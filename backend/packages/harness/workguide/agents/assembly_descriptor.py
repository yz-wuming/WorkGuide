...
:class:`~workguide_extension_api.assembly.AgentAssemblyDescriptor`.
...
from workguide.tools.mcp_metadata import get_mcp_source, is_mcp_tool
...
def _tool_source(tool: object) -> str:
    ...
    if isinstance(metadata, dict):
        declared = metadata.get("workguide_tool_source")
        if isinstance(declared, str) and declared:
            return declared
    callable_object = getattr(tool, "func", None) or getattr(tool, "coroutine", None)
    module = getattr(callable_object, "__module__", "") or ""
    if module.startswith("workguide.tools.builtins") or module.startswith("workguide.agents.memory"):
        return "builtin"
    if "skill" in module:
        return "skill"
...
def _unwrap_middleware(middleware: object) -> tuple[object, str | None]:
    ...
    ``workguide.extensions`` sits below this layer, so importing it here would
...
def _build_identity() -> dict[str, str]:
    ...
    """
    try:
        package_version = version("workguide-harness")
    except PackageNotFoundError:
        package_version = "unknown"
    return {
        "package_version": package_version,
        "image_digest": os.environ.get("WORKGUIDE_IMAGE_DIGEST", "unknown"),
        "git_commit": os.environ.get("WORKGUIDE_GIT_COMMIT", "unknown"),
    }
...
def build_assembly_descriptor(
    *,
    ...
    prompt_template_id: str = "workguide-lead-agent-v1",
    tools: list[object],
