...
from workguide.utils.network import get_free_port, release_port
...
# value can be tuned or disabled through the corresponding WORKGUIDE_SANDBOX_*
...
def _resolve_docker_bind_host(sandbox_host: str | None = None, bind_host: str | None = None) -> str:
    ...
    directly) can restore it with ``WORKGUIDE_SANDBOX_BIND_HOST=0.0.0.0`` —
    ...
    """
    explicit_bind = bind_host if bind_host is not None else os.environ.get("WORKGUIDE_SANDBOX_BIND_HOST", "").strip()
    if explicit_bind:
        ...
        if explicit_bind and not _is_ip_bind_spec(explicit_bind):
            ...
            if resolved is None:
                raise RuntimeError(
                    f"WORKGUIDE_SANDBOX_BIND_HOST={explicit_bind!r} is not an IP literal and could not be resolved; "
    ...
    host = sandbox_host if sandbox_host is not None else os.environ.get("WORKGUIDE_SANDBOX_HOST", "localhost")
    ...
    logger.warning(
        "Could not resolve sandbox host %r for the Docker bind; falling back to the default bridge gateway %s. If the daemon's host-gateway-ip is customized or IPv6, set WORKGUIDE_SANDBOX_BIND_HOST to that address explicitly.",
        host,
        gateway,
    )
    return gateway
...
def _normalize_sandbox_host_for_url(host: str) -> str:
    ...
    WORKGUIDE_SANDBOX_HOST) may carry the address in either bare or
...
def _resolve_sandbox_host_address(host: str) -> str | None:
    ...
    # WORKGUIDE_SANDBOX_HOST) would fail to resolve and silently fall back
...
class LocalContainerBackend(SandboxBackend):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            container_prefix: Prefix for container names (e.g., "workguide-sandbox").
    ...
    def create(
        ...
        *,
        ...
        sandbox_host = _normalize_sandbox_host_for_url(os.environ.get("WORKGUIDE_SANDBOX_HOST", "localhost"))
        return SandboxInfo(
    ...
    def _start_container(
        ...
        if self._runtime == "docker":
            ...
            # WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS=0 (see CONFIGURATION.md),
            # which drops every capability including these three.
            if _env_flag_disabled("WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS"):
            ...
            if memory := _docker_resource_limit("WORKGUIDE_SANDBOX_MEMORY", _DEFAULT_SANDBOX_MEMORY):
                cmd.extend(["--memory", memory])
            if cpus := _docker_resource_limit("WORKGUIDE_SANDBOX_CPUS", _DEFAULT_SANDBOX_CPUS):
                cmd.extend(["--cpus", cpus])
            if pids_limit := _docker_resource_limit("WORKGUIDE_SANDBOX_PIDS_LIMIT", _DEFAULT_SANDBOX_PIDS_LIMIT):
            ...
            if container_user := os.environ.get("WORKGUIDE_SANDBOX_CONTAINER_USER", "").strip():
            ...
            # protections.
            if network := os.environ.get("WORKGUIDE_SANDBOX_NETWORK", "").strip():
                ...
                if target == "host" or target.startswith("container:"):
                    ...
                    raise RuntimeError(
                        f"WORKGUIDE_SANDBOX_NETWORK={network!r} resolves to the {target.split(':', 1)[0]!r} network, "
                ...
                if target == "none":
                    ...
                    raise RuntimeError(
                        f"WORKGUIDE_SANDBOX_NETWORK={network!r} resolves to the 'none' network, which leaves the "
