# WorkGuide Maintainer Orchestrator — design notes

This document explains the *thinking* behind the `workguide-maintainer-orchestrator` skill: what it is for, the boundaries that make it safe to run, and the principles that shape how it reviews. It is written for WorkGuide maintainers who run the skill, and for anyone in the community who wants to understand — or adapt — the pattern of delegating issue and PR triage to an agent.
...
> `.agent/skills/workguide-maintainer-orchestrator/SKILL.md`
