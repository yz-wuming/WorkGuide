...
from workguide.extensions.cli import find_project_root
from workguide.extensions.loader import ExtensionSpec
from workguide.extensions.manager import (
    ExtensionManager,
...
from workguide.tui.cli import main as workguide_main
...
def _write_local_extension(
    ...
    *,
    ...
    distribution: str = "workguide-extension-demo",
    ...
    entry_point = (
        f"""\
[project.entry-points."workguide.extensions"]
demo = "{entry_target}"
"""
        if with_entry_point
        else ""
...
def _assert_demo_entry_point_loads(backend: Path) -> None:
    completed = subprocess.run(
        [
            ...
            "-c",
            "from importlib.metadata import entry_points; eps=entry_points(group='workguide.extensions'); assert [(e.name, e.value) for e in eps] == [('demo', 'demo_extension:install')]; assert callable(next(iter(eps)).load())",
        ],
        check=False,
        capture_output=True,
        text=True,
...
def _write_demo_wheel(directory: Path) -> Path:
    directory.mkdir()
    wheel = directory / "workguide_extension_demo-1.0.0-py3-none-any.whl"
    dist_info = "workguide_extension_demo-1.0.0.dist-info"
    records = {
        "demo_extension/__init__.py": "def install(registry, config):\n    return None\n",
        f"{dist_info}/METADATA": ("Metadata-Version: 2.1\nName: workguide-extension-demo\nVersion: 1.0.0\nRequires-Python: >=3.12\n"),
        f"{dist_info}/WHEEL": ("Wheel-Version: 1.0\nGenerator: deerflow-extension-test\nRoot-Is-Purelib: true\nTag: py3-none-any\n"),
        f"{dist_info}/entry_points.txt": ("[workguide.extensions]\ndemo = demo_extension:install\n"),
    }
...
def test_install_local_directory_makes_it_deployable_and_enabled(tmp_path: Path) -> None:
    ...
    assert result.distribution == "workguide-extension-demo"
    assert result.use == "demo_extension:install"

    managed_source = root / "backend" / "extensions" / "sources" / "workguide-extension-demo"
    ...
    assert project["dependency-groups"]["extensions"] == ["workguide-extension-demo"]
    assert project["tool"]["uv"]["sources"]["workguide-extension-demo"] == {"path": "extensions/sources/workguide-extension-demo"}
    ...
    assert config["plugins"] == [
        {
            "name": "demo",
            "package": "workguide-extension-demo",
...
def test_cli_install_exposes_the_required_opt_in(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    _write_host_project(root)
    _write_local_extension(source)
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(root))

    assert workguide_main(["extensions", "install", str(source), "--yes", "--required"]) == 0
...
def test_workguide_extensions_install_exposes_the_local_install_flow(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    exit_code = workguide_main(["extensions", "install", str(source), "--yes"])
...
def test_hidden_source_env_option_reads_the_install_source_outside_the_shell_recipe(
    ...
    _write_local_extension(source)
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(root))
    monkeypatch.setenv("WORKGUIDE_EXTENSION_SOURCE", str(source))

    exit_code = workguide_main(
        [
            "extensions",
            "install",
            "--source-env",
            "__workguide_extension_source__",
            "--yes",
        ]
    )
...
def test_explicit_invalid_project_root_does_not_fall_back_to_current_checkout(
    ...
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(tmp_path / "not-a-checkout"))
    monkeypatch.chdir(Path(__file__).resolve().parents[2])

    with pytest.raises(FileNotFoundError, match="WORKGUIDE_PROJECT_ROOT"):
        find_project_root()
...
def test_install_git_source_discovers_and_enables_its_packaging_entry_point(tmp_path: Path) -> None:
    ...
    assert result == result.__class__(
        name="demo",
        distribution="workguide-extension-demo",
        use="demo_extension:install",
    )
    assert not (root / "backend" / "extensions" / "sources" / "workguide-extension-demo").exists()
...
def test_install_rejects_a_pypi_requirement_resolved_from_an_external_local_wheel(
    ...
    with pytest.raises(ValueError, match="build context"):
        ExtensionManager(root).install("workguide-extension-demo==1.0.0", yes=True)
...
def _write_audit_host(backend: Path) -> None:
    ...
    (backend / "extensions" / "sources" / "workguide-extension-demo").mkdir(parents=True)
    (backend / "pyproject.toml").write_text(
...
extensions = [{ name = "workguide-extension-demo", directory = "extensions/sources/workguide-extension-demo" }]

[[package]]
name = "workguide-harness"
version = "0.0.0"
source = { editable = "packages/harness" }

[[package]]
name = "workguide-extension-api"
version = "0.0.0"
...
name = "workguide-extension-demo"
version = "1.0.0"
source = { directory = "extensions/sources/workguide-extension-demo" }
...
""",
    ...
    with caplog.at_level("WARNING", logger="workguide.extensions.manager"):
        _validate_locked_local_sources(lock_path, backend)
...
@pytest.mark.parametrize(
    "source",
    [
        "workguide-extension-demo @ ../outside",
        "../outside/workguide-extension-demo",
        "workguide-extension-demo @ /outside/demo.whl",
    ],
)
...
def test_toggle_preserves_the_next_section_header_and_crlf_style(tmp_path: Path) -> None:
    ...
    config_path.write_bytes(
        ...
        b"plugins:\r\n"
        b"  - name: demo\r\n"
        b"    package: workguide-extension-demo\r\n"
...
def test_workguide_extensions_disable_keeps_the_plugin_configuration(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
...
""",
    ...
    exit_code = workguide_main(["extensions", "disable", "demo"])
...
def test_hidden_name_env_option_reads_the_extension_name_outside_the_shell_recipe(
    ...
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(root))
    monkeypatch.setenv("WORKGUIDE_EXTENSION_NAME", "demo")
...
def test_workguide_extensions_enable_reactivates_a_configured_plugin(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
...
""",
    ...
    exit_code = workguide_main(["extensions", "enable", "demo"])
...
def test_distribution_identifier_uses_pep_503_normalization(tmp_path: Path) -> None:
    ...
    config_path = root / "config.yaml"
    config_path.write_text(
        "plugins:\n  - name: demo\n    package: WorkGuide_Extension.Demo\n    use: demo_extension:install\n    enabled: true\n",
        encoding="utf-8",
    )

    ExtensionManager(root).set_enabled("workguide-extension-demo", enabled=False)

    assert yaml.safe_load(config_path.read_text(encoding="utf-8"))["plugins"][0]["enabled"] is False


def test_workguide_extensions_list_reports_activation_and_package(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
...
""",
    ...
    exit_code = workguide_main(["extensions", "list"])

    assert exit_code == 0
    output = capsys.readouterr().out
    assert "demo" in output
    assert "enabled" in output
    assert "workguide-extension-demo" in output
...
def test_cli_reports_invalid_config_without_a_traceback(
    ...
    assert exit_code == 1
    assert "invalid WorkGuide config YAML" in captured.err
...
def test_workguide_extensions_list_rejects_entries_the_runtime_schema_rejects(
    tmp_path: Path,
    monkeypatch,
    capsys,
...
def test_workguide_extensions_remove_uninstalls_dependency_source_and_activation(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    exit_code = workguide_main(["extensions", "remove", "demo"])

    assert exit_code == 0
    output = capsys.readouterr().out
    assert "Removed demo" in output
    assert "Restart WorkGuide" in output
    config = yaml.safe_load((root / "config.yaml").read_text(encoding="utf-8"))
    assert config["plugins"] == []
    assert not (root / "backend" / "extensions" / "sources" / "workguide-extension-demo").exists()
    pyproject = (root / "backend" / "pyproject.toml").read_text(encoding="utf-8")
    assert "workguide-extension-demo" not in pyproject
...
def test_remove_one_configured_instance_keeps_its_shared_distribution_runnable(
    ...
    assert (root / "backend" / "extensions" / "sources" / "workguide-extension-demo").is_dir()
    _assert_demo_entry_point_loads(root / "backend")
...
def test_install_prompts_for_trust_when_yes_is_not_supplied(
    ...
    exit_code = workguide_main(["extensions", "install", str(source)])
...
def test_failed_install_does_not_overwrite_a_concurrent_operator_config_edit(
    ...
    from workguide.extensions import manager as manager_module
    ...
    monkeypatch.setattr("workguide.extensions.manager._sync_environment", _fail_after_operator_edit)
...
def test_uv_add_partial_writes_are_rolled_back_when_the_command_fails(
    ...
    monkeypatch.setattr("workguide.extensions.manager._run_uv", _partially_write_then_fail)
...
def test_install_rejects_identity_collisions_with_a_different_entry_point(
    ...
    assert not (root / "backend" / "extensions" / "sources" / "workguide-extension-demo").exists()
    assert "workguide-extension-demo" not in (root / "backend" / "pyproject.toml").read_text(encoding="utf-8")
...
def test_install_replaces_inline_empty_plugins_with_one_schema_valid_block(tmp_path: Path) -> None:
    ...
    assert parsed.package == "workguide-extension-demo"
...
def test_disable_replaces_nonempty_flow_style_plugins_without_duplicate_key(
    ...
    config_path = root / "config.yaml"
    config_path.write_text(
        f'{plugins_key}: [{{name: demo, package: workguide-extension-demo, use: "demo_extension:install", enabled: true}}]\nlog_level: info\n',
        encoding="utf-8",
    )
...
plugins:
  - name: numeric
    package: workguide-extension-numeric
    use: numeric_extension:install
    enabled: 0
    required: 1
  - name: yaml-booleans
    package: workguide-extension-yaml-booleans
...
def test_cli_install_updates_the_runtime_selected_config_file(
    ...
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(root))
    monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(runtime_config))

    assert workguide_main(["extensions", "install", str(source), "--yes"]) == 0
...
def test_remove_rolls_back_package_lock_config_source_and_environment_when_config_write_fails(
    ...
    monkeypatch.setattr("workguide.extensions.manager.os.replace", _fail_replace)
    ...
    present = subprocess.run(
        [
            str(root / "backend" / ".venv" / "bin" / "python"),
            "-c",
            "from importlib.metadata import version; assert version('workguide-extension-demo') == '1.0.0'",
        ],
        check=False,
    )
...
def test_cli_reports_uv_install_failure_without_traceback_or_partial_state(
    ...
    exit_code = workguide_main(["extensions", "install", "not a valid @ requirement @@", "--yes"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "workguide-extension-demo @ https://user:password@example.com/demo.whl",
    ],
)
...
@pytest.mark.parametrize(
    "source",
    [
        "git+ssh://git@github.com/acme/workguide-extension-demo.git@main",
        "workguide-extension-demo @ git+ssh://git@github.com/acme/workguide-extension-demo.git@main",
        "ssh://git@github.com/acme/workguide-extension-demo.git@main",
    ],
)
...
@pytest.mark.parametrize(
    "source",
    [
        "git@github.com:acme/workguide-extension-demo.git",
        "git+git@github.com:acme/workguide-extension-demo.git",
        "workguide-extension-demo @ git+git@github.com:acme/workguide-extension-demo.git",
        "deploy@internal.example:acme/workguide-extension-demo.git",
    ],
)
...
@pytest.mark.parametrize(
    "source",
    [
        "git+https://github.com/acme/workguide-extension-demo.git@0123456789012345678901234567890123456789",
        "workguide-extension-demo @ git+https://github.com/acme/workguide-extension-demo.git@0123456789012345678901234567890123456789",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "git+git://github.com/acme/workguide-extension-demo.git@main",
...
def test_cli_never_echoes_rejected_source_credentials(
    ...
    assert workguide_main(["extensions", "install", source, "--yes"]) == 1
...
def test_install_uses_one_controlled_uv_project_and_deferred_sync(
    ...
    monkeypatch.setattr("workguide.extensions.manager.subprocess.run", _record_run)
    ...
    assert add[-2:] == ["--", "extensions/sources/workguide-extension-demo"]
...
def test_install_validates_the_config_before_running_third_party_build_hooks(
    ...
    monkeypatch.setattr("workguide.extensions.manager.subprocess.run", _record)
...
def test_failed_recovery_sync_still_restores_the_dependency_files(
    ...
    def _run(command, **_kwargs):
        ...
        if command[1] == "add":
            pyproject_path.write_text(
                original_pyproject.replace("extensions = []", 'extensions = ["workguide-extension-demo"]'),
                encoding="utf-8",
            )
    ...
    monkeypatch.setattr("workguide.extensions.manager.subprocess.run", _run)
...
def test_interrupt_during_install_restores_files_without_a_recovery_resolve(
    ...
    assert not lock_path.exists()
    assert pyproject_path.read_text(encoding="utf-8") == original_pyproject
    assert not (backend / "extensions" / "sources" / "workguide-extension-demo").exists()
...
def test_install_rejects_uv_versions_without_no_workspace_support(
    ...
    monkeypatch.setattr("workguide.extensions.manager.subprocess.run", _old_uv)
...
""",
    ...
    assert remove[-2:] == ["--", "workguide-extension-demo"]
    assert ["--project", backend] == sync[sync.index("--project") : sync.index("--project") + 2]
    assert "--locked" in sync
    assert "--no-sync" not in sync
    for _, kwargs in uv_calls:
...
def test_dependency_sync_uses_the_same_configured_optional_extras_as_startup(
    ...
    monkeypatch.delenv("WORKGUIDE_STREAM_BRIDGE_REDIS_URL", raising=False)
    monkeypatch.delenv("WORKGUIDE_SANDBOX_OWNERSHIP_REDIS_URL", raising=False)
