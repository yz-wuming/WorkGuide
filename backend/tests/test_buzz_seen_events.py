...
def test_new_event_at_same_created_at_is_still_processed(tmp_path):
    ...
    ch, captured = _started_with_store(BuzzSeenEventStore(tmp_path / "seen.json"))
    _dispatch(ch, _event(content="@WorkGuide first", created_at=1700000100))
    _dispatch(ch, _event(content="@WorkGuide second", created_at=1700000100))
...
def test_older_created_at_new_event_is_still_processed(tmp_path):
    ...
    ch, captured = _started_with_store(BuzzSeenEventStore(tmp_path / "seen.json"))
    _dispatch(ch, _event(content="@WorkGuide newer", created_at=1700000200))
    _dispatch(ch, _event(content="@WorkGuide older-clock", created_at=1700000100))
    assert len(captured) == 2
...
def test_service_wiring_injects_persistent_store_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.paths.get_paths", lambda: StubPaths())
    monkeypatch.setattr("workguide.reflection.resolve_class", lambda path, base_class=None: StubChannel)
