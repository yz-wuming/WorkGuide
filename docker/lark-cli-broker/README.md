...
- `install-shim <dest>` — **init container**: writes the launcher + Python shim +
  `.workguide-lark-cli-runtime.json` (`kind: "shim"`) into the shared `emptyDir`
...
(`workguide.integrations.lark_broker`), so the image's copies can never drift from
...
`127` with an actionable message (set `WORKGUIDE_LARK_BROKER_PYTHON` to a known
...
```bash
docker build -t workguide/lark-cli-broker:v1.0.65 \
...
`ghcr.io/<owner>/workguide-lark-cli-broker:<lark-cli-version>` via
`.github/workflows/lark-cli-images.yaml` (run it with a `lark_cli_version` input,
or push a `lark-cli-v*` tag). This is decoupled from the WorkGuide `v*` release
...
- When set, and the Gateway sends `provision_lark_cli_broker` on sandbox create,
  ...
  - the sandbox container gets the runtime RO mount + `WORKGUIDE_LARK_BROKER_URL`
...
tokens could still exfiltrate them. Set `WORKGUIDE_LARK_BROKER_DENY_SUBCOMMANDS`
...
`WORKGUIDE_LARK_BROKER_DENY_SUBCOMMANDS="config show, auth token"`. Denied calls
