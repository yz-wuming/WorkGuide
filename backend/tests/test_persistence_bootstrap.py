"""Tests for ``workguide.persistence.bootstrap.bootstrap_schema``.
...
| legacy (WorkGuide tables, no alembic_version) | create_all (baseline tables only, backfill) + stamp baseline + upgrade head |
...
import workguide.persistence.models  # noqa: F401
from workguide.persistence.base import Base
from workguide.persistence.bootstrap import (
...
from workguide.persistence.migrations._helpers import _normalize_default
...
async def test_legacy_missing_channel_tables_get_backfilled(tmp_path: Path) -> None:
    ...
    try:
        ...
        # branch (has_workguide_tables=True, no alembic_version) while the
...
async def test_legacy_with_manual_workaround_column_warns_on_drift(
    ...
    try:
        ...
        with caplog.at_level("WARNING", logger="workguide.persistence.migrations._helpers"):
            await bootstrap_schema(engine, backend="sqlite")
        ...
        drift_warnings = [r for r in caplog.records if r.levelname == "WARNING" and r.name == "workguide.persistence.migrations._helpers" and "safe_add_column" in r.getMessage() and "token_usage_by_model" in r.getMessage()]
...
def test_type_equivalent_matches_known_dialect_synonyms() -> None:
    from workguide.persistence.migrations._helpers import _type_equivalent
...
async def test_baseline_table_names_constant_matches_0001(tmp_path: Path) -> None:
    ...
    try:
        ...
        # our schema -- the constant is about WorkGuide-owned baseline tables.
...
class TestDecideState:
    def test_empty(self):
        assert _decide_state({"has_alembic_version": False, "has_workguide_tables": False}) == "empty"

    def test_empty_with_unrelated_tables(self):
        # LangGraph checkpointer tables present but WorkGuide has nothing yet.
        # ``has_workguide_tables`` is derived from the metadata intersection in
        # production, so the only thing the decision function needs is the
        # bool itself.
        assert _decide_state({"has_alembic_version": False, "has_workguide_tables": False}) == "empty"

    def test_legacy(self):
        assert _decide_state({"has_alembic_version": False, "has_workguide_tables": True}) == "legacy"

    def test_versioned(self):
        assert _decide_state({"has_alembic_version": True, "has_workguide_tables": True}) == "versioned"
    ...
    def test_versioned_takes_precedence_over_empty(self):
        ...
        assert _decide_state({"has_alembic_version": True, "has_workguide_tables": False}) == "versioned"
...
def test_baseline_revision_id_is_known() -> None:
    ...
    migrations_dir = Path(__file__).resolve().parents[1] / "packages/harness/workguide/persistence/migrations"
    cfg = Config()
