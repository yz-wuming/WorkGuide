...
if [ -n "$WORKGUIDE_CONFIG_PATH" ] && [ -f "$WORKGUIDE_CONFIG_PATH" ]; then
    CONFIG="$WORKGUIDE_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to workguide.*',
        'replacements': [
            ('src.community.', 'workguide.community.'),
            ('src.sandbox.', 'workguide.sandbox.'),
            ('src.models.', 'workguide.models.'),
            ('src.tools.', 'workguide.tools.'),
        ],
    },
