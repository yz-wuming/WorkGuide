...
``from workguide`` line (the ABC contract) and can be vendored into another agent
...
from workguide.agents.memory.backends.deermem.deer_mem import DeerMem
from workguide.agents.memory.backends.deermem.deermem.core.message_processing import (
    filter_messages_for_memory,
)
from workguide.agents.memory.backends.deermem.deermem.core.storage import FileMemoryStorage
from workguide.agents.memory.manager import MemoryCallbacks
...
def test_add_swallows_queue_full_so_backpressure_does_not_break_caller(deermem_data_dir, caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="workguide.agents.memory.backends.deermem.deer_mem"):
...
def test_get_context_injects_facts_only_in_middleware_mode(deermem_data_dir):
    ...
    middleware.import_memory(
        {
            "user": {
                "workContext": {"summary": "Works on WorkGuide memory."},
            },
            "history": {
...
def test_portability_only_abc_contract_imports_workguide():
    """backends/deermem/ has exactly ONE `from workguide` line: the ABC contract in deer_mem.py."""
    import workguide.agents.memory.backends.deermem as pkg
    ...
    for p in root.rglob("*.py"):
        for line in p.read_text(encoding="utf-8").splitlines():
            s = line.strip()
            if s.startswith("from workguide") or s.startswith("import workguide"):
                workguide_imports.append((p.relative_to(root).as_posix(), s))
    assert len(workguide_imports) == 1, workguide_imports
    assert workguide_imports[0][0] == "deer_mem.py"
    assert "memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager" in workguide_imports[0][1]
...
def test_portability_vendor_to_other_agent(tmp_path, monkeypatch):
    ...
    run portability (zero workguide dependency at runtime)."""
    import importlib
    import shutil
    ...
    contract_import = "from workguide.agents.memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager"
...
def test_is_human_clarification_response_matches_host_read():
    ...
    from workguide.agents.human_input import read_human_input_response
    from workguide.agents.memory.backends.deermem.deermem.core.message_processing import _is_human_clarification_response
...
def test_build_llm_returns_none_when_no_model_configured():
    ...
    from workguide.agents.memory.backends.deermem.deermem.config import DeerMemModelConfig
    from workguide.agents.memory.backends.deermem.deermem.core.llm import build_llm
...
def test_build_llm_degrades_to_none_on_init_failure(caplog):
    ...
    llm_logger = "workguide.agents.memory.backends.deermem.deermem.core.llm"
...
def test_from_backend_config_warns_on_unknown_keys(caplog):
    ...
    from workguide.agents.memory.backends.deermem.deermem.config import DeerMemConfig

    cfg_logger = "workguide.agents.memory.backends.deermem.deermem.config"
...
def test_from_backend_config_null_values_fall_back_to_defaults():
    ...
    from workguide.agents.memory.backends.deermem.deermem.config import (
        DeerMemConfig,
        DeerMemModelConfig,
    )
