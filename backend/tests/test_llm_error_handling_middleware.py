...
from workguide.agents.middlewares.llm_error_handling_middleware import (
    LLMErrorHandlingMiddleware,
)
from workguide.config.app_config import AppConfig, LlmCallConfig
...
def _reset_process_limiter() -> Iterator[None]:
    ...
    for.
    """
    from workguide.agents.middlewares import llm_error_handling_middleware as mod
...
def test_async_model_call_retries_busy_provider_then_succeeds(
    ...
    monkeypatch.setattr(
        "workguide.agents.middlewares.llm_error_handling_middleware.aemit_custom_event",
...
def test_async_model_call_returns_user_message_for_quota_errors() -> None:
    ...
    assert result.additional_kwargs["workguide_error_fallback"] is True
...
def test_sync_model_call_uses_retry_after_header(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "workguide.agents.middlewares.llm_error_handling_middleware.emit_custom_event",
        fake_emit_custom_event,
    )
...
def test_async_index_error_exhausted_returns_user_fallback(
    ...
    ``workguide_error_fallback=True``) instead of letting the IndexError
...
async def test_limiter_releases_slot_during_backoff_sleep(
    ...
    assert result_a.additional_kwargs.get("workguide_error_fallback") is True
...
async def test_async_burst_rate_uses_tight_budget_and_longer_base(
    ...
    assert result.additional_kwargs.get("workguide_error_fallback") is True
...
def test_cap_is_frozen_at_first_construction_and_unchanged_by_later_instances() -> None:
    ...
    race on.
    """
    from workguide.agents.middlewares.llm_error_handling_middleware import _get_process_limiter
...
def test_burst_first_retry_uses_jitter_not_fixed_value(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    exc = FakeError("rate increased too quickly", status_code=429, code="limit_burst_rate")
    monkeypatch.setattr(
        "workguide.agents.middlewares.llm_error_handling_middleware.random.randint",
        lambda lo, hi: 7000,
    )
