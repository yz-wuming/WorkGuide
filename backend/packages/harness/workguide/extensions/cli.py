...
from workguide.extensions.manager import ExtensionManager

_NAME_ENV = "WORKGUIDE_EXTENSION_NAME"
_SOURCE_ENV = "WORKGUIDE_EXTENSION_SOURCE"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="workguide extensions",
        description="Install and manage trusted Python extensions for this WorkGuide checkout.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        configured_path = os.environ.get("WORKGUIDE_CONFIG_PATH")
        ...
        if args.command == "install":
            ...
            print(f"Installed and enabled {installed.name} ({installed.distribution}). Restart WorkGuide to load it.")
            return 0
        if args.command == "disable":
            name = manager.set_enabled(_name_argument(args), enabled=False)
            print(f"Disabled {name}. Restart WorkGuide to apply the change.")
            return 0
        if args.command == "enable":
            name = manager.set_enabled(_name_argument(args), enabled=True)
            print(f"Enabled {name}. Restart WorkGuide to apply the change.")
            return 0
        ...
        if args.command == "remove":
            ...
            print(f"Removed {name}. Restart WorkGuide to apply the change.")
            return 0
...
def find_project_root() -> Path:
    configured = os.environ.get("WORKGUIDE_PROJECT_ROOT")
    if configured:
        candidate = Path(configured).expanduser().resolve()
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
        raise FileNotFoundError(f"WORKGUIDE_PROJECT_ROOT is not a WorkGuide checkout: {candidate}")
    ...
    for candidate in candidates:
        ...
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
    raise FileNotFoundError("could not find a WorkGuide checkout; set WORKGUIDE_PROJECT_ROOT")
