#!/usr/bin/env bash
#
# deploy.sh - Build, start, or stop WorkGuide production services
#
# Commands:
...
if [ -f "$ENV_FILE" ]; then
    COMPOSE_CMD=(docker compose --env-file "$ENV_FILE" -p workguide -f "$DOCKER_DIR/docker-compose.yaml")
else
    COMPOSE_CMD=(docker compose -p workguide -f "$DOCKER_DIR/docker-compose.yaml")
fi
...
if [ -z "$WORKGUIDE_HOME" ]; then
    export WORKGUIDE_HOME="$REPO_ROOT/backend/.workguide"
fi
echo -e "${BLUE}WORKGUIDE_HOME=$WORKGUIDE_HOME${NC}"
mkdir -p "$WORKGUIDE_HOME"

# ── WORKGUIDE_REPO_ROOT (for skills host path in DooD) ───────────────────────

export WORKGUIDE_REPO_ROOT="$REPO_ROOT"

# ── config.yaml ───────────────────────────────────────────────────────────────

if [ -z "$WORKGUIDE_CONFIG_PATH" ]; then
    export WORKGUIDE_CONFIG_PATH="$REPO_ROOT/config.yaml"
fi

if  [ "$CMD" != "down" ] && [ ! -f "$WORKGUIDE_CONFIG_PATH" ]; then
    # Try to seed from repo (config.example.yaml is the canonical template)
    if [ -f "$REPO_ROOT/config.example.yaml" ]; then
        cp "$REPO_ROOT/config.example.yaml" "$WORKGUIDE_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded config.example.yaml → $WORKGUIDE_CONFIG_PATH${NC}"
        echo -e "${YELLOW}⚠ config.yaml was seeded from the example template.${NC}"
        echo "  Run 'make setup' to generate a minimal config, or edit $WORKGUIDE_CONFIG_PATH manually before use."
    else
    ...
    echo -e "${GREEN}✓ config.yaml: $WORKGUIDE_CONFIG_PATH${NC}"
fi

# ── extensions_config.json ───────────────────────────────────────────────────

if [ -z "$WORKGUIDE_EXTENSIONS_CONFIG_PATH" ]; then
    export WORKGUIDE_EXTENSIONS_CONFIG_PATH="$REPO_ROOT/extensions_config.json"
fi

if [ ! -f "$WORKGUIDE_EXTENSIONS_CONFIG_PATH" ]; then
    if [ -f "$REPO_ROOT/extensions_config.json" ]; then
        cp "$REPO_ROOT/extensions_config.json" "$WORKGUIDE_EXTENSIONS_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded extensions_config.json → $WORKGUIDE_EXTENSIONS_CONFIG_PATH${NC}"
    else
        # Create a minimal empty config so the gateway doesn't fail on startup
        echo '{"mcpServers":{},"skills":{}}' > "$WORKGUIDE_EXTENSIONS_CONFIG_PATH"
        echo -e "${YELLOW}⚠ extensions_config.json not found, created empty config at $WORKGUIDE_EXTENSIONS_CONFIG_PATH${NC}"
    fi
else
    echo -e "${GREEN}✓ extensions_config.json: $WORKGUIDE_EXTENSIONS_CONFIG_PATH${NC}"
fi
...
_secret_file="$WORKGUIDE_HOME/.better-auth-secret"
...
_internal_auth_token_file="$WORKGUIDE_HOME/.internal-auth-token"
if  [ "$CMD" != "down" ] && [ -z "$WORKGUIDE_INTERNAL_AUTH_TOKEN" ]; then
    if [ -f "$_internal_auth_token_file" ]; then
        export WORKGUIDE_INTERNAL_AUTH_TOKEN
        WORKGUIDE_INTERNAL_AUTH_TOKEN="$(cat "$_internal_auth_token_file")"
        echo -e "${GREEN}✓ WORKGUIDE_INTERNAL_AUTH_TOKEN loaded from $_internal_auth_token_file${NC}"
    else
        export WORKGUIDE_INTERNAL_AUTH_TOKEN
        if command -v python3 > /dev/null 2>&1 && \
            WORKGUIDE_INTERNAL_AUTH_TOKEN="$(python3 -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v python > /dev/null 2>&1 && \
            WORKGUIDE_INTERNAL_AUTH_TOKEN="$(python -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v openssl > /dev/null 2>&1 && \
            WORKGUIDE_INTERNAL_AUTH_TOKEN="$(openssl rand -hex 32)"; then
            true
        else
            echo -e "${RED}✗ Cannot generate WORKGUIDE_INTERNAL_AUTH_TOKEN: python3, python, and openssl are all unavailable.${NC}" >&2
            echo -e "${RED}  Set WORKGUIDE_INTERNAL_AUTH_TOKEN manually before running make up.${NC}" >&2
            exit 1
        fi
        echo "$WORKGUIDE_INTERNAL_AUTH_TOKEN" > "$_internal_auth_token_file"
        chmod 600 "$_internal_auth_token_file"
        echo -e "${GREEN}✓ WORKGUIDE_INTERNAL_AUTH_TOKEN generated → $_internal_auth_token_file${NC}"
    fi
fi
...
detect_sandbox_mode() {
    ...
    [ -f "$WORKGUIDE_CONFIG_PATH" ] || { echo "local"; return; }
    ...
    if [[ "$sandbox_use" == *"workguide.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
if [ "$CMD" = "down" ]; then
    ...
    export WORKGUIDE_HOME="${WORKGUIDE_HOME:-$REPO_ROOT/backend/.workguide}"
    export WORKGUIDE_CONFIG_PATH="${WORKGUIDE_CONFIG_PATH:-$WORKGUIDE_HOME/config.yaml}"
    export WORKGUIDE_EXTENSIONS_CONFIG_PATH="${WORKGUIDE_EXTENSIONS_CONFIG_PATH:-$WORKGUIDE_HOME/extensions_config.json}"
    export WORKGUIDE_REPO_ROOT="${WORKGUIDE_REPO_ROOT:-$REPO_ROOT}"
    export BETTER_AUTH_SECRET="${BETTER_AUTH_SECRET:-placeholder}"
    export WORKGUIDE_INTERNAL_AUTH_TOKEN="${WORKGUIDE_INTERNAL_AUTH_TOKEN:-placeholder}"
    "${COMPOSE_CMD[@]}" down
    exit 0
fi
...
if [ "$CMD" = "build" ]; then
    echo "=========================================="
    echo "  WorkGuide — Building Images"
    echo "=========================================="
    echo ""
...
echo "=========================================="
echo "  WorkGuide Production Deployment"
echo "=========================================="
echo ""
...
# ── WORKGUIDE_DOCKER_SOCKET (aio / pure-DooD mode only) ──────────────────────
...
if [ -z "$WORKGUIDE_DOCKER_SOCKET" ]; then
    export WORKGUIDE_DOCKER_SOCKET="/var/run/docker.sock"
fi

if [ "$sandbox_mode" = "aio" ]; then
    if [ ! -S "$WORKGUIDE_DOCKER_SOCKET" ]; then
        echo -e "${RED}⚠ Docker socket not found at $WORKGUIDE_DOCKER_SOCKET${NC}"
        echo "  AioSandboxProvider (DooD) will not work."
        exit 1
    fi
    echo -e "${GREEN}✓ Docker socket: $WORKGUIDE_DOCKER_SOCKET${NC}"
...
report_startup_failure() {
    echo -e "${RED}✗ WorkGuide services failed to become ready.${NC}" >&2
