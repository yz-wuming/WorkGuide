...
# A ``$``-anchored ``re.match`` would accept a trailing newline (``"workguide\n"``):
# Python's ``$`` matches just before a single trailing ``\n``, which then
# creates a *quoted* schema literally named ``workguide\n`` while the *unquoted*
# ``search_path`` folds to ``workguide`` and misses it, silently landing tables
# in ``public``.
