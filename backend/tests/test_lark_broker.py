...
from workguide.integrations import lark_broker
from workguide.integrations.lark_broker import BrokerConfig, run_lark_cli, serve
...
def test_install_shim_writes_runtime_layout(tmp_path: Path) -> None:
    ...
    marker = json.loads((dest / ".workguide-lark-cli-runtime.json").read_text())
...
def test_launcher_can_pin_interpreter_via_env(broker_server, tmp_path: Path) -> None:
    """WORKGUIDE_LARK_BROKER_PYTHON pins the interpreter for images with no python3
