...
One configured HTTP/SSE MCP server can serve several WorkGuide users, each
...
WorkGuide user ids to credential header values. On every tool call the
...
from workguide.config.extensions_config import ExtensionsConfig, McpUserScopedAuthConfig
