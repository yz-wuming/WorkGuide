...
# shared ``workguide.config.file_signature`` helper also used by
# ``workguide.config.app_config`` for the sibling runtime-editable config file —
...
def _resolve_config_path() -> Path | None:
    ...
    when an explicit `config_path` or `WORKGUIDE_EXTENSIONS_CONFIG_PATH`
...
def reset_mcp_tools_cache() -> None:
    ...
    from workguide.mcp.session_pool import reset_session_pool
