...
externalises that translation to ``workguide.sandbox.tools`` via ``thread_data``,
...
from workguide.sandbox.local.local_sandbox_provider import LocalSandboxProvider
...
def _build_config(skills_dir: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        skills=SimpleNamespace(
            ...
            use="workguide.skills.storage.local_skill_storage:LocalSkillStorage",
        ),
        sandbox=SandboxConfig(use="workguide.sandbox.local:LocalSandboxProvider", mounts=[]),
    )
...
def isolated_paths(monkeypatch, tmp_path):
    ...
    real ``.workguide/`` tree.
    """
    monkeypatch.setenv("WORKGUIDE_HOME", str(tmp_path))
    from workguide.config import paths as paths_module
...
def provider(isolated_paths, tmp_path):
    ...
    with patch("workguide.config.get_app_config", return_value=cfg):
...
def test_is_local_sandbox_accepts_generic_and_per_thread_id_formats():
    from workguide.sandbox.tools import is_local_sandbox
...
def test_concurrent_acquire_same_thread_yields_single_instance(provider):
    ...
    from workguide.sandbox.local import local_sandbox as local_sandbox_module
...
# ──────────────────────────────────────────────────────────────────────────
# 7. Windows desktop mount (/mnt/desktop -> a real writable host directory)
# ──────────────────────────────────────────────────────────────────────────
#
# Regression for the "Access denied on \Desktop\..." bug: the agent emitted a
# POSIX-style root-relative path (`\Desktop\新建文件夹`) that resolved to a
# privileged `C:\Desktop` instead of the configured desktop mount. These tests
# pin the public Sandbox API contract for a desktop-like custom mount: create
# folder, create file, read file, Chinese filenames, and missing paths — all via
# the uniform `/mnt/desktop` virtual root.


class TestWindowsDesktopMount:
    @pytest.fixture
    def desktop_sandbox(self, tmp_path, isolated_paths):
        """Provider + thread sandbox with a writable desktop-like mount."""
        skills_dir = tmp_path / "skills"
        skills_dir.mkdir()
        desktop_host = tmp_path / "Desktop"
        desktop_host.mkdir()
        ...
        cfg = SimpleNamespace(
            ...
            sandbox=SandboxConfig(
                ...
                mounts=[
                    VolumeMountConfig(
                        host_path=str(desktop_host),
                        container_path="/mnt/desktop",
                        read_only=False,
                    )
                ],
            ),
        ...
        provider.acquire("alpha", user_id="default")
        return provider.get("local:default:alpha"), desktop_host

    def test_desktop_create_file_with_chinese_name(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        sandbox.write_file("/mnt/desktop/新建文件.txt", "hello 桌面")
        assert (desktop_host / "新建文件.txt").exists()
        assert sandbox.read_file("/mnt/desktop/新建文件.txt") == "hello 桌面"

    def test_desktop_create_folder_and_read_file_inside(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        sandbox.write_file("/mnt/desktop/新建文件夹/报告.md", "# 报告")
        folder = desktop_host / "新建文件夹"
        assert folder.is_dir(), "desktop folder was not created on the host"
        assert (folder / "报告.md").exists()
        assert sandbox.read_file("/mnt/desktop/新建文件夹/报告.md") == "# 报告"

    def test_desktop_roundtrip_updates_an_existing_file(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        sandbox.write_file("/mnt/desktop/笔记.txt", "v1")
        sandbox.write_file("/mnt/desktop/笔记.txt", "v2")
        assert (desktop_host / "笔记.txt").read_text(encoding="utf-8") == "v2"

    def test_desktop_nonexistent_path_raises(self, desktop_sandbox):
        sandbox, _ = desktop_sandbox
        with pytest.raises(FileNotFoundError):
            sandbox.read_file("/mnt/desktop/不存在的文件.txt")

    def test_desktop_execute_command_resolves_virtual_path(self, desktop_sandbox):
        sandbox, _ = desktop_sandbox
        sandbox.write_file("/mnt/desktop/visible.txt", "x")
        out = sandbox.execute_command("ls /mnt/desktop")
        assert "visible.txt" in out

    # ── Redundant "Desktop" segment right below /mnt/desktop must be folded ──
    def test_desktop_double_desktop_segment_is_collapsed(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        # Agent incorrectly writes the already-virtual desktop again.
        sandbox.write_file("/mnt/desktop/Desktop/新建文件夹/报告.md", "# 报告")
        # Must land on the desktop root, NOT Desktop/Desktop/...
        assert (desktop_host / "新建文件夹" / "报告.md").exists()
        assert not (desktop_host / "Desktop").exists(), "redundant Desktop segment leaked to host"
        assert sandbox.read_file("/mnt/desktop/Desktop/新建文件夹/报告.md") == "# 报告"

    def test_desktop_double_desktop_segment_collapsed_in_command(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        sandbox.execute_command("mkdir /mnt/desktop/Desktop/CommandFolder")
        assert (desktop_host / "CommandFolder").is_dir()
        assert not (desktop_host / "Desktop").exists(), "redundant Desktop leaked in bash command"
        assert not (desktop_host / "Desktop" / "CommandFolder").exists()

    def test_desktop_bare_desktop_segment_is_preserved(self, desktop_sandbox):
        sandbox, desktop_host = desktop_sandbox
        # A literal "/Desktop" at the end (nothing after) is a real entry the user
        # asked for, so it is NOT folded away.
        sandbox.execute_command("mkdir /mnt/desktop/Desktop")
        assert (desktop_host / "Desktop").is_dir()
