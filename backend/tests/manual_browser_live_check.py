...
It:
  ...
  2. builds an isolated WorkGuide config (DeepSeek model + browser tool group),
...
FORM_PAGE = """<!doctype html><html><head><title>WorkGuide Browser Test</title></head>
<body>
<h1>Sign-in demo</h1>
...
tools:
  - name: browser_navigate
    group: browser
    use: workguide.community.browser_automation.tools:browser_navigate_tool
    headless: true
    allow_private_addresses: true
  - name: browser_snapshot
    group: browser
    use: workguide.community.browser_automation.tools:browser_snapshot_tool
  - name: browser_click
    group: browser
    use: workguide.community.browser_automation.tools:browser_click_tool
  - name: browser_type
    group: browser
    use: workguide.community.browser_automation.tools:browser_type_tool
  - name: browser_get_text
    group: browser
    use: workguide.community.browser_automation.tools:browser_get_text_tool
  - name: browser_close
    group: browser
    use: workguide.community.browser_automation.tools:browser_close_tool
...
def main() -> int:
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="workguide-browser-live-"))
    try:
        ...
        # config.yaml. WORKGUIDE_CONFIG_PATH is resolution priority #2.
        os.environ["WORKGUIDE_CONFIG_PATH"] = str(config_path)
        client = WorkGuideClient(config_path=str(config_path))
