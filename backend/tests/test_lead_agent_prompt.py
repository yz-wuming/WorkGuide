...
from workguide.config.subagents_config import CustomSubagentConfig, SubagentsAppConfig
...
def test_apply_prompt_template_threads_explicit_app_config_without_global_config(monkeypatch):
    ...
    explicit_config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=mounts),
        skills=SimpleNamespace(container_path="/mnt/explicit-skills", use="workguide.skills.storage.local_skill_storage:LocalSkillStorage", get_skills_path=lambda: Path("/tmp/explicit-skills")),
        skill_evolution=SimpleNamespace(enabled=False),
        tool_search=SimpleNamespace(enabled=False),
    ...
    monkeypatch.setattr("workguide.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("workguide.config.memory_config.get_memory_config", fail_get_memory_config)
    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", lambda app_config=None: SimpleNamespace(load_skills=lambda enabled_only=True: []))
    monkeypatch.setattr(prompt_module, "get_or_new_user_skill_storage", lambda user_id, app_config=None: SimpleNamespace(load_skills=lambda *, enabled_only: []))
    monkeypatch.setattr(prompt_module, "get_agent_soul", lambda agent_name=None, **kwargs: "")
...
def test_apply_prompt_template_threads_explicit_app_config_to_subagents_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("workguide.config.subagents_config.get_subagents_app_config", fail_get_subagents_app_config)
    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", lambda app_config=None: SimpleNamespace(load_skills=lambda enabled_only=True: []))
    monkeypatch.setattr(prompt_module, "get_agent_soul", lambda agent_name=None, **kwargs: "")
...
def test_build_acp_section_uses_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.acp_config.get_acp_agents", fail_get_acp_agents)
...
def test_get_memory_context_uses_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.memory_config.get_memory_config", fail_get_memory_config)
    monkeypatch.setattr("workguide.runtime.user_context.resolve_runtime_user_id", lambda runtime: "user-1")
    monkeypatch.setattr("workguide.agents.memory.get_memory_manager", lambda: manager)
...
def test_get_memory_context_propagates_fail_closed_manager_error(monkeypatch):
    ...
    monkeypatch.setattr("workguide.agents.memory.get_memory_manager", lambda: manager)
    monkeypatch.setattr("workguide.runtime.user_context.get_effective_user_id", lambda: "user-1")
...
def test_get_memory_context_prefers_explicit_user_id(monkeypatch):
    ...
    monkeypatch.setattr("workguide.runtime.user_context.resolve_runtime_user_id", fail_resolve_runtime_user_id)
    monkeypatch.setattr(
        "workguide.agents.memory.get_memory_manager",
