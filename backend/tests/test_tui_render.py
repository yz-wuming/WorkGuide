...
from workguide.tui.render import render_header, render_status, render_transcript
from workguide.tui.view_state import (
    AssistantDelta,
    RunEnded,
    RunStarted,
    SystemMessage,
...
def test_finalized_long_answer_wraps_instead_of_ellipsis_truncating():
    # Regression: a finalized assistant answer used to be rendered inside a Rich
    # Table cell, which Rich elides to a single ellipsis ("…") line instead of
    # wrapping — so long replies were hard cut off mid-sentence in the TUI.
    # A completed answer must now span multiple lines and keep its full tail.
    text = "很抱歉，但我无法帮助您撰写辱骂上司的邮件。这种行为不仅不专业，而且可能违反道德和法律规定。" * 3
    state = initial_state()
    state = reduce(state, AssistantDelta(id="m1", text=text))
    state = reduce(state, RunEnded())

    out = _render_to_text(render_transcript(state))
    assert not out.rstrip().endswith("…"), f"answer was ellipsis-truncated: {out!r}"
    assert out.count("\n") >= 2, f"expected the answer to wrap onto multiple lines, got: {out!r}"
    # The very tail of the message survives, i.e. it wasn't cut at the column edge.
    assert "法律规定。" in out
...
def test_anonymous_streaming_assistant_stays_plain():
    # Providers that never stamp per-chunk ids (e.g. local/Ollama models) must
    # still stream as plain text instead of reflowing partial text through
    # Markdown (which could collapse the in-flight answer to one line).
    state = initial_state()
    state = reduce(state, RunStarted())
    state = reduce(state, AssistantDelta(id="", text="**partial"))
    state = reduce(state, AssistantDelta(id="", text="**partial heading"))
    out = _render_to_text(render_transcript(state))
    assert "**partial heading" in out
