...
import workguide.skills.storage as skill_storage
from workguide.config.paths import Paths
from workguide.skills.storage import SkillStorage
...
def _patch_storage_resolution(monkeypatch, cls=SlowSkillStorage) -> None:
    monkeypatch.setattr("workguide.config.get_app_config", lambda: _APP_CONFIG)
    monkeypatch.setattr("workguide.reflection.resolve_class", lambda *args, **kwargs: cls)
...
def _patch_user_storage_resolution(monkeypatch, cls=SlowUserSkillStorage) -> None:
    monkeypatch.setattr("workguide.config.get_app_config", lambda: _APP_CONFIG)
    monkeypatch.setattr("workguide.config.paths.get_paths", lambda: Paths(base_dir=Path("/tmp")))
    ...
    monkeypatch.setattr("workguide.skills.storage.UserScopedSkillStorage", cls)
