...
exactly **one** `from workguide` line -- the ABC contract
(`from workguide.agents.memory.manager import MemoryManager` in `deer_mem.py`).
Everything else is relative imports within the folder. So another agent can
adopt DeerMem in three steps, with **zero workguide code**.
...
`get_context` round-trip -- with **zero workguide dependency at runtime**.
