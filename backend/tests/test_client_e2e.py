"""End-to-end tests for WorkGuideClient.
...
Only WORKGUIDE_HOME is redirected to tmp_path for filesystem isolation.
...
from workguide.client import WorkGuideClient, StreamEvent
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - WORKGUIDE_HOME → tmp_path (all thread data lands in a temp dir)
    - WORKGUIDE_PROJECT_ROOT → repository root (shared skills/config assets
    ...
    monkeypatch.setenv(
        "WORKGUIDE_PROJECT_ROOT",
        str(Path(__file__).resolve().parents[2]),
    )
    monkeypatch.setattr("workguide.config.paths._paths", None)
    monkeypatch.setattr("workguide.sandbox.sandbox_provider._default_sandbox_provider", None)
    ...
    monkeypatch.setattr("workguide.config.app_config._app_config", None)
    monkeypatch.setattr("workguide.config.app_config._app_config_is_custom", False)
    config = _make_e2e_config()
    monkeypatch.setattr("workguide.config.app_config._app_config", config)
    monkeypatch.setattr("workguide.config.app_config._app_config_is_custom", True)
    monkeypatch.setattr("workguide.client.get_app_config", lambda: config)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from workguide.config.title_config import TitleConfig

    monkeypatch.setattr("workguide.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from workguide.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "workguide.agents.middlewares.memory_middleware.get_memory_config",
    ...
    monkeypatch.setattr("workguide.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from workguide.agents.lead_agent.agent import build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("workguide.client.build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A WorkGuideClient wired to the isolated e2e_env."""
    return WorkGuideClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = WorkGuideClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = WorkGuideClient(checkpointer=None, plan_mode=False)
        ...
        c2 = WorkGuideClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _allow_skill_security_scan(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.skills.installer.scan_skill_content", _scan)
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "workguide.skills.storage._default_skill_storage",
            local_storage,
        )
        monkeypatch.setattr(
            "workguide.client.get_or_new_user_skill_storage",
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from workguide.config.extensions_config import reload_extensions_config
