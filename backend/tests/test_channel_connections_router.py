...
def _stub_app_config(monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_AUTH_DISABLED", "0")
...
async def _make_repo(tmp_path):
    from workguide.persistence.channel_connections import ChannelConnectionRepository
...
def test_get_providers_reports_connected_without_binding_in_auth_disabled_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("WORKGUIDE_ENV", raising=False)
...
def test_connect_telegram_returns_deep_link_and_persists_state(tmp_path):
    ...
    assert body["url"].startswith("https://t.me/workguide_bot?start=")
    assert body["code"]
...
def test_connect_slack_returns_binding_command_and_persists_state(tmp_path):
    ...
    assert body["instruction"] == f"Send /connect {body['code']} to the WorkGuide Slack bot."
...
def test_connect_discord_returns_binding_command_and_persists_state(tmp_path):
    ...
    assert body["url"] is None
    assert body["code"]
    assert body["instruction"] == f"Send /connect {body['code']} to the WorkGuide Discord bot."
...
def test_connect_existing_binding_code_channels_return_command_and_persist_state(tmp_path):
    ...
    for provider, response in responses.items():
        ...
        assert body["url"] is None
        assert len(body["code"]) >= 22
        assert body["instruction"] == f"Send /connect {body['code']} to the WorkGuide {expected_display_name} bot."
