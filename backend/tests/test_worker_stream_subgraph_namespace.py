...
from workguide.runtime.runs import worker
...
from workguide.runtime.stream_bridge.memory import MemoryStreamBridge
...
def real_executor_module():
    ...
    conftest.py mocks ``workguide.subagents.executor`` to break a package-init
    import cycle; by the time this fixture runs every other workguide module is
    already imported, so a fresh import of the real module is safe.
    """
    original = sys.modules.get("workguide.subagents.executor")
    sys.modules.pop("workguide.subagents.executor", None)
    subagents_pkg = sys.modules.get("workguide.subagents")
    if subagents_pkg is not None and hasattr(subagents_pkg, "executor"):
        delattr(subagents_pkg, "executor")

    module = importlib.import_module("workguide.subagents.executor")
    ...
    if original is not None:
        sys.modules["workguide.subagents.executor"] = original
    else:
        sys.modules.pop("workguide.subagents.executor", None)
    subagents_pkg = sys.modules.get("workguide.subagents")
    if subagents_pkg is not None and hasattr(subagents_pkg, "executor"):
...
def _build_delegating_parent_graph(executor_module, monkeypatch, *, child_emits_error_fallback: bool = False):
    ...
    message carrying the ``workguide_error_fallback`` marker (not as its final
        ...
        lambda _state: {
            "messages": [
                AIMessage(
                    ...
                    additional_kwargs={"workguide_error_fallback": True},
                )
            ]
        },
    )
