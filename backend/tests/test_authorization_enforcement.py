...
from workguide.authz.provider import AuthzDecision, AuthzReason, Principal
...
from workguide.config.guardrails_config import GuardrailProviderConfig, GuardrailsConfig
...
from workguide.tools.builtins.tool_search import assemble_deferred_tools
...
class TestAuthorizationGuardrailWiring:
    ...
    def test_authorization_wires_adapter_with_the_build_provider_instance(self):
        ...
        config = _app_config(
            authorization=AuthorizationConfig(
                enabled=True,
                provider=AuthorizationProviderConfig(use="workguide.authz.rbac:RbacAuthorizationProvider", config={"roles": {"user": {}}}),
            )
        )
...
def test_lead_agent_filters_all_model_visible_tools_and_reuses_provider(monkeypatch, is_bootstrap):
    ...
    monkeypatch.setattr("workguide.skills.describe.build_skill_search_setup", lead_agent_module.build_skill_search_setup)
    monkeypatch.setattr("workguide.tools.get_available_tools", lambda **kwargs: [_tool("safe_tool"), _tool("denied_tool")])
