...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`workguide/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  workguide.skills.installer
workguide.client             ──┘              workguide.uploads.manager
```

- Shared modules live in the harness layer (`workguide.*`), pure business logic, no FastAPI dependency
...
### 3.1 `workguide.skills.installer`
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `WorkGuideClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `WorkGuideClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
