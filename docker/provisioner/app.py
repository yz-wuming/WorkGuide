"""WorkGuide Sandbox Provisioner Service.
...
K8S_NAMESPACE = os.environ.get("K8S_NAMESPACE", "workguide")
...
LARK_CLI_BROKER_DENY_SUBCOMMANDS = os.environ.get("WORKGUIDE_LARK_BROKER_DENY_SUBCOMMANDS", "")
...
THREADS_HOST_PATH = os.environ.get("THREADS_HOST_PATH", "/.workguide/threads")
WORKGUIDE_HOST_BASE_DIR = os.environ.get("WORKGUIDE_HOST_BASE_DIR", "/.workguide")
...
def _host_base_dir_for_extra_mounts() -> str:
    """Return the host-visible WorkGuide state root used for controlled mounts."""
    if WORKGUIDE_HOST_BASE_DIR:
        return os.path.normpath(WORKGUIDE_HOST_BASE_DIR)
...
def _validated_extra_mounts(extra_mounts: list["ExtraMount"] | None) -> list["ExtraMount"]:
    ...
    for mount in extra_mounts:
        ...
        if not _is_path_under_base(host_path, host_base_dir):
            raise HTTPException(status_code=400, detail=f"Extra mount host path is outside WorkGuide state: {mount.host_path}")
...
def _extra_mount_pvc_sub_path(host_path: str) -> str:
    host_base_dir = _host_base_dir_for_extra_mounts()
    if not _is_path_under_base(host_path, host_base_dir):
        raise HTTPException(status_code=400, detail=f"Extra mount host path is outside WorkGuide state: {host_path}")
    ...
    return posixpath.join("workguide", *rel_parts)
        ...
        if exc.status == 404:
            ns = k8s_client.V1Namespace(
                metadata=k8s_client.V1ObjectMeta(
                    name=K8S_NAMESPACE,
                    labels={
                        "app.kubernetes.io/name": "workguide",
...
app = FastAPI(title="WorkGuide Sandbox Provisioner", lifespan=lifespan)
...
def _build_volumes(
    ...
    *,
    ...
    if SKILLS_PVC_NAME:
        ...
        public_path = join_host_path(WORKGUIDE_HOST_BASE_DIR, "skills_view", "public")
        volumes.append(
            k8s_client.V1Volume(
                name="skills-public",
                host_path=k8s_client.V1HostPathVolumeSource(
        ...
        legacy_path = join_host_path(
            WORKGUIDE_HOST_BASE_DIR, "users", user_id, "skills_view", "legacy"
        )
        volumes.append(
            k8s_client.V1Volume(
                name="skills-legacy",
...
def _build_volume_mounts(
    ...
    *,
    ...
    if USERDATA_PVC_NAME:
        userdata_mount.sub_path = f"workguide/users/{user_id}/threads/{thread_id}/user-data"
...
def _build_lark_cli_broker_sidecars(
    ...
    if LARK_CLI_BROKER_DENY_SUBCOMMANDS:
        broker_env.append(
            k8s_client.V1EnvVar(
                name="WORKGUIDE_LARK_BROKER_DENY_SUBCOMMANDS",
        ...
        spec=k8s_client.V1PodSpec(
            containers=[
                k8s_client.V1Container(
                    ...
                    image_pull_policy="IfNotPresent",
                    env=(
                        [k8s_client.V1EnvVar(name="WORKGUIDE_LARK_BROKER_URL", value=LARK_BROKER_URL)]
                        if _lark_cli_broker_enabled(provision_lark_cli_broker)
                        else None
                    ),
                    ports=[
...
def list_sandboxes():
    ...
    try:
        services = core_v1.list_namespaced_service(
            K8S_NAMESPACE,
            label_selector="app=workguide-sandbox",
        )
    except ApiException as exc:
