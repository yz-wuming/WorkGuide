...
def test_upload_files_extracts_zip_members_into_uploads(tmp_path):
    import io
    import zipfile
    ...
    with zipfile.ZipFile(zip_buf, "w") as zf:
        zf.writestr("02_students.csv", "id,name\n1,Alice\n")
        zf.writestr("05_bug_report.md", "# Bug\nreport text\n")
        zf.writestr("nested_dir/03_events.json", "[1,2]\n")
    ...
    with (
        ...
        result = asyncio.run(
            call_unwrapped(
                ...
                "thread-local",
                request=MagicMock(),
                files=[UploadFile(filename="pack.zip", file=io.BytesIO(zip_bytes))],
                config=SimpleNamespace(),
            )
        )

    # zip itself + every flattened inner member are registered
    filenames = [f.filename for f in result.files]
    assert filenames == ["pack.zip", "02_students.csv", "05_bug_report.md", "03_events.json"]
    assert (thread_uploads_dir / "pack.zip").read_bytes() == zip_bytes
    # inner file pulled from a subdir lands flattened in the uploads dir
    assert (thread_uploads_dir / "03_events.json").read_bytes() == b"[1,2]\n"
    assert (thread_uploads_dir / "02_students.csv").read_bytes() == b"id,name\n1,Alice\n"

    # mounted provider: nothing synced to a remote sandbox
    provider.get.return_value.update_file.assert_not_called()


def test_upload_files_non_zip_is_not_extracted(tmp_path):
    thread_uploads_dir = tmp_path / "uploads"
    thread_uploads_dir.mkdir(parents=True)
    ...
    with (
        ...
        result = asyncio.run(
            call_unwrapped(
                ...
                "thread-local",
                request=MagicMock(),
                files=[UploadFile(filename="notes.txt", file=BytesIO(b"plain"))],
                config=SimpleNamespace(),
            )
        )

    assert result.success is True
    assert [f.filename for f in result.files] == ["notes.txt"]
