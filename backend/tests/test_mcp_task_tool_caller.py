...
from workguide.mcp.session_pool import MCPSessionPool
from workguide.mcp.task_tool_caller import McpTaskToolCaller, mcp_task_session_scope_key
...
async def test_stdio_task_call_reuses_exact_scope_and_raw_tool_name() -> None:
    ...
    with (
        patch("workguide.mcp.task_tool_caller.get_session_pool", return_value=pool),
        patch(
            "workguide.mcp.task_tool_caller._prepare_stdio_connection",
...
"""
    ...
    try:
        with (
            patch("workguide.mcp.task_tool_caller.get_paths", return_value=Paths(tmp_path)),
            patch("workguide.mcp.task_tool_caller.get_session_pool", return_value=pool),
        ):
