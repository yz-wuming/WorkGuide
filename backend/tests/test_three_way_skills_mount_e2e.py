...
from workguide.sandbox.local.local_sandbox import PathMapping
from workguide.sandbox.local.local_sandbox_provider import LocalSandboxProvider
from workguide.sandbox.tools import read_file_tool
from workguide.skills.projection import rebuild_skill_projections
from workguide.skills.storage import reset_user_skill_storage
from workguide.skills.storage.user_scoped_skill_storage import UserScopedSkillStorage
from workguide.skills.types import SKILL_MD_FILE, Skill, SkillCategory

_AIO_MODULE = "workguide.community.aio_sandbox.aio_sandbox_provider"
...
class TestThreeWayMountEndToEnd:
    ...
    def test_local_public_skill_mounted(self, skills_fs):
        ...
        with patch("workguide.config.get_app_config", return_value=cfg), patch("workguide.config.paths.get_paths", return_value=paths):
    ...
    def test_local_acquire_recovers_public_mount_after_initial_projection_failure(self, skills_fs):
        ...
        with (
            patch("workguide.config.get_app_config", return_value=cfg),
    ...
    def test_read_file_tool_uses_enabled_projection_for_every_skill_category(self, skills_fs, monkeypatch):
        ...
        with (
            patch("workguide.config.get_app_config", return_value=cfg),
            patch("workguide.config.paths.get_paths", return_value=paths),
            patch("workguide.config.extensions_config.ExtensionsConfig.from_file", return_value=extensions),
            patch("workguide.config.extensions_config.get_extensions_config", return_value=extensions),
        ):
            provider = LocalSandboxProvider()
            sandbox_ids = {
            ...
            monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", _sandbox_for)
            monkeypatch.setattr("workguide.sandbox.tools._resolve_skills_path", _must_not_pre_resolve)
    ...
    def test_registry_to_sandbox_full_pipeline(self, skills_fs):
        ...
        with patch("workguide.config.paths.get_paths", return_value=paths):
    ...
    def test_local_tools_observe_toggle_without_sandbox_recreation(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda _runtime: sandbox)
    ...
    def test_aio_per_user_custom_skill_mount(self, skills_fs, aio_mod, monkeypatch):
        ...
        with patch(_AIO_GET_CONFIG, return_value=cfg), patch("workguide.config.paths.get_paths", return_value=paths):
            mounts = aio_mod.AioSandboxProvider._get_skills_mounts(user_id="user-1")
        idx = {m[1]: m for m in mounts}
        assert "/mnt/skills/custom" in idx
    ...
    def test_aio_extra_mounts_translate_to_docker_bind_mounts(self, skills_fs, aio_mod, monkeypatch):
        ...
        from workguide.community.aio_sandbox.local_backend import _format_container_mount
