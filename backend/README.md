...
WorkGuide is a LangGraph-based AI super agent with sandbox execution, persistent memory, and extensible tool integration. The backend enables AI agents to execute code, browse the web, manage files, delegate tasks to subagents, and retain context across conversations - all in isolated, per-thread environments.
...
- **Skills path**: `/mnt/skills` → `workguide/skills/` directory
...
| `DELETE /api/threads/{id}` | Delete WorkGuide-managed local thread data after LangGraph thread deletion; unexpected failures are logged server-side and return a generic 500 detail |
...
For Feishu card updates, WorkGuide stores the running card's `message_id` per inbound message and patches that same card until the run finishes, preserving the existing `OK` / `DONE` reaction flow. When a follow-up arrives inside an existing Feishu topic while another turn is still running, the later message now waits on the mapped WorkGuide `thread_id`, receives a queued/running card on that exact source message, and keeps a compact source-message blockquote in subsequent patches so rapid consecutive questions remain distinguishable.
...
```bash
uv pip install 'workguide-harness[tui]'   # optional 'textual' dependency
workguide                                 # launch the TUI
workguide --print "summarize this repo"   # headless one-shot
workguide --recursion-limit 250 --print "run a longer task"
```
...
```
backend/
├── packages/harness/           # workguide-harness package (import: workguide.*)
│   └── workguide/
│       ├── agents/             # Agent system
...
│       ├── tui/                # Terminal UI (`workguide` console script)
...
flag permits WorkGuide's synchronous configuration and graph-factory setup
...
Before the locked local runtime loads its persisted development store, WorkGuide
...
- If a provider module is missing, WorkGuide now returns an actionable error with install guidance (for example `uv add langchain-google-genai`).
...
- `WORKGUIDE_CONFIG_PATH` - Override config.yaml location
- `WORKGUIDE_EXTENSIONS_CONFIG_PATH` - Override extensions_config.json location
- Model API keys: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, etc.
- Tool API keys: `TAVILY_API_KEY`, `GITHUB_TOKEN`, etc.

### LangSmith Tracing

WorkGuide has built-in [LangSmith](https://smith.langchain.com) integration for observability. When enabled, all LLM calls, agent runs, tool executions, and middleware processing are traced and visible in the LangSmith dashboard.
...
WorkGuide also supports [Langfuse](https://langfuse.com) observability for LangChain-compatible runs.
...
If both LangSmith and Langfuse are enabled, WorkGuide initializes and attaches both callbacks so the same run data is reported to both systems.

If a provider is explicitly enabled but required credentials are missing, or the provider callback cannot be initialized, WorkGuide raises an error when tracing is initialized during model creation instead of silently disabling tracing.
...
`make dev` pre-creates and excludes `WORKGUIDE_HOME` (by default
`backend/.workguide`) and `backend/sandbox` from Uvicorn's reload watcher. Use
this target instead of a bare `uvicorn --reload`: agent tasks write Python and
other runtime files under `WORKGUIDE_HOME`, and watching that directory can
restart the Gateway during an active run.

### Schema Migrations

WorkGuide's application tables (`runs`, `threads_meta`, `feedback`, `users`,
...
`packages/harness/workguide/persistence/migrations/versions/`:
...
does not need a pre-existing `./data/workguide.db`. Review the generated file
...
# Explicit real-API WorkGuideClient integration suite
make test-live
```
...
`.workguide/blocking-io-findings.json` at the repository root (regardless of
