"""``BoxliteBox`` — WorkGuide :class:`Sandbox` backed by a BoxLite micro-VM.
...
``asyncio.to_thread`` worker WorkGuide invokes us from.

Every operation is a shell command run inside the box (``cat`` / ``find`` /
``grep`` / chunked ``base64``), parsed with the shared ``workguide.sandbox.search``
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: WorkGuide-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so WorkGuide's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        WorkGuide passes a bash command *string*; BoxLite's ``exec`` takes argv, so
