...
run path resolve it through :func:`workguide.config.app_config.get_app_config`,
...
from workguide.community.browser_automation.session import browser_multi_worker_error
from workguide.config.app_config import AppConfig, get_app_config
from workguide.persistence.feedback import FeedbackRepository
from workguide.runtime import ORPHAN_RECOVERY_STOP_REASON, STARTUP_ORPHAN_RECOVERY_ERROR, RunContext, RunManager, StreamBridge
from workguide.runtime.events.store.base import RunEventStore
from workguide.runtime.runs.store.base import RunStore
...
if TYPE_CHECKING:
    ...
    from workguide.persistence.thread_meta.base import ThreadMetaStore
    from workguide.runtime import RunRecord
...
def get_config() -> AppConfig:
    ...
    Routes through :func:`workguide.config.app_config.get_app_config`, which
    ...
    :func:`get_app_config` closes the bytedance/workguide issue #3107 BUG-001
    ...
    :mod:`workguide.config.reload_boundary` and is mirrored by the
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    Usage in ``app.py``::
        ...
        async with langgraph_runtime(app, startup_config):
            yield
    """
    from workguide.persistence.engine import close_engine, get_session_factory, init_engine_from_config
    from workguide.runtime import make_store, make_stream_bridge
    from workguide.runtime.checkpoint_mode import freeze_checkpoint_channel_mode, freeze_checkpoint_snapshot_frequency
    from workguide.runtime.checkpointer.async_provider import make_checkpointer
    from workguide.runtime.events.store import make_run_event_store
    ...
    async with AsyncExitStack() as stack:
        ...
        if sf is not None:
            from workguide.persistence.feedback import FeedbackRepository
            from workguide.persistence.run import RunRepository
        ...
        from workguide.extensions import EMPTY_EXTENSIONS, record_runtime_diagnostics
        from workguide.extensions.gateway import start_services, stop_services
        ...
        from workguide.persistence.thread_meta import make_thread_store

        app.state.thread_store = make_thread_store(sf, app.state.store)
        if sf is not None:
            from workguide.persistence.mcp_tasks import McpTaskRepository
            from workguide.persistence.scheduled_task_runs import (
                ScheduledTaskRunRepository,
            )
            from workguide.persistence.scheduled_tasks import ScheduledTaskRepository
            from workguide.persistence.subagent_batches import SubagentBatchRepository
        ...
        from workguide.utils.time import now_iso
