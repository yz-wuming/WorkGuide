"""Tests for workguide.tracing.metadata.build_langfuse_trace_metadata."""
...
from workguide.tracing import metadata as tracing_metadata
...
def test_workguide_trace_id_comes_from_current_trace_context(monkeypatch):
    _enable_langfuse(monkeypatch)
    ...
    assert result["workguide_trace_id"] == "gateway-trace-1"


def test_workguide_trace_id_explicit_argument_wins(monkeypatch):
    _enable_langfuse(monkeypatch)
    ...
    with request_trace_context("ambient-trace"):
        result = tracing_metadata.build_langfuse_trace_metadata(
            thread_id="thread-abc",
            user_id="user-42",
            workguide_trace_id="explicit-trace",
        )

    assert result["workguide_trace_id"] == "explicit-trace"
