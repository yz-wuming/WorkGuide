...
from workguide.config.database_config import DEFAULT_CHECKPOINT_SNAPSHOT_FREQUENCY
...
def _worker_main(encoded_case: str, *, profile_path: Path | None = None) -> int:
    try:
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="workguide-checkpoint-production-") as temp_dir:
...
async def _run_phase(case: ProductionCase, saver: Any, timing: _TimingSaver, work_dir: Path) -> dict[str, Any]:
    ...
    from workguide.runtime.checkpoint_mode import inject_checkpoint_mode
    ...
    with patch("workguide.agents.lead_agent.agent.create_chat_model", _fake_create_chat_model):
        graph = make_lead_agent({"configurable": {}})
    # Attach the timed saver post-construction, mirroring the run worker
    # (workguide.runs.worker assigns ``agent.checkpointer`` the same way), so
