<!-- Authored by @zhfeng, discussed in https://github.com/bytedance/workguide/issues/4063.
...
**Status:** Draft for feedback (responds to [#3462](https://github.com/bytedance/workguide/issues/3462)).
**Affects:** `backend/packages/harness/workguide/authz/` (new), `backend/app/gateway/authz.py`, `backend/packages/harness/workguide/guardrails/`, `backend/packages/harness/workguide/agents/lead_agent/agent.py`, `backend/packages/harness/workguide/client.py`, `backend/packages/harness/workguide/subagents/executor.py`, `backend/app/gateway/services.py`, `config.example.yaml`.

> Maintainer guidance from the issue ([@WillemJiang](https://github.com/bytedance/workguide/issues/3462)):
...
| Internal (`X-WorkGuide-Internal-Token`) | `"internal"` | IM channel workers, scheduler |
...
The maintainer pointed here for good reason. `GuardrailProvider` (`workguide/guardrails/provider.py:46`) is already a pluggable, class-path-loaded, per-call authorization hook:
...
All tools are merged in `get_available_tools()` (`workguide/tools/tools.py:44`): config tools + built-ins + MCP (cached, hot-reloadable by mtime) + ACP, deduped by name. In the lead agent (`agents/lead_agent/agent.py:561`):
...
| `WorkGuideClient` | `client.py:259` | ❌ *(passes `tools` straight to `assemble_deferred_tools`)* |

The `WorkGuideClient` gap is a pre-existing inconsistency; any new authz filter must be applied on all three.
...
Lives in a new package `workguide/authz/` (harness), mirroring `workguide/guardrails/`.

```python
# workguide/authz/provider.py
...
- New `workguide/authz/` package: `provider.py` (Protocol + dataclasses), `principal.py` (builder), `adapter.py`.
...
- `test_authz_tool_filter.py` — Layer 1: tools removed at assembly on all three build paths (lead / subagent / `WorkGuideClient`); filtered tools absent from `DeferredToolCatalog` (fail-closed promotion).
...
- Issue: [#3462](https://github.com/bytedance/workguide/issues/3462)
- Existing design: `backend/docs/AUTH_DESIGN.md`, `backend/docs/GUARDRAILS.md`
- Guardrail provider: `backend/packages/harness/workguide/guardrails/provider.py`
- Guardrail middleware: `backend/packages/harness/workguide/guardrails/middleware.py`
- Tool assembly: `backend/packages/harness/workguide/agents/lead_agent/agent.py:561`, `workguide/tools/tools.py:44`, `workguide/tools/builtins/tool_search.py:190`
- Skill filter pattern: `backend/packages/harness/workguide/skills/tool_policy.py:42`
- Identity injection: `backend/app/gateway/services.py:278`
- Config resolution: `backend/packages/harness/workguide/reflection/resolvers.py`, `workguide/config/guardrails_config.py`, `workguide/config/reload_boundary.py`
- Middleware wiring: `backend/packages/harness/workguide/agents/middlewares/tool_error_handling_middleware.py:154`

---

## 15. Related work & prior art (upstream issues & PRs)

A search of `bytedance/workguide` confirms **no RBAC implementation exists**; RBAC is a recognized but unowned roadmap item. This RFC situates itself in the prior work below.

### Direct lineage - what this RFC builds on

- **[#1669](https://github.com/bytedance/workguide/issues/1669)** - Q2 Roadmap. Lists *"Implement Role-Based Access Control (RBAC)"* under "Security and Permission Strengthening 🔥🔥🔥🔥" (top priority), referencing #1721 and #3506. **This is the roadmap slot #3462 and this RFC fill.**
- **[#1213](https://github.com/bytedance/workguide/issues/1213)** (closed) - the original RFC proposing OAP `before_tool_call` authorization for tools/skills. The `GuardrailProvider`/`GuardrailMiddleware` system is the implemented result. **Our RFC is the next layer on top of that lineage.**
- **[#3664](https://github.com/bytedance/workguide/issues/3664) / PR [#3665](https://github.com/bytedance/workguide/pull/3665)** (merged) - added `user_id`/`user_role`/`oauth_*`/`run_id`/`tool_call_id` to `GuardrailRequest` and wired Gateway -> context -> middleware. **This is the plumbing our Layer 2 relies on.** Critically, #3665 explicitly scoped guardrails to *not* be an RBAC system (quoted in §11) - the RBAC brain is the deliberately-deferred gap this RFC adds.
- **[#3672](https://github.com/bytedance/workguide/issues/3672) / PR [#3839](https://github.com/bytedance/workguide/pull/3839)** (merged) - propagate the bound connection owner's `role`/`oauth` into the guardrail context for IM/internal-auth runs. Documents the exact `user_role=None` gap for unbound channels ("If owner lookup fails, the run continues with role/oauth attribution unset") **that our `default_role` (§8) closes.**
- **[#2507](https://github.com/bytedance/workguide/issues/2507) / RFC PR [#2504](https://github.com/bytedance/workguide/pull/2504)** (closed) - "Deferred MCP tools can execute before `tool_search` promotion." Proposed direction: *"Add an execution gate before tool execution."* This became `DeferredToolFilterMiddleware.wrap_tool_call`'s execution deny. **Our §7.2 two-layer design is consistent with this precedent** - the project already chose "execution gate" as the pattern for the deferred capability boundary.

### Security precedents - the gaps that motivate #3462

- **GHSA-4693 / [#2996](https://github.com/bytedance/workguide/pull/2996)** (closed) - proposed `@require_permission` for MCP/memory/skills routers after any authenticated user could RCE via MCP stdio config injection. The merged fix was **[#3855](https://github.com/bytedance/workguide/pull/3855)** (admin-gate skills) + **[#3425](https://github.com/bytedance/workguide/pull/3425)** (harden MCP config endpoint) - i.e. the project chose `require_admin_user` over fine-grained permissions for management surfaces. **This RFC respects that precedent** (§9 Phase 2, §12 Q6): management endpoints stay admin-gated; only ordinary routes migrate to the provider.
- **[#1646](https://github.com/bytedance/workguide/issues/1646)**, **[#1648](https://github.com/bytedance/workguide/issues/1648)**, **[#2531](https://github.com/bytedance/workguide/issues/2531)** (open) - unauthenticated/over-broad MCP config + memory disclosure. Further evidence that resource-level authz is the open gap.

### Complementary (distinct axis, not overlapping)

- **[#2470](https://github.com/bytedance/workguide/issues/2470)** (open RFC) - "Pluggable auth *providers* with request-level hook." This is **authentication** (trusted-header/gateway SSO via an `AuthProvider` extension), and its non-goal #4 explicitly excludes authorization policy. **Complementary, not overlapping** - it decides *who you are*; this RFC decides *what you can do*.
- **[#3322](https://github.com/bytedance/workguide/issues/3322)**, **[#3476](https://github.com/bytedance/workguide/issues/3476)**, **[#2761](https://github.com/bytedance/workguide/issues/2761)** (open) - **per-user credential** isolation (per-user MCP tokens, user connectors for GitHub/Linear, per-user model API keys). This is a *different axis* from per-*role* tool authorization: per-user creds = "act as this user on external service X"; this RFC = "may role Y use tool/model Z at all." The `Principal` (§5) and provider hook could eventually support per-user policies, but per-user credential plumbing is a separate effort.
- **[#1721](https://github.com/bytedance/workguide/issues/1721)** (closed RFC) - the original user-authentication module design (the `AUTH_DESIGN.md` lineage). Scoped RBAC out as a non-goal ("当前用户角色只有 admin 和 user，尚未实现细粒度 RBAC"). **This RFC is the RBAC that #1721 deferred.**

### Other relevant security work

- **[#3630](https://github.com/bytedance/workguide/issues/3630) / [#3662](https://github.com/bytedance/workguide/pull/3662) / [#3661](https://github.com/bytedance/workguide/pull/3661)** (merged) - prompt-injection input sanitization + role isolation via system-message injection. Orthogonal defense; our Layer 1 (remove tools from the bound set) is the complementary capability-layer defense.
- **[#3837](https://github.com/bytedance/workguide/pull/3837)** (merged) - persist guardrail interventions as run events. Our Layer 2 reuses this audit trail for free.
- **[#3929](https://github.com/bytedance/workguide/issues/3929)** (open) - sandbox NodePort->ClusterIP (same author family of security hardening RFCs).
