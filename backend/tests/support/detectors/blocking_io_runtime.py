"""Strict Blockbuster runtime context scoped to WorkGuide business code.

Creates a `BlockBuster` instance with `scanned_modules=("app", "workguide")`
...
sync IO whose caller stack passes through `app.*` or `workguide.*` raises
`BlockingError`.
...
_SCANNED_MODULES: tuple[str, ...] = ("app", "workguide")

# Add WorkGuide-local rules here only when Blockbuster's default rule set misses
...
def detect_blocking_io_strict() -> Iterator[BlockBuster]:
    """Activate Blockbuster scoped to app.* and workguide.* callers only."""
