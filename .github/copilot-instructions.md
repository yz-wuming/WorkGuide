# Copilot Onboarding Instructions for WorkGuide
...
WorkGuide is a full-stack "super agent harness".
...
- `backend/packages/harness/workguide/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/workguide/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/workguide/subagents/` - subagent registry/execution
- `backend/packages/harness/workguide/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`workguide.agents:make_lead_agent`)
