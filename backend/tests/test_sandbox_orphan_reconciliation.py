...
from workguide.community.aio_sandbox.aio_sandbox_provider import SandboxBeingDestroyedError
from workguide.community.aio_sandbox.ownership import compute_lease_ttl
from workguide.community.aio_sandbox.sandbox_info import SandboxInfo
...
def test_backend_list_running_default_returns_empty():
    ...
    from workguide.community.aio_sandbox.backend import SandboxBackend
...
def _make_local_backend():
    ...
    from workguide.community.aio_sandbox.local_backend import LocalContainerBackend
...
def test_list_running_returns_containers(monkeypatch):
    ...
    _mock_ps_and_inspect(
        monkeypatch,
        ps_output="workguide-sandbox-abc12345\nworkguide-sandbox-def67890\n",
        inspect_payload=[
            _make_inspect_entry("workguide-sandbox-abc12345", "2026-04-08T01:22:50.000000000Z", "8081"),
            _make_inspect_entry("workguide-sandbox-def67890", "2026-04-08T02:22:50.000000000Z", "8082"),
        ],
    )
...
def test_list_running_skips_non_matching_names(monkeypatch):
    ...
    _mock_ps_and_inspect(
        ...
        inspect_payload=[
            _make_inspect_entry("workguide-sandbox-abc12345", "2026-04-08T01:22:50Z", "8081"),
        ],
    )
...
def test_list_running_includes_containers_without_port(monkeypatch):
    ...
    _mock_ps_and_inspect(
        ...
        ps_output="workguide-sandbox-abc12345\n",
        inspect_payload=[
            _make_inspect_entry("workguide-sandbox-abc12345", "2026-04-08T01:22:50Z", host_port=None),
        ],
    )
...
def test_list_running_handles_malformed_inspect_json(monkeypatch):
    ...
    def mock_run(cmd, **kwargs):
        ...
        if len(cmd) >= 2 and cmd[1] == "ps":
            result.returncode = 0
            result.stdout = "workguide-sandbox-abc12345\n"
            result.stderr = ""
        else:
            result.returncode = 0
...
def test_list_running_uses_single_batch_inspect_call(monkeypatch):
    ...
    def mock_run(cmd, **kwargs):
        ...
        if len(cmd) >= 2 and cmd[1] == "ps":
            result.returncode = 0
            result.stdout = "workguide-sandbox-a\nworkguide-sandbox-b\nworkguide-sandbox-c\n"
            result.stderr = ""
            return result
        if len(cmd) >= 2 and cmd[1] == "inspect":
            inspect_call_count["count"] += 1
            # Expect all three names passed in a single call
            assert cmd[2:] == ["workguide-sandbox-a", "workguide-sandbox-b", "workguide-sandbox-c"]
            result.returncode = 0
            result.stdout = json.dumps(
                [
                    _make_inspect_entry("workguide-sandbox-a", "2026-04-08T01:22:50Z", "8081"),
                    _make_inspect_entry("workguide-sandbox-b", "2026-04-08T01:22:50Z", "8082"),
                    _make_inspect_entry("workguide-sandbox-c", "2026-04-08T01:22:50Z", "8083"),
                ]
            )
            result.stderr = ""
            return result
...
def test_parse_docker_timestamp_with_nanoseconds():
    ...
    from workguide.community.aio_sandbox.local_backend import _parse_docker_timestamp
...
def test_extract_host_port_returns_mapped_port():
    from workguide.community.aio_sandbox.local_backend import _extract_host_port
...
def _make_shared_ownership_store(**kwargs):
    ...
    """
    from workguide.community.aio_sandbox.ownership.memory import MemoryOwnershipStore
...
def _make_provider_for_reconciliation(tmp_path=None, *, worker_id: str = "worker-test", store=None):
    ...
    """
    from workguide.config.sandbox_config import SandboxOwnershipConfig

    aio_mod = importlib.import_module("workguide.community.aio_sandbox.aio_sandbox_provider")
...
def test_reconcile_adopts_old_containers_into_warm_pool(tmp_path):
    ...
    old_info = SandboxInfo(
        ...
        container_name="workguide-sandbox-old12345",
...
def test_reconcile_adopts_young_containers(tmp_path):
    ...
    young_info = SandboxInfo(
        ...
        container_name="workguide-sandbox-young123",
...
def test_reconcile_mixed_containers_all_adopted(tmp_path):
    ...
    old_info = SandboxInfo(
        sandbox_id="old_one",
        sandbox_url="http://localhost:8081",
        container_name="workguide-sandbox-old_one",
        created_at=now - 1200,
    )
    young_info = SandboxInfo(
        sandbox_id="young_one",
        sandbox_url="http://localhost:8082",
        container_name="workguide-sandbox-young_one",
        created_at=now - 60,
    )
...
def test_reconcile_skips_already_tracked_containers(tmp_path):
    ...
    existing_info = SandboxInfo(
        ...
        sandbox_url="http://localhost:8081",
        container_name="workguide-sandbox-existing1",
        created_at=now - 1200,
    )
...
def test_reconcile_skips_container_owned_by_peer():
    ...
    info = SandboxInfo(
        ...
        container_name="workguide-sandbox-shared01",
        created_at=now - 50,
    )
...
def test_idle_reap_does_not_destroy_peer_owned_warm_entry():
    ...
    info = SandboxInfo(
        sandbox_id="a99c8444",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-a99c8444",
        created_at=now - 50,
    )
...
def test_multi_worker_release_then_peer_reconcile_cannot_kill():
    ...
    info = SandboxInfo(
        sandbox_id=sid,
        sandbox_url="http://localhost:8080",
        container_name=f"workguide-sandbox-{sid}",
...
def test_expired_lease_lets_peer_adopt_crashed_owner_container():
    ...
    info = SandboxInfo(
        sandbox_id="crashed1",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-crashed1",
        created_at=time.time() - 50,
    )
...
def test_acquire_fails_closed_when_ownership_cannot_be_published():
    ...
    """
    from workguide.community.aio_sandbox.ownership import OwnershipBackendError
    ...
    info = SandboxInfo(
        sandbox_id="new001",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-new001",
...
def test_reuse_fails_closed_when_ownership_cannot_be_published():
    ...
    info = SandboxInfo(
        sandbox_id="sb1",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-sb1",
        created_at=time.time(),
    )
...
def test_destroy_fails_closed_when_ownership_unknown():
    ...
    info = SandboxInfo(
        sandbox_id="unknown1",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-unknown1",
        created_at=time.time() - 50,
    )
...
def test_reconcile_fails_closed_when_ownership_unknown():
    ...
    info = SandboxInfo(
        sandbox_id="unknown2",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-unknown2",
        created_at=time.time() - 50,
    )
...
def test_renewal_keeps_the_sandbox_when_the_store_cannot_answer():
    ...
    info = SandboxInfo(
        sandbox_id="live02",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-live02",
        created_at=time.time(),
    )
    sandbox = MagicMock()
...
def test_renewal_keeps_the_sandbox_when_lapsed_reclaim_cannot_answer():
    ...
    """
    from workguide.community.aio_sandbox.ownership import OwnershipBackendError, RenewOutcome
...
def test_load_config_carries_the_stream_bridge_section():
    ...
    """
    from workguide.config.stream_bridge_config import StreamBridgeConfig
...
def test_renewal_covers_warm_entries_not_just_active():
    ...
    info = SandboxInfo(
        sandbox_id="warm01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-warm01",
        created_at=time.time(),
    )
...
def test_renewal_does_not_forget_a_warm_entry_mid_teardown():
    ...
    info = SandboxInfo(
        ...
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-warm-stop",
        created_at=time.time(),
    )
...
def test_lost_lease_drops_sandbox_without_destroying_container():
    ...
    info = SandboxInfo(
        sandbox_id="moved01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-moved01",
        created_at=time.time(),
    )
    sandbox = MagicMock()
...
def test_ownership_rollback_on_create_closes_the_client_it_drops():
    ...
    info = SandboxInfo(
        sandbox_id="new002",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-new002",
        created_at=time.time(),
    )
...
def test_acquire_takes_over_ownership_so_a_thread_can_move_instances():
    ...
    info = SandboxInfo(
        sandbox_id="thread01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-thread01",
        created_at=time.time(),
    )
...
def test_store_losing_all_state_does_not_evict_live_sandboxes():
    ...
    info = SandboxInfo(
        sandbox_id="live01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-live01",
        created_at=time.time(),
    )
    sandbox = MagicMock()
...
def test_acquire_refuses_a_container_a_peer_is_destroying():
    ...
    info = SandboxInfo(
        sandbox_id="dying01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-dying01",
        created_at=time.time(),
    )
...
def test_teardown_marker_is_held_for_a_stop_that_outlives_the_lease_ttl():
    ...
    info = SandboxInfo(
        sandbox_id="doomed1",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-doomed1",
        created_at=time.time(),
    )
...
def test_unhealthy_drop_holds_the_teardown_marker_for_its_stop():
    ...
    info = SandboxInfo(
        sandbox_id="sick01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-sick01",
        created_at=time.time(),
    )
...
def test_destroy_holds_the_teardown_marker_for_its_stop():
    ...
    info = SandboxInfo(
        sandbox_id="doomed3",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-doomed3",
        created_at=time.time(),
    )
...
def test_destroy_releases_the_teardown_marker_when_the_stop_fails():
    ...
    info = SandboxInfo(
        sandbox_id="boom01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-boom01",
        created_at=time.time(),
    )
...
def test_teardown_release_waits_for_the_heartbeat_to_exit():
    ...
    info = SandboxInfo(
        sandbox_id="defer1",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-defer1",
        created_at=time.time(),
    )
...
def test_evict_keeps_the_warm_entry_when_the_claim_is_refused():
    ...
    info = SandboxInfo(
        sandbox_id="peer01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-peer01",
        created_at=time.time(),
    )
...
def test_reclaim_drops_a_container_a_peer_is_destroying():
    ...
    info = SandboxInfo(
        sandbox_id="dying02",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-dying02",
        created_at=time.time(),
    )
...
def test_created_sandbox_is_rolled_back_when_a_peer_is_destroying_its_id():
    ...
    info = SandboxInfo(
        sandbox_id="fresh01",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-fresh01",
        created_at=time.time(),
    )
...
def test_teardown_heartbeat_stops_when_the_stop_returns():
    ...
    info = SandboxInfo(
        sandbox_id="doomed2",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-doomed2",
        created_at=time.time(),
    )
...
def test_refused_idle_destroy_keeps_the_warm_entry():
    ...
    info = SandboxInfo(
        sandbox_id="warmpeer",
        sandbox_url="http://localhost:8080",
        container_name="workguide-sandbox-warmpeer",
        created_at=time.time(),
    )
...
def _active_sandbox(provider, sandbox_id, info, *, thread_key=("u", "t1")):
    ...
    from workguide.community.aio_sandbox.aio_sandbox import AioSandbox
...
def _info(sandbox_id, *, created_at=None):
    return SandboxInfo(
        ...
        sandbox_url="http://localhost:8080",
        container_name=f"workguide-sandbox-{sandbox_id}",
