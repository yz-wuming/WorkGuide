...
class SandboxOwnershipConfig(BaseModel):
    ...
    redis_url: str | None = Field(
        default=None,
        description="Redis URL for the redis ownership type. If omitted, WORKGUIDE_SANDBOX_OWNERSHIP_REDIS_URL, WORKGUIDE_STREAM_BRIDGE_REDIS_URL, REDIS_URL, or redis://localhost:6379/0 is used.",
    )
    ...
    key_prefix: str = Field(
        default="workguide:sandbox:owner",
...
class VolumeMountConfig(BaseModel):
    ...
    host_path: str = Field(
        ...
        description=(
            ...
            "``workguide-gateway`` container, so the host directory must also be "
...
class SandboxConfig(BaseModel):
    ...
    AioSandboxProvider specific options:
        ...
        container_prefix: Prefix for container names (default: workguide-sandbox)
    ...
    use: str = Field(
        ...,
        description="Class path of the sandbox provider (e.g. workguide.sandbox.local:LocalSandboxProvider)",
    )
