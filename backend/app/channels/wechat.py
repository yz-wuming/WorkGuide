...
class WechatChannel(Channel):
        ...
        try:
            ...
            send_payload = {
                "msg": {
                    "from_user_id": "",
                    "to_user_id": msg.chat_id,
                    "client_id": f"workguide_img_{int(time.time() * 1000)}",
                    "message_type": 2,
                    "message_state": 2,
                    "context_token": context_token,
                    "item_list": [
        ...
        try:
            ...
            send_payload = {
                "msg": {
                    "from_user_id": "",
                    "to_user_id": msg.chat_id,
                    "client_id": f"workguide_file_{int(time.time() * 1000)}",
                    "message_type": 2,
                    "message_state": 2,
                    "context_token": context_token,
                    "item_list": [
    ...
    async def _bind_connection_from_connect_code(self, *, chat_id: str, context_token: str, code: str) -> bool:
        ...
        await self._send_connection_reply(chat_id, context_token, "WeChat connected to WorkGuide.")
        return True
    ...
    async def _send_connection_reply(self, chat_id: str, context_token: str, text: str) -> None:
        ...
        await self._send_text_message(
            ...
            context_token=context_token,
            text=text,
            client_id_prefix="workguide-connect",
            max_retries=1,
        )
