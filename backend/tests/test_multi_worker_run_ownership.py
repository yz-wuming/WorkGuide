...
from workguide.runtime import ORPHAN_RECOVERY_STOP_REASON, RunManager, RunStatus, ThreadOperationKind
from workguide.runtime.runs.manager import CancelOutcome, ConflictError, _generate_worker_id
...
async def test_periodic_reconciliation_logs_recovered_run_ids_when_callback_fails(caplog):
    ...
    with caplog.at_level("WARNING", logger="workguide.runtime.runs.manager"):
        await manager._reconcile_orphans_periodic()
        await asyncio.sleep(0)
...
class _EndingCrossProcessBridge:
    ...
    def subscribe(self, run_id, *, last_event_id=None, heartbeat_interval=15.0):
        from workguide.runtime import END_SENTINEL
