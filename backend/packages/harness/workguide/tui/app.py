...
pure renderers. Agent runs execute on a worker *thread* (``WorkGuideClient.stream``
...
class WorkGuideTUI(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield ComposerInput(placeholder="(=^･ω･^=) 输入你的任务…   ( / for commands )", id="composer")
    ...
    def _load_session_info(self) -> None:
        ...
        try:
            ...
            self._model = next((m.get("model") or m.get("display_name") or m.get("name") for m in models if m.get("name")), "")
    ...
    def _refresh_header(self) -> None:
        self.query_one("#header", Static).update(render_header(model=self._model))
    ...
    def _refresh_status(self) -> None:
        ...
        self.query_one("#status", Static).update(
            render_status(self.state, spinner=spinner, elapsed=self._elapsed_str())
        )

    def _elapsed_str(self) -> str:
        if not self._run_started_at:
            return ""
        seconds = time.time() - self._run_started_at
        if seconds < 60:
