...
from workguide.skills.skillscan import StaticScanBlockedError, enforce_static_scan, scan_archive_preflight, scan_skill_dir
from workguide.skills.skillscan.orchestrator import _PYTHON_CLIENT_SINK_METHODS
...
def test_python_client_analysis_budget_preserves_prior_findings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setattr("workguide.skills.skillscan.orchestrator._PYTHON_CLIENT_ANALYSIS_BUDGET", 20)
    skill_dir = tmp_path / "demo-skill"
    _write_skill(skill_dir)
    scripts_dir = skill_dir / "scripts"
    scripts_dir.mkdir()
...
def test_archive_member_count_cap_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from workguide.skills.skillscan import orchestrator
...
async def test_llm_scanner_receives_static_findings_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.skills.security_scanner.create_chat_model", lambda **kwargs: FakeModel())
