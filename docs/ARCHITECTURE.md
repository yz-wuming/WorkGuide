# WorkGuide Architecture

This document is the **top-level architecture overview** for WorkGuide. It explains the
...
WorkGuide 2.0 is a ground-up rewrite of the original Deep Research framework (see
...
WorkGuide (**D**eep **E**xploration and **E**fficient **R**esearch **Flow**) is an
...
This lets standard LangGraph SDK clients talk to WorkGuide without a separate LangGraph
...
- **Harness** (`backend/packages/harness/workguide/`, import prefix `workguide.*`) — the
...
**Rule**: App imports `workguide`, but `workguide` never imports `app`. This boundary is
...
There is also an **embedded Python client** (`workguide.client.WorkGuideClient`) used by
...
`RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/workguide/runtime/`). The
...
- **`ThreadState`** extends LangGraph's `AgentState` with `sandbox`, `artifacts`,
  ...
  under `backend/.workguide/threads/{thread_id}/`.
...
- **Skills** — `skills/public/` (committed) and `skills/custom/` (gitignored); managed
  integration packs are global at `.workguide/integrations/skills/{provider}/`. Skills are
...
- **Version sources** — a release version must match in `backend/pyproject.toml`,
  `frontend/package.json`, and `deploy/helm/workguide/Chart.yaml` (`version` + `appVersion`);
