...
from workguide.config.stream_bridge_config import StreamBridgeConfig, set_stream_bridge_config
from workguide.runtime import END_SENTINEL, HEARTBEAT_SENTINEL, MemoryStreamBridge, StreamGap, make_stream_bridge

# RedisStreamBridge is no longer re-exported from workguide.runtime (redis is an
# optional extra; see the NOTE in runtime/stream_bridge/__init__.py). Import it
# directly from the submodule.
from workguide.runtime.stream_bridge.redis import RedisStreamBridge
...
async def test_redis_cleanup_deletes_stream(redis_bridge: RedisStreamBridge):
    ...
    assert fake.deleted == ["workguide:stream_bridge:redis-run-cleanup"]
...
async def test_redis_publish_refreshes_stream_ttl():
    ...
    key = "workguide:stream_bridge:redis-run-ttl"
...
async def test_make_stream_bridge_uses_docker_redis_env(monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_STREAM_BRIDGE_REDIS_URL", "redis://redis:6379/0")
    try:
        async with make_stream_bridge() as bridge:
...
async def test_make_stream_bridge_passes_redis_options(monkeypatch):
    ...
    import workguide.runtime.stream_bridge.redis as redis_module
...
# WORKGUIDE_TEST_REDIS_URL (defaults to redis://localhost:6379/15 — DB 15 to
