...
`workguide.extensions` group, for example
`example = "workguide_extension_example:install"`. The operator CLI is dispatched from
the existing `workguide` console script to `extensions/cli.py` and exposes only these
...
`.workguide/extension-manager.lock`; remove deactivates config before changing the package
...
The public package is `packages/extension-api/` and must never import `workguide` or carry
...
(`workguide_extension_api.assembly.AgentAssemblyDescriptor`) captures the
...
`workguide/agents/assembly_descriptor.py`: a middleware that implements
...
is never authoritative. Absent the key — embedded `WorkGuideClient`, standalone LangGraph
...
System-model-call observers cover WorkGuide-owned model invocations that do not pass
...
described: `WorkGuideSummarizationMiddleware.compact_state()` / `acompact_state()` hash each
...
distinguishes an ordinary user from an administrator through `workguide_extension_api.auth`:
...
host hook, which the WorkGuide-side callbacks translate into an observation and submit
