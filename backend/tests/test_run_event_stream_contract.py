...
from workguide.runtime.events.catalog import (
...
from workguide.subagents.step_events import SUBAGENT_STEP_MAX_CHARS, capture_step_message, subagent_run_event
    ...
    chunks = [
        {"type": "task_started", "task_id": "call-batch", "description": "research"},
        {
            ...
            "message": {
                "type": "ai",
                "content": "searching",
                "tool_calls": [{"name": "web_search", "args": {"query": "workguide"}}],
            },
            "message_index": 1,
        },
...
def test_lower_level_run_event_modules_do_not_import_runtime(relative_path):
    module_path = REPO_ROOT / "backend" / "packages" / "harness" / "workguide" / relative_path
    ...
    assert not [module for module in imports if module == "workguide.runtime" or module.startswith("workguide.runtime.")]
...
async def test_non_database_stores_return_contract_records(backend, tmp_path):
    if backend == "memory":
        ...
        from workguide.runtime.events.store.jsonl import JsonlRunEventStore
...
def test_captured_subagent_message_survives_task_running_conversion():
    ...
    assert capture_step_message(
        AIMessage(
            content="searching",
            id="ai-step-1",
            tool_calls=[{"id": "call-1", "name": "web_search", "args": {"query": "workguide"}}],
        ),
        captured,
        set(),
    )
    ...
    assert event["content"]["tool_calls"] == [{"name": "web_search", "args": {"query": "workguide"}}]
...
async def test_workspace_change_producer_matches_catalog_and_payload(monkeypatch, tmp_path):
    from workguide.workspace_changes import WorkspaceRoot, scan_workspace_roots
    from workguide.workspace_changes import recorder as recorder_module
