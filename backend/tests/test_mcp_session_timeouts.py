...
from workguide.constants import DEFAULT_MCP_SESSION_INIT_TIMEOUT
from workguide.mcp.tools import _make_session_pool_tool, get_mcp_tools
...
async def test_discovery_timeout_skips_hung_server_without_blocking_healthy_server() -> None:
    ...
    with (
        ...
        patch("workguide.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
        ...
        patch("workguide.mcp.tools._make_session_pool_tool", side_effect=lambda tool, *_args, **_kwargs: tool),
    ):
...
async def test_session_init_timeout_raises_when_session_creation_hangs(tmp_path, caplog) -> None:
    ...
    with (
        patch("workguide.mcp.tools.get_session_pool", return_value=mock_pool),
        patch("workguide.mcp.tools.get_paths", return_value=MagicMock()),
        patch(
            "workguide.mcp.tools._prepare_stdio_workspace",
