"""Live integration tests for WorkGuideClient with real external APIs.
...
For direct pytest invocation, set the explicit opt-in flag:
    ...
    WORKGUIDE_RUN_LIVE_TESTS=1 PYTHONPATH=. uv run pytest tests/test_client_live.py -v -s
"""
...
_LIVE_TEST_OPT_IN = "WORKGUIDE_RUN_LIVE_TESTS"
...
def client():
    """Create a real WorkGuideClient (no mocks)."""
    return WorkGuideClient(thinking_enabled=False)
