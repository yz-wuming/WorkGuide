"""BoxLite micro-VM backend for WorkGuide sandboxes.
...
macOS) — behind WorkGuide's :class:`Sandbox` / :class:`SandboxProvider` contract.
Each sandbox is a hardware-isolated VM with its own kernel that runs any OCI
image unchanged. See https://github.com/bytedance/workguide/issues/3936.
...
virtualization when WorkGuide itself runs inside a cloud VM); macOS uses
