# Contributing to WorkGuide

Thank you for your interest in contributing to WorkGuide! This guide will help you set up your development environment and understand our development workflow.
...
3. **Start development services**:
   ...
   version, creates the missing `.env` files, and exports `WORKGUIDE_ROOT`.

   If you do run Compose directly, run it **from the repository root** and set
   `WORKGUIDE_ROOT` to the absolute path of your checkout. Compose interpolates
   that variable into host-side paths (`WORKGUIDE_HOST_BASE_DIR`,
   `THREADS_HOST_PATH`) that the AIO and provisioner sandbox modes bind-mount;
   leaving it unset renders them as `/backend/.workguide`, so those mounts
   silently miss your checkout instead of failing:

   ```bash
   # macOS / Linux
   WORKGUIDE_ROOT="$PWD" docker compose -f docker/docker-compose-dev.yaml up --build
   ```

   ```powershell
   # Windows PowerShell
   $env:WORKGUIDE_ROOT = (Get-Location).Path
...
|---------|-----------|------------|-------|
| `make dev` on one machine | 4 vCPU, 8 GB RAM | 8 vCPU, 16 GB RAM | Best when WorkGuide uses hosted model APIs. |
...
`2 vCPU / 4 GB` environments often fail to start reliably or become unresponsive under normal WorkGuide workloads.
...
```text
unable to get image 'workguide-gateway': permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock
```
...
│   │   └── workguide/       # Agents, tools, sandbox, MCP, skills, config
...
WorkGuide is an AI project and we welcome AI-assisted contributions. To help
...
# Live WorkGuideClient integration tests (explicit opt-in)
...
must also set `WORKGUIDE_RUN_LIVE_TESTS=1`.
...
`.workguide/support-bundles/`. The zip includes toolchain versions, sanitized
...
- Check existing [Issues](https://github.com/bytedance/workguide/issues)
- Read the [Documentation](backend/docs/)
- Ask questions in [Discussions](https://github.com/bytedance/workguide/discussions)

## License

By contributing to WorkGuide, you agree that your contributions will be licensed under the [MIT License](./LICENSE).
