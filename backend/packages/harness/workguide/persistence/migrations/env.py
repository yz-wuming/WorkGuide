"""Alembic environment for WorkGuide application tables.

ONLY manages WorkGuide's tables (runs, threads_meta, feedback, users,
...
try:
    import workguide.persistence.models as models  # register ORM models with Base.metadata

    _ = models
except ImportError:
    # Models not available — migration will work with existing metadata only.
    logging.getLogger(__name__).warning("Could not import workguide.persistence.models; Alembic may not detect all tables")
...
async def run_migrations_online() -> None:
    ...
    pg_schema = config.get_main_option("workguide_pg_schema")
    connect_args: dict = {}
    ...
    if pg_schema and url and url.split("+", 1)[0].split(":", 1)[0] in {"postgresql", "postgres"}:
        from workguide.persistence.postgres_schema import build_asyncpg_connect_args
    ...
    # ``workguide.persistence.engine`` sets this on its own connections, but
