        ...
        "detail": [
            {
                ...
                "loc": ["body", field],
                "msg": f"Run option '{field}' is not supported by WorkGuide",
                "input": value,
                "ctx": {"option": field},
            }
        ]
...
def _sdk_default_payload(method: str) -> dict[str, Any]:
    ...
    if method == "stream":
        ...
        client.runs.stream("thread-id", "workguide", input={"messages": []})
    else:
        client.runs.http.post = capture_post  # type: ignore[method-assign]
        asyncio.run(client.runs.create("thread-id", "workguide", input={"messages": []}))
    return captured
