...
from workguide.config.agents_config import load_agent_soul
...
from workguide.skills.storage import get_or_new_skill_storage, get_or_new_user_skill_storage
from workguide.skills.types import Skill, SkillCategory
from workguide.subagents import get_available_subagent_names
from workguide.tools.builtins.tool_search import get_deferred_tools_prompt_section
...
# ``_user_scoped_storages`` in ``workguide.skills.storage``; the
...
def _start_enabled_skills_refresh_thread() -> None:
    threading.Thread(
        ...
        name="workguide-enabled-skills-loader",
        daemon=True,
    ).start()
...
def _build_available_subagents_description(available_names: list[str], bash_available: bool, *, app_config: AppConfig | None = None) -> str:
    ...
    from workguide.subagents.registry import get_subagent_config
...
is user-managed data (visible and editable via the WorkGuide UI) — you may
...
WorkGuide is an open-source AI agent framework that gained significant traction in early 2026
[citation:GitHub Repository](https://github.com/bytedance/workguide). The project focuses on
providing a production-ready agent system with sandbox execution and memory management
[citation:WorkGuide Documentation](https://workguide.dev/docs).
...
### Primary Sources
- [GitHub Repository](https://github.com/bytedance/workguide) - Official source code and documentation
- [WorkGuide Documentation](https://workguide.dev/docs) - Technical specifications
...
- ✅ RIGHT in Sources: `[GitHub Repository](https://github.com/bytedance/workguide) - 官方源代码和文档`
...
"""
        ...
        from workguide.agents.memory import MemoryManagerError
...
def get_skills_prompt_section(
    ...
    *,
    ...
    if skill_names is not None:
        from workguide.skills.describe import get_skill_index_prompt_section
...
def _build_acp_section(*, app_config: AppConfig | None = None) -> str:
    ...
    if app_config is None:
        try:
            from workguide.config.acp_config import get_acp_agents
...
def _build_custom_mounts_section(*, app_config: AppConfig | None = None) -> str:
    ...
    for mount in mounts:
        ...
        cp = mount.container_path.rstrip("/") or "/"
        lines.append(f"- Custom mount: `{cp}` - Host directory mapped into the sandbox ({access})")
        if cp.endswith("/desktop"):
            lines.append(
                "  - The `/mnt/desktop` mount is the user's REAL Windows desktop. When the user "
                'says "desktop" / "桌面", operate there via `/mnt/desktop/<name>` — not `\\Desktop`, '
                "`~/Desktop`, or a bare `Desktop`."
            )
...
def apply_prompt_template(
    ...
    *,
    ...
    return SYSTEM_PROMPT_TEMPLATE.format(
        agent_name=agent_name or "WorkGuide 2.0",
