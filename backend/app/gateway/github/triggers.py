...
def _mentions(body: str, login: str) -> bool:
    ...
    ``@workguide`` would falsely match ``@workguide-bot`` (a different,
    ...
    character (e.g. ``foo@workguide`` inside an email address) to avoid
...
def event_should_fire(
    ...
    if trigger.require_mention:
        ...
        # Boundary-aware @-mention match: ``@workguide`` must NOT match
        # ``@workguide-bot`` (a distinct, legitimate GitHub login). See
