...
tests pin WorkGuide's adapter contract with a small synchronous fake: lazy
...
from workguide.community.opensandbox.provider import OpenSandboxProvider, _import_sdk
from workguide.community.opensandbox.sandbox import OpenSandboxSandbox
...
def _install(monkeypatch: pytest.MonkeyPatch, *, sdk: _FakeSandboxClass | None = None, config: dict[str, Any] | None = None) -> tuple[OpenSandboxProvider, _FakeSandboxClass]:
    ...
    monkeypatch.setattr("workguide.community.opensandbox.provider.get_app_config", lambda: _stub_config(config))
    monkeypatch.setattr(
        "workguide.community.opensandbox.provider._import_sdk",
...
def test_missing_sdk_has_actionable_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ImportError, match=r"workguide-harness\[opensandbox\]"):
        _import_sdk()


def test_provider_defers_sdk_import_until_acquire(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("workguide.community.opensandbox.provider.get_app_config", lambda: _stub_config())
    calls = 0
    ...
    monkeypatch.setattr("workguide.community.opensandbox.provider._import_sdk", fail_if_called)
...
def test_create_passes_connection_lifetime_scope_and_environment(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert call["metadata"] == {
        "work_guide_provider": "opensandbox",
        "work_guide_thread": "thread-1",
        "work_guide_user": "user-1",
    }
...
def test_remote_http_connection_warns_without_logging_api_key(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.DEBUG, logger="workguide.community.opensandbox.provider")
    provider, _ = _install(
        monkeypatch,
...
def test_download_rejects_oversize_stream(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("workguide.community.opensandbox.sandbox._MAX_DOWNLOAD_SIZE", 4)
    remote = _FakeRemote("remote")
