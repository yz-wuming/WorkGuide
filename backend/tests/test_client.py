...
from workguide.agents.middlewares.view_image_middleware import ViewImageMiddleware
from workguide.agents.thread_state import DeltaThreadState, ThreadState
from workguide.client import WorkGuideClient
from workguide.config.authorization_config import AuthorizationConfig, AuthorizationProviderConfig
from workguide.config.extensions_config import ExtensionsConfig, McpServerConfig
from workguide.config.paths import Paths
from workguide.config.subagent_runtime_config import SubagentRuntimeConfig
from workguide.skills.types import SkillCategory
from workguide.tools.mcp_metadata import tag_mcp_tool
from workguide.uploads.manager import PathTraversalError
...
def client(mock_app_config, tmp_path):
    """Create a WorkGuideClient with mocked config loading."""
    import workguide.skills.storage as _storage_mod
    from workguide.skills.storage.local_skill_storage import LocalSkillStorage

    _storage_mod._default_skill_storage = LocalSkillStorage(host_path=str(tmp_path))
    with patch("workguide.client.get_app_config", return_value=mock_app_config):
        return WorkGuideClient()


@pytest.fixture
def allow_skill_security_scan():
    async def _scan(*args, **kwargs):
        from workguide.skills.security_scanner import ScanResult

        return ScanResult(decision="allow", reason="ok")

    with patch("workguide.skills.installer.scan_skill_content", _scan):
        yield
...
class TestClientInit:
    ...
    def test_custom_params(self, mock_app_config):
        ...
        with patch("workguide.client.get_app_config", return_value=mock_app_config):
            c = WorkGuideClient(model_name="gpt-4", thinking_enabled=False, subagent_enabled=True, plan_mode=True, agent_name="test-agent", available_skills={"skill1", "skill2"}, middlewares=[mock_middleware])
    ...
    def test_invalid_agent_name(self, mock_app_config):
        with patch("workguide.client.get_app_config", return_value=mock_app_config):
            with pytest.raises(ValueError, match="Invalid agent name"):
                WorkGuideClient(agent_name="invalid name with spaces!")
            with pytest.raises(ValueError, match="Invalid agent name"):
                WorkGuideClient(agent_name="../path/traversal")

    def test_custom_config_path(self, mock_app_config):
        with (
            patch("workguide.client.reload_app_config") as mock_reload,
            patch("workguide.client.get_app_config", return_value=mock_app_config),
        ):
            WorkGuideClient(config_path="/tmp/custom.yaml")
    ...
    def test_installs_process_subagent_capacity_from_frozen_config(self, mock_app_config):
        ...
        with (
            patch("workguide.client.get_app_config", return_value=mock_app_config),
            patch("workguide.client.configure_subagent_execution_capacity") as configure,
        ):
            WorkGuideClient()
    ...
    def test_checkpointer_stored(self, mock_app_config):
        cp = MagicMock()
        with patch("workguide.client.get_app_config", return_value=mock_app_config):
            c = WorkGuideClient(checkpointer=cp)
    ...
    def test_delta_snapshot_frequency_is_frozen_from_app_config(self, mock_app_config):
        ...
        from workguide.agents import thread_state
...
class TestConfigQueries:
    ...
    def test_list_skills(self, client):
        ...
        with patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]) as mock_load:
    ...
    def test_list_skills_enabled_only(self, client):
        with patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[]) as mock_load:
    ...
    def test_get_memory(self, client):
        ...
        with patch("workguide.agents.memory.get_memory_manager", return_value=mock_mgr):
...
class TestStream:
    ...
    def test_custom_events_are_forwarded(self, client):
        ...
        # see WorkGuideClient.stream() docstring and GitHub issue #1969.
    ...
    def test_full_mode_overwrites_internal_delta_before_agent_creation(self, client):
        from workguide.runtime.checkpoint_mode import (
    ...
    def test_messages_mode_emits_token_deltas(self, client):
        ...
        Regression for bytedance/workguide#1969 — before the fix the client
...
class TestExtractText:
    def test_string(self):
        assert WorkGuideClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert WorkGuideClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert WorkGuideClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert WorkGuideClient._extract_text([]) == ""

    def test_other_type(self):
        assert WorkGuideClient._extract_text(42) == "42"
...
class TestEnsureAgent:
    def test_authorization_filters_framework_tools_and_reuses_provider(self, client, mock_app_config):
        from workguide.authz.provider import AuthzDecision, AuthzReason
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", return_value=MagicMock()) as mock_create_agent,
            patch("workguide.client.build_middlewares", return_value=[]) as mock_build_middlewares,
            patch("workguide.client.apply_prompt_template", return_value="prompt"),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[MagicMock()]),
            patch("workguide.client.build_skill_search_setup", return_value=SimpleNamespace(describe_skill_tool=describe_tool, skill_names=frozenset({"example"}))),
            patch.object(client, "_get_tools", return_value=[safe_tool, denied_tool]),
            patch("workguide.authz.tool_filter.resolve_authorization_provider", return_value=provider),
            patch("workguide.agents.lead_agent.agent.resolve_authorization_provider", return_value=provider),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=None),
        ):
    ...
    def test_authorization_cache_key_uses_complete_principal(self, client, mock_app_config):
        mock_app_config.authorization = AuthorizationConfig(
            ...
            provider=AuthorizationProviderConfig(
                use="workguide.authz.rbac:RbacAuthorizationProvider",
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", side_effect=[MagicMock(), MagicMock()]) as mock_create_agent,
            patch("workguide.client.build_middlewares", return_value=[]),
            patch("workguide.client.apply_prompt_template", return_value="prompt"),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
    ...
    def test_disabled_authorization_preserves_framework_tool_order(self, client, mock_app_config):
        ...
        with (
            ...
            patch("workguide.client.get_enabled_skills_for_config", return_value=[MagicMock()]),
            patch(
                "workguide.client.build_skill_search_setup",
    ...
    def test_creates_agent(self, client):
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("workguide.client.build_middlewares", return_value=[]) as mock_build_middlewares,
            patch("workguide.client.apply_prompt_template", return_value="prompt") as mock_apply_prompt,
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
            patch.object(client, "_get_tools", return_value=[]),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
        ):
    ...
    def test_delta_mode_selects_state_and_normalizes_middleware(self, client):
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("workguide.client.build_middlewares", return_value=[middleware]),
            patch("workguide.client.apply_prompt_template", return_value="prompt"),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
            patch.object(client, "_get_tools", return_value=[]),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=None),
    ...
    def test_uses_default_checkpointer_when_available(self, client):
        ...
        with (
            ...
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
            patch.object(client, "_get_tools", return_value=[]),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=mock_checkpointer),
        ):
            client._ensure_agent(config)
    ...
    def test_injects_custom_middlewares(self, client):
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("workguide.client.build_middlewares", side_effect=fake_build_middlewares),
            patch("workguide.client.apply_prompt_template", return_value="prompt"),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
            patch.object(client, "_get_tools", return_value=[]),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=MagicMock()),
    ...
    def test_deferred_skill_discovery_wired_when_enabled(self, client, mock_app_config):
        ...
        from workguide.skills.types import Skill, SkillCategory
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", return_value=MagicMock()),
            patch("workguide.client.build_middlewares", return_value=[]),
            patch("workguide.client.apply_prompt_template", return_value="prompt") as mock_apply_prompt,
            patch.object(client, "_get_tools", return_value=[]),
            patch("workguide.runtime.checkpointer.get_checkpointer", return_value=None),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[fake_skill]),
        ):
            client._ensure_agent(config)
    ...
    def test_mcp_routing_middleware_wired_when_tool_search_enabled(self, client, mock_app_config):
        ...
        from workguide.agents.middlewares.mcp_routing_middleware import McpRoutingMiddleware
        from workguide.tools.mcp_metadata import tag_mcp_routing, tag_mcp_tool
...
class TestThreadQueries:
    ...
    def test_list_threads_fallback_checkpointer(self, client):
        ...
        with patch("workguide.runtime.checkpointer.provider.get_checkpointer", return_value=mock_checkpointer):
    ...
    def test_get_thread(self, client):
        ...
        with (
            patch.object(client, "_ensure_agent"),
            patch("workguide.client.CheckpointStateAccessor.bind", return_value=accessor),
        ):
    ...
    def test_get_thread_uses_materialized_snapshot_values(self, client):
        ...
        with patch("workguide.client.CheckpointStateAccessor", create=True) as accessor_type:
    ...
    def test_get_thread_fallback_checkpointer(self, client):
        ...
        with (
            patch("workguide.runtime.checkpointer.provider.get_checkpointer", return_value=mock_checkpointer),
            patch.object(client, "_ensure_agent"),
            patch("workguide.client.CheckpointStateAccessor.bind", return_value=accessor),
        ):
...
class TestMcpConfig:
    def test_get_mcp_config(self, client):
        ...
        with patch("workguide.client.get_extensions_config", return_value=ext_config):
    ...
    def test_update_mcp_config(self, client):
        ...
        try:
            ...
            with (
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("workguide.client.get_extensions_config", return_value=current_config),
                patch("workguide.client.reload_extensions_config", return_value=reloaded_config),
            ):
...
class TestSkillsManagement:
    ...
    def test_get_skill_found(self, client):
        ...
        with patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]):
            result = client.get_skill("test-skill")
        assert result is not None
        assert result["name"] == "test-skill"

    def test_get_skill_not_found(self, client):
        with patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[]):
    ...
    def test_update_skill(self, client):
        ...
        try:
            ...
            with (
                patch(
                    "workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills",
                    side_effect=[[skill], [skill], [updated_skill], [updated_skill]],
                ),
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("workguide.client.reload_extensions_config"),
            ):
    ...
    def test_install_skill(self, client, allow_skill_security_scan):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage._default_skill_storage", local_storage),
                patch("workguide.client.get_or_new_user_skill_storage", lambda user_id, **kwargs: local_storage),
            ):
...
class TestMemoryManagement:
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("workguide.config.memory_config.get_memory_config", return_value=config):
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("workguide.config.memory_config.get_memory_config", return_value=config),
            patch("workguide.agents.memory.get_memory_manager", return_value=mock_mgr),
        ):
...
class TestUploads:
    def test_upload_files(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.client.get_uploads_dir", return_value=uploads_dir), patch("workguide.client.ensure_uploads_dir", return_value=uploads_dir):
    ...
    def test_upload_files_reuses_single_executor_inside_event_loop(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.client.get_uploads_dir", return_value=uploads_dir),
                patch("workguide.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("workguide.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("workguide.utils.file_conversion.convert_file_to_markdown", side_effect=fake_convert),
    ...
    def test_upload_files_converted_markdown_uses_unique_names_on_stem_collision(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.client.get_uploads_dir", return_value=uploads_dir),
                patch("workguide.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("workguide.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".docx", ".pdf"}),
                patch("workguide.utils.file_conversion.convert_file_to_markdown", side_effect=fake_convert),
            ):
    ...
    def test_upload_files_failed_conversion_releases_the_claimed_markdown_name(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("workguide.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("workguide.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".docx", ".pdf"}),
                patch("workguide.utils.file_conversion.convert_file_to_markdown", side_effect=convert_failing_on_docx),
            ):
                result = client.upload_files("thread-1", [docx, pdf])
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("workguide.client.get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestArtifacts:
    def test_get_artifact(self, client):
        from workguide.runtime.user_context import get_effective_user_id
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.client.get_paths", return_value=paths):
...
class TestScenarioConfigManagement:
    ...
    def test_mcp_update_then_skill_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=current_config),
                patch("workguide.client.reload_extensions_config", return_value=reloaded_config),
            ):
            ...
            with (
                patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], [toggled]]),
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=ext_config),
                patch("workguide.client.reload_extensions_config"),
            ):
...
class TestScenarioAgentRecreation:
    ...
    def test_different_model_triggers_rebuild(self, client):
        ...
        with (
            patch("workguide.client.create_chat_model"),
            patch("workguide.client.create_agent", side_effect=fake_create_agent),
            patch("workguide.client.build_middlewares", return_value=[]),
            patch("workguide.client.apply_prompt_template", return_value="prompt"),
            patch("workguide.client.get_enabled_skills_for_config", return_value=[]),
            patch.object(client, "_get_tools", return_value=[]),
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.client.get_uploads_dir", side_effect=get_dir), patch("workguide.client.ensure_uploads_dir", side_effect=get_dir):
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client, allow_skill_security_scan):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[installed_skill]):
                skills_result = client.list_skills()
            ...
            with (
                patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[installed_skill], [disabled_skill]]),
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=ext_config),
                patch("workguide.client.reload_extensions_config"),
            ):
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("workguide.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("workguide.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("workguide.utils.file_conversion.convert_file_to_markdown", side_effect=Exception("conversion failed")),
            ):
...
class TestGatewayConformance:
    """Validate that WorkGuideClient return dicts conform to Gateway Pydantic response models.
    ...
    def test_update_mcp_config(self, client, tmp_path):
        ...
        with (
            patch("workguide.client.get_extensions_config", return_value=ext_config),
            patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
            patch("workguide.client.reload_extensions_config", return_value=ext_config),
        ):
    ...
    def test_get_memory_config(self, client):
        ...
        with patch("workguide.config.memory_config.get_memory_config", return_value=mem_cfg):
            result = client.get_memory_config()
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("workguide.config.memory_config.get_memory_config", return_value=mem_cfg),
            patch("workguide.agents.memory.get_memory_manager", return_value=mock_mgr),
        ):
            result = client.get_memory_status()
...
class TestInstallSkillSecurity:
    ...
    def test_zip_bomb_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            from workguide.skills import installer as _installer
            ...
            with (
                patch("workguide.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("workguide.skills.installer.safe_extract_skill_archive", side_effect=patched_extract),
            ):
    ...
    def test_absolute_path_in_archive_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))):
    ...
    def test_invalid_skill_name_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("workguide.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "../evil")),
            ):
    ...
    def test_existing_skill_rejected(self, client, allow_skill_security_scan):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("workguide.skills.validation._validate_skill_frontmatter", return_value=(True, "OK", "dupe-skill")),
                patch("workguide.client.get_or_new_user_skill_storage", return_value=LocalSkillStorage(host_path=str(skills_root))),
            ):
    ...
    def test_invalid_frontmatter_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage._default_skill_storage", LocalSkillStorage(host_path=str(skills_root))),
                patch("workguide.skills.validation._validate_skill_frontmatter", return_value=(False, "Missing name field", "")),
            ):
...
class TestAtomicWriteJson:
    def test_temp_file_cleaned_on_serialization_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with pytest.raises(TypeError):
                WorkGuideClient._atomic_write_json(target, bad_data)
    ...
    def test_happy_path_writes_atomically(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            WorkGuideClient._atomic_write_json(target, data)
...
class TestConfigUpdateErrors:
    def test_update_mcp_config_no_config_file(self, client):
        ...
        with patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=None):
    ...
    def test_update_skill_no_config_file(self, client):
        ...
        with (
            patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", return_value=[skill]),
            patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=None),
        ):
            with pytest.raises(FileNotFoundError, match="Cannot locate"):
    ...
    def test_update_skill_disappears_after_write(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], []]),
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=ext_config),
                patch("workguide.client.reload_extensions_config"),
            ):
...
class TestSerializeMessage:
    def test_system_message(self):
        ...
        result = WorkGuideClient._serialize_message(msg)
    ...
    def test_tool_message_event_preserves_native_artifact(self):
        ...
        result = WorkGuideClient._tool_message_event(msg)
...
class TestBugListUploadsDeadCode:
    ...
    def test_list_uploads_on_fresh_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("workguide.uploads.manager.get_paths", return_value=mock_paths):
...
class TestBugAgentInvalidationInconsistency:
    ...
    def test_update_mcp_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=current_config),
                patch("workguide.client.reload_extensions_config", return_value=reloaded),
            ):
    ...
    def test_update_skill_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("workguide.skills.storage.local_skill_storage.LocalSkillStorage.load_skills", side_effect=[[skill], [updated]]),
                patch("workguide.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("workguide.client.get_extensions_config", return_value=ext_config),
                patch("workguide.client.reload_extensions_config"),
            ):
