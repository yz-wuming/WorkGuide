"""Hybrid schema bootstrap for WorkGuide's application tables.
...
| DB state                              | Action                                  |
|---------------------------------------|-----------------------------------------|
| empty (no WorkGuide tables)            | ``create_all`` + ``alembic stamp head`` |
| legacy (WorkGuide tables, no alembic)  | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
WorkGuide-owned table. ``create_all`` runs first because stamping at
...
def _get_alembic_config(engine: AsyncEngine, *, postgres_schema: str = "") -> AlembicConfig:
    ...
    When *postgres_schema* is set it is forwarded as the ``workguide_pg_schema``
    ...
    if postgres_schema:
        cfg.set_main_option("workguide_pg_schema", postgres_schema)
    return cfg
...
def _reflect_state(sync_conn: Any) -> dict[str, bool]:
    ...
    - ``has_workguide_tables``: True iff at least one table that ``Base.metadata``
    ...
    try:
        ...
        logger.debug("workguide.persistence.models not found; metadata may be incomplete")
    ...
    return {
        ...
        "has_workguide_tables": bool(reflected & metadata_tables),
    }
...
def _decide_state(state: dict[str, bool]) -> str:
    ...
    if state["has_alembic_version"]:
        return "versioned"
    if not state["has_workguide_tables"]:
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all WorkGuide-owned tables on *sync_conn*."""
    ...
    try:
        ...
        logger.debug("workguide.persistence.models not found; bootstrap will create empty schema")
...
def _run_baseline_create_all_sync(sync_conn: Any) -> None:
    ...
    try:
        ...
        logger.debug("workguide.persistence.models not found; baseline backfill may be incomplete")
...
async def _sqlite_lock(engine: AsyncEngine):
    ...
    WorkGuide. The 30s ``busy_timeout`` plus idempotent revisions cover the
