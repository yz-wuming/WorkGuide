...
Runs each WorkGuide sandbox as a [BoxLite](https://github.com/boxlite-ai/boxlite)
...
[#3439](https://github.com/bytedance/workguide/issues/3439) and
[#3213](https://github.com/bytedance/workguide/issues/3213); discussion in
[#3936](https://github.com/bytedance/workguide/issues/3936).
...
The `boxlite` package is an optional WorkGuide harness extra, not part of the
...
hosts, such as Windows, should use another sandbox provider or run WorkGuide from
a supported Linux/macOS environment.

**Host requirement:** BoxLite boots micro-VMs, so a Linux host needs KVM — i.e.
nested virtualization when WorkGuide runs inside a cloud VM. macOS uses
Hypervisor.framework. This is the main deployment constraint to weigh vs. the
container-based providers.

## Design

WorkGuide's `Sandbox` contract is synchronous; BoxLite's SDK is async-native and
...
inside the box and reuse `workguide.sandbox.search`, mirroring `e2b_sandbox`:
