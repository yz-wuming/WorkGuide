...
(:mod:`workguide.persistence.agents`), so both the ``file`` backend (per-user
...
on the store's :meth:`~workguide.persistence.agents.base.AgentStore.signature`
...
from workguide.config.agents_config import AgentConfig, GitHubTriggerConfig
