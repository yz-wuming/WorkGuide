"""Pure-argument factory for WorkGuide agents.

``create_workguide_agent`` accepts plain Python arguments — it does not load
...
from workguide.agents.features import RuntimeFeatures
...
from workguide.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
from workguide.agents.thread_state import adapt_state_schema_for_mode, get_thread_state_schema, normalize_middleware_state_schemas
from workguide.config.database_config import CheckpointChannelMode
from workguide.tools.builtins import ask_clarification_tool
...
if TYPE_CHECKING:
    ...
    from workguide.subagents.runtime import SubagentRuntime
...
def create_workguide_agent(
    ...
    *,
    ...
    """Create a WorkGuide agent from plain Python arguments.
    ...
    subagent_runtime:
        ...
        caller-managed durable batch worker without Gateway/WorkGuideClient
    ...
    if checkpoint_channel_mode == "delta" and checkpointer is not None:
        raise ValueError(
            "create_workguide_agent does not support checkpoint_channel_mode='delta' with a checkpointer: "
            "persisted graphs built here bypass checkpoint mode marker injection and the fail-closed "
            "compatibility gate (see workguide.runtime.checkpoint_mode), so a mixed-mode store would "
            "silently corrupt thread state.  Use the guarded application paths (make_lead_agent or "
            "WorkGuideClient) for delta persistence; delta without a checkpointer is ephemeral and allowed."
        )
    ...
    if subagent_runtime is not None and subagent_runtime.batch_config is not None and subagent_runtime.batch_submitter is None:
        raise RuntimeError("The explicit durable batch worker is not running; await subagent_runtime.start() or enter it with 'async with' before calling create_workguide_agent")
    if extra_middleware:
    ...
    *,
    ...
    if feat.memory is not False:
        if isinstance(feat.memory, AgentMiddleware):
            ...
            from workguide.config.memory_config import get_memory_config, should_use_memory_tools
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from workguide.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        if isinstance(feat.subagent, AgentMiddleware):
            ...
            from workguide.config.subagents_config import DEFAULT_MAX_TOTAL_SUBAGENTS_PER_RUN
        ...
        from workguide.tools.builtins import task_tool

        if subagent_runtime is None:
            extra_tools.append(task_tool)
        else:
            from workguide.tools.builtins.task_tool import bind_task_tool
        ...
        if subagent_runtime is not None and subagent_runtime.batch_submitter is not None:
            from workguide.tools.builtins.batch_task_tool import bind_batch_tools
            ...
            from workguide.subagents.batch_runtime import is_subagent_batch_runtime_available

            if is_subagent_batch_runtime_available():
                from workguide.tools.builtins import batch_status, batch_task, cancel_batch
