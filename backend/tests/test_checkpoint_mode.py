...
def test_inject_delta_mode_sets_internal_key_and_metadata_marker() -> None:
    ...
    assert config["configurable"]["__workguide_checkpoint_channel_mode"] == "delta"
...
def test_inject_full_mode_does_not_claim_delta_metadata() -> None:
    ...
    assert config["configurable"]["__workguide_checkpoint_channel_mode"] == "full"
...
async def test_full_mode_accessor_rejects_real_delta_checkpoint_on_sqlite(tmp_path) -> None:
    ...
    from workguide.runtime.checkpoint_state import CheckpointStateAccessor, build_state_mutation_graph
...
def test_yaml_mode_change_is_rejected_when_graph_is_reconstructed(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    from workguide.agents.lead_agent import agent as lead_agent
    ...
    def write_config(mode: str) -> None:
        config_path.write_text(
            "\n".join(
                (
                    "sandbox:",
                    "  use: workguide.sandbox.local.provider:LocalSandboxProvider",
                    "database:",
...
async def test_gateway_runtime_rejects_mode_different_from_frozen_process(
    ...
    monkeypatch.setattr(
        "workguide.runtime.checkpointer.async_provider.make_checkpointer",
        resource,
    )
    monkeypatch.setattr("workguide.runtime.make_stream_bridge", resource)
    monkeypatch.setattr("workguide.runtime.make_store", resource)
    monkeypatch.setattr(
        "workguide.persistence.engine.init_engine_from_config",
        noop,
    )
    monkeypatch.setattr("workguide.persistence.engine.close_engine", noop)
    monkeypatch.setattr(
        "workguide.persistence.engine.get_session_factory",
        lambda: None,
    )
    monkeypatch.setattr(
        "workguide.runtime.events.store.make_run_event_store",
        lambda _config: object(),
    )
    monkeypatch.setattr(
        "workguide.persistence.thread_meta.make_thread_store",
...
def test_direct_langgraph_request_cannot_select_delta_in_full_process(
    ...
    app_config = AppConfig.model_validate(
        {
            "sandbox": {"use": "workguide.sandbox.local.provider:LocalSandboxProvider"},
