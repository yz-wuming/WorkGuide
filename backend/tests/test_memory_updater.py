...
from workguide.agents.memory.backends.deermem.deermem.core.prompt import format_conversation_for_update
from workguide.agents.memory.backends.deermem.deermem.core.storage import (
    MemoryManifestRevisionConflict,
    MemoryStorage,
)
from workguide.agents.memory.backends.deermem.deermem.core.updater import (
    MemoryUpdater,
...
from workguide.agents.memory.manager import LangfuseMemoryCallbacks
from workguide.trace_context import get_current_trace_id, request_trace_context
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {**_DURABLE_USER_FACT, "content": "User works on WorkGuide", "category": "context", "confidence": 0.87},
        ],
    }
...
def test_clear_memory_data_clears_facts_and_preserves_shared_summaries() -> None:
    ...
    memory["user"]["workContext"]["summary"] = "Working on WorkGuide"
    ...
    assert result["facts"] == []
    assert result["user"]["workContext"]["summary"] == "Working on WorkGuide"
...
def test_import_memory_data_saves_and_returns_imported_memory() -> None:
    imported_memory = _make_memory(
        facts=[
            {
                "id": "fact_import",
                "content": "User works on WorkGuide.",
                "category": "context",
                "confidence": 0.87,
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_schema_guard_ignores_invalid_update_fields(self):
        ...
        response = (
            ...
            '{"content": "User works on WorkGuide", "category": "context", "confidence": 0.91, '
        ...
        assert result is True
        assert [fact["content"] for fact in storage.memory["facts"]] == ["User works on WorkGuide"]
    ...
    def test_sync_update_memory_returns_false_when_executor_down(self):
        ...
        with (
            patch(
                "workguide.agents.memory.backends.deermem.deermem.core.updater._SYNC_MEMORY_UPDATER_EXECUTOR.submit",
...
class TestUserIdForwarding:
    ...
    def test_sync_update_injects_workguide_trace_metadata_when_langfuse_enabled(self, monkeypatch):
        ...
        assert metadata["workguide_trace_id"] == "memory-trace-1"
...
class TestSyncUpdateBindsTraceContextVar:
    ...
    def test_binds_workguide_trace_id_into_contextvar(self) -> None:
