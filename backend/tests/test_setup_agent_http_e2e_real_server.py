...
@pytest.fixture
def isolated_work_guide_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Stand up an isolated WorkGuide data root + config under tmp_path.

    - Sets ``WORKGUIDE_HOME`` so paths land under tmp_path, not the real
      ``.workguide`` directory.
    - Stages a copy of the project's ``config.yaml`` (or ``config.example.yaml``
      ...
      ``WORKGUIDE_CONFIG_PATH`` to it, so lifespan boot doesn't depend on the
    ...
    # config to tmp_path and pin ``WORKGUIDE_CONFIG_PATH`` to it.
    staged_config = tmp_path / "config.yaml"
    staged_config.write_text(_MINIMAL_CONFIG_YAML, encoding="utf-8")
    monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(staged_config))
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    so they pick up our test-only ``WORKGUIDE_HOME`` and sqlite path:

    - ``workguide.config.app_config`` caches the parsed ``config.yaml``.
    - ``workguide.config.paths`` caches the ``Paths`` singleton derived from
      ``WORKGUIDE_HOME`` at first access.
    - ``workguide.persistence.engine`` caches the SQLAlchemy engine and
    ...
    from workguide.persistence import engine as engine_module
...
@pytest.fixture
def isolated_app(isolated_work_guide_home: Path, monkeypatch: pytest.MonkeyPatch):
    """Build a fresh FastAPI app inside a clean WORKGUIDE_HOME.
    ...
    # Re-resolve the config from the test-only WORKGUIDE_HOME and pin its
    # sqlite path into tmp_path so the lifespan-time engine init lands there.
    from workguide.config import app_config as app_config_module

    cfg = app_config_module.get_app_config()
    cfg.database.sqlite_dir = str(isolated_work_guide_home / "db")
...
def test_real_http_create_agent_lands_in_authenticated_user_dir(
    ...
    """
    # ``workguide.agents.lead_agent.agent`` imports ``create_chat_model`` with
    # ``from workguide.models import create_chat_model`` at module load time,
    ...
    # ``workguide.models.create_chat_model`` would be too late.
    ...
    with (
        patch(
            "workguide.agents.lead_agent.agent.create_chat_model",
        ...
        expected_dir = isolated_work_guide_home / "users" / auth_uid / "agents" / agent_name
        default_dir = isolated_work_guide_home / "users" / "default" / "agents" / agent_name
        ...
        assert _wait_for_file(expected_dir / "SOUL.md", timeout=15.0), (
            ...
            f"tmp tree: {sorted(str(p.relative_to(isolated_work_guide_home)) for p in isolated_work_guide_home.rglob('SOUL.md'))}. "
