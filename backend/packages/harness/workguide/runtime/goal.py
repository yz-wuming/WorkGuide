...
Gateway runs and thin API surfaces. It intentionally lives in ``workguide`` so
...
import workguide.utils.llm_text as llm_text
from workguide.agents.goal_state import GoalBlocker, GoalEvaluation, GoalState
...
def create_goal_evaluator_model(
    *,
    ...
    ``WorkGuideClient.stream``, which attach ``build_tracing_callbacks()`` at
...
def _resolve_environment() -> str | None:
    return os.environ.get("WORKGUIDE_ENV") or os.environ.get("ENVIRONMENT")
...
async def evaluate_goal_completion(
    ...
    *,
    ...
    ``thread_id``/``user_id``/``workguide_trace_id`` are forwarded to Langfuse
...
def make_goal_continuation_message(goal: GoalState, evaluation: GoalEvaluation) -> HumanMessage:
    ...
    return HumanMessage(
        ...
        additional_kwargs={
            "hide_from_ui": True,
            "workguide_goal_continuation": True,
        },
    )
