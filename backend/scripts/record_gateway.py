...
Used by ``frontend/playwright.record.config.ts``. Env:
  ...
  WORKGUIDE_RECORD_OUT               - JSONL path to append captured turns to
...
def _install_capture(out_path: Path) -> None:
    ...
    import workguide.models.factory as factory_mod
...
def main() -> int:
    ...
    record_out = os.environ.get("WORKGUIDE_RECORD_OUT")
    if not record_out:
        print("ERROR: set WORKGUIDE_RECORD_OUT to the JSONL path to append captured turns to", file=sys.stderr)
        return 2
    ...
    # WORKGUIDE_HOME can't leak in and shift prompt-affecting paths/skills.
    os.environ["WORKGUIDE_HOME"] = str(home)
    os.environ["WORKGUIDE_CONFIG_PATH"] = str(cfg)
    os.environ["WORKGUIDE_EXTENSIONS_CONFIG_PATH"] = str(prepare_hermetic_extras(home))
