...
compare candidates with the same WorkGuide workload.
...
python scripts/sandbox_memory_profile.py \
  --namespace workguide \
  --selector app=workguide-sandbox \
  --sample empty \
  --include-processes \
  --format markdown
```
...
same WorkGuide workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible WorkGuide workload data that demonstrates the target memory
