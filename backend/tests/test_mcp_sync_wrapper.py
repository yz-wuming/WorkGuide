...
def test_mcp_tool_sync_wrapper_generation():
    ...
    with (
        ...
        patch("workguide.config.extensions_config.ExtensionsConfig.from_file"),
        patch(
...
def test_mcp_tool_loading_skips_failed_server():
    ...
    with (
        patch("langchain_mcp_adapters.client.MultiServerMCPClient", return_value=mock_client_instance),
        patch("workguide.config.extensions_config.ExtensionsConfig.from_file", return_value=MagicMock(model_extra={})),
        patch("workguide.mcp.tools.build_servers_config", return_value={"good-server": {}, "bad-server": {}}),
        patch("workguide.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
        patch("workguide.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("workguide.mcp.tools.logger.warning") as mock_warning,
    ):
        tools = asyncio.run(get_mcp_tools())
...
def test_mcp_tool_sync_wrapper_exception_logging():
    ...
    with patch("workguide.tools.sync.logger.error") as mock_log_error:
