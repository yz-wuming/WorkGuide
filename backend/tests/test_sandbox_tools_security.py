...
from workguide.sandbox.exceptions import SandboxError
from workguide.sandbox.tools import (
    VIRTUAL_PATH_PREFIX,
    _apply_cwd_prefix,
...
_THREAD_DATA = {
    "workspace_path": "/tmp/workguide/threads/t1/user-data/workspace",
    "uploads_path": "/tmp/workguide/threads/t1/user-data/uploads",
    "outputs_path": "/tmp/workguide/threads/t1/user-data/outputs",
}
...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert Path(replace_virtual_path("/mnt/user-data/workspace/a.txt", _THREAD_DATA)).as_posix() == "/tmp/workguide/threads/t1/user-data/workspace/a.txt"
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/workguide/threads/t1/user-data"
...
def test_replace_virtual_path_preserves_trailing_slash() -> None:
    ...
    assert result == "/tmp/workguide/threads/t1/user-data/workspace/"
...
def test_replace_virtual_path_preserves_trailing_slash_windows_style() -> None:
    ...
    """
    win_thread_data = {
        "workspace_path": r"C:\workguide\threads\t1\user-data\workspace",
        "uploads_path": r"C:\workguide\threads\t1\user-data\uploads",
        "outputs_path": r"C:\workguide\threads\t1\user-data\outputs",
    }
...
def test_replace_virtual_path_preserves_windows_style_for_nested_subdir_trailing_slash() -> None:
    ...
    assert result == "C:\\workguide\\threads\\t1\\user-data\\workspace\\subdir\\"
    assert "/" not in result, f"Mixed separators in Windows path: {result!r}"
...
def test_replace_virtual_paths_in_command_preserves_trailing_slash() -> None:
    ...
    assert "/tmp/workguide/threads/t1/user-data/workspace/" in result, f"Trailing slash lost in: {result!r}"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/workguide/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/workguide/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        patch("workguide.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("workguide.sandbox.tools._get_skills_host_path", return_value="/home/user/workguide/skills"),
    ):
        output = "Reading: /home/user/workguide/skills/public/bootstrap/SKILL.md"
        masked = mask_local_paths_in_output(output, _THREAD_DATA)

        assert "/home/user/workguide/skills" not in masked
...
def test_mask_local_paths_does_not_match_inside_longer_sibling(suffix: str) -> None:
    ...
    with (
        ...
        output = f"found /home/user/workguide/skills{suffix}"
...
def test_mask_local_paths_does_not_match_inside_longer_acp_sibling(suffix: str) -> None:
    ...
    """
    acp_host = "/home/user/.workguide/acp-workspace"
    with patch("workguide.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
...
def test_mask_local_paths_user_data_sibling_is_carried_by_the_virtual_root(suffix: str) -> None:
    ...
    """
    masked = mask_local_paths_in_output(f"wrote /tmp/workguide/threads/t1/user-data/outputs{suffix}", _THREAD_DATA)

    assert masked == f"wrote /mnt/user-data/outputs{suffix}"
    assert replace_virtual_path(f"/mnt/user-data/outputs{suffix}", _THREAD_DATA) == f"/tmp/workguide/threads/t1/user-data/outputs{suffix}"
...
def test_mask_local_paths_still_matches_base_before_non_slash_boundaries(boundary: str, expected: str) -> None:
    ...
    with (
        ...
        masked = mask_local_paths_in_output(f"root is /home/user/workguide/skills{boundary}", None)
...
def test_mask_local_paths_translates_a_bare_base_at_end_of_output(prefix: str) -> None:
    ...
    with (
        ...
        masked = mask_local_paths_in_output(f"{prefix}/home/user/workguide/skills", None)
...
def test_mask_local_paths_compiled_patterns_are_cached() -> None:
    ...
    sources = (("/tmp/workguide/threads/t1/user-data/workspace", "/mnt/user-data/workspace"),)
...
def test_mask_local_paths_stable_across_repeated_and_batched_calls() -> None:
    ...
    output = "a /tmp/workguide/threads/t1/user-data/workspace/x.txt and /tmp/workguide/threads/t1/user-data/outputs/y.log"
    once = mask_local_paths_in_output(output, _THREAD_DATA)
    twice = mask_local_paths_in_output(once, _THREAD_DATA)
    assert "/tmp/workguide/threads/t1/user-data" not in once
...
def test_mask_local_paths_no_thread_data_still_masks_skills() -> None:
    ...
    with (
        ...
        masked = mask_local_paths_in_output("Reading: /home/user/workguide/skills/a/b.md", None)
        assert "/home/user/workguide/skills" not in masked
...
def test_mask_local_paths_hides_global_integration_skill_paths(tmp_path: Path) -> None:
    ...
    with (
        patch("workguide.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("workguide.sandbox.tools._get_skills_host_path", return_value="/home/user/workguide/skills"),
        patch("workguide.config.paths.get_paths", return_value=paths),
        patch("workguide.runtime.user_context.get_effective_user_id", return_value="alice"),
    ):
        masked = mask_local_paths_in_output(output, _THREAD_DATA)
...
def test_validate_local_tool_path_prioritizes_user_data_before_custom_mounts() -> None:
    from workguide.config.sandbox_config import VolumeMountConfig

    mounts = [
        VolumeMountConfig(host_path="/tmp/host-user-data", container_path=VIRTUAL_PATH_PREFIX, read_only=False),
    ]
    with patch("workguide.sandbox.tools._get_custom_mounts", return_value=mounts):
...
def test_validate_local_tool_path_rejects_traversal_in_skills() -> None:
    ...
    with patch("workguide.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
        with pytest.raises(PermissionError, match="path traversal"):
...
def test_validate_local_tool_path_rejects_none_thread_data() -> None:
    ...
    from workguide.sandbox.exceptions import SandboxRuntimeError
...
def test_resolve_skills_path_resolves_correctly() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/workguide/skills/public/bootstrap/SKILL.md"
...
def test_resolve_skills_path_resolves_root() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/workguide/skills"
...
def test_resolve_skills_path_raises_when_not_configured() -> None:
    ...
    with (
        patch("workguide.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("workguide.sandbox.tools._get_skills_host_path", return_value=None),
    ):
...
def test_replace_virtual_paths_in_command_does_not_replace_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/workguide/skills" not in result
...
def test_replace_virtual_paths_in_command_replaces_user_data_only() -> None:
    ...
    with (
        patch("workguide.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("workguide.sandbox.tools._get_skills_host_path", return_value="/home/user/skills"),
    ):
        ...
        assert "/tmp/workguide/threads/t1/user-data/workspace/out.txt" in result
...
def test_replace_virtual_paths_in_command_does_not_rewrite_prefix_siblings(sibling: str) -> None:
    ...
    assert "/tmp/workguide/threads/t1" not in result
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ls /mnt/user-data", "ls /tmp/workguide/threads/t1/user-data"),
        ("ls /mnt/user-data && pwd", "ls /tmp/workguide/threads/t1/user-data && pwd"),
        ("PYTHONPATH=/mnt/user-data:/opt x", "PYTHONPATH=/tmp/workguide/threads/t1/user-data:/opt x"),
        ("echo '/mnt/user-data, done'", "echo '/tmp/workguide/threads/t1/user-data, done'"),
        # Real children still translate.
        ("cat /mnt/user-data/workspace/a.txt", "cat /tmp/workguide/threads/t1/user-data/workspace/a.txt"),
    ],
)
...
def test_bash_tool_rejects_host_bash_when_local_sandbox_default(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "workguide.sandbox.tools.ensure_sandbox_initialized",
        lambda runtime: SimpleNamespace(execute_command=lambda command: pytest.fail("host bash should not execute")),
    )
    monkeypatch.setattr("workguide.sandbox.tools.is_host_bash_allowed", lambda: False)
...
def test_bash_tool_blocks_relative_traversal_before_host_execution(monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("workguide.sandbox.tools.is_host_bash_allowed", lambda: True)
...
def test_validate_local_tool_path_skills_custom_container_path() -> None:
    ...
    with patch("workguide.sandbox.tools._get_skills_container_path", return_value="/custom/skills"):
        # Should not raise
        validate_local_tool_path(
...
def test_resolve_acp_workspace_path_resolves_correctly(tmp_path: Path) -> None:
    ...
    acp_dir.mkdir()
    with patch("workguide.sandbox.tools._get_acp_workspace_host_path", return_value=str(acp_dir)):
...
def test_resolve_acp_workspace_path_raises_when_not_available() -> None:
    ...
    with patch("workguide.sandbox.tools._get_acp_workspace_host_path", return_value=None):
...
def test_replace_virtual_paths_in_command_does_not_replace_acp_workspace() -> None:
    ...
    with patch("workguide.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
        ...
        # User-data paths should still be resolved
        assert "/mnt/user-data" not in result
        assert "/tmp/workguide/threads/t1/user-data/outputs/hello.py" in result
...
def test_apply_cwd_prefix_prepends_workspace() -> None:
    ...
    assert "/tmp/workguide/threads/t1/user-data/workspace" in result
...
def test_validate_local_bash_command_paths_allows_mcp_filesystem_paths() -> None:
    ...
    with patch("workguide.config.extensions_config.get_extensions_config", return_value=mock_config):
        ...
        with patch("workguide.config.extensions_config.get_extensions_config", return_value=disabled_config):
            with pytest.raises(PermissionError, match="Unsafe absolute paths"):
                validate_local_bash_command_paths("ls /mnt/d/workspace", _THREAD_DATA)
...
def test_is_custom_mount_path_recognises_configured_mounts() -> None:
    with patch("workguide.sandbox.tools._get_custom_mounts", return_value=_mock_custom_mounts()):
...
def test_get_custom_mounts_caching(monkeypatch, tmp_path) -> None:
    ...
    from workguide.config.sandbox_config import SandboxConfig, VolumeMountConfig
    ...
    mock_sandbox = SandboxConfig(use="workguide.sandbox.local:LocalSandboxProvider", mounts=mounts)
    mock_config = SimpleNamespace(sandbox=mock_sandbox)

    with patch("workguide.config.get_app_config", return_value=mock_config):
...
def test_str_replace_parallel_updates_should_preserve_both_edits(monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda runtime: sandbox)
    monkeypatch.setattr("workguide.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("workguide.sandbox.tools.is_local_sandbox", lambda runtime: False)
...
def test_write_file_tool_bounds_large_oserror_and_masks_local_paths(monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda runtime: sandbox)
    monkeypatch.setattr("workguide.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
    monkeypatch.setattr("workguide.sandbox.tools.is_local_sandbox", lambda runtime: True)
    monkeypatch.setattr("workguide.sandbox.tools.get_thread_data", lambda runtime: _THREAD_DATA)
    monkeypatch.setattr("workguide.sandbox.tools.validate_local_tool_path", lambda path, thread_data: None)
    monkeypatch.setattr(
        "workguide.sandbox.tools._resolve_and_validate_user_data_path",
    ...
    assert "/tmp/workguide/threads/t1/user-data/workspace" not in result
...
def test_write_file_tool_handles_sandbox_init_failure(monkeypatch) -> None:
    ...
    runtime = SimpleNamespace(state={}, context={"thread_id": "thread-1"}, config={})
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", raise_sandbox_error)
    monkeypatch.setattr("workguide.sandbox.tools.is_local_sandbox", lambda runtime: False)
...
def test_file_operation_lock_memory_cleanup() -> None:
    ...
    from workguide.sandbox.file_operation_lock import _FILE_OPERATION_LOCKS, get_file_operation_lock

    class MockSandbox:
        id = "test_cleanup_sandbox"

    test_path = "/tmp/workguide/memory_leak_test_file.txt"
