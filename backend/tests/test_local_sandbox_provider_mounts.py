...
from workguide.sandbox.local.local_sandbox import LocalSandbox, PathMapping
...
class TestReadOnlyPath:
    ...
    def test_bash_write_to_projected_copy_does_not_mutate_source(self, tmp_path):
        ...
        from workguide.skills.projection import _copy_into_view
...
class TestMultipleMounts:
    ...
    def test_execute_command_path_replacement(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("workguide.sandbox.local.local_sandbox.subprocess.Popen", mock_popen)
        monkeypatch.setattr("workguide.sandbox.local.local_sandbox.LocalSandbox._get_shell", lambda self: "/bin/sh")
...
class TestLocalSandboxProviderMounts:
    def test_thread_mappings_mount_per_user_integration_projections(self, tmp_path):
        ...
        with (
            patch("workguide.config.get_app_config", return_value=config),
    ...
    def test_setup_path_mappings_uses_configured_skills_container_path_as_reserved_prefix(self, tmp_path):
        ...
        config = SimpleNamespace(
            skills=SimpleNamespace(container_path="/custom-skills", get_skills_path=lambda: skills_dir, use="workguide.skills.storage.local_skill_storage:LocalSkillStorage"),
            sandbox=sandbox_config,
        )

        with patch("workguide.config.get_app_config", return_value=config):
    ...
    def test_setup_path_mappings_skips_relative_host_path(self, tmp_path):
        ...
        config = SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills", get_skills_path=lambda: skills_dir, use="workguide.skills.storage.local_skill_storage:LocalSkillStorage"),
            sandbox=sandbox_config,
        )
    ...
    def test_setup_path_mappings_logs_actionable_error_for_missing_host_path(self, tmp_path, caplog):
        ...
        with caplog.at_level("ERROR", logger="workguide.sandbox.local.local_sandbox_provider"):
            with patch("workguide.config.get_app_config", return_value=config):
                provider = LocalSandboxProvider()
...
class TestLocalSandboxProviderResetClearsSingleton:
    ...
    def test_reset_sandbox_provider_clears_local_singleton(self, tmp_path):
        ...
        from workguide.sandbox import local as local_module
        from workguide.sandbox.local import local_sandbox_provider as lsp_module
        from workguide.sandbox.sandbox_provider import (
        ...
        try:
            with patch("workguide.sandbox.sandbox_provider.get_app_config", return_value=first_cfg), patch("workguide.config.get_app_config", return_value=first_cfg):
            ...
            with patch("workguide.sandbox.sandbox_provider.get_app_config", return_value=second_cfg), patch("workguide.config.get_app_config", return_value=second_cfg):
    ...
    def test_shutdown_sandbox_provider_clears_local_singleton(self, tmp_path):
        ...
        try:
            with patch("workguide.sandbox.sandbox_provider.get_app_config", return_value=cfg), patch("workguide.config.get_app_config", return_value=cfg):
                provider = get_sandbox_provider()
                provider.acquire()
