"""``TenkiSandbox`` — WorkGuide :class:`Sandbox` backed by a Tenki cloud sandbox.
...
with the shared ``workguide.sandbox.search`` helpers, the same approach as
...
from workguide.sandbox.search import GrepMatch, path_matches, should_ignore_path, truncate_line
...
# ``/mnt`` is root-owned, so WorkGuide's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """WorkGuide Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: WorkGuide-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map WorkGuide virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        WorkGuide passes a bash command *string*; it runs through ``sh -lc``.
