"""DeerMem's own storage path resolution (no workguide ``get_paths`` / ``AGENT_NAME_PATTERN``).
...
# agent_name validation (inlined; was workguide's AGENT_NAME_PATTERN).
...
def memory_file_path(
    ...
    *,
    ...
    (``$DEERMEM_DATA_DIR`` / ``~/.deermem/``). The host (workguide factory)
