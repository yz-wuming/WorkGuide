...
The value stored here is WorkGuide's request-level correlation id. It is
separate from Langfuse's own trace id and from WorkGuide run ids.
"""
...
WORKGUIDE_TRACE_METADATA_KEY: Final[str] = "workguide_trace_id"
_MAX_TRACE_ID_LENGTH: Final[int] = 512

_current_trace_id: Final[ContextVar[str | None]] = ContextVar("workguide_current_trace_id", default=None)
_trace_id_from_request_header: Final[ContextVar[bool]] = ContextVar(
    "workguide_trace_id_from_request_header",
    default=False,
)
...
def resolve_workguide_trace_id(metadata_trace_id: object) -> str | None:
    """Resolve the effective ``workguide_trace_id`` for a run.

    When Gateway ``TraceMiddleware`` bound a valid inbound ``X-Trace-Id``,
    that value wins over ``config.metadata.workguide_trace_id`` so logs,
