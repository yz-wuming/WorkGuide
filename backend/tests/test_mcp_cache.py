"""Tests for MCP tools cache staleness detection (``workguide.mcp.cache``).
...
The fix mirrors ``workguide.config.app_config``'s ``(path, (mtime, size,
...
import workguide.mcp.cache as cache_module
...
@pytest.fixture()
def cache_globals():
    """Snapshot/restore ``workguide.mcp.cache`` module globals and reset the lock."""
...
def _initialize_against(monkeypatch, config_path: Path) -> None:
    ...
    monkeypatch.setattr("workguide.mcp.tools.get_mcp_tools", _fake_get_mcp_tools)
...
def test_config_path_switch_is_stale(cache_globals, monkeypatch, tmp_path):
    ...
    # The resolver now points at cfg_b (e.g. WORKGUIDE_EXTENSIONS_CONFIG_PATH
...
def test_config_deleted_after_init_via_real_env_resolution_does_not_raise(cache_globals, monkeypatch, tmp_path):
    ...
    ``WORKGUIDE_EXTENSIONS_CONFIG_PATH`` (exactly how Docker dev/prod point at
    ...
    cfg = tmp_path / "extensions_config.json"
    _write_extensions_config(cfg, {"srv1": _server()})
    _initialize_against(monkeypatch, cfg)  # sets WORKGUIDE_EXTENSIONS_CONFIG_PATH=cfg
    assert cache_module._config_signature is not None  # guard: had a real signature
