...
Run explicitly with real credentials:
    ...
    RUN_WORKGUIDE_LEDGER_LIVE=1 PYTHONPATH=. uv run pytest tests/test_delegation_ledger_live.py -v -s
"""
...
from workguide.config.app_config import reload_app_config, reset_app_config, set_app_config
...
elif os.environ.get("RUN_WORKGUIDE_LEDGER_LIVE") != "1":
    _skip_reason = "Set RUN_WORKGUIDE_LEDGER_LIVE=1 to run this real-model test"
...
def real_subagent_executor():
    ...
    original_executor_module = sys.modules.get("workguide.subagents.executor")
    original_subagent_attrs: dict[str, Any] = {}
    original_task_tool_attrs: dict[str, Any] = {}

    import workguide.subagents as subagents_pkg
    ...
    task_tool_module = sys.modules.get("workguide.tools.builtins.task_tool")
    ...
    if original_executor_module is not None:
        sys.modules["workguide.subagents.executor"] = original_executor_module
    else:
        sys.modules.pop("workguide.subagents.executor", None)
...
def _stream_events(client: WorkGuideClient, thread_id: str, prompt: str) -> list[StreamEvent]:
...
def _state_values(client: WorkGuideClient, thread_id: str) -> dict[str, Any]:
