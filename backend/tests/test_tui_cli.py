...
from workguide.tui.cli import LaunchPlan, build_parser, plan_launch
...
def test_top_level_help_points_to_extension_management():
    assert "workguide extensions --help" in build_parser().format_help()
...
def test_env_var_forces_tui():
    p = plan([], stdin_tty=False, stdout_tty=False, env={"WORKGUIDE_TUI": "1"})
    assert p.mode == "tui"
...
def test_transparent_env_is_carried_to_tui_plan():
    p = plan([], env={"WORKGUIDE_TUI_TRANSPARENT": "yes"})
    assert p.mode == "tui"
    assert p.transparent is True
