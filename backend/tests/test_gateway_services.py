...
def _make_start_run_request(run_manager, *, thread_store=None, auth_source=None):
    ...
    from workguide.persistence.thread_meta.memory import MemoryThreadMetaStore
...
async def test_sse_consumer_emits_gap_without_cancelling_run():
    ...
    from workguide.runtime import DisconnectMode, MemoryStreamBridge, RunManager, RunStatus
...
def test_to_langgraph_stream_modes_maps_alias_and_deduplicates(raw, expected):
    from workguide.runtime.stream_modes import to_langgraph_stream_modes
...
def test_normalize_input_strips_external_original_user_content(forged_original):
    from app.gateway.services import normalize_input
    from workguide.utils.messages import ORIGINAL_USER_CONTENT_KEY
...
def test_normalize_input_strips_external_dynamic_context_metadata():
    ...
    from app.gateway.services import normalize_input
    from workguide.agents.middlewares.dynamic_context_middleware import _DYNAMIC_CONTEXT_REMINDER_KEY, _REMINDER_DATE_KEY
...
def test_normalize_input_strips_external_view_image_context_marker():
    from app.gateway.services import normalize_input
    from workguide.agents.middlewares.view_image_middleware import _IMAGE_CONTEXT_MESSAGE_MARKER_KEY
...
def test_normalize_input_strips_external_tool_receipt():
    ...
    from app.gateway.services import normalize_input
    from workguide.agents.middlewares.tool_receipt import TOOL_RECEIPT_KEY
...
def test_build_run_config_ceiling_is_configurable(_stub_app_config):
    ...
    set_app_config(AppConfig.model_validate({"sandbox": {"use": "workguide.sandbox.local:LocalSandboxProvider"}, "max_recursion_limit": 300}))
    try:
        config = build_run_config("thread-1", {"recursion_limit": 100_000_000}, None)
...
def test_resolve_agent_factory_returns_the_explicit_lead_assembly_factory():
    ...
    from workguide.agents.lead_agent.agent import assemble_lead_agent
...
def test_build_checkpoint_state_accessor_uses_frozen_mode_and_binds_runtime_persistence(
    ...
    from workguide.runtime.checkpoint_mode import CHECKPOINT_MODE_METADATA_KEY, INTERNAL_CHECKPOINT_MODE_KEY
...
def test_build_checkpoint_state_accessor_accepts_lead_agent_assembly_factory(_stub_app_config):
    ...
    from app.gateway.services import build_checkpoint_state_accessor
    from workguide.agents.lead_agent.agent import LeadAgentAssembly
    from workguide.config.app_config import get_app_config
...
async def test_checkpoint_history_seed_guard_tolerates_missing_user_context():
    ...
    from app.gateway.services import ensure_checkpoint_history_seeded
    from workguide.runtime.user_context import AUTO, _AutoSentinel
...
async def test_checkpoint_history_seed_guard_is_thread_scoped_under_user_context():
    ...
    from app.gateway.services import ensure_checkpoint_history_seeded
    from workguide.runtime.user_context import AUTO
...
async def test_checkpoint_history_seed_runs_exactly_once_across_principals(tmp_path):
    ...
    from app.gateway.services import ensure_checkpoint_history_seeded
    from workguide.persistence.engine import close_engine, get_session_factory, init_engine
    from workguide.runtime.events.store.db import DbRunEventStore
    from workguide.runtime.user_context import reset_current_user, set_current_user
...
async def test_start_run_checkpoint_validation_failure_does_not_admit_run(_stub_app_config):
    ...
    from workguide.runtime import RunManager
    from workguide.runtime.runs.store.memory import MemoryRunStore
...
async def test_thread_metadata_timeout_logs_and_run_still_starts(_stub_app_config, caplog, monkeypatch):
    ...
    from app.gateway.services import start_run
    from workguide.runtime import RunManager
    from workguide.runtime.runs.manager import RunStartOutcome
    from workguide.runtime.runs.schemas import RunStatus
    from workguide.runtime.runs.store.memory import MemoryRunStore
...
def test_launch_scheduled_thread_run_uses_configured_recursion_limit(_stub_app_config):
    ...
    from app.gateway.services import launch_scheduled_thread_run
    from workguide.config.app_config import AppConfig, set_app_config

    set_app_config(
        AppConfig.model_validate(
            {
                "sandbox": {"use": "workguide.sandbox.local:LocalSandboxProvider"},
...
def test_launch_mcp_task_notification_run_restores_busy_thread_conflict(_stub_app_config):
    ...
    from app.gateway.services import launch_mcp_task_notification_run
    from workguide.runtime.runs.manager import ConflictError
...
async def test_run_agent_invalid_stream_mode_finalizes_run_before_graph_invocation():
    ...
    from workguide.runtime.runs.manager import RunManager
    from workguide.runtime.runs.schemas import RunStatus
    from workguide.runtime.runs.worker import RunContext, run_agent
...
async def test_run_agent_full_mode_rejects_delta_before_graph_invocation():
    ...
    from workguide.runtime.runs.manager import RunRecord, RunStartOutcome
    from workguide.runtime.runs.schemas import DisconnectMode, RunStatus
    from workguide.runtime.runs.worker import RunContext, run_agent
...
async def test_run_agent_full_mode_checks_selected_checkpoint_before_graph():
    ...
    from workguide.runtime.checkpoint_mode import CHECKPOINT_MODE_METADATA_KEY
    from workguide.runtime.runs.manager import RunRecord, RunStartOutcome
    from workguide.runtime.runs.schemas import DisconnectMode, RunStatus
    from workguide.runtime.runs.worker import RunContext, run_agent
