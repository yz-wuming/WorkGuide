# WorkGuide extension example

This directory is a compact, standalone Python package showing all five WorkGuide
extension contribution kinds. It depends on the public
`workguide-extension-api` contract and never imports `workguide.*` or `app.*`.
...
| System-model observer | Counts WorkGuide-owned model calls, including failures |
...
`workguide-extension-api` is currently sourced from this checkout. Install it
first, then install this independent package:

```bash
cd examples/workguide-extension-example
uv venv --python 3.12
...
the WorkGuide harness and Gateway application are not imported.

## Install and load it in WorkGuide

From the WorkGuide checkout root, install this directory through the extension
manager. Use an absolute path because the Make wrapper invokes the manager from
`backend/`:

```bash
make extension-install SOURCE="$PWD/examples/workguide-extension-example"
make extension-list
```
...
- copies a deployable snapshot to
  `backend/extensions/sources/workguide-extension-example/`;
...
plugins:
  - name: example
    package: workguide-extension-example
    use: workguide_extension_example:install
    enabled: true
    required: false
    config: {}
```

Start or restart WorkGuide after installation:
...
```text
uv run --frozen --no-group extensions workguide extensions install <source> [--yes] [--required]
uv run --frozen --no-group extensions workguide extensions list
uv run --frozen --no-group extensions workguide extensions enable <name>
uv run --frozen --no-group extensions workguide extensions disable <name>
uv run --frozen --no-group extensions workguide extensions remove <name>
```
...
`workguide.extensions` group. This example declares:

```toml
[project.entry-points."workguide.extensions"]
example = "workguide_extension_example:install"
```
