...
3. Runtime environment toggles that enable optional backends:
   - WORKGUIDE_STREAM_BRIDGE_REDIS_URL   -> redis
   - WORKGUIDE_SANDBOX_OWNERSHIP_REDIS_URL -> redis
...
def find_config_file() -> Path | None:
    ...
    explicit = os.environ.get("WORKGUIDE_CONFIG_PATH")
    if explicit:
...
def section_value(lines: list[str], section: str, key: str) -> str | None:
    ...
    Only handles the shallow shape WorkGuide uses for these settings:
        database:
          backend: postgres
...
def detect_from_runtime_env() -> list[str]:
    extras: set[str] = set()
    if os.environ.get("WORKGUIDE_STREAM_BRIDGE_REDIS_URL", "").strip():
        extras.add("redis")
    if os.environ.get("WORKGUIDE_SANDBOX_OWNERSHIP_REDIS_URL", "").strip():
        extras.add("redis")
    return sorted(extras)
