#!/usr/bin/env python3
"""Benchmark WorkGuide's full and DeltaChannel checkpoint message storage.
...
from workguide.agents.thread_state import merge_message_writes
...
_HISTORY_CACHE_ENV = "WORKGUIDE_CHECKPOINT_BENCH_HISTORY_CACHE"


def _wrap_history_cache(saver: Any) -> Any:
    """Wrap *saver* in a CachedHistorySaver with a fresh, unbounded memory cache.

    Opt-in via WORKGUIDE_CHECKPOINT_BENCH_HISTORY_CACHE=1 so default rows are
...
def _worker_main(encoded_case: str, *, profile_path: Path | None = None) -> int:
    try:
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="workguide-checkpoint-benchmark-") as temp_dir:
