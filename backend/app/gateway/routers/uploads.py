...
from workguide.sandbox.sandbox_provider import SandboxProvider, get_sandbox_provider
...
from workguide.utils.file_io import run_file_io
...
# Guardrails for on-upload zip extraction: reject directory/symlink escapes,
# encrypted members, and cap the decompressed volume to honor upload limits.
_ZIP_MAX_MEMBERS = 200


def _extract_zip_to_uploads(
    zip_path: os.PathLike[str] | str,
    *,
    uploads_dir: os.PathLike[str] | str,
    sandbox_uploads: os.PathLike[str] | str,
    thread_id: str,
    ...
    """Safely extract a zip archive into the thread uploads dir.

    Members are flattened to sanitized base names (subdirectories are ignored)
    so no entry can escape the uploads directory via ``..`` or absolute paths.
    Symlinks and encrypted members are skipped. Per-member and aggregate size
    limits follow the same caps as direct uploads to guard against zip bombs.

    Returns ``(file_infos, new_total_size)`` where each ``file_info`` carries
    the fields expected by :class:`UploadedFileInfo` plus a private
    ``_real_path`` key that the caller resolves to the on-disk file.
    """
    entries: list[dict] = []
    zip_root = Path(zip_path)
    try:
        with zipfile.ZipFile(str(zip_root)) as archive:
            if any(info.flag_bits & 0x1 for info in archive.infolist()):
                logger.warning("Zip is encrypted; skipping extraction for %s", zip_root.name)
                return entries, total_size
            for info in archive.infolist()[: _ZIP_MAX_MEMBERS]:
                if info.is_dir():
                    continue
                # Reject symlink members (mode 0o120000) without reading them,
                # guarding against a link that points out of the uploads dir.
                if (info.external_attr >> 16) & 0o170000 == 0o120000:
                    continue
                # Flatten to a single basename so traversal is impossible.
                ...
                if total_size + info.file_size > max_total_size:
                    break
                try:
                    display = normalize_filename(name) or "unnamed"
                    display = claim_unique_filename(display, seen_filenames)
                except ValueError:
                    continue
                dest = validate_upload_destination(Path(uploads_dir), display)
                try:
                    # A crafted archive may omit the encryption flag bit but
                    # still encrypt member data; reading such a member without
                    # a password raises RuntimeError, so treat it as skippable.
                ...
                entries.append(
                    {
                        ...
                        "path": str(Path(sandbox_uploads) / display),
                        "virtual_path": upload_virtual_path(display),
                        "artifact_url": upload_artifact_url(thread_id, display),
                    }
                )
    except (zipfile.BadZipFile, OSError, ValueError) as exc:  # noqa: PERF203
        logger.warning("Skipping non-extractable zip %s (%s)", zip_root.name, exc)
        # Best-effort: leave the zip itself uploaded even if members fail.
    return entries, total_size
...
async def upload_files(
    ...
    for file in files:
        ...
        try:
            ...
            if file_ext == ".zip":
                extracted, total_size = await run_file_io(
                    _extract_zip_to_uploads,
                    file_path,
                    uploads_dir=uploads_dir,
                ...
                for inner in extracted:
                    real_path = Path(inner.pop("_real_path"))
                    written_paths.append(real_path)
                    uploaded_files.append(inner)
                    if sync_to_sandbox:
                        sandbox_sync_targets.append((real_path, inner["virtual_path"]))
                if extracted:
                    logger.info(
                        "Extracted %d file(s) from archive %s (thread_id=%s)",
                        len(extracted),
                        safe_filename,
                        thread_id,
                    )
