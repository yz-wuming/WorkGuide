...
from workguide_extension_api import PROVENANCE_KEYS
...
from workguide.agents.middlewares.input_sanitization_middleware import frame_untrusted_text
from workguide.agents.middlewares.tool_receipt import TOOL_RECEIPT_KEY
from workguide.agents.middlewares.tool_transform_meta import TOOL_TRANSFORMS_KEY
...
from workguide.runtime.checkpoint_state import graph_state_schema
from workguide.runtime.goal import goal_thread_lock
from workguide.runtime.journal import build_checkpoint_history_seed_events
from workguide.runtime.runs.naming import resolve_root_run_name
from workguide.runtime.secret_context import (
    LegacyRunMetadataSecretError,
    redact_config_secrets,
    validate_run_metadata_secrets,
)
from workguide.runtime.stream_modes import normalize_stream_modes
...
from workguide.utils.thread_id import validate_thread_id
...
async def resolve_trusted_internal_owner_for_attribution(request: Request, owner_user_id: str | None) -> Any | None:
    """Resolve the WorkGuide user used only for trusted internal attribution."""
...
async def start_run(
    ...
    *,
    ...
    # X-WorkGuide-Owner-User-Id, so they are scoped to that owner instead of
    ...
    try:
        ...
        # Merge WorkGuide-specific context overrides into both ``configurable`` and ``context``.
...
def _mcp_task_notification_prompt(event: dict[str, Any]) -> str:
    ...
    instruction = (
        ...
        "that WorkGuide will continue retrying at a lower frequency."
    )
