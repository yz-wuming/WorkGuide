...
from workguide.persistence.thread_meta import THREAD_PINNED_METADATA_KEY, InvalidMetadataFilterError
from workguide.persistence.thread_meta.memory import THREADS_NS, MemoryThreadMetaStore
from workguide.runtime import ConflictError, ThreadOperationKind
...
def test_delete_thread_route_closes_browser_session(tmp_path):
    ...
    with (
        ...
        patch(
            "workguide.community.browser_automation.get_browser_session_manager",
...
def test_get_thread_history_removes_synthesized_boundary_when_exact_lookup_is_incomplete() -> None:
    ...
    from workguide.runtime.events.store.base import IncompleteMessageRunLookupError
...
def test_branch_thread_real_mutation_graph_finishes_without_scheduling(monkeypatch) -> None:
    ...
    assert branch_snapshot.metadata["workguide_branch"] is True
...
def _wire_extension_agent(monkeypatch, app, checkpointer, mode):
    ...
    from workguide.agents.thread_state import get_thread_state_schema
...
def test_branch_thread_rejects_sidecar_threads() -> None:
    ...
    with TestClient(app) as client:
        created = client.post(
            "/api/threads",
            json={"thread_id": "sidecar-thread", "metadata": {"workguide_sidecar": True}},
        )
        assert created.status_code == 200, created.text
