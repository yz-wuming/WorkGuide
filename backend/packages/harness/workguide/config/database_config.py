...
Controls BOTH the LangGraph checkpointer and the WorkGuide application
...
({sqlite_dir}/workguide.db) with WAL journal mode enabled on every
...
from workguide.config.postgres_schema import POSTGRES_SCHEMA_PATTERN, validate_postgres_schema
...
class CheckpointCacheConfig(BaseModel):
    ...
    redis_url: str | None = Field(
        default=None,
        description=("Redis URL for type=redis. If omitted, WORKGUIDE_CHECKPOINT_CACHE_REDIS_URL, REDIS_URL, or redis://localhost:6379/0 is used."),
    )
...
class DatabaseConfig(BaseModel):
    ...
    sqlite_dir: str = Field(
        default=".workguide/data",
        description=("Directory for the SQLite database file. Both checkpointer and application data share {sqlite_dir}/workguide.db."),
    )
    postgres_url: str = Field(
        ...
        description=(
            ...
            "Example: postgresql://user:pass@host:5432/workguide "
    ...
    def sqlite_path(self) -> str:
        ...
        return os.path.join(self._resolved_sqlite_dir, "workguide.db")
