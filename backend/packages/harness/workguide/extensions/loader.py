...
from workguide_extension_api import API_VERSION
from pydantic import BaseModel, ConfigDict, Field

from workguide.extensions.registry import ExtensionRegistry, LoadedExtensions
...
class ExtensionSpec(BaseModel):
    ...
    table_prefix: str | None = Field(
        ...
        description=(
            ...
            "workguide.persistence.migrations._env_filters so alembic revision --autogenerate "
...
def load_extensions(specs: Sequence[ExtensionSpec]) -> tuple[LoadedExtensions, list[Diagnostic]]:
    ...
    for spec in specs:
        ...
        try:
            declared = getattr(install, "__workguide_api__", None)
        except Exception as exc:
        ...
        if declared is not None and not _compatible(declared, API_VERSION):
            message = f"extension requires extension-api {declared}, host provides {API_VERSION}. Install a matching version: pip install 'workguide-extension-api{_range_for(declared)}'"
            diagnostics.append(Diagnostic.error(spec.use, message))
            logger.error("Extension %s: %s", spec.use, message)
            if spec.required:
