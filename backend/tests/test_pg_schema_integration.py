...
from workguide.persistence.engine import close_engine, get_engine, init_engine_from_config
from workguide.runtime.checkpointer.async_provider import make_checkpointer
from workguide.runtime.checkpointer.provider import _resolve_checkpointer_config, _sync_checkpointer_cm
from workguide.runtime.store.async_provider import make_store
from workguide.runtime.store.provider import _resolve_store_config, _sync_store_cm

POSTGRES_URL = os.getenv("WORKGUIDE_TEST_POSTGRES_URL")

pytestmark = pytest.mark.skipif(
    not POSTGRES_URL,
    reason="set WORKGUIDE_TEST_POSTGRES_URL to run live PostgreSQL schema integration tests",
)
...
async def test_postgres_schema_places_orm_checkpointer_and_store_tables_together():
    ...
    schema = f"workguide_test_{uuid.uuid4().hex[:12]}"
...
def test_sync_postgres_schema_places_checkpointer_and_store_tables_together():
    ...
    try:
        ...
        assert any(table_schema == schema and "checkpoint" in table_name for table_schema, table_name in by_schema)
        assert any(table_schema == schema and ("store" in table_name or "migration" in table_name) for table_schema, table_name in by_schema)
        # The WorkGuide LangGraph tables must NOT leak into public.
