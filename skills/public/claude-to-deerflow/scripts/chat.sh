#!/usr/bin/env bash
# chat.sh — Send a message to WorkGuide and collect the streaming response.
#
# Usage:
...
#   WORKGUIDE_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   WORKGUIDE_URL          — Unified proxy base URL (default: http://localhost:2026)
#   WORKGUIDE_GATEWAY_URL  — Gateway API base URL (default: $WORKGUIDE_URL)
#   WORKGUIDE_LANGGRAPH_URL — LangGraph API base URL (default: $WORKGUIDE_URL/api/langgraph)
#
...
GATEWAY_URL="${WORKGUIDE_GATEWAY_URL:-$WORKGUIDE_URL}"
LANGGRAPH_URL="${WORKGUIDE_LANGGRAPH_URL:-$WORKGUIDE_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: WorkGuide is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure WorkGuide is running. Start it with: cd <workguide-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from WorkGuide: {data_str}", file=sys.stderr)
            sys.exit(1)
