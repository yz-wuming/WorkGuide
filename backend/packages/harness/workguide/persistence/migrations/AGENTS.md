### Schema Migrations (`packages/harness/workguide/persistence/migrations/`)

WorkGuide's application tables (`runs`, `threads_meta`, `feedback`, `users`, `run_events`, plus the four `channel_*` tables) are owned by alembic via a **hybrid bootstrap** strategy. LangGraph's checkpointer tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`, `checkpoint_migrations`) live in the same database but are owned by LangGraph and excluded from alembic's view via `migrations/_env_filters.py::include_object`.
...
end to end and must not register models against `workguide.persistence.base.Base`
...
- every table sharing one prefix, declared via the `plugins:` record's
  ...
  `./data/workguide.db`. That is the same path `LANGGRAPH_OWNED_TABLES` covers,
