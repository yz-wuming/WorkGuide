...
``search_path`` to a target schema. The two PostgreSQL drivers WorkGuide
...
:class:`workguide.config.database_config.DatabaseConfig` to be plain
...
def create_schema_sql(schema: str) -> str | None:
    ...
    if not schema:
        return None
    from workguide.config.postgres_schema import validate_postgres_schema
