#!/usr/bin/env python3
"""WorkGuide Health Check (make doctor).
...
def check_llm_auth(config_path: Path) -> list[CheckResult]:
    ...
    try:
        ...
        for model in data.get("models", []):
            ...
            if use == "workguide.models.openai_codex_provider:CodexChatModel":
            ...
            if use == "workguide.models.claude_provider:ClaudeChatModel":
...
def check_web_tool(config_path: Path, *, tool_name: str, label: str) -> CheckResult:
    ...
    try:
        ...
        free_providers = {
            ...
            "image_search": {"workguide.community.image_search.tools": "DuckDuckGo Images (no key needed)"},
        }
        key_providers = {
            "web_search": {
...
def main() -> int:
    ...
    print()
    print(bold("WorkGuide Health Check"))
    print("═" * 40)
    ...
    if total_fails == 0 and total_warns == 0:
        ...
        print(f"Run {cyan('make dev')} to start WorkGuide")
    elif total_fails == 0:
