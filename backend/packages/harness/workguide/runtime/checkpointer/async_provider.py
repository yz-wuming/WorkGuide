...
For sync usage see :mod:`workguide.runtime.checkpointer.provider`.
"""
...
from workguide.persistence.postgres_schema import create_schema_sql, dsn_with_search_path, normalize_libpq_dsn
from workguide.runtime.checkpointer.provider import (
...
async def make_checkpointer(app_config: AppConfig | None = None) -> AsyncIterator[Checkpointer]:
    ...
    async with _select_inner_checkpointer(app_config) as saver:
        ...
        if mode == "delta":
            from workguide.runtime.checkpoint_cache.provider import (
