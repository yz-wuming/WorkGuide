...
from workguide.config.acp_config import ACPAgentConfig, load_acp_config_from_dict
from workguide.config.agent_storage_config import AgentStorageConfig
from workguide.config.agents_api_config import AgentsApiConfig, load_agents_api_config_from_dict
from workguide.config.auth_config import AuthAppConfig
from workguide.config.authorization_config import AuthorizationConfig, load_authorization_config_from_dict
from workguide.config.channel_connections_config import ChannelConnectionsConfig
from workguide.config.checkpointer_config import CheckpointerConfig, load_checkpointer_config_from_dict
from workguide.config.database_config import DatabaseConfig
from workguide.config.dedupe_storage_config import DedupeStorageConfig
from workguide.config.extensions_config import ExtensionsConfig
from workguide.config.file_signature import ConfigSignature as _ConfigSignature
from workguide.config.file_signature import get_config_signature as _get_config_signature
from workguide.config.guardrails_config import GuardrailsConfig, load_guardrails_config_from_dict
from workguide.config.input_polish_config import InputPolishConfig
from workguide.config.loop_detection_config import LoopDetectionConfig
from workguide.config.mcp_tasks_config import McpTasksConfig
from workguide.config.memory_config import MemoryConfig, load_memory_config_from_dict
from workguide.config.model_config import ModelConfig
from workguide.config.read_before_write_config import ReadBeforeWriteConfig
from workguide.config.reload_boundary import format_field_description
from workguide.config.run_events_config import RunEventsConfig
from workguide.config.run_ownership_config import RunOwnershipConfig
from workguide.config.runtime_paths import existing_project_file
from workguide.config.safety_finish_reason_config import SafetyFinishReasonConfig
from workguide.config.sandbox_config import SandboxConfig
from workguide.config.scheduler_config import SchedulerConfig
from workguide.config.skill_evolution_config import SkillEvolutionConfig
from workguide.config.skill_scan_config import SkillScanConfig
from workguide.config.skills_config import SkillsConfig
from workguide.config.stream_bridge_config import StreamBridgeConfig, load_stream_bridge_config_from_dict
from workguide.config.subagent_batches_config import SubagentBatchesConfig
from workguide.config.subagent_runtime_config import SubagentRuntimeConfig
from workguide.config.subagents_config import SubagentsAppConfig, load_subagents_config_from_dict
from workguide.config.suggestions_config import SuggestionsConfig
from workguide.config.summarization_config import SummarizationConfig, load_summarization_config_from_dict
from workguide.config.title_config import TitleConfig, load_title_config_from_dict
from workguide.config.token_budget_config import TokenBudgetConfig
from workguide.config.token_usage_config import TokenUsageConfig
from workguide.config.tool_config import ToolConfig, ToolGroupConfig
from workguide.config.tool_output_config import ToolOutputConfig
from workguide.config.tool_progress_config import ToolProgressConfig
from workguide.config.tool_search_config import ToolSearchConfig, load_tool_search_config_from_dict
from workguide.config.verification_config import VerificationConfig
...
def is_trace_correlation_enabled(config: Any) -> bool:
    ...
    the Gateway ``TraceMiddleware`` and the embedded ``WorkGuideClient`` so
    the two entry points cannot drift on when ``workguide_trace_id`` is
...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``workguide``/``app`` logger hierarchies.

    Only the ``workguide`` and ``app`` logger levels are changed so that
    ...
    for logger_name in ("workguide", "app"):
...
class AppConfig(BaseModel):
    """Config for the WorkGuide application"""

    log_level: str = Field(
        default="info",
        description=format_field_description(
            "log_level",
            field_doc="Logging level for workguide and app modules (debug/info/warning/error); third-party libraries are not affected.",
        ),
    )
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `WORKGUIDE_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("WORKGUIDE_CONFIG_PATH"):
            path = Path(os.getenv("WORKGUIDE_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `WORKGUIDE_CONFIG_PATH` not found at {path}")
            return path
        else:
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        """
        # Load backend/.env so $VAR references (e.g. $ZHIPU_API_KEY) resolve.
        # Already-set process env vars take precedence and are never overwritten.
        ...
        cls._merge_runtime_models(config_data, resolved_path)
    ...
    @classmethod
    def _merge_runtime_models(cls, config_data: dict[str, Any], resolved_path: Path) -> None:
        """Merge runtime-added models from ``models.runtime.yaml`` into ``models:``.

        Runtime models are stored in a gitignored sidecar next to ``config.yaml``
        and managed through the Gateway ``/api/models`` write endpoints. Merging
        here (before validation) makes them first-class citizens of the model
        allowlist, ``create_chat_model`` and the chat run path — picked up on the
        next ``get_app_config()`` call without a restart. A missing or unreadable
        sidecar is a no-op.
        """
        from workguide.config.runtime_models import load_runtime_models

        runtime_models = load_runtime_models(resolved_path)
        if not runtime_models:
            return
        existing = config_data.get("models") or []
        config_data["models"] = [*existing, *runtime_models]
    ...
    def _apply_singleton_configs(cls, config: Self, acp_agents: dict[str, ACPAgentConfig]) -> None:
        from workguide.config.checkpointer_config import get_checkpointer_config
        ...
        if previous_checkpointer_config != config.checkpointer:
            ...
            from workguide.runtime.checkpointer import reset_checkpointer
            from workguide.runtime.store import reset_store
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("workguide_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("workguide_current_app_config_stack", default=())
...
def get_app_config() -> AppConfig:
    """Get the WorkGuide config instance.
