#!/usr/bin/env python3
"""Create a redacted WorkGuide support bundle for community troubleshooting."""
...
# *CREDENTIAL*/*DSN* sandbox-env denylist (backend/packages/harness/workguide/sandbox/env_policy.py):
...
# Must stay byte-identical to workguide.utils.thread_id.THREAD_ID_PATTERN
...
def _candidate_thread_data_dirs(project_root: Path, thread_id: str) -> list[Path]:
    ...
    candidates = [
        project_root / ".workguide" / "threads" / thread_id / "user-data",
        project_root / "backend" / ".workguide" / "threads" / thread_id / "user-data",
    ]
    for base in (project_root / ".workguide" / "users", project_root / "backend" / ".workguide" / "users"):
        if base.exists():
...
def render_issue_summary(triage: dict[str, Any]) -> str:
    ...
    lines = [
        "## WorkGuide support bundle summary",
        "",
...
def render_ai_issue_draft(triage: dict[str, Any], issue_summary: str, doctor: dict[str, Any] | None) -> str:
    ...
    lines = [
        "# AI issue draft",
        "",
        "Use this when a coding agent or AI assistant files a WorkGuide bug report.",
        ...
        "### Before you start",
        "",
        "- [ ] I searched [existing issues](https://github.com/bytedance/workguide/issues?q=is%3Aissue) and this is not a duplicate.",
...
def _default_out_path(project_root: Path) -> Path:
    ...
    return project_root / ".workguide" / "support-bundles" / f"workguide-support-bundle-{timestamp}.zip"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project-root", type=Path, default=repo_root, help="WorkGuide project root")
