...
from workguide.agents.thread_state import ThreadDataState
from workguide.authz.sandbox_authz import authorize_sandbox_execution, safe_app_config
from workguide.config import get_app_config
from workguide.config.paths import VIRTUAL_PATH_PREFIX
from workguide.constants import DEFAULT_SKILLS_CONTAINER_PATH
from workguide.runtime.secret_context import read_active_secrets
from workguide.runtime.user_context import resolve_runtime_user_id
from workguide.sandbox.exceptions import (
    SandboxError,
    SandboxNotFoundError,
    SandboxRuntimeError,
)
from workguide.sandbox.file_operation_lock import get_file_operation_lock
from workguide.sandbox.overwrite import unwrap_sandbox
from workguide.sandbox.path_patterns import build_output_mask_pattern
from workguide.sandbox.sandbox import Sandbox
from workguide.sandbox.sandbox_provider import get_sandbox_provider
from workguide.sandbox.search import GrepMatch
from workguide.sandbox.security import LOCAL_HOST_BASH_DISABLED_MESSAGE, is_host_bash_allowed
from workguide.tools.types import Runtime
...
# Deployments can override via env var WORKGUIDE_WRITE_FILE_MAX_BYTES; set to
# 0 (or negative) to disable the guard entirely.
_WRITE_FILE_CONTENT_MAX_BYTES = 80 * 1024
_WRITE_FILE_MAX_BYTES_ENV = "WORKGUIDE_WRITE_FILE_MAX_BYTES"
...
def _get_mcp_allowed_paths() -> list[str]:
    ...
    allowed_paths = []
    try:
        from workguide.config.extensions_config import get_extensions_config
...
def _compiled_mask_patterns(sources: tuple[tuple[str, str], ...]) -> tuple[tuple[re.Pattern[str], str, str], ...]:
    ...
    # ``workguide.sandbox.path_patterns``, which owns that rule so the two copies
...
def _apply_cwd_prefix(command: str, thread_data: ThreadDataState | None) -> str:
    ...
    if thread_data and (workspace := thread_data.get("workspace_path")):
        if _is_windows():
            # PowerShell rejects `&&` as a statement separator; use `;` and single-quote the path.
            return f"cd {_powershell_quote(workspace)}; {command}"
        return f"cd {shlex.quote(workspace)} && {command}"
    return command


def _powershell_quote(path: str) -> str:
    """Single-quote a path for PowerShell, doubling internal single quotes."""
...
CHANNEL_USER_ID_ENV = "WORKGUIDE_CHANNEL_USER_ID"
...
def _lark_cli_env_from_runtime(runtime: Runtime, command: str, *, sandbox_paths: bool) -> dict[str, str] | None:
    ...
    Settings authorizes ``lark-cli`` under WorkGuide's per-user integration
    ...
    if not _LARK_CLI_COMMAND_RE.search(command):
        return None
    try:
        from workguide.integrations.lark_cli import lark_cli_env_overlay, sandbox_lark_broker_active
...
def bash_tool(runtime: Runtime, description: str, command: str) -> str:
    """Execute a command in a Windows PowerShell environment (host machine is Windows).

    The machine runs Windows PowerShell, not POSIX/Linux. Follow these rules:

    - Separate statements with `;` — do NOT use `&&` or `||`.
    - Use PowerShell syntax: `$var`, `Get-ChildItem`, `Copy-Item`, `New-Item -ItemType Directory`, etc.
    - Always use absolute paths. When the user says "desktop" / "桌面", operate on
      `/mnt/desktop/<name>` — that virtual path maps to the real Windows desktop
      (`C:\\Users\\yinzi\\Desktop`). Do NOT use `\\Desktop`, `~/Desktop`, or a bare
      relative `Desktop` — those are not the user's desktop.
      Thread workspaces live under `/mnt/user-data/workspace`; skills under `/mnt/skills`.
    - Use `python` to run Python code, and `python -m pip` to install packages.
    - To start a long-lived process, run it in the background with output redirected,
      e.g. `Start-Process -NoNewWindow` or `cmd /c "your-command > /mnt/user-data/workspace/server.log 2>&1"`,
      then poll the log file or port. A foreground long-lived process blocks the turn
      until the command timeout kills it.

    Args:
        description: Explain why you are running this command in short words. ALWAYS PROVIDE THIS PARAMETER FIRST.
        command: The PowerShell command to execute. Always use absolute paths for files and directories.
    """
    try:
...
def _effective_write_file_max_bytes() -> int:
    ...
    Reads ``WORKGUIDE_WRITE_FILE_MAX_BYTES`` at call time (not import time)
...
def write_file_tool(
    ...
    Operators can override the cap via env var `WORKGUIDE_WRITE_FILE_MAX_BYTES`
