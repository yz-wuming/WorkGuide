...
from workguide.community.tenki.provider import _BOOTSTRAP_TIMEOUT, TenkiSandboxProvider, _import_client
from workguide.community.tenki.sandbox import TenkiSandbox
...
def _install(monkeypatch, *, client=None, config_attrs=None):
    ...
    monkeypatch.setattr(
        "workguide.community.tenki.provider.get_app_config",
        lambda: _stub_config(config_attrs),
    )
    if client is not None:
        monkeypatch.setattr(
            "workguide.community.tenki.provider._import_client",
...
def test_import_client_missing_raises_actionable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ImportError, match=r"workguide-harness\[tenki\]"):
        _import_client()


def test_acquire_without_tenki_raises_and_shuts_down_cleanly(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("workguide.community.tenki.provider.get_app_config", lambda: _stub_config())
    _no_tenki(monkeypatch)
    provider = TenkiSandboxProvider()
    try:
        with pytest.raises(ImportError, match=r"workguide-harness\[tenki\]"):
...
def test_download_cap_counts_bytes_actually_received(monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.community.tenki.sandbox._MAX_DOWNLOAD_SIZE", 100)
    fake = _FakeSandbox()
    box = TenkiSandbox("sb", fake)
...
def test_load_config_rejects_invalid_environment_key(monkeypatch):
    ...
    monkeypatch.setattr("workguide.community.tenki.provider.get_app_config", lambda: _stub_config({"environment": {"bad-key": "1"}}))
    with pytest.raises(ValueError, match=r"POSIX"):
...
def test_create_passes_prefixed_name_and_scope(monkeypatch):
    ...
    assert kwargs["name"].startswith("workguide-tenki-")
