...
from workguide.agents.thread_state import merge_artifacts, merge_message_writes
from workguide.config.run_ownership_config import RunOwnershipConfig
from workguide.runtime.checkpoint_state import CheckpointStateAccessor
...
from workguide.runtime.runs.manager import CancelOutcome, ConflictError, RunManager
from workguide.runtime.runs.schemas import RunStatus
from workguide.runtime.runs.store.memory import MemoryRunStore
from workguide.runtime.runs.worker import (
    RollbackPoint,
    RunContext,
...
def _stub_mutation_graph(monkeypatch, *, restored_config):
    ...
    monkeypatch.setattr(
        "workguide.runtime.runs.worker.build_state_mutation_graph",
...
async def test_run_agent_marks_rollback_unusable_when_capture_fails():
    ...
    with patch(
        "workguide.runtime.runs.worker._rollback_to_pre_run_checkpoint",
...
def test_agent_factory_supports_app_config_returns_false_when_signature_lookup_fails(monkeypatch):
    ...
    monkeypatch.setattr("workguide.runtime.runs.worker.inspect.signature", lambda _obj: (_ for _ in ()).throw(ValueError("boom")))
...
async def test_run_agent_ignores_stale_llm_error_fallback_from_prior_run():
    ...
    classified transient and surfaced as a ``workguide_error_fallback`` AIMessage)
...
async def test_worker_finally_block_swallows_helper_exceptions(monkeypatch):
    ...
    the helper itself.
    """
    import workguide.runtime.runs.worker as worker_module
