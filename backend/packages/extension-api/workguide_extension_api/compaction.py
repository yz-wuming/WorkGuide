...
Fields are keyed on canonical content hashes (``workguide_extension_api.canonical_hash``),
...
The exact recipe both sides must use: ``canonical_hash(message.content)`` — the message's ``content`` attribute passed directly, never pre-stringified. WorkGuide
