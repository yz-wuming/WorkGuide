...
from workguide.runtime import DisconnectMode, RunManager, RunStatus, ThreadOperationKind
from workguide.runtime.events.store.memory import MemoryRunEventStore
from workguide.runtime.runs.manager import CancelOutcome, ConflictError, PersistenceRetryPolicy, RunStartOutcome
...
async def test_completion_persistence_warns_when_recreated_row_still_missing(caplog):
    ...
    record = await manager.create("thread-1")
    await manager.set_status(record.run_id, RunStatus.success)
    caplog.set_level(logging.WARNING, logger="workguide.runtime.runs.manager")
...
async def test_list_by_thread(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    # Distinct, increasing timestamps: on Windows the system clock resolution
    # (~15.6ms tick) can make two back-to-back create() calls tie, which the
    # stable sort then resolves by insertion order and breaks the "newest
    # first" assertion. The tie case itself is covered by
    # test_list_by_thread_is_stable_when_timestamps_tie.
    times = iter(
        [
    ...
    monkeypatch.setattr("workguide.runtime.runs.manager._now_iso", lambda: next(times))
...
async def test_list_by_thread_is_stable_when_timestamps_tie(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("workguide.runtime.runs.manager._now_iso", lambda: "2026-01-01T00:00:00+00:00")
...
async def test_model_name_create_or_reject():
    ...
    from workguide.runtime.runs.schemas import DisconnectMode
