...
from workguide.config.extensions_config import ExtensionsConfig, McpServerConfig, resolve_effective_mcp_routing
from workguide.config.paths import VIRTUAL_PATH_PREFIX, Paths, get_paths
from workguide.constants import DEFAULT_MCP_SESSION_INIT_TIMEOUT, MCP_TMP_SUBDIR
from workguide.mcp.client import build_servers_config
from workguide.mcp.headers import apply_header_overrides
from workguide.mcp.interceptors import build_mcp_tool_interceptors, compose_tool_interceptors
from workguide.mcp.oauth import build_oauth_tool_interceptor, get_initial_oauth_headers
from workguide.mcp.session_pool import call_pooled_session_tool, get_session_pool
from workguide.mcp.tasks import ORDINARY_MCP_TASK_DRIVER, TaskSubmitRequest
...
def _local_uri_to_virtual_path(
    ...
    *,
    ...
    only thing missing is the virtual prefix the rest of WorkGuide addresses them
...
def _make_background_submit_tool(
    ...
    *,
    ...
    background_contract = f"Submitted as durable background task {task_name!r}; returns a WorkGuide task ID immediately and status polling is handled automatically."
...
async def get_mcp_tools() -> list[BaseTool]:
    ...
    try:
        ...
        # Patch tools to support sync invocation, as workguide client streams synchronously
