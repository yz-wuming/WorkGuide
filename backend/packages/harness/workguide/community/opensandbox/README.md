...
Runs WorkGuide sandboxes on [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox),
using the synchronous Python SDK behind WorkGuide's `Sandbox` and
...
`OPEN_SANDBOX_DOMAIN` are set. `use_server_proxy` is useful when WorkGuide can
...
Create returns only after the SDK readiness check and WorkGuide's
...
The full WorkGuide surface is implemented:
...
- `execute_command` forwards per-call environment variables and positive
  ...
  non-zero exit information in WorkGuide's string result.
...
- `list_dir`, `glob`, and `grep` use portable `find`/`grep` commands and the
  shared WorkGuide result parsers.
