...
from workguide.community.e2b_sandbox.capacity import (
    CapacityBackendError,
    ReserveStatus,
)
from workguide.config.paths import Paths
from workguide.config.sandbox_config import SandboxConfig
from workguide.sandbox.exceptions import SandboxCapacityExceededError
...
class FakeOwnershipStore:
    ...
    def renew(self, sandbox_id: str):
        from workguide.community.aio_sandbox.ownership import RenewOutcome
...
def _make_provider(*, replicas: int = 3, idle_timeout: int = 1800, overflow_policy: str = "wait", acquire_timeout: int = 30, burst_limit: int = 0) -> Any:
    ...
    mod = importlib.import_module("workguide.community.e2b_sandbox.e2b_sandbox_provider")
...
def _install_shared_deployment_capacity(
    *providers,
    ...
    store = MagicMock()
    store.key = "workguide:test:e2b-capacity"
...
def test_apply_mounts_uploads_only_enabled_skill_projection(monkeypatch, tmp_path):
    from workguide.config.extensions_config import ExtensionsConfig, SkillStateConfig
    ...
    monkeypatch.setattr("workguide.config.extensions_config.ExtensionsConfig.from_file", lambda *_args, **_kwargs: extensions)
    monkeypatch.setattr("workguide.config.extensions_config.get_extensions_config", lambda: extensions)
...
def test_skill_projection_mounts_swallows_projection_failure(monkeypatch):
    ...
    monkeypatch.setattr(mod, "get_app_config", lambda: config)
    monkeypatch.setattr(
        "workguide.skills.projection.ensure_skill_projections",
...
def _make_sandbox(client: FakeClient, *, sandbox_id: str | None = None) -> Any:
    mod = importlib.import_module("workguide.community.e2b_sandbox.e2b_sandbox")
...
def _info(sandbox_id: str, user_id: str, thread_id: str):
    return SimpleNamespace(
        ...
        metadata={
            "work_guide_provider": "e2b_sandbox_provider",
            "work_guide_user": user_id,
            "work_guide_thread": thread_id,
        },
    )
...
def test_reconcile_honors_wall_clock_budget(monkeypatch):
    ...
    provider_mod = importlib.import_module("workguide.community.e2b_sandbox.e2b_sandbox_provider")
...
def test_reconcile_kills_metadata_orphan_only_after_ttl(monkeypatch):
    ...
    fake_cls.list_return = [
        SimpleNamespace(
            ...
            metadata={"work_guide_provider": "e2b_sandbox_provider"},
        )
    ]
...
def _setup_paths(monkeypatch, tmp_path):
    paths_mod = importlib.import_module("workguide.config.paths")
...
def test_sync_outputs_to_host_skips_mtime_restoration_on_overflow(monkeypatch, tmp_path):
    ...
    e2b_provider_mod = importlib.import_module("workguide.community.e2b_sandbox.e2b_sandbox_provider")
...
def test_download_file_streaming_raises_efbig_before_full_buffering():
    ...
    from workguide.community.e2b_sandbox import e2b_sandbox as e2b_sb_mod
...
def test_deployment_capacity_reserves_commits_and_rejects_globally(monkeypatch) -> None:
    ...
    assert metadata["work_guide_capacity_ledger"] == store.key
    assert metadata["work_guide_capacity_reservation"]
    store.track.assert_called_once_with(
        sandbox_id,
        reservation_token=metadata["work_guide_capacity_reservation"],
    )
...
def test_discovery_uses_sdk_query_and_tracks_without_reserving(monkeypatch) -> None:
    ...
    entry = SimpleNamespace(
        ...
        metadata={
            "work_guide_provider": "e2b_sandbox_provider",
            "work_guide_user": "user-a",
            "work_guide_thread": "thread-a",
            "work_guide_capacity_ledger": store.key,
        },
    )
    expected_query = {key: entry.metadata[key] for key in ("work_guide_provider", "work_guide_user", "work_guide_thread")}
    ...
    sdk.list_return = [
        {
            ...
            "metadata": {
                "work_guide_provider": "e2b_sandbox_provider",
                "work_guide_capacity_ledger": store.key,
                "work_guide_capacity_reservation": "reservation-crashed",
            },
        },
        {
            ...
            "metadata": {
                "work_guide_provider": "e2b_sandbox_provider",
                "work_guide_capacity_ledger": "workguide:other:e2b-capacity",
            },
        },
    ]
