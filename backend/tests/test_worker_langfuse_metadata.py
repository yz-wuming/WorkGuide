...
async def test_run_agent_injects_langfuse_metadata(monkeypatch):
    ...
    assert metadata.get(WORKGUIDE_TRACE_METADATA_KEY) == "gateway-trace-1"
    assert fake_agent.captured_config.get("context", {}).get(WORKGUIDE_TRACE_METADATA_KEY) == "gateway-trace-1"
...
async def test_run_agent_falls_back_to_default_user_when_unset(monkeypatch):
    ...
    monkeypatch.setenv("LANGFUSE_SECRET_KEY", "sk-lf-test")
    from workguide.config.tracing_config import reset_tracing_config
    from workguide.runtime import user_context as user_context_module
...
async def test_run_agent_preserves_caller_metadata_overrides(monkeypatch):
    ...
    await run_agent(
        ...
        config={
            ...
            "metadata": {
                WORKGUIDE_TRACE_METADATA_KEY: "explicit-workguide-trace",
    ...
    assert metadata[WORKGUIDE_TRACE_METADATA_KEY] == "explicit-workguide-trace"
    assert fake_agent.captured_config.get("context", {}).get(WORKGUIDE_TRACE_METADATA_KEY) == "explicit-workguide-trace"
...
async def test_run_agent_inbound_header_trace_overrides_metadata(monkeypatch):
    """A valid inbound ``X-Trace-Id`` wins over ``config.metadata.workguide_trace_id``."""
    monkeypatch.setenv("LANGFUSE_TRACING", "true")
    monkeypatch.setenv("LANGFUSE_PUBLIC_KEY", "pk-lf-test")
    monkeypatch.setenv("LANGFUSE_SECRET_KEY", "sk-lf-test")
    from workguide.config.tracing_config import reset_tracing_config
        ...
        try:
            await run_agent(
                ...
                config={
                    ...
                    "metadata": {
                        WORKGUIDE_TRACE_METADATA_KEY: "metadata-trace-ignored",
                    },
                },
            )
        finally:
    ...
    metadata = fake_agent.captured_config.get("metadata") or {}
    assert metadata[WORKGUIDE_TRACE_METADATA_KEY] == "header-trace-1"
    assert fake_agent.captured_config.get("context", {}).get(WORKGUIDE_TRACE_METADATA_KEY) == "header-trace-1"
