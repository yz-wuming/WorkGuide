...
from workguide.agents.lead_agent.prompt import clear_skills_system_prompt_cache, refresh_skills_system_prompt_cache_async, refresh_user_skills_system_prompt_cache_async
...
from workguide.skills import Skill
from workguide.skills.installer import SkillAlreadyExistsError, SkillSecurityScanError
from workguide.skills.security_scanner import scan_skill_content
from workguide.skills.security_static_scanner import (
    StaticFinding,
    StaticScanBlockedError,
    StaticScannerError,
    enforce_static_scan,
)
from workguide.skills.storage import SkillStorage, get_or_new_user_skill_storage
from workguide.skills.types import SKILL_MD_FILE, SkillCategory
from workguide.utils.thread_id import ThreadId
...
class CustomSkillCreateRequest(BaseModel):
    """Request model for creating a user-authored custom skill from a form."""

    name: str = Field(..., description="Skill name, normalized to hyphen-case slug")
    description: str = Field(..., description="One-line description shown in the skill list")
    instructions: str = Field(..., description="Markdown instructions the Agent follows when the skill is used")
...
def _yaml_scalar_safe(value: str) -> str:
    """Render a single-line YAML scalar that parses as a plain string.

    Values made of letters, digits, spaces and a small safe punctuation set are
    emitted verbatim; anything else is double-quoted so a description containing
    ``:``, quotes or other YAML-significant characters cannot corrupt the SKILL.md
    front-matter.
    """
    value = value.replace("\r", " ").replace("\n", " ").strip()
    if not value:
        return '""'
    plain = value[0].isalnum() and all(c.isalnum() or c in " ,.()/-_" for c in value)
    if plain:
        return value
...
def _build_skill_markdown(name: str, description: str, instructions: str) -> str:
    """Assemble a valid SKILL.md file from form fields.

    The body is the skill instructions. Only name/description go into the
    front-matter; user-authored front-matter fields beyond those are not
    synthesized here (powered edit flows preserve existing fields instead).
    """
    return (
        "---\n"
        f"name: {name}\n"
        f"description: {_yaml_scalar_safe(description)}\n"
        "---\n\n"
...
@router.post(
    ...
    description="Create a new user-authored custom skill from name/description/instructions.",
)
async def create_custom_skill(body: CustomSkillCreateRequest, request: Request, config: AppConfig = Depends(get_config)) -> CustomSkillContentResponse:
    await require_admin_user(request, detail=_ADMIN_REQUIRED_DETAIL)
    try:
        name = SkillStorage.validate_skill_name(body.name)
        storage = _get_user_skill_storage(config)
        if storage.custom_skill_exists(name) or storage.public_skill_exists(name):
        ...
        if len(description) > 500:
            raise HTTPException(status_code=400, detail="Skill description must be 500 characters or fewer.")
        if len(instructions) > 20000:
            raise HTTPException(status_code=400, detail="Skill instructions must be 20000 characters or fewer.")

        content = _build_skill_markdown(name, description, instructions)
        storage.validate_skill_markdown_content(name, content)
        static_findings = await _scan_static_skill_markdown_or_raise(name, content, app_config=config)
        scan = await scan_skill_content(content, executable=False, location=f"{name}/{SKILL_MD_FILE}", app_config=config, static_findings=static_findings)
        if scan.decision == "block":
            raise HTTPException(status_code=400, detail=f"Security scan blocked creation: {scan.reason}")

        await asyncio.to_thread(storage.write_custom_skill, name, SKILL_MD_FILE, content)
        ...
        return await _read_custom_skill_response(name, config)
