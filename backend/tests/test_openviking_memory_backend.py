...
from workguide.agents.memory.backends.openviking.config import OpenVikingConfig
from workguide.agents.memory.backends.openviking.openviking_manager import (
    OpenVikingMemoryManager,
    _canonical_peer_id,
    _session_id,
)
from workguide.agents.memory.manager import (
    MemoryManagerError,
    _scan_backends,
...
def official_integration(monkeypatch: pytest.MonkeyPatch) -> None:
    import workguide.agents.memory.backends.openviking.openviking_manager as module
...
def test_manager_refuses_to_share_single_user_key_across_workguide_users(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    official_integration: None,
) -> None:
...
def test_peer_mapping_is_stable_and_namespaces_are_disjoint() -> None:
    assert _canonical_peer_id(None, "workguide") == "workguide"
    assert _canonical_peer_id("Research", "workguide") == "research"
    assert _canonical_peer_id("workguide", "workguide").startswith("df-agent-")
    assert _canonical_peer_id("-research", "workguide").startswith("df-agent-")
    assert _canonical_peer_id("df-agent-custom", "workguide").startswith("df-agent-")
    assert (
        len(
            {
                _canonical_peer_id(None, "workguide"),
                _canonical_peer_id("workguide", "workguide"),
                _canonical_peer_id("-research", "workguide"),
                _canonical_peer_id("df-agent-custom", "workguide"),
            }
        )
        == 4
    )
