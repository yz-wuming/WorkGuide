# WorkGuide - Unified Development Environment
...
help:
	@echo "WorkGuide Development Commands:"
...
detect-thread-boundaries:
	@$(BACKEND_UV_RUN) python ../scripts/detect_thread_boundaries.py --json-output ../.workguide/thread-boundary-inventory.json
...
extension-install: export WORKGUIDE_EXTENSION_SOURCE := $(value SOURCE)
extension-install:
	...
	@cd backend && uv run --frozen --no-group extensions workguide extensions install --source-env __workguide_extension_source__

extension-list:
	@cd backend && uv run --frozen --no-group extensions workguide extensions list

extension-enable: export WORKGUIDE_EXTENSION_NAME := $(value NAME)
extension-enable:
	$(if $(and $(filter command line,$(origin NAME)),$(strip $(value NAME))),,$(error usage: make extension-enable NAME=<extension>))
	@cd backend && uv run --frozen --no-group extensions workguide extensions enable --name-env __workguide_extension_name__

extension-disable: export WORKGUIDE_EXTENSION_NAME := $(value NAME)
extension-disable:
	$(if $(and $(filter command line,$(origin NAME)),$(strip $(value NAME))),,$(error usage: make extension-disable NAME=<extension>))
	@cd backend && uv run --frozen --no-group extensions workguide extensions disable --name-env __workguide_extension_name__

extension-remove: export WORKGUIDE_EXTENSION_NAME := $(value NAME)
extension-remove:
	$(if $(and $(filter command line,$(origin NAME)),$(strip $(value NAME))),,$(error usage: make extension-remove NAME=<extension>))
	@cd backend && uv run --frozen --no-group extensions workguide extensions remove --name-env __workguide_extension_name__
...
clean: stop
	@echo "Cleaning up..."
	@-rm -rf backend/.workguide 2>/dev/null || true
