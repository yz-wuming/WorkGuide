...
import pytest
from workguide_extension_api import AgentScope, MiddlewarePlacement, Placement
from langchain.agents.middleware import AgentMiddleware

from workguide.agents.lead_agent.agent import build_middlewares
from workguide.config.app_config import AppConfig
from workguide.config.sandbox_config import SandboxConfig
from workguide.extensions.isolation import IsolatedMiddleware
from workguide.extensions.registry import ExtensionRegistry
from workguide.extensions.stack import PLACEMENT_ANCHORS
...
def _app_config() -> AppConfig:
    ...
    return AppConfig(sandbox=SandboxConfig(use="workguide.sandbox.local:LocalSandboxProvider"))
...
def test_zero_extensions_skip_policy_projection(monkeypatch):
    ...
    from workguide.extensions import policy as policy_module
...
def test_runtime_isolation_failure_is_recorded_after_stack_composition():
    from workguide.extensions import get_runtime_diagnostics, reset_runtime_diagnostics
...
def test_first_subagent_build_resolves_every_lazy_anchor(monkeypatch):
    ...
    from workguide.extensions import stack as stack_module
...
def test_core_ordering_table_is_enforced_against_a_real_stack(monkeypatch):
    ...
    from workguide.extensions import ordering as ordering_mod
...
def test_ordering_violation_raises_and_names_the_extension():
    ...
    from workguide.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
    from workguide.extensions.ordering import OrderingConstraint, assert_ordering
