...
from workguide_extension_api.runtime_bridge import (
...
def test_gateway_contribution_points_are_part_of_the_public_surface():
    import workguide_extension_api

    for name in (
        "ExtensionRuntimeDeps",
        "ExtensionService",
    ):
        assert name in workguide_extension_api.__all__
        assert hasattr(workguide_extension_api, name)
    assert callable(ExtensionRegistry.service)
    assert callable(ExtensionRegistry.routers)
    assert not hasattr(workguide_extension_api, "RouterContributor")
...
def test_extension_decorator_stamps_api_requirement():
    ...
    assert install.__workguide_api__ == "0.1"
    assert install.__workguide_name__ == "demo"
...
def test_registry_and_install_alias_are_part_of_the_public_surface():
    ...
    assert "ExtensionRegistry" in workguide_extension_api.__all__
    assert "ExtensionInstall" in workguide_extension_api.__all__
...
def test_distribution_marks_the_contract_package_as_typed():
    marker = importlib.resources.files("workguide_extension_api").joinpath("py.typed")
...
def test_harness_pins_the_contract_package_exactly():
    ...
    requirement = next(Requirement(dep) for dep in dependencies if Requirement(dep).name == "workguide-extension-api")

    expected = f"=={version('workguide-extension-api')}"
    assert str(requirement.specifier) == expected, f"the host must pin workguide-extension-api exactly ({expected}); a range lets pip resolve a contract newer than the host implements"
...
def test_runtime_api_version_matches_the_installed_contract_package():
    ...
    assert API_VERSION == version("workguide-extension-api")
