"""ChannelManager — consumes inbound messages and dispatches them to the WorkGuide agent via Gateway."""
...
from workguide.config.agents_config import load_agent_config
from workguide.config.paths import make_safe_user_id
from workguide.runtime import END_SENTINEL, StreamBridge
from workguide.runtime.goal import parse_goal_command
from workguide.runtime.user_context import get_effective_user_id
from workguide.skills.slash import parse_slash_skill_reference
from workguide.skills.storage import get_or_new_skill_storage
from workguide.skills.storage.skill_storage import SkillStorage
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from WorkGuide Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the WorkGuide operator."
...
def _stream_payload_type(payload: Mapping[str, Any]) -> str:
    ...
    * The shape WorkGuide's own gateway emits (``runtime/serialization.py``
...
def _is_assistant_stream_type(payload_type: str) -> bool:
    ...
    type contains 'tool'") published every other message type, and WorkGuide
...
def _accumulate_stream_text(
    ...
    attributed to the assistant.  Nothing in WorkGuide produces it (the gateway
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical WorkGuide user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound WorkGuide owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the WorkGuide agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives WorkGuide-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the WorkGuide account that
    ...
    async def start(self) -> None:
        ...
        self._worker_tasks = {
            asyncio.create_task(
                ...
                name=f"workguide-channel-worker-{worker_index}",
            )
    ...
    async def stop(self) -> None:
        ...
        join_task = asyncio.create_task(self.bus.join_inbound(), name="workguide-channel-inbound-drain")
        drained = False
        try:
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # enforced at the webhook route by HMAC, and the "sender → WorkGuide
        ...
        # the binding by provider identity before creating WorkGuide threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing WorkGuide thread, creating one if this is the
