...
from workguide.config.acp_config import load_acp_config_from_dict
from workguide.config.agents_api_config import get_agents_api_config, load_agents_api_config_from_dict
from workguide.config.app_config import AppConfig, get_app_config, reset_app_config
from workguide.config.checkpointer_config import get_checkpointer_config, load_checkpointer_config_from_dict
from workguide.config.database_config import DatabaseConfig
from workguide.config.guardrails_config import get_guardrails_config, load_guardrails_config_from_dict
from workguide.config.memory_config import get_memory_config, load_memory_config_from_dict
from workguide.config.stream_bridge_config import get_stream_bridge_config, load_stream_bridge_config_from_dict
from workguide.config.subagents_config import get_subagents_app_config, load_subagents_config_from_dict
from workguide.config.summarization_config import get_summarization_config, load_summarization_config_from_dict
from workguide.config.title_config import get_title_config, load_title_config_from_dict
from workguide.config.tool_search_config import get_tool_search_config, load_tool_search_config_from_dict
...
def test_resolve_checkpoint_graph_cache_max_tolerates_stub_configs() -> None:
    ...
    from workguide.config.database_config import resolve_checkpoint_graph_cache_max
...
def test_app_config_defaults_missing_database_to_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(extensions_path))

    config = AppConfig.from_file(str(config_path))

    assert config.database.backend == "sqlite"
    assert config.database.sqlite_dir == ".workguide/data"
...
def test_app_config_warns_when_no_models_configured(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="workguide.config.app_config"):
        AppConfig.from_file(str(config_path))
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(config_b))
...
def test_get_app_config_does_not_reset_persistence_singletons_when_database_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("workguide.runtime.checkpointer.reset_checkpointer", _reset_checkpointer)
    monkeypatch.setattr("workguide.runtime.store.reset_store", _reset_store)
