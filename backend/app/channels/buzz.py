"""Buzz (Nostr) channel: WorkGuide as a member of a Buzz workspace relay.
...
_CONNECT_REPLY_TEXT = {
    "success": "Buzz connected to WorkGuide.",
...
# WorkGuide's hidden model-context wrappers. These are literal tags this codebase
...
class BuzzChannel(Channel):
    ...
    def _membership_filter(self) -> dict:
        ...
        socket opened covers both that window and relay/WorkGuide clock skew. Cost of
    ...
    async def handle_relay_frame(self, raw: str) -> None:
        ...
        is not necessarily the WorkGuide operator on a team-run Buzz relay, and
        ...
        to an attacker's WorkGuide account through ``/connect``. Both handlers make
    ...
    def _strip_own_mention(self, text: str) -> str:
        ...
        (e.g. "@Alice, @WorkGuide help"), guessing that the first one is ours risks
        ...
        alone rather than guess. The common single-mention case ("@WorkGuide
        ...
        if not sep:
            return stripped  # "@WorkGuide" alone: nothing to strip without losing the whole message
    ...
    async def _attach_connection_identity(self, inbound: InboundMessage) -> InboundMessage:
        ...
        bound WorkGuide account, and revoking the connection has no runtime effect.
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        DEFENSE IN DEPTH: text carrying one of WorkGuide's hidden model-context
