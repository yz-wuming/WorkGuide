...
def reset_api_key_warned():
    import workguide.community.tencent_wsa.tools as wsa
...
class TestTencentWsaApiKey:
    def test_config_key_takes_precedence_over_environment(self, monkeypatch):
        ...
        with patch("workguide.community.tencent_wsa.tools.get_app_config") as get_config:
            get_config.return_value.get_tool_config.return_value = _tool_config({"api_key": "config-key"})

            from workguide.community.tencent_wsa.tools import _get_api_key
    ...
    def test_missing_key_returns_a_structured_error(self, monkeypatch):
        ...
        with patch("workguide.community.tencent_wsa.tools.get_app_config") as get_config:
            ...
            from workguide.community.tencent_wsa.tools import web_search_tool
...
class TestTencentWsaSearch:
    def test_search_normalizes_documented_pages_and_honors_config(self):
        ...
        with (
            patch("workguide.community.tencent_wsa.tools.get_app_config") as get_config,
            patch("workguide.community.tencent_wsa.tools.httpx.Client", return_value=context_manager),
        ):
    ...
    def test_empty_query_does_not_call_paid_api(self):
        with (
            patch("workguide.community.tencent_wsa.tools.get_app_config") as get_config,
            patch("workguide.community.tencent_wsa.tools.httpx.Client") as client,
        ):
            get_config.return_value.get_tool_config.return_value = _tool_config({"api_key": "test-key"})
...
class TestTencentWsaConfiguration:
    ...
    def test_coerce_max_results(self, value, expected):
        from workguide.community.tencent_wsa.tools import _coerce_max_results
    ...
    def test_invalid_mode_is_omitted(self, value):
        with patch("workguide.community.tencent_wsa.tools.get_app_config") as get_config:
            ...
            from workguide.community.tencent_wsa.tools import _get_mode
