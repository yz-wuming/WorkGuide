...
``workguide.config.file_signature.get_config_signature`` was extracted from
verbatim-duplicate implementations that used to live independently in
``workguide.config.app_config`` and ``workguide.mcp.cache`` (flagged in review
on PR #4124: "now a verbatim duplicate of
``workguide/config/app_config.py::_get_config_signature`` / ``_ConfigSignature``
...
from workguide.config.file_signature import ConfigSignature, get_config_signature
