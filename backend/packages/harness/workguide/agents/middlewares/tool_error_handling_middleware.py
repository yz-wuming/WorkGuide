...
from workguide.agents.middlewares.skill_context import (
    SKILL_CONTEXT_ENTRY_KEY,
    _tool_call_path,
    build_skill_entry_metadata_from_read,
)
from workguide.agents.middlewares.tool_result_meta import (
    normalize_tool_result,
    stamp_exception_meta,
)
from workguide.config.app_config import AppConfig
from workguide.config.summarization_config import DEFAULT_SKILL_FILE_READ_TOOL_NAMES
from workguide.constants import DEFAULT_SKILLS_CONTAINER_PATH
from workguide.subagents.status_contract import (
...
def _build_runtime_middlewares(
    *,
    ...
    from workguide.agents.middlewares.tool_result_sanitization_middleware import ToolResultSanitizationMiddleware
    ...
    # gap the ledger. Stamping out here still sees workguide_tool_meta on
    ...
    if guardrails_config.enabled and guardrails_config.provider:
        ...
        from workguide.guardrails.middleware import GuardrailMiddleware
        from workguide.reflection import resolve_variable
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "workguide"
    ...
    # a ToolProgress slot.  The middleware stamps workguide_tool_meta on the blocked
    ...
    # chain includes ToolErrorHandlingMiddleware (inner), which stamps workguide_tool_meta
    ...
    # Ordering invariants are declared in workguide.extensions.ordering and
...
def build_subagent_runtime_middlewares(
    *,
    ...
    if app_config is None:
        from workguide.config import get_app_config
    ...
    # WorkGuideSummarizationMiddleware — subagents inherit none of the lead's
    ...
    from workguide.agents.middlewares.summarization_middleware import create_summarization_middleware
