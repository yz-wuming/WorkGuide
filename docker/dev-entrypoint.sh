#!/usr/bin/env sh
#
# WorkGuide gateway dev entrypoint — runs inside the docker-compose-dev gateway
...
#      workspace member extras (workguide-harness's
...
# WORKGUIDE_PROJECT_ROOT points at /app for runtime path translation. Prefer
# both locations, then the checkout-relative path used by direct invocations.
ENTRYPOINT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
DETECTOR_PATH=""
for candidate in \
    "${WORKGUIDE_PROJECT_ROOT:+$WORKGUIDE_PROJECT_ROOT/scripts/detect_uv_extras.py}" \
...
# means `sandbox` must be created here too, not just `.workguide`.
: "${WORKGUIDE_HOME:=/app/backend/.workguide}"
export WORKGUIDE_HOME
mkdir -p "$WORKGUIDE_HOME" /app/backend/.workguide /app/backend/sandbox
...
# docker-compose-dev's default WORKGUIDE_STREAM_BRIDGE_REDIS_URL is translated
...
PYTHONPATH=. exec uv run --no-sync uvicorn app.gateway.app:app \
    ...
    --reload-exclude="$WORKGUIDE_HOME" \
    --reload-exclude=/app/backend/.workguide
