#!/usr/bin/env bash
#
# serve.sh — Unified WorkGuide service launcher
#
# Usage:
...
# Every workguide worktree (the main checkout + each linked worktree) hardcodes
...
# WORKGUIDE_ROOTS is that set of roots; processes living outside all of them
...
# True if PID has an open file/cwd under any workguide worktree root. The
# trailing slash keeps a sibling dir like ".../workguide-notes" from matching
# the ".../workguide" root.
_is_workguide_pid() {
    local pid=$1 files root

    # Daemon children inherit WORKGUIDE_DAEMON_ROOT from run_service. Checking
    ...
    if [ -r "/proc/$pid/environ" ] &&
        tr '\0' '\n' < "/proc/$pid/environ" 2>/dev/null | grep -Fxq "WORKGUIDE_DAEMON_ROOT=$REPO_ROOT"; then
        return 0
    fi
...
_report_reclaimed_ports() {
    ...
    for port in 8001 3000 2026; do
        for pid in $(lsof -nP -iTCP:"$port" -sTCP:LISTEN -t 2>/dev/null); do
            _is_workguide_pid "$pid" || continue
...
_kill_repo_processes() {
    ...
    while IFS= read -r pid; do
        if [ -n "$pid" ] && _is_workguide_pid "$pid"; then
            case " $pids " in
                *" $pid "*) ;;
                *) pids="$pids $pid" ;;
            esac
...
stop_all() {
    ...
    # so a lingering nginx (or any workguide process) that _kill_repo_nginx did
    ...
    ./scripts/cleanup-containers.sh workguide-sandbox 2>/dev/null || true
...
if ! WORKGUIDE_PNPM_PYTHON="$(_pick_python)"; then
    echo "Python 3 is required to run pnpm."
    exit 1
fi
WORKGUIDE_PNPM_RUNNER="$REPO_ROOT/scripts/pnpm.py"
export WORKGUIDE_PNPM_PYTHON WORKGUIDE_PNPM_RUNNER

# Frontend command
if $DEV_MODE; then
    FRONTEND_CMD='env PORT=3000 "$WORKGUIDE_PNPM_PYTHON" "$WORKGUIDE_PNPM_RUNNER" run dev'
    ...
    FRONTEND_CMD="env PORT=3000 BETTER_AUTH_SECRET=$($WORKGUIDE_PNPM_PYTHON -c 'import secrets; print(secrets.token_hex(16))') \"\$WORKGUIDE_PNPM_PYTHON\" \"\$WORKGUIDE_PNPM_RUNNER\" run start"
else
    FRONTEND_CMD="env PORT=3000 BETTER_AUTH_SECRET=$($WORKGUIDE_PNPM_PYTHON -c 'import secrets; print(secrets.token_hex(16))') \"\$WORKGUIDE_PNPM_PYTHON\" \"\$WORKGUIDE_PNPM_RUNNER\" run preview"
fi

# Runtime path defaults. Local `make dev` launches Gateway from `backend/`,
# so pin WorkGuide-owned state to the expected backend runtime directory and
# create it before uvicorn builds its reload exclude filter.
if [ -z "$WORKGUIDE_PROJECT_ROOT" ]; then
    export WORKGUIDE_PROJECT_ROOT="$REPO_ROOT"
fi

BACKEND_RUNTIME_HOME="$REPO_ROOT/backend/.workguide"
if [ -z "$WORKGUIDE_HOME" ]; then
    export WORKGUIDE_HOME="$BACKEND_RUNTIME_HOME"
fi
...
mkdir -p "$WORKGUIDE_HOME" "$BACKEND_RUNTIME_HOME" "$REPO_ROOT/backend/sandbox"
WORKGUIDE_HOME="$(cd "$WORKGUIDE_HOME" && pwd -P)"
BACKEND_RUNTIME_HOME="$(cd "$BACKEND_RUNTIME_HOME" && pwd -P)"
export WORKGUIDE_HOME

# Extra flags for uvicorn
if $DEV_MODE && ! $DAEMON_MODE; then
    GATEWAY_EXTRA_FLAGS="--reload --reload-include='*.yaml' --reload-include='.env' --reload-exclude='*.pyc' --reload-exclude='__pycache__' --reload-exclude='$REPO_ROOT/backend/sandbox' --reload-exclude='$WORKGUIDE_HOME' --reload-exclude='$BACKEND_RUNTIME_HOME'"
else
    GATEWAY_EXTRA_FLAGS=""
fi
...
if ! { \
        [ -n "$WORKGUIDE_CONFIG_PATH" ] && [ -f "$WORKGUIDE_CONFIG_PATH" ] || \
        [ -f backend/config.yaml ] || \
        [ -f config.yaml ]; \
    }; then
    echo "✗ No WorkGuide config file found."
...
if ! $SKIP_INSTALL; then
    ...
    # `--all-packages` propagates extras into workspace members (workguide-harness
    ...
    (cd frontend && "$WORKGUIDE_PNPM_PYTHON" "$WORKGUIDE_PNPM_RUNNER" install --silent) || { echo "✗ Frontend dependency install failed"; exit 1; }
...
run_service() {
    ...
    if $DAEMON_MODE; then
        ...
        # carries WORKGUIDE_DAEMON_ROOT in its environment, letting
        # _is_workguide_pid recognize it at stop time.
        nohup env WORKGUIDE_DAEMON_ROOT="$REPO_ROOT" sh -c "$cmd" > /dev/null 2>&1 &
    else
        sh -c "$cmd" &
    fi
...
echo ""
echo "=========================================="
echo "  ✓ WorkGuide is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""
