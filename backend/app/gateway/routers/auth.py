    ...
    if getattr(request.state, "auth_source", None) == AUTH_SOURCE_AUTH_DISABLED:
        raise HTTPException(
            ...
            detail=AuthErrorResponse(
                ...
                message="Password changes are not available when WORKGUIDE_AUTH_DISABLED=1.",
            ).model_dump(),
        )
...
async def oauth_callback(
    ...
    the ID token, provisions/links the WorkGuide user, and sets the
    session cookie.
    """
    from workguide.config.app_config import get_app_config
