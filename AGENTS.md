...
WorkGuide is a LangGraph-based AI super-agent system with a full-stack architecture. The
...
│   ├── packages/harness/           # workguide-harness package (import: workguide.*) — agent framework
│   └── app/                        # FastAPI Gateway + IM channels (import: app.*)
├── frontend/                       # Next.js frontend (pnpm) — see frontend/AGENTS.md
├── docker/                         # docker-compose files, nginx config, provisioner
├── skills/                         # Agent skills: public/ (committed), custom/ (gitignored)
│                                    # Managed integration skill packs are global at .workguide/integrations/skills/{provider}/
...
five. Manage them with `workguide extensions install/list/enable/disable/remove` or the root
...
[the extensions guide](backend/packages/harness/workguide/extensions/AGENTS.md).
...
Docker log and restart commands resolve `WORKGUIDE_ROOT` from the current
...
cd frontend && pnpm dev       # Dev server: Webpack on Windows, Turbopack elsewhere (override with WORKGUIDE_DEV_BUNDLER)
...
- **Version sources must stay in lockstep** — a release version must match identically in
  `backend/pyproject.toml`, `frontend/package.json`, and `deploy/helm/workguide/Chart.yaml`
