...
def test_api_package_does_not_import_workguide():
    """The API package must stay independent of the host so extensions can
    depend on it alone. A `workguide` import here would silently couple every
    extension to the harness release cadence."""
    import pathlib

    import workguide_extension_api

    root = pathlib.Path(workguide_extension_api.__file__).parent
    offenders = []
    for path in root.rglob("*.py"):
        text = path.read_text(encoding="utf-8")
        for lineno, line in enumerate(text.splitlines(), start=1):
            stripped = line.strip()
            if stripped.startswith(("import workguide", "from workguide")) and not stripped.startswith(("import workguide_extension_api", "from workguide_extension_api")):
                offenders.append(f"{path.name}:{lineno}: {stripped}")
    assert offenders == [], "workguide-extension-api must not import workguide: " + "; ".join(offenders)
