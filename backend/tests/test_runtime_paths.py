...
from workguide.config import app_config as app_config_module
from workguide.config import extensions_config as extensions_config_module
from workguide.config import skills_config as skills_config_module
...
from workguide.config.runtime_paths import project_root
from workguide.config.skills_config import SkillsConfig
from workguide.skills.storage import get_or_new_skill_storage


def _clear_path_env(monkeypatch):
    for name in (
        "WORKGUIDE_CONFIG_PATH",
        "WORKGUIDE_EXTENSIONS_CONFIG_PATH",
        "WORKGUIDE_HOME",
        "WORKGUIDE_PROJECT_ROOT",
        "WORKGUIDE_SKILLS_PATH",
    ):
...
def test_default_runtime_paths_resolve_from_current_project(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / "config.yaml").write_text(
        yaml.safe_dump({"sandbox": {"use": "workguide.sandbox.local:LocalSandboxProvider"}}),
        encoding="utf-8",
    )
    ...
    assert Paths().base_dir == tmp_path / ".workguide"
...
def test_work_guide_project_root_overrides_current_directory(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    ...
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(project_root))
    ...
    assert Paths().base_dir == project_root / ".workguide"
    assert SkillsConfig(path="custom-skills").get_skills_path() == project_root / "custom-skills"


def test_work_guide_skills_path_overrides_project_default(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("WORKGUIDE_SKILLS_PATH", "team-skills")
...
def test_work_guide_project_root_must_exist(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    missing_root = tmp_path / "missing"
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(missing_root))

    with pytest.raises(ValueError, match="does not exist"):
        project_root()


def test_work_guide_project_root_must_be_directory(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    project_root_file = tmp_path / "project-root"
    project_root_file.write_text("", encoding="utf-8")
    monkeypatch.setenv("WORKGUIDE_PROJECT_ROOT", str(project_root_file))
...
def test_app_config_falls_back_to_legacy_when_project_root_lacks_config(tmp_path: Path, monkeypatch):
    """When WORKGUIDE_PROJECT_ROOT is unset and cwd has no config.yaml, the
...
def test_skills_config_falls_back_to_legacy_when_project_root_lacks_skills(tmp_path: Path, monkeypatch):
    """When WORKGUIDE_PROJECT_ROOT is unset and cwd has no `skills/`, the legacy
...
def test_extensions_config_falls_back_to_legacy_when_project_root_lacks_file(tmp_path: Path, monkeypatch):
    ...
    fake_paths_module_file = fake_backend / "packages" / "harness" / "workguide" / "config" / "extensions_config.py"
...
def test_extensions_config_env_var_missing_file_raises(tmp_path: Path, monkeypatch):
    """``WORKGUIDE_EXTENSIONS_CONFIG_PATH`` pointing at a file that has since
    ...
    backend/AGENTS.md: "Docker development ... points `WORKGUIDE_CONFIG_PATH`
    / `WORKGUIDE_EXTENSIONS_CONFIG_PATH`" at the mounted config directory), so
    ...
    ``workguide.mcp.cache._resolve_config_path`` calls
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(cfg))

    assert ExtensionsConfig.resolve_config_path() == cfg  # sanity: resolves while present

    cfg.unlink()

    with pytest.raises(FileNotFoundError, match="WORKGUIDE_EXTENSIONS_CONFIG_PATH"):
...
def test_extensions_config_search_finds_nothing_returns_none(tmp_path: Path, monkeypatch):
    ...
    ``WORKGUIDE_EXTENSIONS_CONFIG_PATH``) must still return ``None`` — not
    ...
    though the explicit `config_path`/`WORKGUIDE_EXTENSIONS_CONFIG_PATH`
    ...
    ``workguide.mcp.cache._is_cache_stale`` depends on this fallback ``None``
