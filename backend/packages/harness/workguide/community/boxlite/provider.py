"""``BoxliteProvider`` — WorkGuide :class:`SandboxProvider` backed by BoxLite.

Integrates `BoxLite <https://github.com/boxlite-ai/boxlite>`_ — a daemonless,
OCI-native micro-VM runtime — as a WorkGuide sandbox backend. See
https://github.com/bytedance/workguide/issues/3936.
...
_BOX_NAME_PREFIX = "workguide-boxlite-"
_NO_ACTIVE_IDENTITY = object()
# WorkGuide's virtual prefixes, materialised on the box rootfs at start so the
...
def _import_simplebox() -> type[SimpleBox]:
    ...
    try:
        ...
        raise ImportError("BoxliteProvider requires the optional 'boxlite' dependency. Install it with: pip install 'workguide-harness[boxlite]' or pip install boxlite.") from e
    return SimpleBox
...
class _EventLoopThread:
    ...
    BoxLite is async-native and its box handles are loop-affine, while WorkGuide's
...
class BoxliteProvider(WarmPoolLifecycleMixin[BoxliteBox], SandboxProvider):
    """Run each WorkGuide sandbox as a BoxLite micro-VM."""
    ...
    def _reconcile_orphans(self) -> None:
        """Adopt WorkGuide-owned BoxLite boxes left by a previous provider/process.

        BoxLite boxes are discovered by a WorkGuide-specific name prefix. Adopted
    ...
    def _create_box(self, sandbox_id: str) -> BoxliteBox:
        ...
        async def _make() -> SimpleBox:
            ...
            await box.start()
            # Materialise WorkGuide's virtual prefixes so file ops resolve natively.
