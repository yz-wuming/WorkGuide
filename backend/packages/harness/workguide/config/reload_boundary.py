...
Bytedance/workguide issue #3144: gateway request dependencies resolve
...
STARTUP_ONLY_FIELDS: dict[str, str] = {
    ...
    "log_level": (
        "apply_logging_level() runs only during app.py startup; it sets the workguide/app logger levels and may lower root handler thresholds so configured messages can propagate. A freshly reloaded AppConfig does not retrigger it."
    ),
    "logging": (
        ...
        "workguide_trace_id stay coherent. A freshly reloaded AppConfig does not retrigger any of this."
    ),
