...
integration tier: point at a server with ``WORKGUIDE_TEST_REDIS_URL`` (defaults
...
from workguide.community.aio_sandbox.ownership import (
...
REDIS_TEST_URL = os.environ.get("WORKGUIDE_TEST_REDIS_URL", "redis://localhost:6379/15")
...
class _StoreFactory:
    ...
    def __init__(self, kind: str, ttl_seconds: float = 60.0):
        ...
        self._key_prefix = f"workguide:test:{uuid.uuid4().hex}"
        self._made: list = []
    ...
    def make(self, owner_id: str, *, ttl_seconds: float | None = None):
        ...
        if self.kind == "memory":
            ...
            from workguide.community.aio_sandbox.ownership.redis import RedisOwnershipStore
...
def test_stream_bridge_redis_env_implies_redis_ownership(monkeypatch):
    ...
    """
    monkeypatch.setenv("WORKGUIDE_STREAM_BRIDGE_REDIS_URL", "redis://somewhere:6379/0")
...
def test_redis_backend_error_is_wrapped_not_leaked():
    ...
    store = RedisOwnershipStore(
        ...
        ttl_seconds=60,
        key_prefix=f"workguide:test:{uuid.uuid4().hex}",
    )
...
def test_redis_store_declares_cross_process_support():
    ...
    store = RedisOwnershipStore(owner_id="A", redis_url=REDIS_TEST_URL, ttl_seconds=60, key_prefix=f"workguide:test:{uuid.uuid4().hex}")
    try:
