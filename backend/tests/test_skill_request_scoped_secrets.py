...
from workguide.sandbox.local.local_sandbox import LocalSandbox
from workguide.skills.types import SecretRequirement, Skill, SkillCategory
...
class TestLocalSandboxEnvInjection:
    ...
    def test_injected_env_visible_to_command(self):
        ...
        out = sandbox.execute_command(
            "echo $WORKGUIDE_TEST_SECRET",
            env={"WORKGUIDE_TEST_SECRET": "s3cret-value"},
        )
    ...
    def test_env_none_keeps_inherited_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("WORKGUIDE_INHERITED_VAR", "inherited-value")
        sandbox = LocalSandbox(id="local")
        out = sandbox.execute_command("echo $WORKGUIDE_INHERITED_VAR")
    ...
    def test_injected_env_is_per_call_only(self):
        ...
        sandbox = LocalSandbox(id="local")
        sandbox.execute_command("true", env={"WORKGUIDE_EPHEMERAL": "leaky"})
        out = sandbox.execute_command("echo [$WORKGUIDE_EPHEMERAL]")
    ...
    def test_benign_env_still_inherited_after_scrub(self, monkeypatch):
        ...
        monkeypatch.setenv("WORKGUIDE_PLAIN_VAR", "harmless-value")
        sandbox = LocalSandbox(id="local")
        out = sandbox.execute_command("echo [$WORKGUIDE_PLAIN_VAR]")
...
class TestAioSandboxEnvInjection:
    @pytest.fixture
    def sandbox(self):
        with patch("workguide.community.aio_sandbox.aio_sandbox.AioSandboxClient"):
    ...
    def test_env_path_retries_on_error_observation_signature(self, sandbox):
        ...
        from workguide.community.aio_sandbox.aio_sandbox import _ERROR_OBSERVATION_SIGNATURE
...
class TestEnvPolicy:
    ...
    def test_secret_like_names_are_blocked(self, name):
        from workguide.sandbox.env_policy import is_blocked_env_name
    ...
    def test_db_password_vars_do_not_reach_the_subprocess_env(self, monkeypatch):
        ...
        """
        from workguide.sandbox.env_policy import build_sandbox_env
...
class TestRequiredSecretsParsing:
    ...
    def test_absent_field_defaults_to_empty(self, tmp_path):
        from workguide.skills.parser import parse_skill_file
...
class TestSecretCarrier:
    ...
    def test_runtime_context_carries_secrets(self):
        from workguide.runtime.runs.worker import _build_runtime_context
    ...
    def test_slash_skill_source_path_public_contract(self):
        from workguide.runtime.secret_context import read_slash_skill_source_path, write_slash_skill_source_path
    ...
    def test_slash_skill_source_path_rejects_malformed_shapes(self):
        from workguide.runtime.secret_context import read_slash_skill_source_path
...
class TestActivationBindsSecrets:
    ...
    def _activate(self, tmp_path, monkeypatch, skill, context):
        from workguide.agents.middlewares import skill_activation_middleware as mw
    ...
    def test_activation_fires_after_input_sanitization_wrapping(self, tmp_path, monkeypatch):
        ...
        from workguide.agents.middlewares.skill_activation_middleware import SkillActivationMiddleware
        from workguide.config.app_config import AppConfig, reset_app_config, set_app_config
        from workguide.runtime.secret_context import read_active_secrets
        ...
        set_app_config(AppConfig.model_validate({"sandbox": {"use": "workguide.sandbox.local:LocalSandboxProvider"}}))
        try:
...
class TestInContextBindsSecrets:
    ...
    def test_forged_slash_source_cannot_bypass_gates(self, tmp_path, monkeypatch):
        ...
        from workguide.runtime.secret_context import _SLASH_SECRET_SOURCE_KEY, read_active_secrets
...
class TestBashToolInjectsActiveSecrets:
    ...
    def _run_bash(self, context):
        from workguide.sandbox import tools as tools_mod
    ...
    def test_local_bash_forwards_env_and_timeout(self, monkeypatch):
        ...
        with (
            ...
            patch("workguide.config.app_config.get_app_config", return_value=fake_cfg),
        ):
...
class TestLeakSurfaces:
    ...
    def test_trace_metadata_has_no_secret(self, monkeypatch):
        from workguide.tracing import metadata as meta
    ...
    def test_redact_helper_strips_secret_keys(self):
        from workguide.runtime.secret_context import SKILL_TOOL_POLICY_DECISION_CONTEXT_KEY, redact_secret_context_keys
    ...
    def test_redact_config_secrets_strips_from_persisted_config(self):
        ...
        from workguide.runtime.secret_context import SKILL_TOOL_POLICY_DECISION_CONTEXT_KEY, redact_config_secrets
    ...
    def test_redact_config_secrets_handles_none_and_no_context(self):
        from workguide.runtime.secret_context import redact_config_secrets
    ...
    def test_stdout_surface_redacted(self):
        from workguide.sandbox.tools import mask_secret_values
