...
rule** (read before writing a backend):
    ...
    It MUST NOT import workguide modules or hardcode workguide paths. The ONLY
    ``from workguide`` line allowed in the whole backend folder is the ABC
...
What the factory (``manager.py::get_memory_manager``) provides to each backend:
  - ``backend_config["storage_path"]`` (str): a writable state dir (the host's
    ...
    storage root** -- do NOT call a workguide path helper yourself.
