...
def reset_api_key_warned():
    ...
    import workguide.community.serply.tools as serply_mod
...
def _patch_config(extra: dict | None):
    ...
    patcher = patch("workguide.community.serply.tools.get_app_config")
    mock = patcher.start()
    if extra is None:
...
def _run(query: str = "test query", max_results: int = 5) -> dict:
    from workguide.community.serply.tools import web_search_tool
...
class TestGetApiKey:
    def test_returns_config_key_when_present(self):
        ...
        try:
            from workguide.community.serply.tools import _get_api_key
...
class TestCoerceMaxResults:
    def test_returns_value_when_valid(self):
        from workguide.community.serply.tools import _coerce_max_results
...
class TestCoerceVertical:
    def test_accepts_known_verticals(self):
        from workguide.community.serply.tools import _coerce_vertical
...
class TestWebSearchTool:
    def test_basic_search_returns_normalized_results(self, mock_config_with_key):
        with patch("workguide.community.serply.tools.httpx.Client") as mock_client:
