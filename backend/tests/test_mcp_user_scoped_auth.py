...
from workguide.mcp.user_scoped_auth import build_user_scoped_auth_interceptor
...
def test_default_user_fallback_is_denied_when_unmapped():
    ...
    interceptor = build_user_scoped_auth_interceptor(_config(users={"u1": "Bearer t1"}))
    with patch("workguide.mcp.user_scoped_auth._current_runtime", return_value=None), pytest.raises(ToolException):
...
def test_stdio_server_user_auth_is_skipped_with_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="workguide.mcp.user_scoped_auth"):
...
def test_user_credential_wins_over_oauth_set_header_through_real_composition():
    ...
    tool path uses."""
    from workguide.mcp.interceptors import compose_tool_interceptors
