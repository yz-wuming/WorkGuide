...
def _enable_runtime_authorization(monkeypatch, provider) -> AuthorizationConfig:
    ...
    """
    monkeypatch.setattr(
        "workguide.agents.lead_agent.agent.resolve_authorization_provider",
...
def test_authorize_model_name_disabled_is_noop():
    ...
    from workguide.agents.lead_agent.agent import _authorize_model_name
...
# ── WorkGuideClient._ensure_agent path ─────────────────────────────────
# Regression for willem-bd's Round 4 coverage observation: the embedded/library
# lead-agent construction path (``WorkGuideClient._ensure_agent``) must enforce
...
def _stub_client_assembly(monkeypatch) -> dict[str, str]:
    ...
    monkeypatch.setattr(
        "workguide.client.create_chat_model",
        lambda **kwargs: captured.__setitem__("name", kwargs.get("name")) or object(),
    )
    monkeypatch.setattr("workguide.client.create_agent", lambda **kwargs: object())
    monkeypatch.setattr("workguide.client.build_middlewares", lambda *args, **kwargs: [])
    monkeypatch.setattr("workguide.client.WorkGuideClient._get_tools", staticmethod(lambda *, model_name, subagent_enabled: []))  # noqa: ARG005
    monkeypatch.setattr("workguide.client.get_enabled_skills_for_config", lambda app_config: [])  # noqa: ARG005
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "workguide.client.assemble_deferred_tools",
        lambda tools, *, enabled: ([], SimpleNamespace(deferred_names=frozenset())),  # noqa: ARG005
    )
    monkeypatch.setattr("workguide.client.build_mcp_routing_middleware", lambda *args, **kwargs: None)  # noqa: ARG005
    monkeypatch.setattr("workguide.client.get_mcp_routing_hints_prompt_section", lambda *args, **kwargs: "")  # noqa: ARG005
    monkeypatch.setattr("workguide.client.apply_prompt_template", lambda **kwargs: "")  # noqa: ARG005
    monkeypatch.setattr("workguide.client.get_thread_state_schema", lambda *args, **kwargs: object())  # noqa: ARG005
    monkeypatch.setattr("workguide.client.normalize_middleware_state_schemas", lambda schemas, mode, freq: [])  # noqa: ARG005
    monkeypatch.setattr("workguide.client.get_effective_user_id", lambda: "user-123")
    ...
    monkeypatch.setattr(
        "workguide.authz.tool_filter.resolve_authorization_provider",
...
def _bare_client(app_config):
    """Construct a ``WorkGuideClient`` without running ``__init__``."""
    from workguide.client import WorkGuideClient

    client = WorkGuideClient.__new__(WorkGuideClient)
