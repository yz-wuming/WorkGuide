...
def test_sqlite_round_trip_new_fields():
    ...
    run_events, and feedback use). The old separate .workguide/users.db
    file is gone.
    """
    import asyncio
    import tempfile
    ...
    async def _run() -> None:
        from workguide.persistence.engine import (
            close_engine,
            get_session_factory,
            init_engine,
        )
...
def test_oidc_login_blocked_by_existing_local_account_across_case(tmp_path):
    ...
    from workguide.config.auth_config import OIDCProviderConfig
    ...
    async def _run() -> None:
        ...
        try:
            ...
            cfg = OIDCProviderConfig(display_name="Test SSO", issuer="https://issuer.example.com", client_id="workguide", auto_create_users=True)
