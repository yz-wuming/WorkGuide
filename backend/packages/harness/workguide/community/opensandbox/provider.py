"""OpenSandbox-backed community ``SandboxProvider`` for WorkGuide."""
...
def _import_sdk() -> tuple[type[SandboxSync], type[ConnectionConfigSync], type[RunCommandOpts]]:
    ...
    try:
        ...
        raise ImportError("OpenSandboxProvider requires the optional 'opensandbox' dependency. Install it with: pip install 'workguide-harness[opensandbox]' or pip install 'opensandbox>=0.1.15,<0.2.0'.") from exc
...
class OpenSandboxProvider(WarmPoolLifecycleMixin[OpenSandboxSandbox], SandboxProvider):
    ...
    def _create_sandbox(self, sandbox_id: str, *, thread_id: str | None, user_id: str | None) -> OpenSandboxSandbox:
        ...
        metadata = {"work_guide_provider": "opensandbox"}
        if thread_id is not None:
            metadata["work_guide_thread"] = thread_id
        if user_id is not None:
            metadata["work_guide_user"] = user_id
