...
**Lead Agent** (`packages/harness/workguide/agents/lead_agent/agent.py`):
...
- `assemble_lead_agent(config, *, app_config=None) -> LeadAgentAssembly(graph, descriptor)`
  ...
  `workguide/agents/assembly_descriptor.py::build_assembly_descriptor()` and captures what
...
**ThreadState** (`packages/harness/workguide/agents/thread_state.py`):
...
- `max_total_subagents` - Optional per-run total delegation cap override (falls back to `subagents.max_total_per_run`, clamped to 1-50)
  Gateway and `WorkGuideClient.stream()` always provide the runtime `run_id`; custom
