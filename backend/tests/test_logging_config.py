...
from workguide.logging_config import TraceContextFilter, configure_logging
from workguide.trace_context import request_trace_context


def test_trace_context_filter_injects_current_trace_id() -> None:
    record = logging.LogRecord("workguide.test", logging.INFO, __file__, 1, "hello", (), None)
...
def test_configure_logging_enhanced_text_includes_trace_id() -> None:
    ...
    try:
        ...
        with request_trace_context("trace-log-2"):
            logging.getLogger("workguide.test").info("hello")
