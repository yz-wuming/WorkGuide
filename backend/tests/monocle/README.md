# WorkGuide behavioural tests (Monocle Test Tools)

Trace-based tests for WorkGuide. Monocle records each run as a structured trace
...
- **One offline example** (`test_assertion_api_example`) loads a recorded trace
  ...
  format and the asserter wiring, not WorkGuide's behaviour. Treat it as the
...
- `test_workguide.py` — the offline example + two live tests
- `conftest.py` — the `run_agent` fixture (live path only)
- `_helpers.py` — paths and `run_workguide()`
...
it embeds the WorkGuide system prompt as of the recording date and the content
...
they still skip if the WorkGuide app is not importable or `config.yaml` is
...
so there is no hard-coded key requirement. WorkGuide's `web_search` is
...
1. Run WorkGuide under Monocle and capture a trace of a run you are happy with
