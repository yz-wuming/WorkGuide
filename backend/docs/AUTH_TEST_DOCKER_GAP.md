...
|---|---|---|---|
| TC-DOCKER-01 | `workguide.db` volume persistence | Verify the `WORKGUIDE_HOME` bind mount survives container restart | needs `docker compose up` |
...
| TC-DOCKER-05 (credential surfacing) | `reset_admin` writes `.workguide/admin_initial_credentials.txt` with mode 0600 and logs only the path — the only Docker-unique step is whether the bind mount projects this path onto the host, which is a `docker compose` config check, not a runtime behavior change |
...
# Optional: pin WORKGUIDE_HOME to a stable host path
echo "WORKGUIDE_HOME=$HOME/workguide-data" >> .env
```
