# Contributing to WorkGuide Backend

Thank you for your interest in contributing to WorkGuide! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/workguide.git
   cd workguide
   ```
...
```
backend/
├── packages/harness/workguide/  # workguide-harness package (import: workguide.*)
│   ├── agents/                 # Agent system
...
```python
import pytest
from workguide.models.factory import create_chat_model
...
1. Create tool in `packages/harness/workguide/tools/builtins/` or `packages/harness/workguide/community/`:

```python
# packages/harness/workguide/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: workguide.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/workguide/agents/middlewares/`:

```python
# packages/harness/workguide/agents/middlewares/my_middleware.py
...
1. Update `packages/harness/workguide/config/app_config.py` with new fields
...
Thank you for contributing to WorkGuide!
