...
from workguide.extensions.loader import (
    Diagnostic,
    ExtensionLoadError,
    ExtensionSpec,
    load_extensions,
...
def test_host_disabled_required_extension_is_skipped_before_resolution(monkeypatch):
    ...
    monkeypatch.setattr("workguide.extensions.loader.resolve_variable", _must_not_resolve)
...
def test_manager_metadata_is_accepted_without_reaching_the_install_hook():
    ...
    assert spec.package == "workguide-extension-demo"
...
def test_optional_extension_with_non_string_api_marker_is_skipped_with_a_diagnostic(monkeypatch):
    monkeypatch.setattr(demo_extensions.install_ok, "__workguide_api__", 101, raising=False)
    spec = ExtensionSpec(use=f"{_FIXTURE}:install_ok")
...
def test_extension_api_marker_getter_failure_obeys_required_policy(monkeypatch, required):
    ...
    monkeypatch.setattr(
        "workguide.extensions.loader.resolve_variable",
...
def test_compatible_follows_semver_windows():
    ...
    from workguide.extensions.loader import _compatible
...
def test_a_successful_load_is_reported(caplog):
    ...
    with caplog.at_level("INFO", logger="workguide.extensions.loader"):
...
def test_host_registry_satisfies_the_public_contract():
    ...
    from workguide_extension_api import ExtensionRegistry as ContractRegistry

    from workguide.extensions.registry import ExtensionRegistry as HostRegistry
