...
def _deploy_fixture(tmp_path: Path, *, docker_script: str) -> tuple[Path, dict[str, str]]:
    ...
    (worktree / "config.yaml").write_text("sandbox:\n  use: workguide.sandbox:LocalSandboxProvider\n", encoding="utf-8")
    ...
    env["WORKGUIDE_INTERNAL_AUTH_TOKEN"] = "test-internal-auth-token"
    return worktree, env
...
def test_deploy_waits_for_gateway_readiness_before_success(tmp_path: Path) -> None:
    ...
    assert "WorkGuide is running!" in result.stdout
...
def test_deploy_failure_prints_gateway_diagnostics_and_never_claims_success(tmp_path: Path) -> None:
    ...
    assert "WorkGuide is running!" not in result.stdout
    assert "WorkGuide services failed to become ready" in result.stderr
