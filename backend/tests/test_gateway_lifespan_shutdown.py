...
async def _run_lifespan_with_hanging_stop() -> float:
    ...
    with (
        ...
        patch("workguide.agents.memory.get_memory_manager", return_value=MagicMock()),
    ):
...
async def _run_lifespan_with_mcp_task_config_snapshot() -> None:
    ...
    from workguide.mcp.tasks.runtime import McpTaskConfigurationError, validate_mcp_task_config_snapshot
    ...
    with (
        ...
        patch("workguide.skills.projection.ensure_public_skill_projection"),
        patch("workguide.agents.memory.get_memory_manager", return_value=MagicMock()),
        patch("workguide.config.extensions_config.ExtensionsConfig.from_file", return_value=startup_extensions),
    ):
        async with lifespan(app):
...
async def _run_lifespan_with_memory_flush(
    *,
    ...
    with (
        ...
        patch("app.channels.service.start_channel_service", side_effect=fake_start),
        patch("app.channels.service.stop_channel_service", stop_channel_service),
        patch("workguide.agents.memory.get_memory_manager", return_value=manager),
        patch("workguide.extensions.notify.suspend_extension_system_observations", suspend_system_observations),
    ):
        async with lifespan(app):
            pass
