"""Tests for WorkGuideClient's graph-root tracing wiring.
...
make sure ``WorkGuideClient`` injects the tracing callbacks at the graph
...
from workguide.trace_context import WORKGUIDE_TRACE_METADATA_KEY, request_trace_context
...
def _stub_agent_creation(monkeypatch, fake_agent: _FakeAgent) -> dict[str, Any]:
    ...
    monkeypatch.setattr(WorkGuideClient, "_ensure_agent", _stub_ensure_agent)
    return captured


def _make_client(_monkeypatch, *, enhance_enabled: bool = True) -> WorkGuideClient:
    ...
    :func:`WorkGuideClient.stream` consults to gate request-trace binding
...
def test_stream_injects_langfuse_metadata_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr("workguide.client.build_tracing_callbacks", lambda: [sentinel])
    ...
    assert metadata.get(WORKGUIDE_TRACE_METADATA_KEY)
...
def test_stream_is_inert_when_langfuse_disabled(monkeypatch):
    monkeypatch.setattr("workguide.client.build_tracing_callbacks", lambda: [])
...
def test_stream_preserves_caller_metadata_overrides(monkeypatch):
    ...
    original_get_config = WorkGuideClient._get_runnable_config

    def patched_get_runnable_config(self, thread_id, **overrides):
        cfg = original_get_config(self, thread_id, **overrides)
        cfg["metadata"] = {
            WORKGUIDE_TRACE_METADATA_KEY: "explicit-client-trace",
            "langfuse_session_id": "explicit-session-override",
            "langfuse_user_id": "explicit-user",
        }
        return cfg

    monkeypatch.setattr(WorkGuideClient, "_get_runnable_config", patched_get_runnable_config)
    ...
    assert metadata[WORKGUIDE_TRACE_METADATA_KEY] == "explicit-client-trace"
    # ``trace_name`` was not supplied by caller so the worker still fills it.
    assert metadata["langfuse_trace_name"] == "lead-agent"


def test_stream_omits_workguide_trace_id_when_enhance_disabled(monkeypatch):
    ...
    default config would silently gain a new indexed ``workguide_trace_id``
    ...
    assert WORKGUIDE_TRACE_METADATA_KEY not in metadata
...
def test_stream_respects_caller_bound_trace_when_enhance_disabled(monkeypatch):
    ...
    metadata = captured["config"].get("metadata") or {}
    assert metadata.get(WORKGUIDE_TRACE_METADATA_KEY) == "caller-opt-in"
...
def test_stream_does_not_leak_trace_id_to_caller_context_between_yields(monkeypatch):
    ...
    from workguide.trace_context import get_current_trace_id
