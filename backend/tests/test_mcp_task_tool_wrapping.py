...
from workguide.mcp.tools import _configure_task_tools_for_server, get_mcp_tools
...
def test_submit_wrapper_preserves_server_description_and_appends_background_contract() -> None:
    ...
    assert configured[0].description == (
        "Generate a quarterly financial report for the requested topic.\n\nSubmitted as durable background task 'report-generation'; returns a WorkGuide task ID immediately and status polling is handled automatically."
    )
...
async def test_tool_reload_rejects_task_server_runtime_config_drift() -> None:
    ...
    try:
        with (
            patch(
                "workguide.mcp.tools.ExtensionsConfig.from_file",
