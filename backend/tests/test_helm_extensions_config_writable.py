...
CHART = REPO_ROOT / "deploy" / "helm" / "workguide"
GATEWAY_TEMPLATE = CHART / "templates" / "gateway-deployment.yaml"
RUNTIME_CONFIG_PATH = "/app/backend/.workguide/extensions-config/extensions_config.json"
...
def _render_chart(*settings: str) -> list[dict]:
    ...
    command = [helm, "template", "workguide", str(CHART)]
...
def test_rendered_helm_extensions_config_is_writable_and_seeded(persistence_enabled: bool) -> None:
    ...
    assert env["WORKGUIDE_EXTENSIONS_CONFIG_PATH"]["value"] == RUNTIME_CONFIG_PATH
    ...
    assert runtime_mount["subPath"] == "workguide/extensions-config"

    home_mount = _named(gateway["volumeMounts"], "home")
    assert home_mount["mountPath"] == "/app/backend/.workguide"
    assert home_mount["subPath"] == "workguide"
