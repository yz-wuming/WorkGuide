...
from workguide.config.paths import Paths, join_host_path
...
def test_get_lark_cli_runtime_mounts_uses_user_auth_dirs(tmp_path, monkeypatch):
    ...
    aio_mod = importlib.import_module("workguide.community.aio_sandbox.aio_sandbox_provider")
    lark_cli = importlib.import_module("workguide.integrations.lark_cli")
    monkeypatch.setattr(aio_mod, "get_paths", lambda: Paths(base_dir=tmp_path))
    monkeypatch.setattr(aio_mod, "get_effective_user_id", lambda: "default")
...
def test_get_extra_mounts_provisioner_payload_has_unique_container_paths(tmp_path, monkeypatch, provisioner_module):
    ...
    aio_mod = importlib.import_module("workguide.community.aio_sandbox.aio_sandbox_provider")
    lark_cli = importlib.import_module("workguide.integrations.lark_cli")
    remote_backend = importlib.import_module("workguide.community.aio_sandbox.remote_backend")
    skills_root = tmp_path / "skills"
    (skills_root / "public").mkdir(parents=True)
    home = tmp_path / "home"
    config = SimpleNamespace(
    ...
    provisioner_module.WORKGUIDE_HOST_BASE_DIR = str(home)
...
def test_join_host_path_preserves_windows_drive_letter_style():
    base = r"C:\Users\demo\workguide\backend\.workguide"

    joined = join_host_path(base, "threads", "thread-9", "user-data", "outputs")

    assert joined == r"C:\Users\demo\workguide\backend\.workguide\threads\thread-9\user-data\outputs"
...
def test_get_thread_mounts_preserves_windows_host_path_style(tmp_path, monkeypatch):
    ...
    aio_mod = importlib.import_module("workguide.community.aio_sandbox.aio_sandbox_provider")
    monkeypatch.setenv("WORKGUIDE_HOST_BASE_DIR", r"C:\Users\demo\workguide\backend\.workguide")
    monkeypatch.setattr(aio_mod, "get_paths", lambda: Paths(base_dir=tmp_path))
    monkeypatch.setattr(aio_mod, "get_effective_user_id", lambda: None)
    ...
    assert container_paths["/mnt/user-data/workspace"] == r"C:\Users\demo\workguide\backend\.workguide\threads\thread-10\user-data\workspace"
    assert container_paths["/mnt/user-data/uploads"] == r"C:\Users\demo\workguide\backend\.workguide\threads\thread-10\user-data\uploads"
    assert container_paths["/mnt/user-data/outputs"] == r"C:\Users\demo\workguide\backend\.workguide\threads\thread-10\user-data\outputs"
    assert container_paths["/mnt/acp-workspace"] == r"C:\Users\demo\workguide\backend\.workguide\threads\thread-10\acp-workspace"
...
def test_remote_backend_create_forwards_effective_user_id(monkeypatch):
    ...
    remote_mod = importlib.import_module("workguide.community.aio_sandbox.remote_backend")
...
def test_acquire_drops_dead_cached_sandbox(tmp_path, monkeypatch):
    ...
    provider._backend.create = MagicMock(
        return_value=aio_mod.SandboxInfo(
            ...
            container_name="workguide-sandbox-sandbox-dead",
        )
    )
    ...
    provider._warm_pool = {
        "sandbox-warm-dead": (
            aio_mod.SandboxInfo(
                ...
                container_name="workguide-sandbox-sandbox-warm-dead",
            ),
            0.0,
        )
    }
