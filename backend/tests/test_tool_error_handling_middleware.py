...
from workguide.agents.middlewares.tool_error_handling_middleware import (
    ToolErrorHandlingMiddleware,
    build_lead_runtime_middlewares,
    build_subagent_runtime_middlewares,
)
from workguide.agents.middlewares.tool_result_meta import TOOL_META_KEY
from workguide.agents.middlewares.view_image_middleware import ViewImageMiddleware
from workguide.config import summarization_config
from workguide.config.app_config import AppConfig, CircuitBreakerConfig
from workguide.config.extensions_config import ExtensionsConfig
from workguide.config.guardrails_config import GuardrailsConfig
from workguide.config.model_config import ModelConfig
from workguide.config.sandbox_config import SandboxConfig
from workguide.subagents.status_contract import SUBAGENT_ERROR_KEY, SUBAGENT_STATUS_KEY
...
def _stub_runtime_middleware_imports(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.llm_error_handling_middleware",
        _module(
            "workguide.agents.middlewares.llm_error_handling_middleware",
    ...
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.thread_data_middleware",
        _module("workguide.agents.middlewares.thread_data_middleware", ThreadDataMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "workguide.sandbox.middleware",
        _module("workguide.sandbox.middleware", SandboxMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.dangling_tool_call_middleware",
        _module("workguide.agents.middlewares.dangling_tool_call_middleware", DanglingToolCallMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.sandbox_audit_middleware",
        _module("workguide.agents.middlewares.sandbox_audit_middleware", SandboxAuditMiddleware=FakeMiddleware),
    )
...
def test_build_subagent_runtime_middlewares_threads_app_config_to_llm_middleware(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.input_sanitization_middleware",
        _module(
            "workguide.agents.middlewares.input_sanitization_middleware",
    ...
    from workguide.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    from workguide.agents.middlewares.dynamic_context_middleware import SubagentDateContextMiddleware
    from workguide.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
    from workguide.agents.middlewares.skill_activation_middleware import SkillActivationMiddleware
    from workguide.agents.middlewares.skill_tool_policy_middleware import SkillToolPolicyMiddleware
    from workguide.agents.middlewares.system_message_coalescing_middleware import SystemMessageCoalescingMiddleware
    from workguide.agents.middlewares.token_budget_middleware import TokenBudgetMiddleware
    from workguide.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
    from workguide.agents.middlewares.tool_receipt_middleware import ToolReceiptMiddleware
    ...
    # workguide_tool_meta status it stamps (guard-enforced, like ToolProgress).
...
def test_tool_progress_middleware_is_outer_relative_to_error_handling(monkeypatch: pytest.MonkeyPatch):
    ...
    # Only then can it read workguide_tool_meta stamped by ToolErrorHandlingMiddleware.
    from workguide.agents.middlewares.tool_progress_middleware import ToolProgressMiddleware
    from workguide.config.tool_progress_config import ToolProgressConfig
...
def test_middleware_ordering_guard_moved_to_declarative_constraints(monkeypatch: pytest.MonkeyPatch):
    ...
    declared in workguide.extensions.ordering (core_ordering_constraints / assert_ordering) and
    ...
    must no longer raise inside this builder; workguide.extensions.ordering has the equivalent
...
def test_lead_runtime_middlewares_thread_app_config_to_tool_error_handling(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setitem(
        sys.modules,
        "workguide.agents.middlewares.input_sanitization_middleware",
        _module("workguide.agents.middlewares.input_sanitization_middleware", InputSanitizationMiddleware=object),
    )
    app_config = _make_app_config()
    _stub_runtime_middleware_imports(monkeypatch)
...
def test_build_lead_runtime_middlewares_orders_thread_data_before_uploads():
    ...
    upload endpoint.
    """
    from workguide.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from workguide.agents.middlewares.uploads_middleware import UploadsMiddleware
...
def test_build_lead_runtime_middlewares_chain_order_matches_agents_md():
    ...
    """
    from workguide.agents.middlewares.dangling_tool_call_middleware import DanglingToolCallMiddleware
    from workguide.agents.middlewares.input_sanitization_middleware import InputSanitizationMiddleware
    from workguide.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
    from workguide.agents.middlewares.read_before_write_middleware import ReadBeforeWriteMiddleware
    from workguide.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
    from workguide.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from workguide.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
    from workguide.agents.middlewares.uploads_middleware import UploadsMiddleware
    from workguide.sandbox.middleware import SandboxMiddleware
...
def test_subagent_runtime_middlewares_attach_deferred_filter_when_setup_has_names(monkeypatch):
    ...
    from workguide.agents.middlewares.deferred_tool_filter_middleware import DeferredToolFilterMiddleware
    from workguide.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
    from workguide.tools.builtins.tool_search import build_deferred_tool_setup
...
def test_subagent_runtime_middlewares_place_mcp_routing_before_deferred_filter(monkeypatch):
    ...
    from workguide.tools.builtins.tool_search import DeferredToolSetup
...
def test_subagent_runtime_middlewares_attach_loop_detection_when_enabled(monkeypatch):
    ...
    from workguide.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
...
def test_subagent_runtime_middlewares_omit_loop_detection_when_disabled(monkeypatch):
    ...
    from workguide.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
    from workguide.config.loop_detection_config import LoopDetectionConfig
...
def test_subagent_runtime_middlewares_attach_durable_context_before_summarization(monkeypatch):
    ...
    """
    from workguide.agents.middlewares import summarization_middleware as sm
    from workguide.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    ...
    from workguide.config.summarization_config import SummarizationConfig
...
def test_subagent_compaction_injects_summary_before_assistant_tool_tail(monkeypatch):
    ...
    from workguide.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    from workguide.agents.middlewares.summarization_middleware import WorkGuideSummarizationMiddleware
    from workguide.agents.middlewares.system_message_coalescing_middleware import SystemMessageCoalescingMiddleware
    from workguide.agents.thread_state import ThreadState
    from workguide.config.summarization_config import ContextSize, SummarizationConfig
    ...
    monkeypatch.setattr(
        "workguide.agents.middlewares.summarization_middleware.create_chat_model",
    ...
    compaction_middlewares = [middleware for middleware in runtime_middlewares if isinstance(middleware, (DurableContextMiddleware, WorkGuideSummarizationMiddleware, SystemMessageCoalescingMiddleware))]
    agent = create_agent(
        model=strict_model,
        tools=[],
...
def test_subagent_chain_injects_date_without_memory_and_coalesces_for_strict_provider(monkeypatch):
    ...
    from workguide.agents.middlewares import dynamic_context_middleware as dynamic_context
    from workguide.agents.middlewares.system_message_coalescing_middleware import SystemMessageCoalescingMiddleware
    from workguide.agents.thread_state import ThreadState
    ...
    monkeypatch.setattr("workguide.agents.lead_agent.prompt._get_memory_context", _unexpected_memory_lookup)
...
def test_subagent_runtime_middlewares_omit_summarization_when_factory_returns_none(monkeypatch):
    ...
    assert not any(isinstance(m, WorkGuideSummarizationMiddleware) for m in middlewares)
...
def test_subagent_summarization_fires_mid_run_and_produces_usable_result(monkeypatch):
    ...
    ``WorkGuideSummarizationMiddleware`` (the exact instance the subagent chain
    ...
    from workguide.agents.middlewares.summarization_middleware import (
        WorkGuideSummarizationMiddleware,
        create_summarization_middleware,
    )
    from workguide.agents.thread_state import ThreadState
    from workguide.config.memory_config import MemoryConfig
    from workguide.config.summarization_config import ContextSize, SummarizationConfig
    ...
    assert isinstance(middleware, WorkGuideSummarizationMiddleware), "the real middleware must be built"
    ...
    before_model_chunks = [c for c in chunks if "WorkGuideSummarizationMiddleware.before_model" in c]
    assert before_model_chunks, "summarization before_model must fire when messages exceed the trigger"
    summary_update = before_model_chunks[0]["WorkGuideSummarizationMiddleware.before_model"]
