"""Command-line entry point and launch-mode planning for the WorkGuide TUI.
...
headless one-shot. ``main`` wires that decision to the embedded ``WorkGuideClient``
and lazily imports the Textual app only when actually launching the UI, so the
``workguide`` console script still runs headless commands without Textual present.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="workguide",
        description="WorkGuide terminal workbench — a TUI over the embedded WorkGuide harness.",
        epilog="Extension management: workguide extensions --help",
        add_help=True,
    )
...
def plan_launch(
    ...
    *,
    ...
    if args.cli:
        ...
        return LaunchPlan(
            mode="headless-help",
            reason='--cli needs a message. Try: workguide --print "your question".',
        )

    forced_tui = bool(args.tui)
    transparent = bool(args.tui_transparent) or _truthy(env.get("WORKGUIDE_TUI_TRANSPARENT"))
    if forced_tui or _truthy(env.get("WORKGUIDE_TUI")) or (stdin_isatty and stdout_isatty):
        return LaunchPlan(
            mode="tui",
            message=positional,
            thread_id=resume,
...
_HEADLESS_HELP = """\
workguide — WorkGuide terminal workbench

  workguide                      launch the terminal UI (TTY required)
  workguide --tui                force the terminal UI
  workguide --tui-transparent    use the terminal's default background
  workguide --continue           resume the most recent thread in the UI
  workguide --resume THREAD      resume a thread by id or title
  workguide --print "question"   one-shot answer to stdout
  workguide --json "question"    stream newline-delimited JSON events
  workguide --recursion-limit N --print "question"
                              set the headless agent-loop super-step limit
  workguide extensions --help  install and manage trusted Python extensions
  echo "question" | workguide --print
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if argv and argv[0] == "extensions":
        from workguide.extensions.cli import main as extensions_main
...
def _run_tui(plan: LaunchPlan) -> int:
    try:
        ...
        # `workguide.tui.app` module for the forbidden top-level `app` package.
        from workguide.tui.app import run_tui
    except ModuleNotFoundError as exc:  # textual missing
        if getattr(exc, "name", "") == "textual" or "textual" in str(exc):
            msg = "The terminal UI needs the optional 'textual' dependency.\nInstall it with:  uv pip install 'workguide-harness[tui]'   (or: pip install textual)\n"
            if plan.forced_tui:
