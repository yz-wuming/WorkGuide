...
WorkGuide's own tables in the same database. Alembic must NEVER emit DDL for
...
from workguide.persistence.migrations._env_filters import (
...
def test_filter_includes_workguide_tables() -> None:
...
def test_filter_includes_indexes_on_workguide_tables() -> None:
    md = sa.MetaData()
...
def test_env_module_wires_busy_timeout_for_sqlite() -> None:
    ...
    env_path = Path(__file__).resolve().parents[1] / "packages/harness/workguide/persistence/migrations/env.py"
...
class TestExtensionOwnedTables:
    ...
    whose `alembic.ini` points at a real `./data/workguide.db` — the same path
    `LANGGRAPH_OWNED_TABLES` covers.
    """

    def setup_method(self):
        from workguide.persistence.migrations import _env_filters
    ...
    def test_a_registered_prefix_is_excluded(self):
        from workguide.persistence.migrations._env_filters import include_object, register_extension_table_prefix
    ...
    def test_an_unregistered_table_is_still_included(self):
        from workguide.persistence.migrations._env_filters import include_object
    ...
    def test_registration_rejects_an_empty_prefix(self):
        ...
        from workguide.persistence.migrations._env_filters import register_extension_table_prefix
    ...
    def test_registration_accepts_a_prefix_that_matches_no_host_table(self):
        from workguide.persistence.migrations._env_filters import EXTENSION_TABLE_PREFIXES, register_extension_table_prefix
...
import workguide.persistence.models  # noqa: F401 - populate Base.metadata
...
class TestPrefixesReachTheAlembicProcess:
    ...
    def _probe(self, tmp_path, config_yaml: str) -> dict:
        ...
        proc = subprocess.run(
            ...
            cwd=str(backend),
            env={**os.environ, "WORKGUIDE_CONFIG_PATH": str(tmp_path / "config.yaml"), "PYTHONPATH": str(backend)},
            capture_output=True,
            text=True,
            timeout=180,
        )
    ...
    def test_env_module_registers_configured_prefixes(self):
        ...
        env_py = Path(__file__).resolve().parents[1] / "packages/harness/workguide/persistence/migrations/env.py"
