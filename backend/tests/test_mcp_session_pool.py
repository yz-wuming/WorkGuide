...
from workguide.mcp.session_pool import MCPSessionPool, call_pooled_session_tool, get_session_pool, reset_session_pool
...
def _make_test_pool_tool(*, pool, call_tool, tool_interceptors=None):
    ...
    from workguide.mcp.tools import _make_session_pool_tool
    ...
    with patch("workguide.mcp.tools.get_session_pool", return_value=pool):
...
"""
    ...
    with patch("workguide.mcp.tools.get_paths", return_value=Paths(tmp_path)):
...
async def test_session_pool_tool_evicts_session_after_transport_disconnect(tmp_path, transport_error):
    ...
    with (
        patch("workguide.mcp.tools.get_paths", return_value=Paths(tmp_path)),
...
async def test_session_pool_tool_pins_cwd_and_temp_env(tmp_path):
    ...
    from workguide.config.paths import Paths
    from workguide.constants import MCP_TMP_SUBDIR
    from workguide.mcp.tools import _make_session_pool_tool
    ...
    with (
        patch("workguide.mcp.tools.get_paths", return_value=paths),
...
async def test_session_pool_tool_skips_fs_work_for_non_stdio_transport(tmp_path):
    ...
    with (
        patch("workguide.mcp.tools.get_paths", return_value=paths) as get_paths,
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm) as create_session,
    ):
...
async def test_session_pool_tool_skips_after_walk_when_no_text_content(tmp_path):
    ...
    with (
        ...
        patch("workguide.mcp.tools._changed_workspace_files") as changed_files,
    ):
        wrapped = _make_session_pool_tool(original_tool, "playwright", connection)
        await wrapped.coroutine(runtime=mock_runtime, url="https://example.com")
...
async def test_session_pool_tool_runs_after_walk_when_text_content_present(tmp_path):
    ...
    with (
        patch("workguide.mcp.tools.get_paths", return_value=paths),
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm),
        patch("workguide.mcp.tools._changed_workspace_files", return_value=[]) as changed_files,
    ):
        wrapped = _make_session_pool_tool(original_tool, "playwright", connection)
        await wrapped.coroutine(runtime=mock_runtime, url="https://example.com")
...
async def test_session_pool_tool_get_config_fallback():
    ...
    with (
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm),
        patch("workguide.mcp.tools.get_config", return_value=fake_config),
    ):
        wrapped = _make_session_pool_tool(original_tool, "server", {"transport": "stdio", "command": "x", "args": []})
...
def test_session_pool_tool_sync_wrapper_path_is_safe():
    ...
    from workguide.mcp.tools import _make_session_pool_tool
    from workguide.tools.sync import make_sync_tool_wrapper
...
async def test_http_transport_tools_not_pooled():
    ...
    from workguide.mcp.tools import get_mcp_tools
    ...
    with (
        patch("workguide.mcp.tools.ExtensionsConfig.from_file", return_value=extensions_config),
        patch("workguide.mcp.tools.build_servers_config", return_value=servers_config),
        patch("workguide.mcp.tools.get_initial_oauth_headers", return_value={}),
        patch("workguide.mcp.tools.build_oauth_tool_interceptor", return_value=None),
...
async def test_non_stdio_tool_call_timeout_warns_that_it_is_ignored(caplog):
    ...
    from workguide.config.extensions_config import McpServerConfig
    from workguide.mcp.tools import get_mcp_tools
    ...
    with (
        ...
        patch("workguide.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("langchain_mcp_adapters.client.MultiServerMCPClient") as MockClient,
        caplog.at_level(logging.WARNING, logger="workguide.mcp.tools"),
    ):
        mock_client_instance = MockClient.return_value
...
def test_reset_mcp_tools_cache_from_running_loop_is_bounded():
    ...
    """
    from workguide.mcp.cache import reset_mcp_tools_cache
    from workguide.mcp.session_pool import get_session_pool
...
async def test_mcp_tools_routed_to_source_server_with_prefix_overlap():
    ...
    with (
        ...
        patch("workguide.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("langchain_mcp_adapters.client.MultiServerMCPClient") as MockClient,
        patch("workguide.mcp.tools._make_session_pool_tool", side_effect=fake_wrap),
    ):
