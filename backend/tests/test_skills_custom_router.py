...
from workguide.skills.security_static_scanner import StaticScannerError
...
def test_install_skill_archive_runs_security_scan(monkeypatch, tmp_path):
    ...
    config = SimpleNamespace(
        skills=SimpleNamespace(get_skills_path=lambda: skills_root, container_path="/mnt/skills", use="workguide.skills.storage.local_skill_storage:LocalSkillStorage"),
...
def test_uploaded_skill_archive_installs_sandbox_readable_tree(monkeypatch, tmp_path):
    ...
    # Monkeypatch paths BEFORE constructing UserScopedSkillStorage
    monkeypatch.setattr("workguide.config.paths.get_paths", lambda: Paths(base_dir=tmp_path))
    monkeypatch.setattr("workguide.config.paths._paths", None)
    monkeypatch.setenv("WORKGUIDE_HOME", str(home))
...
def test_custom_skill_delete_continues_when_history_write_is_readonly(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("workguide.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage.append_history", _readonly_history)
    monkeypatch.setattr("app.gateway.routers.skills.refresh_user_skills_system_prompt_cache_async", _refresh)
    monkeypatch.setattr("app.gateway.routers.skills.get_effective_user_id", lambda: "default")
...
def test_custom_skill_delete_fails_when_skill_dir_removal_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("workguide.skills.storage.local_skill_storage.shutil.rmtree", _fail_rmtree)
    monkeypatch.setattr("app.gateway.routers.skills.refresh_user_skills_system_prompt_cache_async", _refresh)
    monkeypatch.setattr("app.gateway.routers.skills.get_effective_user_id", lambda: "default")
...
def test_update_skill_refreshes_prompt_cache_before_return(monkeypatch, tmp_path):
    ...
    from workguide.skills.storage import user_scoped_skill_storage as uss_module
    ...
    monkeypatch.setattr("workguide.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage", _FakeUserScopedStorage)
...
def test_public_skill_toggle_rebuilds_projection_before_response(monkeypatch, tmp_path):
    from workguide.config.extensions_config import ExtensionsConfig, SkillStateConfig, reset_extensions_config, set_extensions_config
    from workguide.config.paths import Paths
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(config_path))
