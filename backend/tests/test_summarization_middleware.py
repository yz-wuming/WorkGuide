...
from workguide.agents.memory.summarization_hook import memory_flush_hook
from workguide.agents.middlewares.dynamic_context_middleware import _DYNAMIC_CONTEXT_REMINDER_KEY, DynamicContextMiddleware, is_dynamic_context_reminder
from workguide.agents.middlewares.summarization_middleware import WorkGuideSummarizationMiddleware, SummarizationEvent, SummaryGenerationError, create_summarization_middleware
...
def test_summarization_middleware_emits_frontend_update_key_in_agent_stream() -> None:
    middleware = WorkGuideSummarizationMiddleware(
    ...
    update = next(
        (chunk["WorkGuideSummarizationMiddleware.before_model"] for chunk in chunks if "WorkGuideSummarizationMiddleware.before_model" in chunk),
        None,
    )
...
def test_dynamic_context_reminder_is_preserved_across_summarization() -> None:
    ...
    with mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt:
...
def test_memory_flush_hook_skips_when_memory_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = MagicMock()
    monkeypatch.setattr("workguide.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=False))
    monkeypatch.setattr("workguide.agents.memory.summarization_hook.get_memory_manager", lambda: manager)
...
def test_memory_flush_hook_skips_when_thread_id_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = MagicMock()
    monkeypatch.setattr("workguide.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=True))
    monkeypatch.setattr("workguide.agents.memory.summarization_hook.get_memory_manager", lambda: manager)
...
def test_factory_attaches_memory_flush_hook_by_default(monkeypatch):
    ...
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", lambda **kw: fake_model)
...
def test_null_model_summarizes_with_the_run_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    built: list = []
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", _tracking_create_chat_model(built))
...
def test_both_summary_models_failing_returns_none_on_automatic_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    built: list = []
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", _tracking_create_chat_model(built, fail=True))
...
def test_fallback_construction_error_does_not_escape_automatic_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", _failing_build)
...
def test_blank_summary_response_is_not_committed_null_case(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", lambda **kwargs: run_model)
...
def test_factory_configured_constructor_failure_falls_back_to_run_model(monkeypatch):
    ...
    monkeypatch.setattr("workguide.agents.middlewares.summarization_middleware.create_chat_model", _factory)
