...
from workguide.client import StreamEvent
from workguide.tui.app import WorkGuideTUI
from workguide.tui.cli import LaunchPlan
from workguide.tui.view_state import SystemMessage
...
async def _fill_scrollable_transcript(app: WorkGuideTUI, pilot) -> VerticalScroll:
...
async def test_app_runs_a_turn_and_renders_streamed_assistant():
    app = WorkGuideTUI(_FakeSession(), LaunchPlan(mode="tui"))
...
async def test_help_text_matches_command_registry():
    from workguide.tui.command_registry import format_command_help
...
async def test_clear_command_clears_display_without_resetting_thread_or_calling_agent():
    ...
    app = WorkGuideTUI(session, LaunchPlan(mode="tui"))
    async with app.run_test() as pilot:
        await pilot.pause()
