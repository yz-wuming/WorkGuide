...
from workguide.skills.storage import reset_skill_storage, reset_user_skill_storage
...
def base_dir(tmp_path: Path) -> Path:
    """Provide a temp directory as the WorkGuide base_dir."""
    return tmp_path
...
def user_storage(base_dir: Path, skills_root, config) -> UserScopedSkillStorage:
    ...
    with patch("workguide.config.paths.get_paths", return_value=Paths(base_dir=base_dir)):
        with patch("workguide.config.paths._paths", None):
...
class TestSkillToggleIsolation:
    ...
    def test_alice_disable_does_not_affect_bob(self, base_dir: Path, skills_root, config):
        ...
        from workguide.agents.lead_agent.prompt import clear_skills_system_prompt_cache, get_skills_prompt_section
        from workguide.sandbox.tools import _is_disabled_skill_path
        from workguide.skills.storage import get_or_new_user_skill_storage
        ...
        with patch("workguide.config.paths.get_paths", return_value=Paths(base_dir=base_dir)):
            with patch("workguide.config.paths._paths", None):
                with patch("workguide.config.get_app_config", return_value=rich_config):
...
class TestSkillStateFailClosed:
    ...
    def test_returns_true_when_state_lookup_raises(self) -> None:
        ...
        with patch("workguide.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage.get_skill_enabled_state", side_effect=_boom):
    ...
    def test_returns_true_when_public_extensions_config_raises(self) -> None:
        ...
        with patch("workguide.config.extensions_config.ExtensionsConfig.from_file", side_effect=_boom):
...
class TestSkillLoadingRespectsGlobalDisable:
    ...
    def test_global_disable_wins_when_per_user_state_missing(self, tmp_path: Path) -> None:
        ...
        with patch("workguide.config.paths.get_paths", return_value=Paths(base_dir=base)):
            with patch("workguide.config.paths._paths", None):
                cfg = SimpleNamespace(
                    skills=SimpleNamespace(
                        get_skills_path=lambda: skills_root,
                ...
                with patch("workguide.config.get_app_config", return_value=cfg):
                    ...
                    with patch("workguide.config.extensions_config.ExtensionsConfig.from_file", return_value=ext_cfg):
