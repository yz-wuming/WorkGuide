...
async def test_oidc_validate_id_token_refreshes_jwks_once_on_kid_miss(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {"iss": metadata.issuer, "sub": "subject", "aud": "workguide", "exp": 9999999999},
    )

    claims = await service.validate_id_token(metadata, "workguide", "id-token")
...
async def test_oidc_validate_id_token_rejects_hmac_algorithms(monkeypatch):
    ...
    with pytest.raises(OIDCValidationError, match="unsupported algorithm"):
        await service.validate_id_token(metadata, "workguide", "id-token")
