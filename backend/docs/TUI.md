# WorkGuide Terminal Workbench (TUI)

`workguide` is a terminal-native workbench for the WorkGuide harness. It runs
**embedded** over `WorkGuideClient` — no Gateway, frontend, nginx, or Docker
services required — while honoring the same `config.yaml`, checkpointer, skills,
memory, MCP, and sandbox settings as the rest of WorkGuide.

![WorkGuide TUI](../../docs/tui/tui-preview.svg)
...
```bash
uv pip install 'workguide-harness[tui]'    # or: pip install textual
```
...
| Command | Behavior |
|---|---|
| `workguide` | Launch the TUI when stdin/stdout are TTYs |
| `workguide --tui` | Force the TUI (clear diagnostic if `textual` is missing) |
| `workguide --tui-transparent` | Use the terminal's default background when launching the TUI |
| `workguide --cli` | Force headless/classic mode for one invocation |
| `workguide chat` | Same TUI conversation surface |
| `workguide --continue` | Resume the most recent thread |
| `workguide --resume THREAD` | Resume a thread by id |
| `workguide --print "question"` | Headless one-shot answer to stdout |
| `workguide --json "question"` | Headless newline-delimited `StreamEvent`s |
| `workguide --recursion-limit 250 --print "question"` | Set the headless agent-loop super-step limit |
| `echo "q" \| workguide --print` | Read the message from stdin |
| `WORKGUIDE_TUI=1 workguide` | Force the TUI via environment |
| `WORKGUIDE_TUI_TRANSPARENT=1 workguide` | Persist terminal-background rendering via environment |

If no TTY is available and no headless flag is given, `workguide` prints guidance
instead of hanging.

Transparent rendering is opt-in; the solid WorkGuide palette remains the default.
...
semantics as elsewhere in WorkGuide). `/model` and `/threads` open modal pickers.
...
session.py      builds WorkGuideClient (+ checkpointer) and the persistence writer
...
`WorkGuideClient.stream()` is a **synchronous** generator, so the app runs it on a
...
`workguide.persistence.engine.init_engine_from_config`), **not** the Gateway
