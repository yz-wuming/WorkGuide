...
from workguide.community.boxlite.box import BoxliteBox
from workguide.community.boxlite.provider import BoxliteProvider, _import_simplebox
...
def _no_existing_boxlite_boxes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.community.boxlite.provider._import_sync_boxlite_runtime", lambda: _EmptyBoxlite)


def test_import_simplebox_missing_raises_actionable(monkeypatch: pytest.MonkeyPatch) -> None:
    _no_boxlite(monkeypatch)
    with pytest.raises(ImportError, match=r"workguide-harness\[boxlite\]"):
        _import_simplebox()
...
def test_acquire_without_boxlite_raises_and_shuts_down_cleanly(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.community.boxlite.provider.get_app_config", lambda: stub)
    _no_boxlite(monkeypatch)
...
def test_execute_command_closed_box_returns_without_error_log(caplog) -> None:
    ...
    with caplog.at_level(logging.ERROR, logger="workguide.community.boxlite.box"):
        output = box.execute_command("echo hi")
...
def test_sandbox_id_deterministic(monkeypatch):
    ...
    monkeypatch.setattr(
        "workguide.community.boxlite.provider.get_app_config",
        lambda: _stub_config(),
    )
    provider = BoxliteProvider()
...
def test_create_box_passes_prefixed_sandbox_id_as_name(monkeypatch):
    """_create_box gives BoxLite a WorkGuide-owned name prefix."""
    monkeypatch.setattr(
        "workguide.community.boxlite.provider.get_app_config",
        lambda: _stub_config(),
    )
    ...
    monkeypatch.setattr(
        "workguide.community.boxlite.provider._import_simplebox",
    ...
    assert created_boxes[0]["name"] == "workguide-boxlite-test-sandbox-id"
    assert box.id == "test-sandbox-id"


def test_startup_reconciliation_adopts_prefixed_existing_boxes(monkeypatch):
    """Existing WorkGuide-named BoxLite boxes are adopted into the warm pool."""
    monkeypatch.setattr(
        "workguide.community.boxlite.provider.get_app_config",
        lambda: _stub_config(),
    )
    ...
    class _Runtime:
        ...
        def list_info(self):
            return [
                types.SimpleNamespace(name="workguide-boxlite-adopted"),
                types.SimpleNamespace(name="unrelated-box"),
                types.SimpleNamespace(name=None),
            ]

        def get(self, name):
            if name == "workguide-boxlite-adopted":
                return _NativeBox()
    ...
    monkeypatch.setattr("workguide.community.boxlite.provider._import_sync_boxlite_runtime", lambda: _Boxlite)
...
def test_adopted_warm_pool_box_still_health_checks(monkeypatch):
    ...
    adopted = BoxliteBox(
        "adopted",
        _FakeBox(name="workguide-boxlite-adopted"),
        _fake_run,
        default_env={},
    )
...
def test_late_same_tenant_collision_reuses_active_box(monkeypatch):
    ...
    active = BoxliteBox(
        sandbox_id,
        _FakeBox(name=f"workguide-boxlite-{sandbox_id}"),
        _fake_run,
        default_env={},
    )
...
def test_failed_health_check_does_not_remove_swapped_warm_entry(monkeypatch):
    ...
    stale = BoxliteBox(
        "shared-id",
        _FakeBox(name="workguide-boxlite-shared-id"),
        _fake_run,
        default_env={},
    )
