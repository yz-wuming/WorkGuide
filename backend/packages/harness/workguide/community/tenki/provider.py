"""``TenkiSandboxProvider`` — WorkGuide :class:`SandboxProvider` backed by Tenki.

Integrates `Tenki <https://tenki.cloud>`_ cloud sandboxes as a WorkGuide sandbox
...
_SANDBOX_NAME_PREFIX = "workguide-tenki-"
# Tenki terminates a sandbox at its max lifetime (~30 min by default), which
# would silently drop a long-running thread's state mid-conversation. WorkGuide
...
def _bootstrap_script(home_dir: str) -> str:
    """Materialise WorkGuide's virtual path layout inside the sandbox.
...
def _import_client() -> type[Client]:
    ...
    try:
        ...
        raise ImportError("TenkiSandboxProvider requires the optional 'tenki-sandbox' dependency. Install it with: pip install 'workguide-harness[tenki]' or pip install tenki-sandbox.") from e
    return Client


class TenkiSandboxProvider(WarmPoolLifecycleMixin[TenkiSandbox], SandboxProvider):
    """Run each WorkGuide sandbox as a Tenki cloud microVM."""
    ...
    def _create_sandbox(self, sandbox_id: str) -> TenkiSandbox:
        ...
        # Materialise WorkGuide's virtual path layout under the writable HOME.
