...
def test_browser_stream_closes_4501_when_browser_runtime_unavailable():
    ...
    def fail_browser_runtime_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "workguide.community.browser_automation" and "get_browser_session_manager" in fromlist:
...
def test_browser_navigate_rejects_legacy_null_owner_thread():
    ...
    with (
        patch.object(browser_router, "_browser_tools_enabled", return_value=True),
        patch("workguide.community.browser_automation.navigate_and_capture", new=AsyncMock()),
    ):
...
def test_browser_tools_disabled_when_cdp_risk_not_explicitly_accepted():
    ...
    with (
        patch("workguide.config.get_app_config", return_value=app_config),
...
def test_browser_navigate_redacts_failure_url_from_logs_and_response(caplog):
    ...
    with (
        patch.object(browser_router, "_browser_tools_enabled", return_value=True),
        patch(
            "workguide.community.browser_automation.navigate_and_capture",
...
def test_browser_stream_seed_applies_to_blank_page():
    assert _should_apply_browser_seed("about:blank", "https://github.com/bytedance/workguide")
...
def test_browser_frames_dirname_shared_between_tools_and_scanner():
    ...
    """
    from workguide.community.browser_automation import tools as browser_tools
    from workguide.constants import BROWSER_FRAMES_DIRNAME
    from workguide.workspace_changes.scanner import EXCLUDED_DIR_NAMES
...
def test_validate_browser_url_rejects_private_and_non_http(monkeypatch):
    ...
    """
    from workguide.community.browser_automation import tools as browser_tools
    from workguide.community.browser_automation import validate_browser_url
    ...
    assert validate_browser_url("https://github.com/bytedance/workguide") is None
