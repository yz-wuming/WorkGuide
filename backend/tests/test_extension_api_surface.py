...
A typo in a re-exported name breaks `import workguide_extension_api` for every
...
def test_public_surface_imports_and_matches_all():
    module = importlib.import_module("workguide_extension_api")
...
def test_runtime_deps_is_exported_under_its_documented_name():
    from workguide_extension_api import ExtensionRuntimeDeps
...
def test_every_contract_kind_added_for_the_0_2_series_is_exported():
    import workguide_extension_api as api
...
def test_registry_protocol_declares_every_registration_method():
    from workguide_extension_api import ExtensionRegistry
