...
from workguide.runtime import goal
...
def test_evaluate_goal_completion_uses_non_thinking_model(monkeypatch):
    ...
    # tracing callbacks from (unlike make_lead_agent/WorkGuideClient.stream,
...
def test_evaluate_goal_completion_injects_langfuse_metadata(monkeypatch):
    ...
    try:
        result = asyncio.run(
            goal.evaluate_goal_completion(
                ...
                user_id="alice",
                workguide_trace_id="gateway-trace-1",
            )
        )
    finally:
        reset_tracing_config()
    ...
    assert metadata.get("workguide_trace_id") == "gateway-trace-1"
