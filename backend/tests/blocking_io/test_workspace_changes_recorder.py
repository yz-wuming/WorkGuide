...
from workguide.workspace_changes import recorder
...
async def test_capture_workspace_snapshot_cleanup_does_not_block_event_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    import workguide.config.paths as paths_mod
    ...
    leftovers = await asyncio.to_thread(lambda: sorted(cache_root.glob("workguide-workspace-changes-*")))
...
async def test_capture_workspace_snapshot_cancelled_handoff_leaks_no_text_cache(tmp_path: Path, monkeypatch) -> None:
    ...
    parked = await asyncio.to_thread(lambda: sorted(cache_root.glob("workguide-workspace-changes-*")))
