...
from workguide.agents.memory import (
    MemoryManager,
    get_memory_manager,
    reset_memory_manager,
)
from workguide.agents.memory.backends.deermem.deer_mem import DeerMem
from workguide.agents.memory.backends.noop.noop_manager import NoopMemoryManager
from workguide.config.memory_config import MemoryConfig, get_memory_config, set_memory_config
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("workguide.agents.memory.backends.deermem.deer_mem.DeerMem", DeerMem),
        ("workguide.agents.memory.backends.deermem.deer_mem:DeerMem", DeerMem),
        ("workguide.agents.memory.backends.noop.noop_manager.NoopMemoryManager", NoopMemoryManager),
        ("workguide.agents.memory.backends.noop.noop_manager:NoopMemoryManager", NoopMemoryManager),
    ],
)
...
def test_migration_drops_file_style_legacy_storage_path(caplog) -> None:
    ...
    from workguide.config.memory_config import load_memory_config_from_dict

    with caplog.at_level("WARNING", logger="workguide.config.memory_config"):
...
def test_empty_storage_path_factory_injects_runtime_home(tmp_path, monkeypatch) -> None:
    ...
    import workguide.config.runtime_paths as rp
...
def test_deermem_shutdown_flush_drains_a_pending_update() -> None:
    ...
    from workguide.agents.memory.backends.deermem.deermem.core.queue import ConversationContext
