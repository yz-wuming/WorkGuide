...
# sandbox as WORKGUIDE_LARK_BROKER_URL.
LARK_BROKER_DEFAULT_HOST = "127.0.0.1"
LARK_BROKER_DEFAULT_PORT = 8788
LARK_BROKER_URL_ENV = "WORKGUIDE_LARK_BROKER_URL"
...
LARK_BROKER_DENY_SUBCOMMANDS_ENV = "WORKGUIDE_LARK_BROKER_DENY_SUBCOMMANDS"
...
# an actionable message instead of an opaque ENOEXEC. ``WORKGUIDE_LARK_BROKER_PYTHON``
...
LARK_BROKER_PYTHON_ENV = "WORKGUIDE_LARK_BROKER_PYTHON"
...
LARK_CLI_BROKER_LAUNCHER_TEMPLATE = (
    "#!/bin/sh\n"
    "# WorkGuide lark-cli broker launcher (Pattern B). Resolves a Python 3\n"
    ...
    "# so it still works when PATH is empty and WORKGUIDE_LARK_BROKER_PYTHON pins\n"
    "# the interpreter.\n"
    "set -eu\n"
    'shim="' + _LARK_CLI_BROKER_SHIM_PATH_PLACEHOLDER + '"\n'
    'if [ -n "${WORKGUIDE_LARK_BROKER_PYTHON:-}" ]; then\n'
    '  exec "$WORKGUIDE_LARK_BROKER_PYTHON" "$shim" "$@"\n'
    "fi\n"
    ...
    'echo "          set WORKGUIDE_LARK_BROKER_PYTHON to a python3 path in the sandbox image." >&2\n'
    "exit 127\n"
)
...
"""WorkGuide lark-cli broker shim (Pattern B). Forwards argv/stdin to the broker.
...
BROKER_URL = os.environ.get("WORKGUIDE_LARK_BROKER_URL", "http://127.0.0.1:8788")
...
def install_shim(dest_dir: str, *, version: str | None = None) -> str:
    ...
    the same ``bin/lark-cli`` + ``.workguide-lark-cli-runtime.json`` layout Pattern
    ...
    marker = os.path.join(dest, ".workguide-lark-cli-runtime.json")
...
def _config_from_env() -> BrokerConfig:
    return BrokerConfig(
        lark_cli_path=os.environ.get("WORKGUIDE_LARK_BROKER_CLI", "lark-cli"),
        ...
        host=os.environ.get("WORKGUIDE_LARK_BROKER_HOST", LARK_BROKER_DEFAULT_HOST),
        port=int(os.environ.get("WORKGUIDE_LARK_BROKER_PORT", str(LARK_BROKER_DEFAULT_PORT))),
        timeout_seconds=int(os.environ.get("WORKGUIDE_LARK_BROKER_TIMEOUT", str(LARK_BROKER_DEFAULT_TIMEOUT_SECONDS))),
