...
# Make 'app' and 'workguide' importable from any working directory
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "scripts"))

# Break the circular import chain that exists in production code:
#   workguide.subagents.__init__
#     -> .executor (SubagentExecutor, SubagentResult)
#       -> workguide.agents.thread_state
#         -> workguide.agents.__init__
#           -> lead_agent.agent
#             -> subagent_limit_middleware
#               -> workguide.subagents.executor  <-- circular!
#
# By injecting a mock for workguide.subagents.executor *before* any test module
...
sys.modules["workguide.subagents.executor"] = _executor_mock
...
# (see ``workguide.runtime.user_context``). Without this fixture, every
...
def _restore_title_config_singleton():
    ...
    """
    try:
        from workguide.config.title_config import reset_title_config
    except ImportError:
        yield
        return
