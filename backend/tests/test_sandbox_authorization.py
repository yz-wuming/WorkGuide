...
from workguide.sandbox.exceptions import SandboxAuthorizationError
...
def test_authorize_sandbox_rbac_allow(monkeypatch):
    ...
    monkeypatch.setattr(
        "workguide.authz.sandbox_authz.resolve_authorization_provider",
...
def test_ensure_sandbox_initialized_denies_on_authz_reject(monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.get_app_config", lambda: app_config)
...
def test_authorize_sandbox_no_config_file_is_noop(monkeypatch):
    ...
    """
    from workguide.authz import sandbox_authz as mod

    def _no_config():
        raise FileNotFoundError("config.yaml file not found in the project root")

    monkeypatch.setattr("workguide.config.get_app_config", _no_config)
    # safe_app_config imports get_app_config lazily from workguide.config.
