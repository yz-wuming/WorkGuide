...
from workguide.sandbox.local.list_dir import list_dir
from workguide.sandbox.path_patterns import build_output_mask_pattern
from workguide.sandbox.sandbox import Sandbox, _validate_extra_env
from workguide.sandbox.search import GrepMatch, find_glob_matches, find_grep_matches
...
class LocalSandbox(Sandbox):
    ...
    def _msys_path_conversion_exclusions(self) -> str:
        ...
        Excluding only the configured virtual roots preserves WorkGuide path
    ...
    def _reverse_output_patterns(self) -> list[re.Pattern[str]]:
        ...
        # ``workguide.sandbox.path_patterns`` and shared with
    ...
    def _resolve_path_with_mapping(self, path: str) -> ResolvedPath:
        ...
        relative = self._strip_redundant_desktop_segment(mapping.container_path, relative)
    ...
    def _strip_redundant_desktop_segment(self, container_path: str, relative: str) -> str:
        """Drop an accidental `Desktop` segment right below a desktop mount.

        If the agent emits ``/mnt/desktop/Desktop/NewFolder``, the virtual path
        already names the user's desktop, so the extra ``Desktop`` segment is
        redundant and would otherwise produce ``Desktop/Desktop/NewFolder`` on the
        host. Collapse it to ``/mnt/desktop/NewFolder``.
        A bare ``/mnt/desktop/Desktop`` (nothing after the segment) is left alone so
        the user can still create an item literally named "Desktop" on the desktop.
        This runs inside the mount, so the sandbox containment boundary is unchanged.
        """
        if not relative or not container_path.rstrip("/").endswith("/desktop"):
            return relative
        parts = [p for p in relative.split("/") if p]
        if len(parts) >= 2 and parts[0].lower() == "desktop":
            # relative is spliced without a leading slash; keep the same form
            # so ``local_root / relative`` stays under the mount root.
            return "/".join(parts[1:])
        return relative
    ...
    def _run_posix_command(
        ...
        stdout_capture, stdout_thread = LocalSandbox._start_pipe_drain(stdout_read_fd, "workguide-bash-stdout-drain")
        stderr_capture, stderr_thread = LocalSandbox._start_pipe_drain(stderr_read_fd, "workguide-bash-stderr-drain")
        try:
