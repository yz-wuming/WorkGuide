...
async def _make_channel_connection_repo(tmp_path: Path):
    from workguide.persistence.channel_connections import ChannelConnectionRepository, ChannelCredentialCipher
    from workguide.persistence.engine import get_session_factory, init_engine
...
class TestChannelManager:
    def test_get_client_includes_csrf_header_and_cookie(self):
        ...
        assert headers["X-WorkGuide-Internal-Token"]
    ...
    def test_fetch_gateway_includes_internal_auth_headers(self, monkeypatch):
        ...
        async def go():
            ...
            assert calls[0]["headers"]["X-WorkGuide-Internal-Token"]
    ...
    def test_fetch_gateway_uses_bound_owner_headers(self, monkeypatch):
        ...
        async def go():
            ...
            msg = InboundMessage(
                ...
                chat_id="C123",
                user_id="U-platform",
                owner_user_id="workguide-user-1",
            ...
            assert calls[0]["headers"][INTERNAL_OWNER_USER_ID_HEADER_NAME] == "workguide-user-1"
    ...
    def test_ingest_inbound_files_uses_explicit_owner_bucket(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("workguide.uploads.manager.get_paths", lambda: paths)
    ...
    def test_clarification_follow_up_preserves_history(self, monkeypatch):
        ...
        monkeypatch.delenv("WORKGUIDE_AUTH_DISABLED", raising=False)
    ...
    def test_each_topic_creates_new_thread(self):
        """Messages with distinct topic_ids should each create a new WorkGuide thread."""
        from app.channels.manager import ChannelManager
    ...
    def test_same_topic_reuses_thread(self, monkeypatch):
        """Messages with the same topic_id should reuse the same WorkGuide thread."""
        from app.channels.manager import ChannelManager
...
class TestResolveRunParamsUserId:
    ...
    @pytest.mark.parametrize(
        "kwargs",
        [
            {"user_id": "U-platform", "owner_user_id": "workguide-user-1"},  # bound
    ...
    def test_connection_owner_user_id_takes_precedence_over_platform_user_id(self, monkeypatch):
        ...
        assert run_context["user_id"] == "workguide-user-1"
    ...
    def test_auth_disabled_user_id_is_used_for_unbound_channel_messages(self, monkeypatch):
        ...
        manager = self._manager()
        monkeypatch.setenv("WORKGUIDE_AUTH_DISABLED", "1")
    ...
    def test_unsafe_user_id_is_normalized_but_raw_preserved(self, monkeypatch):
        from workguide.config.paths import make_safe_user_id
    ...
    def test_unsafe_user_id_migrates_unique_legacy_bucket(self, tmp_path, monkeypatch):
        from workguide.config.paths import Paths, make_safe_user_id

        paths = Paths(tmp_path)
        legacy_dir = paths.base_dir / "users" / "user-example-com-63a710569261a24b"
        legacy_dir.mkdir(parents=True)
        (legacy_dir / "memory.json").write_text('{"legacy": true}\n', encoding="utf-8")
        monkeypatch.setattr("workguide.config.paths.get_paths", lambda: paths)
...
class TestGithubFollowupBuffer:
    ...
    def test_run_watcher_drains_buffer_on_end_sentinel(self):
        ...
        from workguide.runtime import MemoryStreamBridge
...
class TestChannelManagerBoundIdentityPolicy:
    ...
    def test_webhook_channel_run_policy_opts_out_of_bound_identity_gate(self, monkeypatch):
        ...
        agent: they are HMAC-authenticated at the route, and the sender→WorkGuide
...
class TestChannelManagerConnectionRouting:
    def test_connection_scoped_conversations_do_not_share_threads(self, tmp_path, monkeypatch):
        from app.channels.manager import ChannelManager
        from app.gateway.internal_auth import INTERNAL_OWNER_USER_ID_HEADER_NAME
        from workguide.persistence.engine import close_engine
...
class TestChannelService:
    ...
    def test_service_urls_fall_back_to_env(self, monkeypatch):
        ...
        monkeypatch.setenv("WORKGUIDE_CHANNELS_LANGGRAPH_URL", "http://gateway:8001/api")
        monkeypatch.setenv("WORKGUIDE_CHANNELS_GATEWAY_URL", "http://gateway:8001")
    ...
    def test_from_app_config_uses_explicit_config(self):
        ...
        with patch("workguide.config.app_config.get_app_config", side_effect=AssertionError("should not read global config")):
    ...
    def test_from_app_config_does_not_create_runtime_channels_from_channel_connections(
        ...
        app_config = SimpleNamespace(
            ...
            channel_connections=ChannelConnectionsConfig.model_validate(
                {
                    "enabled": True,
                    "telegram": {"enabled": True, "bot_username": "workguide_bot"},
                    "slack": {"enabled": True},
    ...
    def test_start_retries_configured_channel_until_ready(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "workguide.reflection.resolve_class",
    ...
    def test_restart_channel_reloads_config_from_disk(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.config.app_config.get_app_config", mock_get_app_config)
    ...
    def test_configure_channel_keeps_explicit_config_over_stale_file_entry(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.config.app_config.get_app_config", fail_get_app_config)
    ...
    def test_restart_channel_reload_applies_runtime_store_overlay(self, monkeypatch, tmp_path):
        ...
        def mock_get_app_config():
            return SimpleNamespace(
                ...
                channel_connections=ChannelConnectionsConfig.model_validate({"enabled": True, "telegram": {"enabled": True, "bot_username": "workguide_bot"}}),
            )
    ...
    def test_restart_channel_falls_back_to_cached_config_on_error(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.config.app_config.get_app_config", _raise)
...
class TestTelegramInboundMessages:
    ...
    def test_slash_skill_addressed_to_telegram_bot_strips_username(self):
        ...
        async def go():
            ...
            update = _make_telegram_update(
                "group",
                message_id=13,
                text="/data-analysis@WorkGuideBot analyze uploads/foo.csv",
            )
            context = SimpleNamespace(bot=SimpleNamespace(username="WorkGuideBot"))
    ...
    def test_cmd_generic_strips_addressed_telegram_bot_username(self):
        ...
        async def go():
            ...
            update = _make_telegram_update("group", message_id=33, text="/status@WorkGuideBot")
            context = SimpleNamespace(bot=SimpleNamespace(username="WorkGuideBot"))
...
class TestSlackTextEscaping:
    ...
    def test_real_markdown_link_still_converts_without_double_escaping(self):
        ...
        sent = self._sent_text("See [WorkGuide docs](https://example.com/docs) for more.")
        assert "<https://example.com/docs|WorkGuide docs>" in sent
        assert "&lt;" not in sent
        assert "&gt;" not in sent
...
# only payloads whose ``type`` contained "tool".  WorkGuide injects hidden
...
_MEMORY_LEAK_TEXT = "<memory>\nFacts:\n- [context | 0.70] User interacts with the assistant through the WorkGuide chat channel.\n</memory>"
