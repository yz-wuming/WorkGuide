...
from workguide.agents.goal_state import GoalEvaluation, GoalState
from workguide.agents.middlewares.input_sanitization_middleware import neutralize_untrusted_tags
from workguide.config.app_config import AppConfig
from workguide.config.database_config import CheckpointChannelMode
from workguide.constants import TOOL_RESULTS_DIRNAME
from workguide.runtime.checkpoint_mode import (
    aensure_checkpoint_mode_compatible,
    inject_checkpoint_mode,
)
from workguide.runtime.checkpoint_state import (
...
from workguide.runtime.context_keys import CURRENT_RUN_PRE_EXISTING_MESSAGE_IDS_KEY
from workguide.runtime.goal import (
...
from workguide.runtime.serialization import serialize
from workguide.runtime.stream_bridge import StreamBridge
from workguide.runtime.stream_modes import normalize_stream_modes, to_langgraph_stream_modes
from workguide.runtime.user_context import get_effective_user_id, resolve_runtime_user_id
from workguide.trace_context import (
    WORKGUIDE_TRACE_METADATA_KEY,
    is_trace_id_from_request_header,
    resolve_workguide_trace_id,
)
from workguide.tracing import inject_langfuse_metadata
from workguide.utils.messages import message_to_text
from workguide.workspace_changes import capture_workspace_snapshot, get_changed_output_paths, record_workspace_changes
from workguide.workspace_changes.types import WorkspaceSnapshot
...
def _build_runtime_context(
    ...
    if task_store is not None:
        from workguide_extension_api import EXTENSION_TASK_STORE_KEY
    ...
    from workguide.extensions import EXTENSION_SNAPSHOT_CONTEXT_KEY
...
def _install_runtime_context(config: dict, runtime_context: dict[str, Any]) -> None:
    ...
    if isinstance(existing_context, dict):
        ...
        if WORKGUIDE_TRACE_METADATA_KEY in runtime_context:
            existing_context.setdefault(WORKGUIDE_TRACE_METADATA_KEY, runtime_context[WORKGUIDE_TRACE_METADATA_KEY])
...
def _agent_graph(agent_result: Any) -> Any:
    ...
    try:
        from workguide.agents.lead_agent.agent import unwrap_agent_graph
    except Exception:
...
class _SubagentEventBuffer:
    ...
    async def add(self, chunk: Any) -> None:
        ...
        if self._event_store is None:
            return
        # Lazy import: importing workguide.subagents at module load triggers its
        # package __init__ (executor → agents → tools → task_tool), which imports
        # back from workguide.subagents and deadlocks at gateway startup. Deferring
        # it to call time (after all modules are loaded) breaks that cycle.
        from workguide.subagents.step_events import subagent_run_event
...
async def run_agent(
    ...
    *,
    ...
    from workguide_extension_api import ExtensionData, TaskInfo

    from workguide.extensions import get_loaded_extensions
    from workguide.extensions.notify import (
        lead_task_id,
        lead_task_outcome,
        notify_task_start,
        notify_task_stop,
    ...
    # this set to mask out ``workguide_error_fallback`` markers that belong to
    ...
    try:
        ...
        if event_store is not None:
            from workguide.runtime.journal import RunJournal
        ...
        workguide_trace_id = resolve_workguide_trace_id(incoming_metadata.get(WORKGUIDE_TRACE_METADATA_KEY))
        if workguide_trace_id:
            runtime_ctx[WORKGUIDE_TRACE_METADATA_KEY] = workguide_trace_id
            if is_trace_id_from_request_header():
                merged_metadata = dict(incoming_metadata)
                merged_metadata[WORKGUIDE_TRACE_METADATA_KEY] = workguide_trace_id
        ...
        # Shared helper with ``WorkGuideClient.stream`` so both entry points stay
        ...
        inject_langfuse_metadata(
            ...
            environment=os.environ.get("WORKGUIDE_ENV") or os.environ.get("ENVIRONMENT"),
        ...
        from workguide.extensions import bind_agent_build_extensions
...
async def _prepare_goal_continuation_input(
    *,
    ...
    workguide_trace_id: str | None = None,
    task_store: Any | None = None,
    extensions: Any | None = None,
) -> dict[str, Any] | None:
...
def _try_extract_from_message(obj: Any, pre_existing_ids: set[str] | None = None) -> str | None:
    ...
    if isinstance(additional_kwargs, dict) and additional_kwargs.get("workguide_error_fallback"):
    ...
    if isinstance(obj, dict):
        ...
        if isinstance(nested_kwargs, dict) and nested_kwargs.get("workguide_error_fallback"):
