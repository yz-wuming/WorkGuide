...
def _run(
    ...
    *,
    ...
    env.pop("WORKGUIDE_CONFIG_PATH", None)
    env.pop("WORKGUIDE_STREAM_BRIDGE_REDIS_URL", None)
    if uv_extras is not None:
        env["UV_EXTRAS"] = uv_extras
    if config_path is not None:
        env["WORKGUIDE_CONFIG_PATH"] = str(config_path)
    if stream_bridge_redis_url is not None:
        env["WORKGUIDE_STREAM_BRIDGE_REDIS_URL"] = stream_bridge_redis_url
...
def test_entrypoint_excludes_runtime_state_from_uvicorn_reload():
    ...
    assert ': "${WORKGUIDE_HOME:=/app/backend/.workguide}"' in content
    # sandbox must be created too, not just .workguide (#3459 / #3454).
    assert 'mkdir -p "$WORKGUIDE_HOME" /app/backend/.workguide /app/backend/sandbox' in content
    ...
    assert '--reload-exclude="$WORKGUIDE_HOME"' in content
    assert "--reload-exclude=/app/backend/.workguide" in content
