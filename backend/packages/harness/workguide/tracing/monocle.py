...
def setup_monocle_tracing_if_enabled() -> bool:
    ...
    try:
        ...
        raise RuntimeError("MONOCLE_TRACING is enabled but monocle_apptrace is not installed. Install the 'monocle' extra: `uv sync --extra monocle` in backend/, or `pip install 'workguide-harness[monocle]'`.") from exc

    # monocle_exporters_list takes the comma-separated string as-is (monocle_apptrace's API).
    setup_monocle_telemetry(workflow_name="workguide", monocle_exporters_list=exporters)
