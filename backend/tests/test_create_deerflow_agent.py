"""Tests for create_workguide_agent SDK entry point."""
...
from workguide.agents.factory import create_workguide_agent
from workguide.agents.features import Next, Prev, RuntimeFeatures
...
from workguide.subagents import SubagentRuntime
...
def test_custom_state_schema_is_preserved_in_full_mode_and_adapted_in_delta_mode(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), state_schema=_CustomState)
...
def test_compiled_factory_graph_selects_full_and_delta_message_channels():
    full_graph = create_workguide_agent(
        _FakeModel(responses=[AIMessage(id="full-response", content="done")]),
    )
    delta_graph = create_workguide_agent(
...
def test_with_tools(mock_create_agent):
    ...
    create_workguide_agent(model, tools=[tool])
...
def test_with_system_prompt(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), system_prompt=prompt)
...
def test_features_mode(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), features=feat)
...
def test_middleware_takeover(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), middleware=[custom_mw])
...
def test_tool_deduplication(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), tools=[user_clarification], features=RuntimeFeatures(sandbox=False))
...
def test_checkpointer_passthrough(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), checkpointer=cp)
...
def test_loop_detection_always_present(mock_create_agent):
    mock_create_agent.return_value = MagicMock()
    create_workguide_agent(_make_mock_model(), features=RuntimeFeatures(sandbox=False))
...
def test_plan_mode_adds_todo_middleware(mock_create_agent):
    mock_create_agent.return_value = MagicMock()
    create_workguide_agent(_make_mock_model(), features=RuntimeFeatures(sandbox=False), plan_mode=True)
...
def test_full_chain_order(mock_create_agent):
    ...
    create_workguide_agent(_make_mock_model(), features=feat, plan_mode=True)
