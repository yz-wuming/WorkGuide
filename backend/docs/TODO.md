...
- [x] Replace `time.sleep(5)` with `asyncio.sleep()` in `packages/harness/workguide/tools/builtins/task_tool.py` (subagent polling)
...
- [ ] Replace `subprocess.run()` with `asyncio.create_subprocess_shell()` in `packages/harness/workguide/sandbox/local/local_sandbox.py`
