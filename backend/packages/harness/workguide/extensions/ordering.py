...
def core_ordering_constraints() -> tuple[OrderingConstraint, ...]:
    ...
    return (
        OrderingConstraint(
            ...
            reason=("ToolProgressMiddleware reads workguide_tool_meta in _update_state_from_result, so its wrap_tool_call chain must enclose the ToolErrorHandlingMiddleware step that stamps it"),
        ),
        OrderingConstraint(
            outer=ToolReceiptMiddleware,
            inner=ToolErrorHandlingMiddleware,
            reason=("ToolReceiptMiddleware reads the workguide_tool_meta status stamped by ToolErrorHandlingMiddleware when building each receipt, so its wrap_tool_call chain must enclose the stamping step"),
        ),
        *(
            OrderingConstraint(
                outer=ToolReceiptMiddleware,
