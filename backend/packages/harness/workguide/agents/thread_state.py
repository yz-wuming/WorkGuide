...
import workguide.checkpoint_patches as _checkpoint_patches  # noqa: F401 - import-time saver fixes
from workguide.agents.goal_state import GoalState
from workguide.config.database_config import DEFAULT_CHECKPOINT_SNAPSHOT_FREQUENCY, CheckpointChannelMode
from workguide.subagents.status_contract import SUBAGENT_STATUS_VALUES
...
def _resolve_snapshot_frequency(snapshot_frequency: int | None) -> int:
    ...
    else default. Imported lazily — ``workguide.runtime.__init__`` reaches this
    module via ``checkpoint_state``, so a top-level import would cycle."""
    if snapshot_frequency is not None:
        return snapshot_frequency
    from workguide.runtime.checkpoint_mode import resolve_checkpoint_snapshot_frequency
