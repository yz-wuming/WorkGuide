...
DEFAULT_SCAN_PATHS = (
    ...
    REPO_ROOT / "backend" / "packages" / "harness" / "workguide",
)
IGNORED_DIR_NAMES = {
    ".git",
    ".mypy_cache",
...
EXACT_CALL_RULES: dict[str, _CallRule] = {
    ...
    "workguide.utils.file_io.run_file_io": _CallRule(
        "INFO",
        "ASYNC_FILE_IO_OFFLOAD",
        DEDICATED_EXECUTOR,
        "Offloads filesystem work into WorkGuide's dedicated file-IO executor.",
    ),
    ...
    "make_sync_tool_wrapper": _CallRule(
        ...
        "SYNC_TOOL_WRAPPER",
        DEDICATED_EXECUTOR,
        "Adapts an async tool for sync invocation through WorkGuide's dedicated tool executor.",
    ),
    "workguide.tools.sync.make_sync_tool_wrapper": _CallRule(
        "INFO",
        "SYNC_TOOL_WRAPPER",
        DEDICATED_EXECUTOR,
        "Adapts an async tool for sync invocation through WorkGuide's dedicated tool executor.",
    ...
    def _discover_thread_helpers(self, tree: ast.Module) -> None:
        ...
        for definition in tree.body:
            ...
            for node in ast.walk(definition):
                ...
                elif call_name == "workguide.utils.file_io.run_file_io":
...
def inspect_configured_components(
    ...
    *,
    ...
    if resolve_tool is None or resolve_model is None:
        from workguide.reflection import resolve_class, resolve_variable
