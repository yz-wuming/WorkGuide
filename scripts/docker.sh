...
_refresh_compose_cmd() {
    COMPOSE_CMD="${COMPOSE_BIN[*]} -p workguide-dev -f ${COMPOSE_FILE}"
}
_refresh_compose_cmd
...
require_compose_file() {
    ...
    echo "Run this from the WorkGuide repository root, e.g. 'make docker-start'."
...
# Compose interpolates ${WORKGUIDE_ROOT} into host-side paths
# (WORKGUIDE_HOST_BASE_DIR, THREADS_HOST_PATH) that AIO/provisioner sandbox
# modes bind-mount. Unset, those render as /backend/.workguide — a plausible
# looking absolute path on the wrong root, so mounts silently miss the checkout.
ensure_work_guide_root() {
    if [ -z "$WORKGUIDE_ROOT" ]; then
        export WORKGUIDE_ROOT="$PROJECT_ROOT"
    fi
}
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"workguide.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"workguide.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  WorkGuide Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting WorkGuide Docker Development"
    echo "=========================================="
    echo ""
    ...
    if [ "$sandbox_mode" = "aio" ]; then
        local docker_socket="${WORKGUIDE_DOCKER_SOCKET:-/var/run/docker.sock}"
    ...
    echo -e "${BLUE}Using WORKGUIDE_ROOT=$WORKGUIDE_ROOT${NC}"
    echo ""
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting WorkGuide.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  WorkGuide Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" workguide-sandbox 2>/dev/null || true
...
restart() {
    compose_preflight
    echo "========================================"
    echo "  Restarting WorkGuide Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "WorkGuide Docker Management Script"
    echo ""
