...
The simplest option. Ships with WorkGuide. Block or allow tools by name. No external packages, no passport, no network.
...
aport setup --framework workguide
```

This creates:
- `~/.aport/workguide/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/workguide/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; WorkGuide doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the WorkGuide middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start WorkGuide and ask: "Create a file called test.txt with content hello"
...
class MyGuardrailProvider:
    ...
    def evaluate(self, request):
        from workguide.guardrails.provider import GuardrailDecision, GuardrailReason
...
2. Add the config
3. Start WorkGuide and ask: "Use bash to delete test.txt"
...
| Field | Example use |
|---|---|
| `user_id` | Attach the authenticated WorkGuide user to a provider-side policy or audit record |
...
class ContextAwareGuardrailProvider:
    ...
    async def aevaluate(self, request):
        ...
        # (WorkGuide enforces a blocking-IO gate in CI). If your policy
    ...
    def _decide(self, request):
        # 1. Normalize WorkGuide request data into policy context.
        context = {
        ...
        # 3. Convert the policy result back to WorkGuide's decision object.
        return GuardrailDecision(
            allow=result["allow"],
            reasons=[
                GuardrailReason(
...
WorkGuide loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="workguide"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="workguide" tells you which config dir to use
        ...
```
...
```
packages/harness/workguide/guardrails/
...
packages/harness/workguide/config/
    guardrails_config.py     # GuardrailsConfig Pydantic model + singleton

packages/harness/workguide/agents/middlewares/
