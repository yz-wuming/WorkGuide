...
consumer side of this module lives in ``workguide.persistence.*``):
...
_current_user: Final[ContextVar[CurrentUser | None]] = ContextVar("workguide_current_user", default=None)
...
def _storage_user_id_from_auth_identity(identity: object | None) -> str | None:
    ...
    # BaseUser.identity, while WorkGuide's user directories require a narrower
...
def resolve_config_user_id(config: object | None) -> str:
    ...
    values. Gateway runtime context remains the next source for WorkGuide's
