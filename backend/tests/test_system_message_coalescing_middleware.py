...
from workguide.agents.middlewares.system_message_coalescing_middleware import (
...
class TestCoalesceRequest:
    ...
    def test_system_message_plus_one_in_msg_system_coalesces(self):
        ...
        prompt = SystemMessage(content="You are WorkGuide.", id="sys-1")
        ...
        assert "You are WorkGuide." in result.system_message.content
    ...
    def test_merged_kwargs_combine_all_parts(self):
        ...
        assert result.system_message.additional_kwargs == {
            ...
            "workguide_content_kind": "middleware_injection",
            "workguide_producer_kind": "system_coalescing",
        }
    ...
    def test_merged_handles_list_content(self):
        ...
        prompt = SystemMessage(
            content=[{"type": "text", "text": "You are WorkGuide."}],
            id="sys-1",
        )
        reminder = SystemMessage(content="<system-reminder>date</system-reminder>", id="msg-1")
        request = _make_request(system_message=prompt, messages=[reminder])
...
class TestRealisticScenario:
    def test_first_turn_single_system_message_in_final_payload(self):
        ...
        request = _make_request(
            system_message=SystemMessage(content="You are WorkGuide 2.0, an AI assistant...", id="sys-prompt"),
            messages=[
                SystemMessage(
        ...
        assert "WorkGuide 2.0" in final[0].content
...
class TestStrictBackendStub:
    ...
    def test_first_turn_without_middleware_rejects(self):
        ...
        request = _make_request(
            system_message=SystemMessage(content="You are WorkGuide.", id="sys-prompt"),
            messages=[
