...
DeerMem wraps the WorkGuide memory machinery (the five ``core/`` modules:
storage / queue / updater / prompt / message_processing) behind the
backend-neutral :class:`~workguide.agents.memory.manager.MemoryManager`
...
from workguide.agents.memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager
...
def _resolve_agent_name(agent_name: str | None) -> str:
    """Return WorkGuide's case-insensitive canonical agent identifier."""
