"""Helpers for the WorkGuide Monocle behavioural tests.
...
``monocle_trace_asserter`` fixture), so ``run_workguide`` only drives the agent;
...
def run_workguide(message: str) -> str:
    """Run the WorkGuide agent once and return its response text.

    The model is resolved from ``config.yaml`` (no hardcoded override) so the
    live test exercises WorkGuide's own model-resolution path.
    """
