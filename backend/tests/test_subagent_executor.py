...
workguide.subagents.executor. This test file uses delayed import via fixture to test
...
from workguide.subagents.capacity import SubagentCapacityRejected

# Module names that need to be mocked to break circular imports
_MOCKED_MODULE_NAMES = [
    "workguide.agents",
    "workguide.agents.thread_state",
    "workguide.agents.middlewares",
    "workguide.agents.middlewares.thread_data_middleware",
    "workguide.sandbox",
    "workguide.sandbox.middleware",
    "workguide.sandbox.security",
    "workguide.models",
    "workguide.skills.storage",
]
...
def _clear_stale_executor_package_attr() -> None:
    subagents_pkg = sys.modules.get("workguide.subagents")
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("workguide.subagents.executor")
    original_tool_search = sys.modules.get("workguide.tools.builtins.tool_search")
    ...
    tool_search_module = importlib.import_module("workguide.tools.builtins.tool_search")

    # Remove mocked executor if exists (from conftest.py)
    if "workguide.subagents.executor" in sys.modules:
        del sys.modules["workguide.subagents.executor"]
    _clear_stale_executor_package_attr()

    # Set up mocks
    for name in _MOCKED_MODULE_NAMES:
        sys.modules[name] = MagicMock()
    storage_module = ModuleType("workguide.skills.storage")
    ...
    sys.modules["workguide.skills.storage"] = storage_module
    sys.modules["workguide.tools.builtins.tool_search"] = tool_search_module

    # Import real classes inside fixture
    from langchain_core.messages import AIMessage, HumanMessage, ToolMessage

    from workguide.subagents.config import SubagentConfig
    from workguide.subagents.executor import (
        SubagentExecutor,
        SubagentResult,
        SubagentStatus,
    )

    executor_module = sys.modules["workguide.subagents.executor"]
    ...
    if original_executor is not None:
        sys.modules["workguide.subagents.executor"] = original_executor
    elif "workguide.subagents.executor" in sys.modules:
        del sys.modules["workguide.subagents.executor"]
    if original_tool_search is not None:
        sys.modules["workguide.tools.builtins.tool_search"] = original_tool_search
    else:
        sys.modules.pop("workguide.tools.builtins.tool_search", None)
...
class TestAgentConstruction:
    ...
    def test_create_agent_threads_explicit_app_config_to_model_and_middlewares(
        ...
        import workguide.config as config_module
        from workguide.subagents import executor as executor_module
        ...
        monkeypatch.setitem(
            sys.modules,
            "workguide.agents.middlewares.tool_error_handling_middleware",
            _module(
                "workguide.agents.middlewares.tool_error_handling_middleware",
    ...
    async def test_load_skills_uses_explicit_app_config_for_skill_storage(
        ...
        monkeypatch.setattr(sys.modules["workguide.skills.storage"], "get_or_new_user_skill_storage", fake_get_or_new_user_skill_storage)
    ...
    async def test_load_skills_uses_each_subagent_users_scoped_storage(
        ...
        monkeypatch.setattr(sys.modules["workguide.skills.storage"], "get_or_new_skill_storage", global_storage)
        monkeypatch.setattr(sys.modules["workguide.skills.storage"], "get_or_new_user_skill_storage", user_storage)
    ...
    async def test_build_initial_state_consolidates_system_prompt_and_skill_discovery(
        ...
        monkeypatch.setattr(
            sys.modules["workguide.skills.storage"],
...
class TestAsyncExecutionPath:
    ...
    async def test_aexecute_step_capture_survives_history_contraction(self, classes, base_config, mock_agent, msg):
        """Regression for #3875 Phase 3: WorkGuideSummarizationMiddleware rewrites the
    ...
    async def test_aexecute_recursion_error_with_llm_error_fallback_surfaces_failed(self, classes, base_config, mock_agent, msg):
        ...
        ``additional_kwargs.workguide_error_fallback``, and the
...
class TestSyncExecutionPath:
    ...
    async def test_execute_in_running_event_loop_calls_isolated_loop_directly(self, classes, base_config, mock_agent, msg):
        ...
        from workguide.runtime.user_context import (
...
class TestThreadSafety:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        executor = importlib.import_module("workguide.subagents.executor")
...
class TestCooperativeCancellation:
    ...
    def test_execute_async_drops_parent_callbacks_at_isolated_loop_boundary(self, executor_module, classes, base_config):
        ...
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentStatus = classes["SubagentStatus"]
        parent_callback = SimpleNamespace(workguide_loop_bound=True)
    ...
    def test_isolated_context_filters_callback_manager_without_mutating_parent(self, executor_module):
        ...
        loop_bound = SimpleNamespace(workguide_loop_bound=True)
...
class TestSubagentCheckpointLineage:
    ...
    async def test_aexecute_leaves_checkpoint_coordinates_to_parent_context(
        ...
        """
        executor_module = importlib.import_module("workguide.subagents.executor")
...
class TestSubagentTracingWiring:
    ...
    def _clear_langfuse_env(self, monkeypatch):
        ...
        """
        from workguide.config.tracing_config import reset_tracing_config
    ...
    def _make_executor(self, classes, *, user_id=None, name="general-purpose", parent_model="test-model", workguide_trace_id=None):
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentConfig = classes["SubagentConfig"]
        config = SubagentConfig(
            name=name,
    ...
    async def test_aexecute_injects_langfuse_session_user_and_trace_name(
        ...
        executor = self._make_executor(classes, user_id="alice", name="general_purpose", workguide_trace_id="gateway-trace-sub")
        fake_agent = _FakeStreamAgent()
        monkeypatch.setattr(executor, "_build_initial_state", self._noop_build_initial_state)
        monkeypatch.setattr(executor, "_create_agent", lambda *a, **kw: fake_agent)
        ...
        assert metadata.get("workguide_trace_id") == "gateway-trace-sub"
        assert fake_agent.captured_context.get("workguide_trace_id") == "gateway-trace-sub"
    ...
    async def test_user_id_defaults_when_not_supplied(
        ...
        metadata = (fake_agent.captured_config or {}).get("metadata") or {}
        # DEFAULT_USER_ID is "default" (see workguide.runtime.user_context).
    ...
    @pytest.mark.anyio
    async def test_environment_tag_emitted_from_work_guide_env(
        self,
        classes,
        executor_module,
        monkeypatch,
    ):
        """``WORKGUIDE_ENV`` must surface as an ``env:<value>`` tag so Langfuse
        cost aggregation can split traces by deployment environment.
        """
        monkeypatch.setenv("LANGFUSE_TRACING", "true")
        monkeypatch.setenv("LANGFUSE_PUBLIC_KEY", "pk-lf-test")
        monkeypatch.setenv("LANGFUSE_SECRET_KEY", "sk-lf-test")
        monkeypatch.setenv("WORKGUIDE_ENV", "staging")
        from workguide.config.tracing_config import reset_tracing_config
...
class TestSubagentGuardrailAttribution:
    ...
    async def test_aexecute_propagates_channel_user_id_to_subagent_context(
        ...
        dispatching turn's ``WORKGUIDE_CHANNEL_USER_ID`` (group chats share one
