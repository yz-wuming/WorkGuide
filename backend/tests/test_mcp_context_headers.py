...
from workguide.mcp.context_headers import build_context_headers_interceptor
from workguide.mcp.interceptors import build_mcp_tool_interceptors
...
def test_stdio_server_is_skipped_with_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="workguide.mcp.context_headers"):
        assert build_context_headers_interceptor(config) is None
...
def test_falls_back_to_ambient_runtime_when_request_runtime_is_missing():
    interceptor = build_context_headers_interceptor(_config(headers={"X-Tenant-Token": "tenant_token"}))
    with patch(
        "workguide.mcp.context_headers._current_runtime",
...
def _connection_headers_for_adapter_call(config: ExtensionsConfig) -> dict[str, str]:
    ...
    from workguide.mcp.client import build_server_params
...
def test_absent_run_context_is_denied():
    interceptor = build_context_headers_interceptor(_config(headers={"X-Tenant-Token": "tenant_token"}))
    with patch("workguide.mcp.context_headers._current_runtime", return_value=None), pytest.raises(ToolException):
...
def _run_adapter_tool_in_graph(*, isolate_request_runtime: bool = False) -> dict[str, Any]:
    ...
    WorkGuide does not wrap HTTP/SSE MCP tools, so the tool under test here is the
    ...
    if isolate_request_runtime:
        with patch("workguide.mcp.context_headers._current_runtime", return_value=None):
            _invoke()
    else:
        _invoke()
    return seen_headers
...
def test_durable_submit_carries_the_request_scoped_headers():
    ...
    from workguide.mcp.tasks import TaskSubmitRequest
    from workguide.mcp.tasks.ordinary import OrdinaryMcpTaskDriver
...
async def test_durable_status_poll_keeps_the_server_credential():
    ...
    from workguide.mcp.tasks.models import TaskReference
    from workguide.mcp.tasks.ordinary import OrdinaryMcpTaskDriver
