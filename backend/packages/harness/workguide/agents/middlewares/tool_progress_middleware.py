...
Architecture:
  ToolProgressMiddleware (outer)
    └── handler → ToolErrorHandlingMiddleware (inner) → actual tool
                                                              ↓
  ToolProgressMiddleware reads workguide_tool_meta from the normalized result
...
from workguide.agents.middlewares.tool_result_meta import TOOL_META_KEY, ToolResultMeta
    ...
    def _update_state_from_result(
        ...
        if meta is None:
            if tool_name not in self._exempt_tools:
                logger.warning(
                    "tool_progress: workguide_tool_meta missing for non-exempt tool %s — verify ToolProgressMiddleware is outer of ToolErrorHandlingMiddleware",
                    tool_name,
                )
            return result
