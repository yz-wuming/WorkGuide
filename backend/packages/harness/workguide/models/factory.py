...
def _apply_stream_chunk_timeout_default(model_class: type, model_settings_from_config: dict) -> None:
    ...
    it is accepted by ``ChatOpenAI`` and by every WorkGuide provider that subclasses
...
def create_chat_model(name: str | None = None, thinking_enabled: bool = False, *, app_config: AppConfig | None = None, attach_tracing: bool = True, model_overrides: dict | None = None, **kwargs) -> BaseChatModel:
    ...
    model_settings_from_config = model_config.model_dump(
        ...
        exclude={
            ...
            "thinking",
            "supports_vision",
            # UI metadata stored on runtime-added models (see
            # workguide.config.runtime_models) — must never reach the provider
            # client, which would forward it into the completion request.
            "provider",
    ...
    from workguide.models.openai_codex_provider import CodexChatModel
