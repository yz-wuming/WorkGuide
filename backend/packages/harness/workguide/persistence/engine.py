...
async def init_engine(
    ...
    *,
        ...
        def _enable_sqlite_wal(dbapi_conn, _record):  # noqa: ARG001 — SQLAlchemy contract
            ...
            try:
                try:
                    ...
                    # Some filesystems (e.g. Docker Desktop bind mounts on
                    # Windows) cannot create the -wal/-shm sidecar files and
                    # raise "disk I/O error". Fall back to the default journal
                    # mode so the app still boots and persists there. The async
                    # driver surfaces the failure as sqlalchemy.exc.OperationalError
                    # (wrapping sqlite3.OperationalError), so catch both.
                    logger.warning(
    ...
    # See workguide.persistence.bootstrap for the full state machine.
    from workguide.persistence.bootstrap import bootstrap_schema
