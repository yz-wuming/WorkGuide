...
Uses Honcho (self-hosted or hosted, v3 API) as WorkGuide's
...
memory:
  ...
  backend_config:
    ...
    workspace_prefix: workguide-u-   # one isolated workspace per user id
...
`MemoryManager` contract. WorkGuide offloads it at every async boundary via
