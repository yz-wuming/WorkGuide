...
WORKGUIDE_ROOTS={shlex.quote(str(repo_root))}
FAKE_COMMAND={shlex.quote(command)}
FAKE_ARGS={shlex.quote(args)}
FAKE_WORKGUIDE_PID={1 if workguide_pid else 0}

_is_workguide_pid() {{
    [ "$FAKE_WORKGUIDE_PID" = "1" ]
}}
...
@pytest.mark.parametrize(
    ("command", "args", "workguide_pid"),
    [
        ("nginx: worker process", "nginx: worker process", False),
        ("python", "python -m nginx /tmp/workguide/docker/nginx/nginx.local.conf", True),
    ],
)
