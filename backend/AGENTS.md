...
WorkGuide is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, persistent memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.
...
**Runtime**:
- `make dev`, Docker dev, and production all run the agent runtime in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/workguide/runtime/`). Nginx exposes that runtime at `/api/langgraph/*` and rewrites it to Gateway's native `/api/*` routers.
...
│   ├── packages/
│   │   ├── extension-api/     # public, host-independent extension contracts (import: workguide_extension_api.*)
│   │   └── harness/           # workguide-harness package (import: workguide.*)
│   │       ├── pyproject.toml
│   │       └── workguide/
...
│   │           └── client.py          # Embedded Python client (WorkGuideClient)
...
make test-live          # Explicitly run live WorkGuideClient tests with real APIs
...
The backend `make dev` target pre-creates and excludes `WORKGUIDE_HOME`
(default: `backend/.workguide`) and `backend/sandbox` from Uvicorn's reload
watcher. Do not replace it with a bare `uvicorn --reload`: agent tasks write
Python and other runtime files below `WORKGUIDE_HOME`, which would otherwise
...
- **Harness** (`packages/harness/workguide/`): Publishable agent framework package (`workguide-harness`). Import prefix: `workguide.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config — everything needed to build and run agents.
- **App** (`app/`): Unpublished application code. Import prefix: `app.*`. Contains the FastAPI Gateway API and IM channel integrations (Feishu, Slack, Telegram, DingTalk).

**Dependency rule**: App imports workguide, but workguide never imports app. This boundary is enforced by `tests/test_harness_boundary.py` which runs in CI.

**Import conventions**:
```python
# Harness internal
from workguide.agents import make_lead_agent
...
Package import hygiene: the `workguide.agents` and `workguide.subagents` package
roots expose heavyweight graph/executor entrypoints lazily. The
`workguide.agents:make_lead_agent` LangGraph Server entrypoint is a concrete thin
...
- If a module causes circular import issues in tests, add a `sys.modules` mock in `tests/conftest.py` (see existing example for `workguide.subagents.executor`)
...
skipped unless `WORKGUIDE_RUN_LIVE_TESTS=1` is set. Do not add that opt-in to
...
- Gateway HTTP upload/list/delete handlers offload filesystem work through `workguide.utils.file_io.run_file_io`, a dedicated ContextVar-preserving file IO executor. Non-mounted sandbox uploads acquire sandboxes with `SandboxProvider.acquire_async()` and offload `read_bytes()` plus `sandbox.update_file()` together.
...
- Manual compaction uses `POST /api/threads/{id}/compact`, reuses the same
  `WorkGuideSummarizationMiddleware`, writes a new checkpoint with updated
