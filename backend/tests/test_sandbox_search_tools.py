...
from workguide.sandbox.tools import glob_tool, grep_tool, ls_tool
...
def test_glob_tool_returns_virtual_paths_and_ignores_common_dirs(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda runtime: LocalSandbox(id="local"))
...
def test_grep_tool_truncates_results(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.get_app_config", lambda: SimpleNamespace(get_tool_config=lambda name: None))
...
def test_glob_tool_honors_smaller_requested_max_results(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda runtime: LocalSandbox(id="local"))
    monkeypatch.setattr(
        "workguide.sandbox.tools.get_app_config",
...
def test_ls_tool_skills_path_uses_sandbox_mapping_user_id_not_contextvar(tmp_path, monkeypatch) -> None:
    ...
    try:
        with patch("workguide.config.paths.get_paths", return_value=Paths(base_dir=base_dir)):
            with patch("workguide.config.get_app_config", return_value=app_config):
                result = ls_tool.func(
                    runtime=_make_runtime(tmp_path),
...
def _make_skills_sandbox(tmp_path, monkeypatch, *, disabled: str):
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(ext))
...
def test_extract_skill_name_treats_category_dir_with_trailing_slash_as_root() -> None:
    ...
    """
    from workguide.sandbox.tools import _extract_skill_name_from_skills_path as extract
...
def _make_custom_skills_sandbox(tmp_path, monkeypatch, *, user_id: str, disabled: str):
    ...
    """
    from workguide.skills.storage import reset_skill_storage
    ...
    reset_skill_storage()
    monkeypatch.setattr("workguide.sandbox.tools.resolve_runtime_user_id", lambda runtime: user_id)
