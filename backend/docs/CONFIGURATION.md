...
This guide explains how to configure WorkGuide for your environment.
...
- Xiaomi MiMo (`workguide.models.patched_mimo:PatchedChatMiMo`)
- Claude Code OAuth (`workguide.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`workguide.models.openai_codex_provider:CodexChatModel`)
...
models:
  - name: gpt-5.4
    ...
    use: workguide.models.openai_codex_provider:CodexChatModel
    model: gpt-5.4
  ...
  - name: claude-sonnet-4.6
    ...
    use: workguide.models.claude_provider:ClaudeChatModel
...
- On macOS, WorkGuide does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `workguide.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: workguide.models.patched_openai:PatchedChatOpenAI
...
models:
  - name: mimo-v2.5-pro
    ...
    use: workguide.models.patched_mimo:PatchedChatMiMo
    model: mimo-v2.5-pro
...
`knowledge_search`. WorkGuide does not persist a copy of dataset or document
...
tools:
  - name: knowledge_search
    group: knowledge
    use: workguide.community.ragflow.tools:knowledge_search_tool
...
it contains RAGFlow dataset IDs selected by the deployment operator, WorkGuide
...
scores from different embedding spaces are not globally comparable, WorkGuide
...
deletion remain in RAGFlow and are not exposed as Agent tools or WorkGuide APIs.
...
tools:
  - name: web_search
    group: web
    use: workguide.community.tavily.tools:web_search_tool
    max_results: 5
...
tools:
  - name: web_capture
    group: web
    use: workguide.community.browserless.tools:web_capture_tool
...
WorkGuide supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: workguide.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: workguide.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```

**BoxLite micro-VM Sandbox** (runs sandbox code in daemonless OCI micro-VMs):
```yaml
sandbox:
   use: workguide.community.boxlite:BoxliteProvider
...
```bash
pip install "workguide-harness[boxlite]"
```
...
When using Docker development (`make docker-start`), WorkGuide starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.

Remote/provisioner backends default to explicit file synchronization because
WorkGuide cannot infer whether their `/mnt/user-data` mount points reference the
...
```yaml
sandbox:
   use: workguide.community.e2b_sandbox:E2BSandboxProvider
...
`e2b-code-interpreter` is bundled as a core dependency of `workguide-harness`,
...
- Each WorkGuide thread is bound to its E2B sandbox via metadata
  (`work_guide_user`, `work_guide_thread`). Startup and periodic reconciliation
...
```yaml
sandbox:
   use: workguide.community.opensandbox:OpenSandboxProvider
   image: python:3.11
...
```bash
pip install "workguide-harness[opensandbox]"
```
...
`allow_host_bash` is intentionally `false` by default. WorkGuide's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.

When `LocalSandboxProvider` runs under `make up`, it runs inside the `workguide-gateway` container. In that mode, `sandbox.mounts[].host_path` is resolved from the gateway container's filesystem, not from your Docker host. If you need a local-sandbox custom mount in production Docker, bind the host directory into the gateway service first, then use the in-container path in `config.yaml`:
...
services:
  gateway:
    volumes:
      - ${WORKGUIDE_REPO_ROOT}/.workguide/knowledge:/app/.workguide/knowledge:ro
```
...
sandbox:
  use: workguide.sandbox.local:LocalSandboxProvider
  mounts:
    - host_path: /app/.workguide/knowledge
      container_path: /mnt/knowledge
      read_only: true
```

If the configured `host_path` is not visible to the gateway process, WorkGuide logs an error and ignores that mount.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: workguide-sandbox
...
When you configure `sandbox.mounts`, WorkGuide exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

#### Sandbox container network exposure and hardening

The sandbox HTTP API (`/v1/shell/*` and friends) has no authentication: anyone who can reach a published sandbox port can execute arbitrary commands in that sandbox. For bare-metal Docker sandbox runs that use localhost, WorkGuide binds the sandbox port to `127.0.0.1` so it is not exposed on other host interfaces. For Docker-outside-of-Docker deployments that connect through `host.docker.internal`, the port is bound to the address that hostname actually resolves to — the daemon's `host-gateway-ip` mapping (customizable, possibly IPv6) — so the published port and the address the gateway connects to always match, and the port is no longer published on external network interfaces (previously it was bound to `0.0.0.0`). If resolution fails, the Docker default bridge gateway (via `docker network inspect bridge`, falling back to `172.17.0.1`) is used as a best-effort bind and a warning is logged. Set `WORKGUIDE_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address; setting it to `0.0.0.0` restores the legacy broad bind, which re-exposes the unauthenticated exec API on every interface and should be paired with an external firewall.
...
A custom image that is already fully initialized as a non-root user (no runtime root handoff) should set `WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS=0` to drop every capability including those three: leaving them on would let sandboxed code chown bind-mounted paths or impersonate mounted-file UIDs/GIDs for the container's lifetime. Note that `no-new-privileges` does **not** mitigate that risk — it only blocks gaining privileges across exec; the risk comes from the retained `CAP_SETUID`/`CAP_SETGID` themselves. One hardening knob is relaxed by default: the shipped AIO image runs with `seccomp=unconfined` because its Chromium browser does not start under Docker's default seccomp profile (syscall filtering is disabled — see the two seccomp variables below to change that). The following environment variables (set them in the gateway process, e.g. via `.env` loaded by docker-compose, or the gateway service `environment:`) tune or disable each knob:

| Environment variable | Default | Purpose |
| --- | --- | --- |
| `WORKGUIDE_SANDBOX_BIND_HOST` | loopback / bridge gateway (see above) | Host interface for the sandbox `-p` publish. Must be an IP literal (bare or bracketed IPv6) or a hostname, which is resolved to an address first — Docker publish specs do not accept hostnames. `0.0.0.0` restores the legacy broad bind (risky). |
| `WORKGUIDE_SANDBOX_SECCOMP_UNCONFINED` | on | The shipped AIO image's Chromium browser does not start under Docker's default seccomp profile (see the upstream agent-infra sandbox FAQ), so `seccomp=unconfined` remains the default. Set to `0` to run with the built-in profile — passed explicitly as `seccomp=builtin`, so a daemon configured with a different default cannot weaken the opt-out — and only for images verified to start and pass browser checks with it. |
| `WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS` | on | Keeps the four capabilities (`CHOWN`/`SETUID`/`SETGID`/`DAC_OVERRIDE`) that the shipped image needs: three for the entrypoint's runtime user handoff, plus `DAC_OVERRIDE` because the root nginx master writes gem-owned log files for the container's lifetime. Set to `0` for images already fully initialized as a non-root user — every capability is then dropped, so sandboxed code cannot chown bind-mounted paths or impersonate mounted-file UIDs/GIDs. |
| `WORKGUIDE_SANDBOX_SECCOMP_PROFILE` | unset | Path to a custom seccomp profile (e.g. a restricted, Chromium-compatible one built from Docker's default plus the namespace syscalls Chromium needs). Takes precedence over the unconfined default. |
| `WORKGUIDE_SANDBOX_MEMORY` | `2g` | `--memory` limit per sandbox container. `0`/`none` disables the limit. |
| `WORKGUIDE_SANDBOX_CPUS` | `2` | `--cpus` limit per sandbox container. `0`/`none` disables the limit. |
| `WORKGUIDE_SANDBOX_PIDS_LIMIT` | `512` | `--pids-limit` per sandbox container (fork-bomb guard). `0`/`none` disables the limit. |
| `WORKGUIDE_SANDBOX_CONTAINER_USER` | unset (image default) | Passed through as `--user` (e.g. `1000:1000`). The default AIO image's user is upstream-controlled, so WorkGuide does not force one; set this only if you know your image's runtime user. |
| `WORKGUIDE_SANDBOX_NETWORK` | unset (daemon default network) | Passed through as `--network`. Point it at a dedicated, egress-controlled Docker network so sandbox egress can be filtered by that network's policy; by default sandbox code can otherwise reach internal networks and cloud metadata endpoints directly. `host`, `container:<name>`, and `none` are rejected at startup (including through Docker's extended `name=<network>` syntax, whose effective target is validated): Docker drops `-p/--publish` in host mode (and shares the namespace for `container:<name>`), which would void the hardened port bind and re-expose the unauthenticated exec API; `none` leaves the container loopback-only, so the published sandbox API port cannot receive traffic and every acquisition would time out. |
...
`AioSandboxProvider` talks to the sandbox container through the `agent-sandbox` SDK. The Dockerfile for the default `enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest` image is not part of this repository; WorkGuide treats that image as an upstream AIO sandbox runtime.
...
USER root
# Example user dependency; not required by WorkGuide itself.
RUN apt-get update \
...
If you rebuild the runtime from scratch instead of extending the published image, it must expose the same HTTP API used by `agent-sandbox`. WorkGuide currently depends on:

- `sandbox.get_context()`, including `home_dir`
- `shell.exec_command(...)`
- `bash.exec(...)` — only exercised for per-command environment injection (skills that declare `required-secrets`). The `/v1/bash/*` routes exist since upstream all-in-one-sandbox `1.9.3`; on older images (including a `latest` tag still frozen on the `1.0.0.x` line) WorkGuide fails fast with an actionable error instead of surfacing the raw 404. Pin `sandbox.image` to `1.9.3` or newer (e.g. `1.11.0`) and recreate the sandbox container to use `required-secrets` with the AIO sandbox.
- `file.read_file(...)`
...
- `/mnt/user-data` must remain writable because WorkGuide mounts thread workspace, uploads, and outputs there.
- `home_dir` comes from the sandbox context endpoint; do not assume WorkGuide hardcodes it.
- Shell command handling must remain compatible with serialized `exec_command` calls. WorkGuide serializes shell access on the host side to avoid corrupting the sandbox's persistent shell session.
...
**How Skills Work**:
- Skills are stored in `workguide/skills/{public,custom}/`
...
2. Restart the WorkGuide service to apply changes

## Environment Variables

WorkGuide supports environment variable substitution using the `$` prefix:
...
- `WORKGUIDE_PROJECT_ROOT` - Project root for relative runtime paths
- `WORKGUIDE_CONFIG_PATH` - Custom config file path
- `WORKGUIDE_EXTENSIONS_CONFIG_PATH` - Custom extensions config file path
- `WORKGUIDE_HOME` - Runtime state directory (defaults to `.workguide` under the project root)
- `WORKGUIDE_SKILLS_PATH` - Skills directory when `skills.path` is omitted
...
The configuration file should be placed in the **project root directory** (`workguide/config.yaml`). Set `WORKGUIDE_PROJECT_ROOT` when the process may start from another working directory, or set `WORKGUIDE_CONFIG_PATH` to point at a specific file.

## Configuration Priority

WorkGuide searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `WORKGUIDE_CONFIG_PATH` environment variable
3. `config.yaml` under `WORKGUIDE_PROJECT_ROOT`, or under the current working directory when `WORKGUIDE_PROJECT_ROOT` is unset
4. Legacy backend/repository-root locations for monorepo compatibility

## Security Notes
### Sandbox Isolation and the Docker Socket (DooD)

WorkGuide executes agent-generated shell/code through a configurable sandbox
...
|------|---------------|--------------------|-----------|
| `local` (default) | `workguide.sandbox.local:LocalSandboxProvider` | Not mounted | Commands run **inside the gateway container** on its filesystem. Not a strong boundary — `allow_host_bash` is `false` by default and should stay off for untrusted workloads. |
| `aio` (pure DooD) | `workguide.community.aio_sandbox:AioSandboxProvider` (no `provisioner_url`) | **Mounted** (opt-in overlay) | Sandbox containers are started via the host Docker daemon. |
...
matters for WorkGuide because the gateway executes model-generated commands, so a
...
WorkGuide can reuse your Claude Code / Codex CLI subscription login as a model
...
adapters authenticate independently of WorkGuide via their own documented env —
...
Docker container. WorkGuide forwards enabled MCP servers to the MCode session.
...
1. **Place `config.yaml` in project root** - Set `WORKGUIDE_PROJECT_ROOT` if the runtime starts elsewhere
...
- Ensure `config.yaml` exists in the **project root** directory (`workguide/config.yaml`)
- If the runtime starts outside the project root, set `WORKGUIDE_PROJECT_ROOT`
- Alternatively, set `WORKGUIDE_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `workguide/skills/` directory exists
- Verify skills have valid `SKILL.md` files
- Check `skills.path` or `WORKGUIDE_SKILLS_PATH` if using a custom path
