...
from workguide.skills.security_scanner import _extract_json_object, scan_skill_content
...
def _make_env(monkeypatch, response_content):
    ...
    monkeypatch.setattr("workguide.skills.security_scanner.get_app_config", lambda: config)
    monkeypatch.setattr("workguide.skills.security_scanner.create_chat_model", _fake_create_chat_model)
    return model
...
def _make_traced_env(monkeypatch, *, model_name, response_content='{"decision":"allow","reason":"ok"}'):
    ...
    monkeypatch.setattr("workguide.skills.security_scanner.get_app_config", lambda: config)
    monkeypatch.setattr("workguide.skills.security_scanner.create_chat_model", _fake_create_chat_model)
    monkeypatch.setattr("workguide.skills.security_scanner.get_effective_user_id", lambda: "scanner-user")
    return model
...
def _enable_langfuse_env(monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_ENV", "production")
...
async def test_scan_skill_content_blocks_when_model_unavailable(monkeypatch):
    config = SimpleNamespace(skill_evolution=SimpleNamespace(moderation_model_name=None))
    monkeypatch.setattr("workguide.skills.security_scanner.get_app_config", lambda: config)
    monkeypatch.setattr("workguide.skills.security_scanner.create_chat_model", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("boom")))
...
# packages/harness/workguide/agents/lead_agent/agent.py and the Tracing System
...
def _make_unavailable_env(monkeypatch, *, security_fail_closed):
    ...
    monkeypatch.setattr("workguide.skills.security_scanner.get_app_config", lambda: config)
    monkeypatch.setattr(
        "workguide.skills.security_scanner.create_chat_model",
...
async def test_fail_open_logs_operator_visible_warning(monkeypatch, caplog):
    _make_unavailable_env(monkeypatch, security_fail_closed=False)
    with caplog.at_level(logging.WARNING, logger="workguide.skills.security_scanner"):
        result = await scan_skill_content(SKILL_CONTENT, executable=False)
    assert result.decision == "warn"
