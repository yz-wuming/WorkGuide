...
from workguide.authz.outcome import pop_authorization_outcome
...
class TestAllowlistProvider:
    def test_release_policy_contract_has_stable_identity_and_effective_rules(self):
        ...
        assert policy == {
            ...
            "passport": "ops-policy",
            "policy": {
                "id": "workguide.guardrails.allowlist",
                "version": "1.0.0",
            },
            "provider_parameters": {
...
class TestGuardrailMiddleware:
    ...
    def test_protocol_isinstance_check(self):
        ...
        from workguide.guardrails.provider import GuardrailProvider
...
class TestGuardrailsConfig:
    ...
    def test_config_from_dict(self):
        ...
        assert config.provider.use == "workguide.guardrails.builtin:AllowlistProvider"
        assert config.provider.config == {"denied_tools": ["bash"]}

    def test_singleton_load_and_get(self):
        from workguide.config.guardrails_config import get_guardrails_config, load_guardrails_config_from_dict, reset_guardrails_config
...
class TestGuardrailWritesAuthorizationOutcome:
    ...
    def test_the_outcome_store_is_bounded_so_an_unpopped_run_cannot_grow_forever(self):
        ...
        from workguide.authz.outcome import _MAX_TRACKED_OUTCOMES
