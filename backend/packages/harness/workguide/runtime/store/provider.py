...
and the embedded :class:`~workguide.client.WorkGuideClient`.
...
Usage::
    ...
    from workguide.runtime.store.provider import get_store, store_context
...
POSTGRES_STORE_INSTALL = (
    "langgraph-checkpoint-postgres is required for the PostgreSQL store. Install the package extra with: pip install 'workguide-harness[postgres]' (or use: uv sync --all-packages --extra postgres when developing locally)"
)
...
def _sync_store_cm(config) -> Iterator[BaseStore]:
    ...
    :class:`~workguide.config.checkpointer_config.CheckpointerConfig` instance —
