...
def _patch_runtime_resources(monkeypatch, events: list[str]) -> None:
    ...
    monkeypatch.setattr("workguide.runtime.make_stream_bridge", lambda _config: _resource(object()))
    monkeypatch.setattr("workguide.runtime.make_store", lambda _config: _resource(object()))
    monkeypatch.setattr("workguide.runtime.checkpointer.async_provider.make_checkpointer", lambda _config: _resource(object()))
    monkeypatch.setattr("workguide.persistence.engine.init_engine_from_config", init_engine)
    monkeypatch.setattr("workguide.persistence.engine.close_engine", close_engine)
    monkeypatch.setattr("workguide.persistence.engine.get_session_factory", lambda: None)
...
async def test_runtime_owns_engine_cleanup_before_initialization(monkeypatch):
    ...
    monkeypatch.setattr("workguide.runtime.make_stream_bridge", lambda _config: _resource(object()))
    monkeypatch.setattr("workguide.persistence.engine.init_engine_from_config", fail_engine_init)
    monkeypatch.setattr("workguide.persistence.engine.close_engine", close_engine)
...
async def test_later_startup_failure_stops_same_snapshot_and_appends_diagnostics(monkeypatch):
    import workguide.extensions as extensions_module
    from app.gateway.deps import langgraph_runtime
