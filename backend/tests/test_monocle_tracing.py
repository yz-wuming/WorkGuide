...
importing ``workguide.agents`` no longer sets up telemetry at import time.
"""
...
from workguide.config import is_monocle_tracing_enabled
from workguide.config.tracing_config import get_tracing_config, reset_tracing_config
...
def clear_monocle_env(monkeypatch):
    ...
    monkeypatch.setattr("workguide.tracing.monocle._setup_completed", False)
...
def test_toggles_on_and_sets_up(monkeypatch):
    ...
    assert captured == {"workflow_name": "workguide", "monocle_exporters_list": "file"}
...
def test_embedded_hint_when_enabled_but_uninitialized(monkeypatch, caplog):
    ...
    The embedded ``WorkGuideClient`` and the TUI never hit the Gateway lifespan,
    ...
    with caplog.at_level(logging.DEBUG, logger="workguide.tracing.factory"):
...
def test_no_import_time_setup():
    """Regression: importing workguide.agents must not install telemetry.

    The setup call used to live at module import in ``workguide/agents/__init__``.
    ...
    script = textwrap.dedent(
        ...
        import workguide.agents as agents
...
def test_gateway_lifespan_initializes_monocle():
    ...
    with (
        ...
        patch("workguide.skills.projection.ensure_public_skill_projection"),
