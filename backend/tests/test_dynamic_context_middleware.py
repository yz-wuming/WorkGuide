...
from workguide.agents.middlewares.dynamic_context_middleware import (
...
def test_injects_system_reminder_into_first_human_message():
    ...
    with mock.patch("workguide.agents.lead_agent.prompt._get_memory_context", return_value=""), mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt:
...
def test_memory_included_when_present():
    ...
    with (
        mock.patch(
            "workguide.agents.lead_agent.prompt._get_memory_context",
            return_value="<memory>\nUser prefers Python.\n</memory>",
        ),
        mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt,
    ):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
        result = mw.before_agent(state, _fake_runtime())
...
def test_first_run_records_exact_effective_memory():
    ...
    with (
        mock.patch("workguide.agents.lead_agent.prompt._get_memory_context", return_value=context),
        mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt,
    ):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
...
def test_second_turn_with_memory_does_not_reinject():
    ...
    with (
        mock.patch("workguide.agents.lead_agent.prompt._get_memory_context", return_value="<memory>\nUser prefers Python.\n</memory>"),
        mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt,
    ):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
        result = mw.before_agent(state, _fake_runtime())
...
def test_no_human_message_returns_none():
    ...
    with mock.patch("workguide.agents.lead_agent.prompt._get_memory_context", return_value=""):
        result = mw.before_agent(state, _fake_runtime())
    assert result is None
...
def test_memory_message_carries_reminder_key_for_title_eligibility():
    ...
    """
    from workguide.agents.middlewares.dynamic_context_middleware import is_dynamic_context_reminder
...
def test_user_suffix_message_is_not_injection_target():
    ...
    """
    from workguide.agents.middlewares.dynamic_context_middleware import _is_user_injection_target
...
def test_no_recursive_id_swap_in_full_middleware_flow():
    ...
    with mock.patch("workguide.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt, mock.patch("workguide.agents.lead_agent.prompt._get_memory_context", return_value=""):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
