...
This lets the existing :class:`~workguide.guardrails.middleware.GuardrailMiddleware`
enforce :class:`~workguide.authz.provider.AuthorizationProvider` decisions at
tool-call time — no new middleware class required (see RFC §6.1).

The adapter maps :class:`~workguide.guardrails.provider.GuardrailRequest`
fields to :class:`~workguide.authz.provider.AuthzRequest` fields, calls the
authorization provider, and converts the :class:`~workguide.authz.provider.AuthzDecision`
back to a :class:`~workguide.guardrails.provider.GuardrailDecision`.

Principal construction delegates to
:func:`~workguide.authz.principal.build_principal_from_context` so Layer 1
...
from workguide.authz.provider import AuthorizationProvider, AuthzDecision, AuthzRequest
from workguide.guardrails.provider import GuardrailDecision, GuardrailReason, GuardrailRequest
...
class GuardrailAuthorizationAdapter:
    ...
    def evaluate(self, request: GuardrailRequest) -> GuardrailDecision:
        ...
        adapter is consumed by :class:`~workguide.guardrails.middleware.GuardrailMiddleware`,
