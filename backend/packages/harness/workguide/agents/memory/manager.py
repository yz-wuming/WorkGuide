...
and set ``manager_class: <name>``. Nothing else in workguide changes.
...
class MemoryCallbacks:
    ...
    def on_memory_llm_result(
        ...
        *,
        ...
        WorkGuide's extension API. It is invoked for both provider success and
...
class MemoryManager(BaseModel):
    ...
    with the workguide conversation thread. The contract is deliberately
    neutral so a third-party memory system can be adapted without workguide
    code changes:
...
def _scan_backends() -> dict[str, type[MemoryManager]]:
    ...
    for entry in sorted(_BACKENDS_DIR.iterdir()):
        ...
        if not (entry / "__init__.py").is_file():
            continue
        dotted = f"workguide.agents.memory.backends.{entry.name}"
        try:
...
# LLM). The portable backend package never names a workguide concept; the host
    ...
    def on_memory_llm_result(
        ...
        *,
        ...
        if not extensions.has_system_model_observers:
            return
        try:
            from workguide_extension_api import (
                SystemModelRequest,
                SystemModelResult,
                SystemOperationKind,
            )
...
def _host_default_should_keep_hidden_message(additional_kwargs: Any) -> bool:
    """workguide default for DeerMem's ``should_keep_hidden_message`` slot.
...
def _host_default_llm() -> Any:
    """workguide default for DeerMem's ``host_llm`` slot (zero-config extraction).
...
def _host_default_extraction_callback(payload: Any) -> None:
    """workguide default for DeerMem's ``extraction_callback`` slot.
...
def _collect_host_hooks() -> dict[str, Any]:
    ...
    *consumer* that decides which hooks to use (so adding a backend does not
    ...
    """
    from workguide.trace_context import request_trace_context
...
def get_memory_manager() -> MemoryManager:
    ...
    # workguide is multi-threaded: memory injection runs via asyncio.to_thread,
    ...
    with _manager_lock:
        ...
        # Zero-config UX: default DeerMem storage to workguide's state dir
        # (absolute, CWD-independent) so memory lands at
        # {runtime_home}/users/{user_id}/memory.json (workguide's base_dir,
        # same as pre-abstraction) unless the host explicitly sets storage_path.
        if not backend_config.get("storage_path"):
            from workguide.config.runtime_paths import runtime_home
