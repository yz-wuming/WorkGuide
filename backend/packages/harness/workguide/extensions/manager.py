"""Operator-facing installation management for packaged WorkGuide extensions."""
...
_ENTRY_POINT_GROUP = "workguide.extensions"
...
class ExtensionManager:
    """Install trusted Python extensions into one WorkGuide checkout."""
    ...
    def _read_plugins(self) -> tuple[str, list[Any]]:
        if not self.config_path.is_file():
            raise FileNotFoundError(f"WorkGuide config not found: {self.config_path}")
        ...
        try:
            ...
            raise ValueError("invalid WorkGuide config YAML") from exc
        ...
        if not isinstance(config, dict):
            raise ValueError("WorkGuide config root must be a mapping")
...
def _manager_lock(project_root: Path) -> Iterator[None]:
    ...
    lock_directory = project_root / ".workguide"
...
def _validate_remote_source(source: str) -> None:
    ...
    if not scheme:
        raise ValueError("local path references are not deployable; pass a local directory so WorkGuide can snapshot it")
    if scheme == "file":
        raise ValueError("file URLs are not deployable; pass a local directory so WorkGuide can snapshot it")
    if scheme == "ssh":
...
def _detect_extra_flags(project_root: Path, config_path: Path) -> list[str]:
    ...
    environment["WORKGUIDE_CONFIG_PATH"] = str(config_path)
    completed = subprocess.run(
