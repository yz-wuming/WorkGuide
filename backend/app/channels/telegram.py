...
from workguide.uploads.manager import is_upload_staging_file, normalize_filename
...
class TelegramChannel(Channel):
    ...
    async def _bind_connection_from_start_token_on_main(
        ...
        logger.info("[Telegram] bound chat=%s user=%s to WorkGuide user=%s connection=%s", chat_id, user_id, owner_user_id, connection["id"])
        await self._run_on_telegram_loop(update.message.reply_text("Telegram connected to WorkGuide."))
        return True
    ...
    async def _cmd_start(self, update, context) -> None:
        ...
        if not self._check_user(update.effective_user.id):
            return
        await update.message.reply_text("Welcome to WorkGuide! Send me a message to start a conversation.\nType /help for available commands.")
    ...
    async def _on_text(self, update, context) -> None:
        ...
        # topic_id determines which WorkGuide thread the message maps to.
