...
```text
backend/packages/harness/workguide/authz/principal.py
```
...
```text
backend/packages/harness/workguide/tools/builtins/task_tool.py
backend/packages/harness/workguide/subagents/executor.py
```
...
```text
backend/packages/harness/workguide/guardrails/provider.py
backend/packages/harness/workguide/guardrails/middleware.py
backend/packages/harness/workguide/authz/adapter.py
...
修改 `workguide/authz/__init__.py` 导出 builder。向 implementation notes 的决策日志
...
```text
backend/packages/harness/workguide/authz/rbac.py
```
...
```text
backend/packages/harness/workguide/authz/runtime.py
```
...
修改 `workguide/authz/__init__.py` 导出 RBAC provider 和 factory。向 implementation
...
uv run ruff check packages/harness/workguide/authz app/gateway/services.py tests
uv run ruff format --check packages/harness/workguide/authz app/gateway/services.py tests
```
...
- `WorkGuideClient` 现有 skill filter 缺口修复。
