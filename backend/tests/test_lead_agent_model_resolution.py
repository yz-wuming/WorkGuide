...
from workguide.agents.middlewares import summarization_middleware as summarization_middleware_module
...
from workguide.config.subagents_config import SubagentsAppConfig
from workguide.config.summarization_config import SummarizationConfig
from workguide.runtime.checkpoint_mode import INTERNAL_CHECKPOINT_MODE_KEY
from workguide.runtime.secret_context import write_slash_skill_source_path
...
def test_make_lead_agent_uses_server_auth_identity_for_all_user_scoped_inputs(monkeypatch):
    ...
    import workguide.tools as tools_module
...
def test_create_summarization_middleware_uses_configured_model_alias(monkeypatch):
    ...
    monkeypatch.setattr(summarization_middleware_module, "WorkGuideSummarizationMiddleware", lambda **kwargs: kwargs)
...
def test_create_summarization_middleware_uses_frontend_supported_update_key(monkeypatch):
    ...
    assert update_key == "WorkGuideSummarizationMiddleware.before_model"
...
def test_memory_middleware_uses_explicit_memory_config_without_global_read(monkeypatch):
    from workguide.agents.middlewares import memory_middleware as memory_middleware_module
