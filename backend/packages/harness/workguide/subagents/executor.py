...
from workguide.agents.thread_state import SandboxState, ThreadDataState, ThreadState
from workguide.authz.principal import normalize_authz_attributes
...
from workguide.runtime.user_context import DEFAULT_USER_ID
from workguide.skills.types import Skill
from workguide.subagents.capacity import (
    SubagentCapacityError,
    SubagentExecutionCapacity,
    get_subagent_execution_capacity,
)
from workguide.subagents.config import SubagentConfig, resolve_subagent_model_name
from workguide.subagents.step_events import capture_new_step_messages
from workguide.subagents.token_collector import SubagentTokenCollector
from workguide.trace_context import WORKGUIDE_TRACE_METADATA_KEY
...
if TYPE_CHECKING:
    ...
    # `from workguide.subagents import SubagentExecutor`, which re-enters this
...
def _extract_llm_error_fallback(final_state: Any) -> str | None:
    ...
    for message in reversed(final_state.get("messages", [])):
        ...
        if metadata.get("workguide_error_fallback") is not True:
            return None
...
def _copy_isolated_subagent_context() -> Context:
    ...
    if isinstance(callbacks, BaseCallbackManager):
        ...
        isolated_callbacks.handlers = [handler for handler in callbacks.handlers if not getattr(handler, "workguide_loop_bound", False)]
        isolated_callbacks.inheritable_handlers = [handler for handler in callbacks.inheritable_handlers if not getattr(handler, "workguide_loop_bound", False)]
    elif isinstance(callbacks, (list, tuple)):
        isolated_callbacks = [handler for handler in callbacks if not getattr(handler, "workguide_loop_bound", False)]
    elif getattr(callbacks, "workguide_loop_bound", False):
...
class SubagentExecutor:
    ...
    def __init__(
        ...
        Args:
            ...
            workguide_trace_id: WorkGuide request-level correlation id propagated
            ...
            execution_capacity: Optional explicitly shared admission controller.
                Direct ``create_workguide_agent`` callers pass one through their
        ...
        self.workguide_trace_id = workguide_trace_id
    ...
    def _create_agent(
        ...
        *,
        ...
        from workguide.agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares
        ...
        if deferred_setup is not None and deferred_setup.deferred_names:
            from workguide.tools.builtins.tool_search import build_mcp_routing_middleware
        ...
        *,
        ...
        try:
            ...
            descriptor = build_assembly_descriptor(
                ...
                prompt_template_id="workguide-subagent-v1",
                tools=tools,
                middlewares=middlewares,
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        from workguide.tools.builtins.tool_search import assemble_deferred_tools, get_deferred_tools_prompt_section, get_mcp_routing_hints_prompt_section
        ...
        from workguide.skills.describe import build_skill_search_setup, get_skill_index_prompt_section
    ...
    async def _aexecute_admitted(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            inject_langfuse_metadata(
                ...
                workguide_trace_id=self.workguide_trace_id,
            )
            ...
            if self.workguide_trace_id:
                context[WORKGUIDE_TRACE_METADATA_KEY] = self.workguide_trace_id
