...
WorkGuide releases are **tag-driven**: pushing a `v*` git tag triggers the
...
| `deploy/helm/workguide/Chart.yaml`     | `version: X.Y.Z`     |
| `deploy/helm/workguide/Chart.yaml`     | `appVersion: "X.Y.Z"`|
...
2. **Update `CHANGELOG.md`**: rename the `## [Unreleased]` section to
   ...
   ```
   [2.1.0]: https://github.com/bytedance/workguide/releases/tag/v2.1.0
   ```
...
- `.github/workflows/chart.yaml` — packages the Helm chart and pushes it as an
  ...
  ```bash
  helm install workguide oci://ghcr.io/<owner>/charts/workguide --version 2.1.0
  ```
...
is gated on `github.repository == 'bytedance/workguide'`, so it only runs on
...
- Images: `ghcr.io/<owner>/workguide-{backend,frontend,provisioner}:nightly`
...
- Chart: `oci://ghcr.io/<owner>/charts/workguide`, version `<base>-nightly.<date>-<sha>`
...
- Builds multi-arch (`linux/amd64,linux/arm64`) and pushes
  `ghcr.io/<owner>/workguide-{lark-cli-init,lark-cli-broker}:<lark-cli-version>`.
- Gated on `github.repository == 'bytedance/workguide'`; not tied to the
  `verify-versions` gate (its version is the lark-cli release, not the WorkGuide
...
builds remain at the legacy bare `ghcr.io/<owner>/workguide` package. That
