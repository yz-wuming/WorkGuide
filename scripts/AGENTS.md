...
boundaries under `backend/app/` and `backend/packages/harness/workguide/`. It
prints a concise count by execution domain and writes the complete, versioned
JSON payload to `.workguide/thread-boundary-inventory.json`. Every finding has
...
python scripts/detect_thread_boundaries.py \
  ...
  --json-output .workguide/thread-boundary-inventory.json
```
...
The `detect-blocking-io` target parses `app/`, `packages/harness/workguide/`,
...
findings to `.workguide/blocking-io-findings.json` at the repository root
...
- Wraps every item under `tests/blocking_io/` with a strict Blockbuster
  context scoped to `app.*` and `workguide.*` (see
  `tests/support/detectors/blocking_io_runtime.py`). Any sync blocking IO
  call whose stack passes through WorkGuide business code while running on
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/workguide/` never imports from `app.*`
