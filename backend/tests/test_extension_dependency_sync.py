...
def test_the_app_layer_declares_the_contract_package_it_imports_directly() -> None:
    """The app imports ``workguide_extension_api`` itself, so it declares it.

    Only ``workguide-harness`` guarantees the package transitively. That is the
    ...
    importers = sorted(
        ...
        if (isinstance(node, ast.ImportFrom) and (node.module or "").split(".")[0] == "workguide_extension_api") or (isinstance(node, ast.Import) and any(alias.name.split(".")[0] == "workguide_extension_api" for alias in node.names))
    )
    if not importers:
    ...
    assert "workguide-extension-api" in declared, f"imported directly by {importers} but only guaranteed transitively"
...
def test_root_makefile_exposes_extension_management_commands() -> None:
    ...
    assert "workguide extensions install" in install
    assert "--source-env __workguide_extension_source__" in install
    assert "WORKGUIDE_EXTENSION_SOURCE" not in install
    assert "$(SOURCE)" not in install
    assert "uv run --frozen --no-group extensions" in install
    assert "--yes" not in install

    for target, command in (
        ("extension-list", "workguide extensions list"),
        ("extension-enable", "workguide extensions enable"),
        ("extension-disable", "workguide extensions disable"),
        ("extension-remove", "workguide extensions remove"),
    ):
        recipe = _make_recipe(makefile, target)
...
@pytest.mark.parametrize(
    ...
    [
        ("extension-install", "SOURCE", "--source-env __workguide_extension_source__"),
        ("extension-enable", "NAME", "--name-env __workguide_extension_name__"),
        ("extension-disable", "NAME", "--name-env __workguide_extension_name__"),
        ("extension-remove", "NAME", "--name-env __workguide_extension_name__"),
    ],
)
def test_root_extension_shortcuts_keep_command_line_arguments_out_of_the_shell_recipe(
    ...
    assert "$WORKGUIDE_EXTENSION_" not in result.stdout
    assert "%WORKGUIDE_EXTENSION_" not in result.stdout
...
def test_docker_image_builds_from_the_lock_and_never_syncs_at_runtime() -> None:
    ...
    assert "ARG UV_IMAGE=ghcr.io/astral-sh/uv:0.12.5" in dockerfile
    ...
    for compose_name in ("docker-compose.yaml", "docker-compose-dev.yaml"):
        ...
        assert "ghcr.io/astral-sh/uv:0.12.5" in compose
...
def test_docker_builder_can_resolve_locked_git_extensions() -> None:
    ...
    builder_packages = re.search(
        r"apt-get install -y \\\n(?P<packages>.*?)    && rm -rf /var/lib/apt/lists/\*",
        dockerfile,
        re.DOTALL,
    )
