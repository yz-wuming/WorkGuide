...
async def test_goal_evaluator_observes_success_and_failure_with_explicit_snapshot():
    from workguide.runtime.goal import evaluate_goal_completion
...
async def test_title_middleware_uses_build_bound_snapshot_and_live_task_store(monkeypatch):
    from workguide.agents.middlewares import title_middleware as title_module
...
async def test_async_summarization_observes_each_provider_attempt_and_live_store():
    ...
    middleware = WorkGuideSummarizationMiddleware.__new__(WorkGuideSummarizationMiddleware)
...
async def test_memory_callback_dispatches_the_captured_snapshot_and_live_store():
    ...
    from workguide.extensions import reset_loaded_extensions, set_loaded_extensions
...
def test_memory_callback_does_not_swallow_interpreter_shutdown(monkeypatch):
    ...
    from workguide.agents.memory.manager import LangfuseMemoryCallbacks
    from workguide.extensions import notify as notify_module
...
async def test_system_model_failure_logs_identify_the_task_scope(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="workguide.extensions.notify"):
        await notify_system_model_call(
            _extensions(_Broken()),
    ...
    messages = [record.getMessage() for record in caplog.records if record.name == "workguide.extensions.notify"]
