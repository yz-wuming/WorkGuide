...
This document describes how to use and maintain WorkGuide backend blocking-IO
...
```text
.workguide/blocking-io-findings.json
```
...
focused test when code under `app.*` or `workguide.*` performs blocking IO on
...
Keeping rules centralized makes it clear which extra primitives WorkGuide
