...
from workguide.config.agents_config import GitHubTriggerConfig
...
# ---------------------------------------------------------------------------
# @-mention boundary: ``@workguide`` must NOT match ``@workguide-bot``
# ---------------------------------------------------------------------------


def test_mention_prefix_does_not_match_longer_login() -> None:
    # Agent with mention_login='workguide' must NOT fire on a comment that
    # addresses a different account, '@workguide-bot'. Regression for the
    # naive substring ``f'@{login}' in body`` check.
    trigger = _resolve("issue_comment", GitHubTriggerConfig(require_mention=True, mention_login="workguide"))
    fire, reason = event_should_fire(
        "issue_comment",
        {
            "action": "created",
            "issue": {"number": 1, "user": {"login": "alice"}},
            "comment": {"body": "Hey @workguide-bot please review", "user": {"login": "alice"}},
            "repository": {"full_name": "a/b"},
        },
        trigger,
        "workguide",
