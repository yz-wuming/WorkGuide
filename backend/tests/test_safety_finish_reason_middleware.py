...
from workguide.config.safety_finish_reason_config import (
...
class TestFromConfig:
    ...
    def test_custom_detectors_loaded_via_reflection(self):
        cfg = SafetyFinishReasonConfig(
            detectors=[
                SafetyDetectorConfig(
                    use="workguide.agents.middlewares.safety_termination_detectors:OpenAICompatibleContentFilterDetector",
...
class TestStreamEvent:
    def test_emits_event_when_writer_available(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "workguide.agents.middlewares.safety_finish_reason_middleware.emit_custom_event",
    ...
    async def test_async_hook_uses_async_event_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "workguide.agents.middlewares.safety_finish_reason_middleware.aemit_custom_event",
            fake_emit_custom_event,
        )
