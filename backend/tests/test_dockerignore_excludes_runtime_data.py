...
into the image. The runtime directories are written by a *running* WorkGuide, not by
...
- ``WORKGUIDE_HOME`` (``backend/.workguide`` by default) holds the sqlite database,
...
host that has run WorkGuide bakes that state — including the JWT secret and the user
...
fails outright::
    ...
    open .../.workguide/users/<uuid>/integrations/lark-cli: permission denied
...
HOST_LOCAL_PATHS = [
    ...
    "backend/.env",
    "frontend/.env",
    ".workguide/integrations/skills/provider/pack/SKILL.md",
    "backend/.workguide/data/workguide.db",
    "backend/.workguide/.jwt_secret",
    "backend/.workguide/users/some-user/agents/my-agent/config.yaml",
...
BUILD_INPUT_PATHS = [
    ...
    "backend/packages/harness/workguide/config/extensions_config.py",
]
