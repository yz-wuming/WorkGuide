...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable WorkGuide state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.workguide``).  Using
...
class Paths:
    """
    Centralized path configuration for WorkGuide application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. WORKGUIDE_HOME environment variable
        3. Caller project fallback: `{project_root}/.workguide`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set WORKGUIDE_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("WORKGUIDE_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("WORKGUIDE_HOME"):
