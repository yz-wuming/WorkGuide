...
tools:
  - name: ls
    group: file:read
    use: workguide.sandbox.tools:ls_tool
  - name: read_file
    group: file:read
    use: workguide.sandbox.tools:read_file_tool
  - name: write_file
    group: file:write
    use: workguide.sandbox.tools:write_file_tool
...
def prepare_hermetic_extras(home: Path) -> Path:
    ...
    ``WORKGUIDE_EXTENSIONS_CONFIG_PATH`` at it. Call before starting the gateway.
    """
