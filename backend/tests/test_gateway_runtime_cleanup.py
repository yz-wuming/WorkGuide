...
def test_docker_dev_mounts_mutable_configs_through_project_directory():
    ...
    assert "WORKGUIDE_CONFIG_PATH=/app/project/config.yaml" in compose
    assert "WORKGUIDE_EXTENSIONS_CONFIG_PATH=/app/project/extensions_config.json" in compose


def test_local_dev_gateway_reload_excludes_runtime_state_with_absolute_dirs():
    serve_sh = _read("scripts/serve.sh")

    assert 'export WORKGUIDE_PROJECT_ROOT="$REPO_ROOT"' in serve_sh
    assert 'BACKEND_RUNTIME_HOME="$REPO_ROOT/backend/.workguide"' in serve_sh
    assert 'export WORKGUIDE_HOME="$BACKEND_RUNTIME_HOME"' in serve_sh
    ...
    assert 'mkdir -p "$WORKGUIDE_HOME" "$BACKEND_RUNTIME_HOME" "$REPO_ROOT/backend/sandbox"' in serve_sh
    assert "--reload-exclude='$WORKGUIDE_HOME'" in serve_sh
    assert "--reload-exclude='$BACKEND_RUNTIME_HOME'" in serve_sh
    assert "--reload-exclude='sandbox/'" not in serve_sh
    assert "--reload-exclude='.workguide/'" not in serve_sh


def test_backend_make_dev_gateway_reload_excludes_runtime_state_with_absolute_dirs():
    makefile = _read("backend/Makefile")

    assert "WORKGUIDE_HOME ?= $(CURDIR)/.workguide" in makefile
    assert "WORKGUIDE_HOME := $(abspath $(WORKGUIDE_HOME))" in makefile
    assert "BACKEND_SANDBOX_HOME := $(abspath $(CURDIR)/sandbox)" in makefile
    assert 'mkdir -p "$(WORKGUIDE_HOME)" "$(BACKEND_SANDBOX_HOME)"' in makefile
    # The launch line may carry runtime-only uv flags (`--locked` pins the
    # extension lock); what this guards is that WORKGUIDE_HOME is exported on it,
    # so the reload-excludes below resolve to the same absolute directories.
    assert re.search(r'WORKGUIDE_HOME="\$\(WORKGUIDE_HOME\)" uv run(?: --(?:locked|no-sync))? uvicorn', makefile)
    assert '--reload-exclude="$(WORKGUIDE_HOME)"' in makefile
...
def test_frontend_rewrites_langgraph_prefix_to_gateway():
    ...
    assert "WORKGUIDE_INTERNAL_LANGGRAPH_BASE_URL" not in next_config
...
def test_smoke_test_docs_do_not_expect_standalone_langgraph_server():
    ...
    for path, content in smoke_files.items():
        ...
        assert "workguide-langgraph" not in content, path
