...
WorkGuide supports configurable MCP servers and skills to extend its capabilities, which are loaded from a dedicated `extensions_config.json` file in the project root directory.
...
WorkGuide connects to it through the same generic MCP client used for other HTTP
servers:
...
`X-API-Key` is used here because WorkGuide expands a whole-string `$ENV_VAR`
...
authentication fails and WorkGuide skips that MCP server, so no OpenViking tools
appear. Changing only the environment variable does not invalidate WorkGuide's
already-populated, file-signature-based MCP tool cache; after setting or fixing
the key, restart WorkGuide, modify and re-save the extensions config, or call the
MCP cache-reset endpoint at `POST /api/mcp/cache/reset`.

OpenViking owns the tool schemas and behavior. WorkGuide performs the standard
...
For capability parity with other official OpenViking harnesses, WorkGuide exposes
...
confirmation; WorkGuide does not enforce that confirmation.

Operators who do not want agents to call `forget` can block its default visible
name with WorkGuide's existing guardrail configuration:

```yaml
guardrails:
  enabled: true
  provider:
    use: workguide.guardrails.builtin:AllowlistProvider
    config:
...
WorkGuide prefixes discovered MCP tool names with `<server_name>_` by default.
...
unique across the enabled servers. Stdio tools continue to use WorkGuide's
...
WorkGuide already provides built-in file tools for thread-scoped workspace access.
Do not add an MCP filesystem server for the same WorkGuide workspace. The
overlapping file tools use different path semantics, which can make LLM tool
selection and file access behavior unstable.

WorkGuide does not currently adapt the MCP Roots mode for filesystem servers. In
particular, it does not publish per-thread MCP roots or map WorkGuide sandbox
paths such as `/mnt/user-data/...` to paths accepted by
`@modelcontextprotocol/server-filesystem`. Use WorkGuide's built-in file tools
for WorkGuide workspace files.

## Durable Background Tasks with Ordinary MCP Tools

An MCP server can expose a fast `submit` tool plus `status` and `cancel` tools
for long-running work. WorkGuide keeps the remote task ID in SQL and polls it
...
the server's raw names, before WorkGuide adds any `<server_name>_` prefix:
...
- `get_report_status({"task_id":"remote-123"})` returns a status from
  ...
  `poll_after_seconds`. WorkGuide caps that remote scheduling hint at 24 hours.
...
WorkGuide records a bounded snippet of its first text content block and retries
...
cut into invalid JSON: WorkGuide stores a text preview, marks
...
the task alive and recognize its ID after WorkGuide reconnects. A stdio server
...
those polls. Declaring both on one server logs a warning at startup. Restart WorkGuide after changing
...
WorkGuide rejects task-tool reloads that no longer match the Gateway's startup
...
For `http` and `sse` MCP servers, WorkGuide supports OAuth token acquisition and automatic token refresh.
...
Use `user_auth` instead when the credential belongs to a configured WorkGuide
...
```python
from workguide.runtime.secret_context import extract_request_secrets
...
`workguide/mcp/context_headers.py::_current_runtime`).
...
material already retained by those systems. Restarting or upgrading WorkGuide does
not rotate credentials or perform historical cleanup; operators must complete
both actions explicitly.

## How It Works

MCP servers expose tools that are automatically discovered and integrated into WorkGuide’s agent system at runtime. Once enabled, these tools become available to agents without additional code changes.
