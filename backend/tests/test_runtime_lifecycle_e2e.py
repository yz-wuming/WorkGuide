...
from workguide.runtime.checkpoint_state import build_state_mutation_graph
...
@pytest.fixture
def isolated_work_guide_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("WORKGUIDE_EXTENSIONS_CONFIG_PATH", str(staged_extensions_config))
    return home
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``config.yaml``/``extensions_config.json`` and ``WORKGUIDE_HOME``, so the
    caches below must be reset before app creation:

    - app_config / extensions_config: parsed config file caches.
    - paths: ``WORKGUIDE_HOME``-derived filesystem paths.
...
@pytest.fixture
def isolated_app_with_title(isolated_work_guide_home: Path, monkeypatch: pytest.MonkeyPatch):
    config_path = isolated_work_guide_home.parent / "config-title-enabled.yaml"
...
def test_stream_run_executes_real_lead_agent_setup_agent_business_path(isolated_app, isolated_work_guide_home: Path):
    ...
    with (
        ...
        expected_soul = isolated_work_guide_home / "users" / auth_user_id / "agents" / agent_name / "SOUL.md"
        assert expected_soul.exists(), f"setup_agent did not write SOUL.md. tmp tree: {sorted(str(p.relative_to(isolated_work_guide_home)) for p in isolated_work_guide_home.rglob('SOUL.md'))}"
        assert f"Agent name: {agent_name}" in expected_soul.read_text(encoding="utf-8")
        assert not (isolated_work_guide_home / "users" / "default" / "agents" / agent_name).exists()
