...
from workguide.community.aio_sandbox.local_backend import (
...
def test_format_container_mount_uses_mount_syntax_for_docker_windows_paths():
    args = _format_container_mount("docker", "D:/workguide/backend/.workguide/threads", "/mnt/threads", False)

    assert args == [
        "--mount",
        "type=bind,src=D:/workguide/backend/.workguide/threads,dst=/mnt/threads",
    ]
...
def test_start_container_logs_redacted_env_values(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="workguide.community.aio_sandbox.local_backend"):
...
def test_resolve_docker_bind_host_defaults_loopback_for_localhost(monkeypatch):
    monkeypatch.delenv("WORKGUIDE_SANDBOX_BIND_HOST", raising=False)
    monkeypatch.delenv("WORKGUIDE_SANDBOX_HOST", raising=False)
...
def test_resolve_docker_bind_host_follows_host_gateway_mapping_for_dood(monkeypatch):
    ...
    monkeypatch.delenv("WORKGUIDE_SANDBOX_BIND_HOST", raising=False)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_HOST", "host.docker.internal")
    monkeypatch.setattr(
        "workguide.community.aio_sandbox.local_backend._resolve_sandbox_host_address",
...
def test_resolve_docker_bind_host_brackets_bare_ipv6_override(monkeypatch):
    ...
    """
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "fd00::1")
    assert _resolve_docker_bind_host() == "[fd00::1]"
    ...
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "192.168.64.1")
    assert _resolve_docker_bind_host() == "192.168.64.1"
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "0.0.0.0")
...
def test_resolve_docker_bind_host_resolves_hostname_override(monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "host.docker.internal")
    monkeypatch.setattr(
        "workguide.community.aio_sandbox.local_backend._resolve_sandbox_host_address",
...
def test_resolve_docker_bind_host_rejects_unresolvable_hostname_override(monkeypatch):
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "not-a-resolvable-host.invalid")
    monkeypatch.setattr(
        "workguide.community.aio_sandbox.local_backend._resolve_sandbox_host_address",
        lambda host: None,
    )
    with pytest.raises(RuntimeError, match="WORKGUIDE_SANDBOX_BIND_HOST"):
        _resolve_docker_bind_host()
...
def test_resolve_docker_bind_host_uses_discovered_bridge_gateway_when_resolution_fails(monkeypatch):
    ...
    monkeypatch.setattr(
        "workguide.community.aio_sandbox.local_backend._docker_bridge_gateway_ip",
        lambda: "192.168.64.1",
    )
...
def test_resolve_docker_bind_host_uses_ipv6_loopback_for_ipv6_sandbox_host(monkeypatch):
    monkeypatch.delenv("WORKGUIDE_SANDBOX_BIND_HOST", raising=False)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_HOST", "[::1]")

    assert _resolve_docker_bind_host() == "[::1]"


def test_resolve_docker_bind_host_logs_selected_bind_reason(caplog):
    with caplog.at_level(logging.DEBUG, logger="workguide.community.aio_sandbox.local_backend"):
...
def test_resolve_docker_bind_host_allows_explicit_override(monkeypatch):
    monkeypatch.setenv("WORKGUIDE_SANDBOX_HOST", "localhost")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "192.0.2.10")
...
def test_resolve_docker_bind_host_allows_restoring_legacy_broad_bind(monkeypatch):
    """WORKGUIDE_SANDBOX_BIND_HOST=0.0.0.0 restores the pre-hardening bind."""
    monkeypatch.setenv("WORKGUIDE_SANDBOX_HOST", "host.docker.internal")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "0.0.0.0")
...
def _clear_hardening_env(monkeypatch):
    for var in (
        "WORKGUIDE_SANDBOX_HOST",
        "WORKGUIDE_SANDBOX_BIND_HOST",
        "WORKGUIDE_SANDBOX_SECCOMP_UNCONFINED",
        "WORKGUIDE_SANDBOX_SECCOMP_PROFILE",
        "WORKGUIDE_SANDBOX_MEMORY",
        "WORKGUIDE_SANDBOX_CPUS",
        "WORKGUIDE_SANDBOX_PIDS_LIMIT",
        "WORKGUIDE_SANDBOX_CONTAINER_USER",
        "WORKGUIDE_SANDBOX_NETWORK",
        "WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS",
    ):
...
def test_start_container_keeps_apple_container_port_format(monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_SANDBOX_BIND_HOST", "127.0.0.1")
...
def test_start_container_seccomp_can_opt_out_to_default_profile(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_SECCOMP_UNCONFINED", "0")
...
def test_start_container_seccomp_profile_env_selects_custom_profile(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_SECCOMP_PROFILE", "/etc/docker/chromium-seccomp.json")
...
def test_resolve_sandbox_host_address_formats_and_filters(monkeypatch):
    ...
    monkeypatch.setattr("workguide.community.aio_sandbox.local_backend.socket.getaddrinfo", fake_getaddrinfo)

    from workguide.community.aio_sandbox.local_backend import _resolve_sandbox_host_address
...
def test_start_container_resource_limits_env_override(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_MEMORY", "4g")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_CPUS", "4")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_PIDS_LIMIT", "1024")
...
def test_start_container_resource_limits_can_be_disabled(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_MEMORY", "0")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_CPUS", "none")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_PIDS_LIMIT", "0")
...
def test_start_container_passes_through_user_and_network(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_CONTAINER_USER", "1000:1000")
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "workguide-sandbox-egress")

    captured_cmd = _capture_start_container_command(monkeypatch, backend)

    assert captured_cmd[captured_cmd.index("--user") + 1] == "1000:1000"
    assert captured_cmd[captured_cmd.index("--network") + 1] == "workguide-sandbox-egress"
...
def test_start_container_rejects_host_networking(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "host")

    with pytest.raises(RuntimeError, match="WORKGUIDE_SANDBOX_NETWORK"):
...
def test_start_container_rejects_shared_container_network_namespace(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "container:gateway")
...
def test_start_container_rejects_none_network(monkeypatch):
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "none")
...
def test_start_container_rejects_extended_network_syntax_host(monkeypatch):
    ...
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=host")
...
def test_start_container_rejects_extended_network_syntax_none(monkeypatch):
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=none")
...
def test_start_container_rejects_extended_network_syntax_container(monkeypatch):
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=container:gateway")
...
def test_start_container_passes_extended_network_syntax_for_custom_networks(monkeypatch):
    ...
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=workguide-sandbox-egress")

    captured_cmd = _capture_start_container_command(monkeypatch, backend)

    assert captured_cmd[captured_cmd.index("--network") + 1] == "name=workguide-sandbox-egress"
...
def test_discover_brackets_ipv6_sandbox_host_for_url(monkeypatch, sandbox_host):
    ...
    backend = _backend_for_inspect_tests()
    monkeypatch.setenv("WORKGUIDE_SANDBOX_HOST", sandbox_host)
    ...
    monkeypatch.setattr("workguide.community.aio_sandbox.local_backend.wait_for_sandbox_ready", fake_ready)
...
def test_create_brackets_ipv6_sandbox_host_for_url(monkeypatch, sandbox_host):
    ...
    monkeypatch.setattr("workguide.community.aio_sandbox.local_backend.get_free_port", lambda start_port=None: 18082)
...
def test_start_container_rejects_host_with_additional_long_syntax_fields(monkeypatch, network):
    ...
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", network)
...
def test_start_container_rejects_none_with_additional_long_syntax_fields(monkeypatch):
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "gw-priority=0,name=none")
...
def test_start_container_rejects_container_mode_with_additional_long_syntax_fields(monkeypatch):
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=container:gateway,gw-priority=0")
...
def test_start_container_passes_long_syntax_custom_network_with_fields(monkeypatch):
    ...
    backend = _backend_for_inspect_tests()
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_NETWORK", "name=egressnet,gw-priority=1")
...
def test_effective_network_target_last_name_field_wins():
    ...
    from workguide.community.aio_sandbox.local_backend import _effective_docker_network_target as target
...
def test_default_image_starts_under_hardened_capabilities(monkeypatch):
    ...
    """
    from workguide.community.aio_sandbox.backend import SANDBOX_LOCAL_PROVIDER_READY_TIMEOUT
    from workguide.community.aio_sandbox.local_backend import wait_for_sandbox_ready
    ...
    backend = LocalContainerBackend(
        ...
        image=os.environ.get("WORKGUIDE_SANDBOX_SMOKE_IMAGE", _DEFAULT_AIO_IMAGE),
        base_port=18210,
...
def test_start_container_preinitialized_image_can_drop_startup_caps(monkeypatch):
    ...
    WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS=0 drops every capability."""
    backend = LocalContainerBackend(
    ...
    _clear_hardening_env(monkeypatch)
    monkeypatch.setenv("WORKGUIDE_SANDBOX_IMAGE_STARTUP_CAPS", "0")
