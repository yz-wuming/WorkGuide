...
from workguide.config.app_config import reset_app_config
from workguide.config.database_config import DatabaseConfig
from workguide.persistence.agents import get_agent_store, make_agent_store
from workguide.persistence.agents.file import FileAgentStore
from workguide.persistence.agents.model import AgentRow
from workguide.persistence.agents.sql import SqlAgentStore
from workguide.persistence.base import Base
from workguide.persistence.managed_subagents.base import ManagedSubagentDefinition
from workguide.persistence.managed_subagents.file import FileManagedSubagentStore
from workguide.persistence.managed_subagents.model import ManagedSubagentRow
from workguide.persistence.managed_subagents.sql import SqlManagedSubagentStore
...
def file_home(tmp_path, monkeypatch):
    """Root file stores at a temp WORKGUIDE_HOME with seeded definitions."""
...
def _patch_importer(monkeypatch, cfg):
    ...
    monkeypatch.setattr("workguide.persistence.engine.init_engine_from_config", _noop_init)
    return importer
...
def test_read_free_functions_dispatch_to_db_backend(file_home, monkeypatch):
    ...
    monkeypatch.setattr("workguide.config.app_config.get_app_config", lambda: cfg)

    from workguide.config.agents_config import list_custom_agents, load_agent_config, load_agent_soul
...
def test_file_create_race_maps_file_exists_to_agent_exists(tmp_path, monkeypatch):
    ...
    from workguide.persistence.agents.base import AgentExistsError
...
def test_get_agent_store_resolves_db_backend_from_on_disk_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(cfg_path))
    try:
...
def test_get_agent_store_falls_back_to_file_without_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORKGUIDE_CONFIG_PATH", str(tmp_path / "does-not-exist.yaml"))
    try:
        reset_app_config()
