...
- Sandbox: ``bash_tool`` exposes the id as the fixed env var
  ``WORKGUIDE_CHANNEL_USER_ID`` via an ``export`` prefix on the command string.
...
class TestBashToolChannelIdentityPrefix:
    ...
    def test_windows_local_sandbox_skips_prefix(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.sandbox.tools._is_windows", lambda: True)
    ...
    def test_posix_local_sandbox_gets_prefix(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
        monkeypatch.setattr("workguide.sandbox.tools.is_host_bash_allowed", lambda: True)
        monkeypatch.setattr("workguide.sandbox.tools._is_windows", lambda: False)
