...
from workguide.mcp.tasks.runtime import get_mcp_task_submitter
from workguide.tools.builtins.list_uploaded_files_tool import _resolve_thread_id, _resolve_user_id
...
async def cancel_background_task(
    ...
    task: Annotated[
        str | None,
        "Optional exact task name or WorkGuide task ID. Omit it only when one active task exists.",
    ] = None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "task": public,
        "message": "Cancellation requested. WorkGuide will keep retrying safely if the remote server is temporarily unavailable.",
    }
