...
Runs each WorkGuide sandbox as a [Tenki](https://tenki.cloud) cloud sandbox — an
...
```yaml
sandbox:
  use: workguide.community.tenki:TenkiSandboxProvider
...
```bash
pip install "workguide-harness[tenki]"
```

The `tenki-sandbox` package is an optional WorkGuide harness extra, not part of
...
API; directory and content search shell out and reuse `workguide.sandbox.search`,
...
WorkGuide's `/mnt/user-data` virtual prefix is remapped under the writable
