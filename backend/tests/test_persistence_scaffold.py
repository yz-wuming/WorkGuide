...
class TestDatabaseConfig:
    ...
    def test_sqlite_paths_unified(self):
        ...
        assert c.sqlite_path.endswith("workguide.db")
    ...
    @pytest.mark.parametrize("schema", ["workguide", "my_schema", "_private", "s", "a" * 63])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # quoted schema literally named ``workguide\n`` while the unquoted
            # search_path folds to ``workguide`` and misses it (tables land in
    ...
    def test_postgres_schema_does_not_pollute_url(self):
        c = DatabaseConfig(backend="postgres", postgres_url="postgresql://u:p@h:5432/db", postgres_schema="workguide")
        url = c.app_sqlalchemy_url
        assert "workguide" not in url.replace("/db", "")
        assert url.startswith("postgresql+asyncpg://")
...
class TestEngineLifecycle:
    ...
    async def test_postgres_without_asyncpg_gives_actionable_error(self):
        ...
        from workguide.persistence.engine import init_engine
...
class TestPostgresSchemaInit:
    ...
    async def test_passes_search_path_connect_args(self, monkeypatch):
        import workguide.persistence.engine as engine_module
        ...
        monkeypatch.setattr("workguide.persistence.bootstrap.bootstrap_schema", AsyncMock())

        await engine_module.init_engine(
            "postgres",
            url="postgresql+asyncpg://u:p@h:5432/db",
            postgres_schema="workguide",
        )

        assert captured["connect_args"] == {
            "command_timeout": engine_module.POSTGRES_COMMAND_TIMEOUT_SECONDS,
            "server_settings": {"search_path": "workguide"},
        }
    ...
    async def test_creates_schema_before_bootstrap(self, monkeypatch):
        ...
        monkeypatch.setattr("workguide.persistence.bootstrap.bootstrap_schema", calls.bootstrap_schema)
        ...
        assert "workguide" in str(execute_arg)
        await engine_module.close_engine()
