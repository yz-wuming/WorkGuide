...
Lives at the top-level package (not ``workguide.runtime``) so it can be
imported from ``workguide.agents.thread_state`` without pulling in the heavy
``workguide.runtime`` package __init__ (which eagerly imports the runs
machinery). Anchored from ``workguide.agents.thread_state`` so every process
that builds a WorkGuide graph (gateway, workers, in-process LangGraph
...
_PATCH_FLAG = "_workguide_delta_history_patched"
...
_BINOP_PATCH_FLAG = "_workguide_overwrite_first_write_patched"
...
def _as_overwrite(value: Any) -> tuple[bool, Any]:
    ...
    WorkGuide's write paths produce for these Union channels (the branch and
    ...
    an internal serialization detail WorkGuide never emits into these channels.
    """
...
def _binop_first_write_stores_overwrite_wrapper() -> bool:
    ...
    channel.key = "workguide-overwrite-probe"
