...
from workguide.community.warm_pool_lifecycle import (
...
from workguide.config.paths import VIRTUAL_PATH_PREFIX, get_paths, join_host_path
from workguide.integrations.lark_cli import INTEGRATION_ID as LARK_CLI_INTEGRATION_ID
from workguide.integrations.lark_cli import LARK_CLI_SANDBOX_CONFIG_DIR, LARK_CLI_SANDBOX_DATA_DIR, LARK_CLI_SANDBOX_LOCKS_DIR, LARK_CLI_SANDBOX_RUNTIME_DIR, ensure_lark_cli_credential_tree, lark_skills_installed
...
DEFAULT_PORT = 8080
DEFAULT_CONTAINER_PREFIX = "workguide-sandbox"
...
class AioSandboxProvider(WarmPoolLifecycleMixin[SandboxInfo], SandboxProvider):
    ...
    def _get_skills_mounts(*, user_id: str | None = None) -> list[tuple[str, str, bool]]:
        ...
        Mount sources use ``WORKGUIDE_HOST_BASE_DIR`` when running inside
    ...
    def _lark_broker_active(user_id: str | None = None) -> bool:
        ...
        try:
            if not AioSandboxProvider._lark_integration_active(user_id):
                return False
            from workguide.integrations.lark_cli import sandbox_lark_broker_active
