...
async def _seed_state(repo, provider: str, state: str, owner_user_id: str = "workguide-user-1") -> None:
...
def test_feishu_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        connections = await repo.list_connections("workguide-user-1")
        ...
        channel._reply_card.assert_awaited_once_with("om-message-1", "Feishu connected to WorkGuide.")
        await repo.close()
...
def test_wechat_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        channel._send_connection_reply.assert_awaited_once_with("wx-user-1", "ctx-1", "WeChat connected to WorkGuide.")
        await repo.close()
...
def test_wecom_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        channel._ws_client.reply.assert_awaited_once_with(frame, {"msgtype": "text", "text": {"content": "WeCom connected to WorkGuide."}})
        await repo.close()
...
def test_additional_channels_attach_owner_identity(tmp_path):
    ...
    async def go():
        ...
        for channel, inbound in cases:
            ...
            assert attached.owner_user_id == "workguide-user-1"
