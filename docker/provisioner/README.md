# WorkGuide Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the WorkGuide backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `workguide` namespace with:
...
When the Gateway mounts that same storage at its WorkGuide home and the PVC
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `workguide` | Kubernetes namespace for sandbox resources |
...
| `WORKGUIDE_HOST_BASE_DIR` | `/.workguide` | **Host machine** WorkGuide data root containing global and per-user `skills_view` projections |
...
| `USERDATA_PVC_NAME` | empty (use hostPath) | PVC name for user-data volume; when set, uses PVC with `subPath: workguide/users/{user_id}/threads/{thread_id}/user-data` |
...
Older provisioner versions mounted PVC user-data from `threads/{thread_id}/user-data`. The user-scoped layout mounts from `workguide/users/{user_id}/threads/{thread_id}/user-data`.

If an existing deployment already has PVC-backed user-data under the legacy layout, migrate the WorkGuide data directory before relying on the new PVC subPath. Mount the same PVC path that the gateway uses as its WorkGuide base directory, then run the existing user-isolation migration script:
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `workguide` namespace
     - Create/read/delete Services in the `workguide` namespace
     - Read Namespaces (to create `workguide` if missing)

4. **Host Paths**:
   - `WORKGUIDE_HOST_BASE_DIR` and `THREADS_HOST_PATH` must be **absolute paths on the host machine**
...
docker compose -p workguide-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec workguide-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001","user_id":"user-001"}'

# Check sandbox status
docker exec workguide-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec workguide-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n workguide -l sandbox-id=test-001

# Delete sandbox
docker exec workguide-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec workguide-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec workguide-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec workguide-provisioner ls -ld /root/.kube/config
  ```
...
**Solution**: 
- Use absolute paths for `WORKGUIDE_HOST_BASE_DIR` and `THREADS_HOST_PATH`
- Verify the paths exist on your host machine:
  ```bash
  ls -la /path/to/skills
  ls -la /path/to/backend/.workguide/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n workguide`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n workguide`
- In NodePort mode, test from the backend container: `curl http://$NODE_HOST:NODE_PORT/v1/sandbox`
- In ClusterIP mode, test from the backend Pod: `curl http://sandbox-XXX-svc.workguide.svc.cluster.local:8080/v1/sandbox`
