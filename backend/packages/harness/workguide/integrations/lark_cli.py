...
* a **content** SHA-256 over the extracted skill tree, after WorkGuide's shared
...
also manages a writable WorkGuide-owned Gateway CLI under
``.workguide/integrations/lark-cli/gateway-cli`` and prefers it over the system
...
from workguide.config.paths import Paths, get_paths
from workguide.integrations.lark_broker import LARK_BROKER_URL_ENV
from workguide.skills.installer import is_executable_binary_prefix, is_symlink_member, is_unsafe_zip_member
from workguide.skills.parser import parse_skill_file
from workguide.skills.permissions import make_skill_tree_sandbox_readable
...
LARK_CLI_SOURCE_ARCHIVE_ENV = "WORKGUIDE_LARK_CLI_SKILLS_ARCHIVE"
LARK_CLI_SANDBOX_RUNTIME_SOURCE_ENV = "WORKGUIDE_LARK_CLI_SANDBOX_RUNTIME_DIR"
...
LARK_CLI_MANIFEST_FILE = ".workguide-lark-cli-manifest.json"
...
LARK_CLI_RUNTIME_MANIFEST_FILE = ".workguide-lark-cli-runtime.json"
LARK_CLI_FLOW_STATE_FILE = ".workguide-lark-cli-flow.json"
...
_WORKGUIDE_LARK_SHARED_GUIDANCE_MARKER = "<!-- workguide-lark-cli-auth-guidance-v2 -->"
_WORKGUIDE_LARK_SHARED_GUIDANCE_LEGACY_MARKERS = ("<!-- workguide-lark-cli-auth-guidance-v1 -->",)
...
def lark_cli_managed_gateway_dir() -> Path:
    """Gateway-scoped WorkGuide-managed lark-cli install root."""
...
def _download_lark_release_asset(version: str, asset_name: str, *, max_bytes: int = LARK_CLI_MAX_RUNTIME_ASSET_BYTES) -> bytes:
    ...
    request = urllib.request.Request(
        ...
        headers={"Accept": "application/octet-stream", "User-Agent": "workguide"},
    )
    try:
...
def lark_cli_env_overlay(user_id: str, *, sandbox_paths: bool = False, broker: bool = False) -> dict[str, str]:
    """Environment overlay for lark-cli using WorkGuide-managed credentials.
...
def _probe_provisioner_capabilities(config: AppConfig, *, timeout: float = 5.0) -> dict[str, bool] | None:
    ...
    try:
        request = urllib.request.Request(url, headers={"User-Agent": "workguide", **headers})
...
def _ensure_managed_gateway_lark_cli() -> LarkCliProbe:
    """Install/update the WorkGuide-managed Gateway lark-cli.
...
def _resolve_latest_lark_cli_version() -> str:
    ...
    try:
        request = urllib.request.Request(
            ...
            headers={"Accept": "application/vnd.github+json", "User-Agent": "workguide"},
        )
        with urllib.request.urlopen(request, timeout=LARK_HTTP_TIMEOUT_SECONDS) as response:
            raw = response.read().decode("utf-8")
        data = json.loads(raw)
...
def _content_sha256(root: Path, skill_names: set[str]) -> str:
    ...
    The caller computes this after injecting WorkGuide's shared guidance, so the
...
def _install_lark_skills_from_archive_locked(
    ...
    *,
    ...
    try:
        ...
        _append_workguide_lark_shared_guidance(staging_target)
...
def _append_workguide_lark_shared_guidance(root: Path) -> None:
    skill_file = root / "lark-shared" / SKILL_MD_FILE
    content = skill_file.read_text(encoding="utf-8")
    if _WORKGUIDE_LARK_SHARED_GUIDANCE_MARKER in content:
        return
    for legacy_marker in _WORKGUIDE_LARK_SHARED_GUIDANCE_LEGACY_MARKERS:
        if legacy_marker in content:
            content = content.split(legacy_marker, maxsplit=1)[0].rstrip()
            break
    guidance = f"""

{_WORKGUIDE_LARK_SHARED_GUIDANCE_MARKER}

## WorkGuide 授权入口

在 WorkGuide 中，如果 `lark-cli auth status` 或业务命令提示未配置、未登录、token 过期或缺少用户授权：
