...
def _read_marked_message(path, content, tool_call_id="r1"):
    ...
    msg.additional_kwargs["workguide_read_mark"] = {"path": path, "hash": _sha(content)}
    return msg
...
class TestReadCurrentFileContent:
    def test_reads_via_sandbox_with_resolution(self):
        from workguide.sandbox import tools as sandbox_tools
...
class TestReadMarkStamping:
    def test_read_file_success_stamps_mark(self):
        ...
        mark = result.additional_kwargs["workguide_read_mark"]
    ...
    def test_ranged_read_stamps_full_file_hash(self):
        ...
        result = mw.wrap_tool_call(request, handler)
        assert result.additional_kwargs["workguide_read_mark"]["hash"] == _sha("line1\nline2\nline3")
    ...
    def test_error_tool_message_gets_no_mark(self):
        ...
        result = mw.wrap_tool_call(request, handler)
        assert "workguide_read_mark" not in result.additional_kwargs
...
class TestWriteGate:
    ...
    def test_blocked_write_has_workguide_tool_meta(self):
        ...
        assert meta is not None, "blocked write must carry workguide_tool_meta"
...
class TestAsyncPaths:
    ...
    def test_async_blocked_write_has_workguide_tool_meta(self):
        import asyncio
        ...
        result = asyncio.run(mw.awrap_tool_call(request, handler))
        meta = (result.additional_kwargs or {}).get(TOOL_META_KEY)
        assert meta is not None, "async blocked write must carry workguide_tool_meta"
        assert meta["recoverable_by_model"] is True
    ...
    def test_async_read_stamps_mark(self):
        ...
        result = asyncio.run(mw.awrap_tool_call(request, handler))
        assert result.additional_kwargs["workguide_read_mark"]["hash"] == _sha("v1")
...
class TestChainWiring:
    def test_enabled_by_default_in_runtime_chain(self):
        ...
        from workguide.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware, build_lead_runtime_middlewares
...
class TestErrorStringSandboxes:
    ...
    def test_existing_file_read_still_marked_and_write_allowed(self):
        ...
        assert read_result.additional_kwargs["workguide_read_mark"]["hash"] == _sha("v1")
...
class TestSamePathSerialization:
    ...
    def test_read_mark_matches_content_shown_to_model(self):
        ...
        mark = read_result.additional_kwargs.get("workguide_read_mark")
