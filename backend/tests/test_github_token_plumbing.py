...
from workguide.sandbox.tools import _github_env_from_runtime, bash_tool
...
def test_local_sandbox_env_overlay_reaches_subprocess(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import workguide.sandbox.local.local_sandbox as local_sandbox
...
def test_extra_env_rejects_invalid_keys(bad_key) -> None:
    ...
    """
    from workguide.sandbox.sandbox import _validate_extra_env
...
def test_bash_tool_passes_token_as_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("workguide.sandbox.tools.ensure_sandbox_initialized", lambda runtime: _Sandbox())
