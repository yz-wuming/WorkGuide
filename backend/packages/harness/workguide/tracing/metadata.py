...
from workguide.config import get_enabled_tracing_providers
from workguide.trace_context import WORKGUIDE_TRACE_METADATA_KEY, get_current_trace_id, normalize_trace_id

# Lazy-imported below to avoid a circular import: ``workguide.runtime`` eagerly
# imports the run worker, which in turn needs ``workguide.tracing``.
...
def build_langfuse_trace_metadata(
    *,
    ...
    Args:
        ...
        workguide_trace_id: Optional WorkGuide request trace id; falls back to
    ...
    request_trace_id = normalize_trace_id(workguide_trace_id) or get_current_trace_id()
    if request_trace_id:
        metadata[WORKGUIDE_TRACE_METADATA_KEY] = request_trace_id
