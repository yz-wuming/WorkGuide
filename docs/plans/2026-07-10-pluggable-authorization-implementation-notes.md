...
> pluggable authorization RFC ([#4063](https://github.com/bytedance/workguide/issues/4063)).
...
本文档是可插拔授权 RFC（[#4063](https://github.com/bytedance/workguide/issues/4063)）
...
PR [#4127](https://github.com/bytedance/workguide/pull/4127) 于 2026-07-15
...
- Layer 1 必须覆盖 lead agent、native subagent 和 `WorkGuideClient` 三条装配路径。
...
- [ ] 选择配置版本号前，已 fetch 并 rebase 最新 `upstream/main`。不能使用本地缓存的
      ...
      `config.example.yaml` + `deploy/helm/workguide/values.yaml` + `deploy/helm/workguide/README.md`
      三处同步。
...
- **延期：** Layer 1 工具过滤、Layer 2 自动接线、WorkGuideClient、RBAC 配置示例
  移至 Phase 1B。
...
- `WorkGuideClient.stream()` 的调用方属于可信进程内边界，可通过关键字参数传入与
...
- **决策（embedded/library 路径）：** `_authorize_model_name` 同样接入
  `WorkGuideClient._ensure_agent`（client.py），与 Gateway runtime 路径 `_make_lead_agent`
...
- **证据：** `tests/test_models_authorization.py`（24 tests）覆盖 disabled/anonymous/
  ...
  no-usable-fallback 运行时场景，以及 `WorkGuideClient._ensure_agent` 的 model:use 强制 +
